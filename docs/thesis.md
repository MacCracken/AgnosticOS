...
- **The knowledge library** (the v1.0+ science crates in [`applications/libs/`](applications/libs/README.md), pre-1.0 + planned in [`shared-crates.md`](development/planning/shared-crates.md)) — scientific domains must travel with the proof. No downloading from outside. The library is the card.
