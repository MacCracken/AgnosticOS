...
- **AGNOS kernel (260 KB, 33 subsystems)** — an operating system that depends on another operating system to build itself is scaffolding, not structure.
- **The knowledge library (78 crates)** — scientific domains must travel with the proof. No downloading from outside. The library is the card.
...
- **AGNOS kernel** — 260 KB, 33 subsystems. Not *"Linux in Cyrius."* A totally different decomposition of the kernel problem for a platform that doesn't need to support 30 years of legacy hardware.
...
**Shipped:**
- **AGNOS kernel v1.22.0** — 260 KB, 33 subsystems, 26 syscalls, TCP/IP, FAT16, VirtIO, SMP, ELF loader, kybernet as PID 1
- **Cyrius v5.5.27** — self-hosting from 29 KB seed; x86_64 Linux, aarch64 Linux, Apple Silicon Mach-O, Windows PE32+ — all byte-identical
- **bhava v2.0.0** — 37 modules, 5 bridges, 63 bridge functions, 1,117 tests; Scales 0–3 implemented
- **30+ subsystems** ported from Rust to Cyrius
- **48 KB** sovereign Cyrius boot pipeline
- **ISO pipeline Stage 0** (component verification)

**In flight / next:**
- Cyrius v5.5.34 platform closeout → v5.6.x compiler-optimization arc → v5.7.0 RISC-V → v5.8.0 bare-metal
