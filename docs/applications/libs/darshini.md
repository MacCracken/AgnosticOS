# darshini

`eza`/`ls`-equivalent — modern directory listing for the terminal (Cyrius-native).

- **Repository**: [github.com/MacCracken/darshini](https://github.com/MacCracken/darshini)
- **License**: GPL-3.0-only
- **Status**: Stable — v1.0+
- **Language**: Cyrius

Part of the terminal-aesthetics cohort (with commandress / bannermanor / iam / kii). Sanskrit दर्शिनी — "she who shows / reveals."

See the [libs registry](../README.md) for the full catalog and dependency context.
