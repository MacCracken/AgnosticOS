...
| rosnet | 1.0.0 | **Sovereign dense f64 tensor algebra — the BLAS substrate under the whole ML family** (storage, BLAS-1, matmul + its hand-derived FD-gated gradient; no BLAS/libc/autodiff). Dual-profile: CPU `dist/rosnet.cyr` (mabda-free) + GPU `dist/rosnet-gpu.cyr` (`[lib.gpu]`, mabda-gated, bit-exact/~1e-13 vs the CPU oracle; provider coverage evolves under mabda — 4.0 added basic NVIDIA). **1.0.0 froze the surface six shipping consumers exercised** (attn11/tarka/tentib/prajna/rupantara/anukūlana); the no-bounds-checks caller-guarantees contract is the audited substrate design (`docs/api.md` + `docs/audit/`). Post-1.0 additive lane: conv2d fwd+bwd (the modality/Type-4 gate), f32/f16/bf16 wideners |
