...
> **Last refresh**: 2026-05-28 (mini-sweep after agnos 1.38.x JBD2 arc closed; user-driven post-arc doc sweep). **Touchpoints**: `README.md` (kernel badge 1.34.6→1.38.8, Cyrius badge 6.0.1→6.0.9, capability paragraph adds ext4 extent allocation + JBD2 crash-safe journaling + DNS/NTP/ICMP + kashi 1.0.0 vendoring), the JBD2 arc's prior-art audit + iron-burn audit (`ext4-jbd2-prior-art.md` and `ext4-jbd2-iron-burn-audit.md` — new files, landed dated in-cycle, not edited in this sweep). **Left alone** (correctly historical / current via pointer): `state.md` (kept current per-cut across 1.36 → 1.38 cycles), `roadmap.md` (forward-facing structure stable from 2026-05-26), `architecture.md` / `philosophy.md` / `thesis.md` / `installation/*` (versions already pointer-deferred to state.md), all `articles/` + `*-prior-art.md` + dated iron-burn audits (dated engineering artifacts — don't re-touch).
>
