...
> **Last refresh**: 2026-07-01 — **structural sweep (Phase 2 of a full doc + agent-memory audit)**. (1) **Merged the stray `docs/development/issue/` (singular) into `issues/` (plural)** — plural matches the active issue's location + the cyrius/agnos-side `issues/` convention; moved `issue/archive/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md` → `issues/archive/`, fixed the two inbound links (`roadmap.md`, `iron-nuc-zen-photos/README.md`). (2) **Dead-link scan** of the load-bearing docs (top-level + `development/` + READMEs): confirmed the two loose `*-prior-art.md` files (`shell-separation`, `uefi-boot`) are **correctly in `development/` per the documented active-vs-completed split** (`prior-art/README.md`: "active research stays in `../`") — NOT moved. Pre-existing dead links inside *dated historical logs* (`iron-nuc-zen-log*`, cross-repo cyrius-issue refs, an `applications/roadmap.md` / `planning/usb-hid-keyboard-driver.md` ghost) left per the dated-artifact rule (flagged, not chased). (3) **Repointed a live `state.md` memory-link** broken by the same-session agent-memory consolidation (the `project_amd_zen_scanout_residue` memory folded into the new `project_archaemenid_hardware_target` group). NOTE: the **agent-memory folder was heavily consolidated this session** (131→66 files, index 48K→15K — per-project detail pushed down to its repos, iron/cyrius working-discipline grouped); a few repo→memory links in historical logs now dangle (fragile by design; not chased). **Phase 3 DONE (workflow, 2026-07-01)** — a 32-agent parallel staleness sweep of the current-facing docs (one auditor+fixer per doc, adversarially-verified, name-at-time preserved): **21 of 32 fixed, 11 clean.** Recurring corrections: kernel/cyrius/gnoboot versions → current (agnos **1.51.2**, cyrius **6.3.21**, gnoboot **0.6.0**; deferred to state.md where volatile); the **GRUB/ISO9660/multiboot2 boot path → gnoboot `BOOTX64.EFI` on GPT+FAT-ESP** (README/AGNOS/architecture/iso-pipeline/iso-stage4/system-requirements/cis-benchmarks/path-c); **agnosys → agnodrm** with the whole `kernel-guide.md` agnosys section reframed historical (kernel-guide/philosophy/iso-stage4); **beta timeline early-June/Q4 → late-Aug 2026 + founder solo month + GA late-fall** (README/AGNOS/thesis/installation); plus **cc5→cycc / cyrc→cybs**, CalVer→SemVer, `.cyrius-toolchain`→`cyrius.cyml`, stale Rust namespaces (`agent_runtime::`/`agnos_sys::`) → Cyrius names, and "systemd-boot is the default bootloader" → gnoboot. First-instance misses the per-doc auditors left were cleaned by hand (AGNOS.md `6.0.1` ×3, binaries.md agnos pin `6.0.56`→`6.3.9`, README `1.44.x iron-pending`→iron-validated, iso-pipeline early-June note). **DEFERRED to a separate registry version-drift sweep:** the per-crate toolchain pins in `applications/` + `planning/shared-crates.md` (bannermanor 6.0.1, chakshu 6.0.1, anuenue 6.0.56, …) — those are per-crate build-pin *records*, refreshed on a registry drift pass, not this ecosystem-fact staleness pass.
>
> **2026-06-19 sweep** — **agnos → agnodrm decomposition sweep** (handoff prep). The over-scoped `agnosys` kernel-interface lib decomposed to **agnodrm** (the AGNOS device / DRM model — udev/drm on error/util); its other subsystems moved to their proper homes: trust → **sigil 3.9.0** (promoted to `dist`), security/mac/audit → **kavach 3.5.0**, pam → **aegis 1.1.0**, logging → **sakshi 2.4.0**, the per-arch syscall layer → **cyrius** (value-add filed). **Synced the load-bearing handoff anchors**: `planning/shared-crates.md` (agnosys row → **agnodrm 1.4.4** + the 8 touched repos' version cells + `depends on agnosys` refs argonaut/shakti → agnodrm + a header sweep entry), `development/state.md` (a current-ecosystem decomposition note + the tag order), `CLAUDE.md` (Standalone Repos table row — done earlier in-session), `README.md` (port-receipt row → "agnodrm (was agnosys)"). Downstream consumers were rewired + patched **in their own repos** (libro 2.7.5 / nein 1.5.4 / kybernet 1.3.5; **mihi / iam deferred** on the cyrius uname/sysinfo request — redirect-safe). **Also swept the current-facing prose**: `architecture.md` (ASCII module-map box agnosys→agnodrm; the **SYSCALL/SYSRET layer** line corrected — those bindings moved to **cyrius**, agnodrm is the device/DRM model now), `AGNOS.md` (kernel-interface subsystem row → device/DRM model + the port-receipt name), `roadmap.md` (the CVE-audit clean-repos list `sigil, agnosys, phylax` → agnodrm). **Disposition rule applied** (and the boundary for the remaining tail): rename `agnosys` → `agnodrm` in **current-facing prose**; **preserve it as the name-at-the-time** in ADRs / `archive/` / `history.md` / `timeline.md` / dated `articles/` / port-sequence receipts (renaming history would falsify the record). **Remaining (left as historical, correctly)**: `design-patterns.md`'s 30+-ports list + the "`check-api-surface.sh` originated in agnosys" origin note (both name-at-the-time), `roadmap.md`'s port-dependency chain (`agnostik → agnosys → libro …` — the historical port order), and the `planning/` / `vision/` / `guides/` low-traffic tail (verify opportunistically; none known to be load-bearing). The registry + state.md + CLAUDE.md (what a new agent loads first) + the current-facing prose are all synced.
>
> **2026-06-04 sweep** (post-agnos-1.41.x-arc kernel-fact refresh — the shell-separation arc went **1.41.0 → 1.41.11 software-complete** since the 2026-05-31 sweep had set the docs at 1.41.0). Ground-truth dossier (agnos **1.41.11** / cyrius pin **6.0.56** / cyrius latest 6.0.61 / gnoboot **0.5.0** / build 1,070,720 B / `shell.cyr` 813 LOC / syscall surface 0–33) mined + adversarially fact-checked, then a parallel per-doc refresh+verify pass (two multi-agent workflows; same QEMU-vs-iron honesty bar as the vidya kernel field-notes work). **Touchpoints**: `README.md` (badges 1.41.0→**1.41.11** + 6.0.14→**6.0.56**; capability paragraph adds shell-separation **software-complete / QEMU-validated / iron-pending** with the explicit "not yet booted on real hardware" honesty bar — `#tracker-141x-cycle`), `AGNOS.md` (1.41.x completion + `agnsh` as the userland interactive shell + gnoboot 0.5.0), `architecture.md` (Layer-5: interactive shell → userland `agnsh`, in-kernel shell = recovery REPL ~813 LOC; gnoboot "recently shipped v0.2.0" → milestone phrasing), `history.md` + `timeline.md` (1.41.x shell-separation milestone rows; **also fixed a pre-existing days-from-start off-by-one** 107/108→108/109; dropped a stale "248KB … current" tag → trajectory + state.md pointer), `roadmap.md` (1.41.x slot active→software-complete/iron-pending), `installation/system-requirements.md` (kernel → 1.41.11; `.cyrius-toolchain`→`cyrius.cyml` path fix), **`SECURITY.md`** (**stripped the stale "26 syscalls / agnos v1.26.1 / 248KB" hardcode** the 2026-05-31 count-reframe had missed → the durable "no socket/splice/AF_ALG surface" anchor + state.md pointer, noting the table grew 26→34). **`development/state.md` heavily refreshed** (the anchor): header advanced 1.40.14-OPEN/1.41.0-OPEN → **1.41.11 software-complete + iron-pending**; the ~3,000-word 1.40.x forensic-triage line **COLLAPSED to a one-liner** (it had drifted into a LOG, violating the file's own *NOT A LOG* rule — full detail already in CHANGELOG + iron-log); jbd2 + 1.39.x verbose bite-ladders condensed to one-liners; gnoboot 0.4.3→0.5.0; **pin-lag spectrum re-surveyed** — the 1.41.x arc (`CYRIUS_TARGET_AGNOS` at cyrius 6.0.55/56) pulled the boot-path core off the uniform 6.0.14 pin into a **6.0.14→6.0.61 spread** (agnos/agnoshi/kybernet/argonaut 6.0.56, agnosys/libro 6.0.52/53, agnostik 6.0.26, mabda 6.0.43, sigil 6.0.61, only **hisab** still 6.0.14); toolchain 6.0.24→6.0.61; the **fossil agnos table row (was 1.32.1!)** + the agnoshi row updated to current. **`CLAUDE.md` verified current** — no edit (already defers to `cyrius/VERSION` + state.md, and correctly states there's no `.cyrius-toolchain` file). **Left alone** (correctly dated/historical): all `articles/`, `prior-art/`, iron-logs, `archive/`; the broader pin-lag ecosystem rows (.0/.1/.3 — didn't move with the arc, last surveyed 2026-06-01).
>
...
| `CHANGELOG.md` | 2026-06-04 | ✅ Fresh | **2026-06-04: brief `[Unreleased]` entry** — 0.1.0 clean+prep doc-currency note pointed at ecosystem **release-readiness** (iron-validated/shippable: kernel base maturity + cyrius 6.0.62 + 94 v1.0+ crates + agora 1.0 + gnoboot 0.5.0; software-complete/iron-pending: 1.41.x shell-sep; 0.1.0 close gates on the kernel iron burn + ISO + Docker). Prior **[2026.5.11]** entry (v5.10.x close / v5.11.x open) retained. |
...
| `planning/multimodal-substrate.md` | 2026-06-13 | ✅ Fresh — **NEW** | **NEW 2026-06-13** — forward design + reference map for sight/hearing ML on the attn11 transformer core. The reframe: attn11 is the modality-agnostic body; each modality = a frontend + 1–2 new differentiable primitives. Gating new work: `conv2d`/`conv1d` fwd+bwd (learned) + sovereign FFT/STFT+mel (fixed, no backprop) + cross-attention. Carries a curated references section (ViT/CLIP/Whisper/AST/Conformer/LLaVA papers; nanoGPT/whisper.cpp/tinygrad prior-art; in-ecosystem attn11/abaco-dsp/kii/vani-stack/drishti). Roadmap entry at Phases 17–18 + Demand-Gated table. Demand-gated, post-beta — not on any critical path. |
| `planning/desktop-design-ideas.md` | 2026-06-22 | ✅ Fresh — **NEW** | **NEW 2026-06-22** — fermenting idea log for desktop-stage generative visuals (aethersafha/mabda surface): organic loaders (procedural geometry on bsp/abaco), compositor-owned global motion language, shader wallpapers (mabda surface). Frame: concept-survives-substrate, borrow-concept-drop-the-JS. Sibling to the generative-ML planning docs (same compute-don't-stream instinct, GUI surface). HW design (Jetson/Olares as references only) explicitly parked until desktop ships. Not on any critical path — capture-and-ferment. |
...
| `port-ledger-volume-3.md` | 2026-06-20 | ✅ Fresh (COMPLETE — 10/10 Vol 1 ports) — Post-arc re-measurement ledger; **seeded 2026-06-01** (abaco + hisab), **expanded 2026-06-04** with six more Volume 1 port receipts (agnosys, kybernet, ai-hwaccel, avatara, kavach, nous) — **ALL TEN Vol 1 ports now have published receipts** (completed 2026-06-20). Gathered by fan-out extract+verify workflows reading each port's real CSV/bench-doc + adversarial source-check (cardinal rule: **no fabricated numbers**; noisy single-run trails = direction-of-motion, not absolute speed). The last three were **freshly bench-run at 6.2.x on 2026-06-20** (agnostik serde from_json collapse 6000→803 / 3000→503 ns; hoosh n=1→2-point baseline; ark graduated 5.1.10→1.0.0, fresh three-tier 6.2.x baseline (DCE-confirmed)), and **agnosys reframed → agnodrm** (decomposition; old syscall benches confirmed gone). Still accreting: held-cluster, cross-arch, opt-arc verdict. |
