...
> **Last refresh**: 2026-05-18 (2026-05-18 doc-staleness audit, four-stage sweep + five follow-ups: **S1** agnos `security-hardening.md` S7 KASLR flipped ✅ Done v1.28.0 (13/13 closed); **S2** agnos `state.md` body sync (Open Investigation superseded — xHCI Enable Slot CCE arc replaces scheduler-under-UEFI hypothesis, Build artifacts table extended through v1.30.7, Cyrius pin corrected 5.11.53 → 5.11.59, ecosystem block single-pin retirement, In-flight + Recently closed rebuilt); **S3** agnos `doc-health.md` full re-bucket (22 → 25 files, v1.28.4 → v1.30.7 era, forward-policy commitment to refresh on every minor cut); **S4** agnosticos `README.md` + `architecture.md` lib-doc precedent sweep (Stack-table Version column removed, ASCII diagrams stripped of version cells, Named Subsystems / Technology Stack / Design Decisions defer to state.md + shared-crates.md). Follow-ups: agnos `roadmap.md` 1.30.x arc-recap insertion (replaced stale "scheduler-under-UEFI" framing); agnos `README.md` "Size Comparison" lib-doc precedent; agnos `architecture/overview.md` v1.27.2 → v1.30.7 header + Path-C boot-flow rewrite + memory-map size-cell elision; agnos `kybernet-bridge.md` + `syscall-additions.md` one-line bumps. Earlier 2026-05-17 sweep: 1.30.4 xHCI Linux-diff hardening closeout + 1.30.5 Phase 4/5 USB HID-boot keyboard driver landed + Attempt 54 prep + Phase 1-5 status block + planning/usb-hid-keyboard-driver / planning/shared-crates row.) | **Refresh cadence**: when docs are touched, update the affected row.
...
| `monolith-extraction.md` | 2026-04-17 | 🗄 Archived | **Moved to `docs/archive/` on 2026-05-12** — historical milestone, extraction completed 2026-04-01. Live work tracked in `state.md` + `iron-nuc-zen-log.md`. |
...
| `iron-bring-up-process.md` | 2026-05-15 | ✅ Fresh | **NEW 2026-05-15** — generic process example for bringing AGNOS up on a new hardware target. Distilled from the NUC AMD Zen arc (Attempts 1–29) post-cycle-close. Patterns: diagnostic-channel ladder, repair-letter convention, premise-audit gate, MVP-to-X gap re-audit, walls common to UEFI x86_64 targets, Cyrius-side gotchas. Companion to `iron-nuc-zen-log.md` (the canonical example arc). Read alongside `uefi-boot-prior-art.md`. |
| `iron-nuc-zen-log.md` | 2026-05-18 | 🔴 In-flight | NUC AMD (Beelink SER / archaemenid) target. **~6800 lines, Attempts 1–62** (was 52 at last touch). 2026-05-18: Attempt 62 (NN+OO bundled, falsified) entry appended; vendor-cap research dry-well result logged; MSI-X audit divergence + Attempt 63 (QQ + QQ'') prep with line-by-line edit audit + pre-bound outcome grid appended. Phase 3 silent-absorb arc closed Attempt 55 (Repair EE — root cause: `xhci_portsc_write` inner re-mask stripping RW1S PR bit, NOT a "non-spec gate" as Attempt 52 framing suggested). Phase 4 cmd-path silent-absorb open after 9 falsified spec-path repairs FF→OO. Photo dir: `iron-nuc-zen-photos/`. Previous touch 2026-05-17 added handoff/AMD-quirk audit + H1-H4 spec-discipline closeout + Attempt 54 prep. |
...
| `xhci-prior-art-audit.md` | 2026-05-18 | ✅ Fresh — **NEW** | **NEW 2026-05-18** — convergent-prior-art audit doc for the xHCI cmd-path silent-absorb arc on AMD FCH 1022:1639 (archaemenid). Four-source diff (Linux v6.13 + FreeBSD HEAD + Haiku master + EDK2 master) of register-write ordering between reset-complete and R/S=1. Surfaced two convergent divergences (ERDP-before-ERSTBA per 3-of-4; CRCR-after-IMOD per 2-of-4) staged as Repair (NN) for Attempt 62 (falsified). Companion to `iron-nuc-zen-log.md` § Attempts 56-62. Replaces "stack the next letter on symptom dictionary" with "diff against four independent impls; bundle only what ≥2 of them agree on" — the pattern that broke the FF→MM letter ladder. |
| `planning/usb-hid-keyboard-driver.md` | 2026-05-17 | 🟠 Plan complete | **All 5 phases landed** across agnos 1.30.0–1.30.5: Phase 1 (PCIe discovery) → Phase 5 (HID-usage→PS/2 translation + `kb_buf` feed). Iron-side blocker remains: archaemenid USB2 silent-absorb gates Phase 3 enumeration completion, so Phases 4/5 dormant on this hardware (compile-verified; QEMU xhci-pci is the active validation surface). Iron-progress tracking moves to `iron-nuc-zen-log.md`; this doc keeps the architectural-decision record. Status header refreshed 2026-05-17. |
