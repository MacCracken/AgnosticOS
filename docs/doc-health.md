...
>
> **T2** held-outline mechanical drift on `articles/why-gigacenters.md` — "Last verified" date 2026-05-06 → 2026-05-22; status-line extended with body-figures-pinned + state.md pointer + iron-validation summary (storage trio + networking) + thesis-survives-larger-kernel note. Body left alone (held outline; receipts still pending murti+seema demo).
>
> **T3** structural extension on `articles/doom-in-cyrius.md` — added "Extension — 2026-05-22" subsection after the 2026-05-09 footer covering Cyrius v6.0.x landing (cyrc → cybs / cc5 → cycc rename ceremony), AGNOS MVP gate at Attempt 68 (cyrius gvar-init lesson w/ pointer to kernel.cyml/the_mvp_gate_at_attempt_68), storage-trio iron debuts (NVMe/AHCI/USB-MS), networking arc in flight (Attempt 92/93 DHCP gate fix verified). **Mid-edit correction per user feedback** — cyrius-doom is NOT at 5.7.48 anymore: pin graduated to 6.0.1 at v0.27.3 (2026-05-21), with Result<T,E> adoption at WAD IO/parse boundary + first use of v5.8.x sum types in doom's own code. Sprint 3 (Black Book v1.0 audit) still gated, but on typed-error rollout + optimization arc, NOT pin graduation.
>
> **T4** structural extension on `articles/sovereign-compiler-vs-brute-force.md` — added "Extension 2026-05-22 — v6.x landed, kernel iron-validated, MVP gate carries forward" subsection after the 2026-05-15 footer. Closes the "young kernel" framing line in the body — kernel now mounts real Linux ext4 partitions on real silicon, MVP gate green across 25 iron burns. Multi-source convergent audit pattern from feedback_redesign_dont_reinvent codified as shippable artifact (audits land BEFORE iron burns).
>
> **T5** structural extension on `articles/the-2-dollar-sd-card.md` — added "Extension 2026-05-22 — kernel on iron, storage + networking with the receipts" subsection. Three iron-validated milestones (MVP gate Attempt 68, storage trio cycle Attempts 80-91, networking cycle Attempts 92-93 in flight) + Cyrius v6.0.x landing. The Dandelion Core claim from the original article ("fits on a $2 SD card") no longer hypothetical — every layer between the SD card and the typed shell prompt has receipts.
>
> **T6** + **T7** — **two new vidya field_notes entries** in `/home/macro/Repos/vidya/content/cyrius/field_notes/kernel.cyml` (file grew 667 → ~960 lines):
>
> - **`the_mvp_gate_at_attempt_68`** (~210 lines) — bridges from `shell_on_iron`'s Attempt 29 cutoff through the xHCI silent-absorb phantom arc (Attempts 30-67, repair letters F-N) to the cyrius gvar-init-order discovery in Repair (O) cleanup pass and the function-wrapped banner pattern that fixed it. Codifies `feedback_stop_letter_laddering` (write the escape plan BEFORE the next falsification at ~5-letter depth, not after) + `feedback_cyrius_hands_off` (workaround in consumer, file upstream issue, never block on producer cadence) + the MVP-gate-stays-green discipline (25 consecutive iron burns without regression).
>
> - **`the_storage_trio_iron_debut`** (~280 lines) — covers Attempts 80-91 (1.31.x cycle): NVMe iron debut (one-shot clean), AHCI/SATA carry-forward triplet (Attempts 81 PARTIAL → 82 PASS), USB MS five-attempt arc (83-86 falsified → 87 PASS via Phase 2.8 eight-bug repair stack), no-regression burns 88+89, ext4 victory laps 90+91 (first end-to-end real-filesystem read on iron via mkfs.ext4-written NVMe partition). Codifies the multi-source convergent audit pattern per `feedback_redesign_dont_reinvent` (Linux + FreeBSD + OpenBSD + NetBSD + Haiku + spec — audit lands BEFORE iron burn as shippable artifact) + the AGNOS kernel-growth rule per `project_agnos_kernel_growth_rules` (kernel grows per native workload, never to chase Linux ABIs).
>
> **Deliberately deferred this stage**:
> - **`r8169_and_the_dhcp_gate`** field_notes entry — premature; networking arc is in flight at Attempt 94+ PENDING IRON. The lesson ("the next layer down is always there waiting; gate predicate bugs hide driver-level questions") is fully present but the arc hasn't closed. Lands at 1.32.1 cycle close.
> - **Most articles' "Since This Was Written" footers** — only the kernel-most-affected three articles got structural extensions (doom-in-cyrius, sovereign-compiler-vs-brute-force, the-2-dollar-sd-card). Other articles (methodology-is-the-trap, what-justifies-a-stdlib-foldin, the-price-of-porting-early, port-ledger-volume-2, etc.) have moving parts but the cyrius v6.x + AGNOS iron arc is the dominant new material — folded into the three kernel-thesis articles where it lands cleanest. Deferred articles get refreshed when their specific arc moves (e.g., what-justifies-a-stdlib-foldin updates with the next foldin landing).
>
> Prior refresh entry (kept for traceability):

> **Prior**: 2026-05-21 PM (post-kernel-week sweep, eight touchpoints — covers the 1.31.1→1.31.4 delta after a week of kernel-focused work): **T1** `README.md` — kernel badge `1.30.7 → 1.31.4`, Cyrius badge `5.11.59 → 6.0.1`, Path-C labelling retired from milestone row (sovereign UEFI is the only UEFI path now; nothing to disambiguate). **T2** `docs/AGNOS.md` — full version-cell sweep: kernel `1.30.5 (~365KB) → 1.31.4 (~510KB, open cycle)`, compiler `cc5 5.11.55 → cycc 6.0.1` (v6.0.0 cycle-open rename cybs/cycc 2026-05-19), bootstrap chain shortened (`seed → cyrc → bridge → cc5` retired; bridge step eliminated at cyrius v5.11.66, current chain `seed → cybs → cycc`), kernel subsystem table extended with NVMe / AHCI/SATA / USB-MS / GPT / VirtIO-modern / RAM-disk + iron debuts (Attempts 68/80/81/87), Technical Statistics dated 2026-05-21, Last-Updated footer refreshed. **T3** `docs/applications/gnoboot.md` — version `0.1.0 (2026-05-13) → 0.4.2`, Cyrius pin `5.11.53 → 6.0.1`, Iron Attempt 5 pending → MVP gate cleared at Attempt 68 + storage debuts 80/81/87, status section restructured (MVP done, FB-handoff observability done, Quiet-Boot scanout parked), Path C plan reference relabelled to "Sovereign UEFI plan" (file literal preserved). **T4** `docs/architecture.md` — Last Updated `2026-05-18 → 2026-05-21`; stripped redundant Version stamp. **T5** `docs/development/README.md` — Last Updated `2026-05-09 → 2026-05-21`; index extended with two new sections — *Iron Boot Bring-up* (iron-bring-up-process.md, iron-nuc-zen-log.md, iron-nuc-zen-log-mvp.md, iron-nuc-zen-photos/) and *Prior-Art Audits + Kernel Plans* (9 docs: path-c-sovereign-uefi, uefi-boot-prior-art, xhci-prior-art-audit, true-font-swap-plan, ahci-iron-burn-audit, usb-ms-iron-burn-audit, msc-reset-recovery-prior-art, ramdisk-virtio-modern-prior-art, path-a-elf64-multiboot2-archived). **T6** `docs/philosophy.md` — bootstrap chain at line 76 updated `seed → cyrc → bridge → cc5 (self-hosting)` → `seed → cybs → cycc (self-hosting)` per cyrius v5.11.66 chain shortening + v6.0.0 rename. **T7** Archive — `docs/development/planning/usb-hid-keyboard-driver.md → docs/archive/usb-hid-keyboard-driver-shipped.md` (git mv, archive notice prepended). All 5 phases shipped + MVP gate cleared on iron at Attempt 68 (2026-05-18, agnos 1.30.9); planning doc was fossil. `planning/README.md` Kernel-scope row repointed to archive + note about prior-art-audit pattern superseding the explicit kernel-plan table. **T8** scripts/build.sh — `Path-C` labelling retired from comments and `Boot:` echo (sovereign UEFI is just "the UEFI path" now). **Tier 3 (articles)** deliberately skipped this sweep per user. Earlier 2026-05-20 sweep (post-1.31.1 code-complete, T1-T4 there): preserved below for audit history.

Prior refresh entry (kept for traceability):

> 2026-05-20 sweep — post-1.31.1-code-complete, four touchpoints: **T1** agnos `state.md` full body sync (v1.30.7 → v1.31.1 era — Open Investigation rewritten with MVP-gate-cleared / NVMe-iron-debut framing + parked AMD Zen scanout pin + AHCI iron-burn-audit pointer, Build artifacts table extended through 1.31.1 with bookmark cuts only, Subsystem status expanded 35 → 40+ rows incl. NVMe / AHCI / GPT / block-layer dispatch, Source rollup 49 → 66 .cyr files + 6,306 → 15,048 lines, Ecosystem block updated for cyrius 6.0.1 / kernel-pin 5.11.x bedrock, In-flight table rebuilt with AHCI iron debut #1 / AMD Zen scanout #2 / SMP AP wakeup #3 / Process derive #4, Recently closed catalogs 1.30.8 → 1.31.1 inclusive, closing "What changed" rewritten); **T2** agnos `roadmap.md` body sync (Current header subsystem count + gnoboot version + iron-validation framing refreshed; Shipped arc table adds rows for v1.30.8 / .9 (MVP GATE HIT) / .10-12 / 1.31.0 / 1.31.1; 1.30.x Arc Recap reframed "iron-validated except CCE gate" → "MVP gate hit at Attempt 68 via cyrius gvar-init-order fix"; Next-cycle table replaces xHCI #1 with AHCI iron debut audit + adds USB MS / ext2 as planned-1.31.2/3; 1.31.0-KASLR-Option-A section reframed as "slot TBD on cyrius v6.1.x PIE track" with second slot-move documented; Security Hardening note aligned); **T3** agnosticos `CLAUDE.md` Standalone Repos table — added bannermanor / hapi / darshana rows (Native, 2026-05-20 v6.0.x leading-edge cluster); trimmed mabda version-specific note per intentionally-version-free convention; **T4** new doc `ahci-iron-burn-audit.md` drafted (11 sections covering Phase 1-4 risk surface, LBA-5 sentinel write danger byte-for-byte, AHCI_RW_DEMO compile-gate mitigation proposal matching KTEST/XHCI_VERBOSE pattern, expected iron output, CMOS checkpoint trail 0x4B → 0x50). Cross-linked from agnos CHANGELOG [Unreleased] § AHCI Phase 4 + agnosticos `state.md` 1.31.1 closing handoff. Earlier 2026-05-18 sweep (xHCI Enable Slot CCE framing): preserved below for audit history. | **Refresh cadence**: when docs are touched, update the affected row.
...
|---|---|---|---|
| `README.md` | 2026-05-18 | ✅ Fresh | **Refreshed 2026-05-18 (doc-staleness audit S4 — lib-doc precedent sweep)**: Stack-table Version column dropped (defer to state.md + shared-crates.md), Architecture ASCII diagram version cells stripped, badges bumped (Kernel 1.30.5 → 1.30.7, Cyrius 5.11.55 → 5.11.59), Security section version cells removed (defer to state.md), Milestone table reflows (sovereign boot pipeline → Path-C UEFI handoff via gnoboot; security stack → kavach/sigil/libro/aegis at v1.0+). Previous refresh 2026-05-06: badges, architecture diagram, stack table, security versions, beta target. |
...
| `docs/architecture.md` | 2026-05-18 | ✅ Fresh | **Refreshed 2026-05-18 (doc-staleness audit S4 — lib-doc precedent sweep)**: header bumped to 2026-05-18 / 2026.5.18; ASCII kernel-block dropped version+size cells; Named Subsystems section heading-versions stripped (`### ark — Unified Package Manager (v0.8.0, Cyrius)` → `### ark — Unified Package Manager`); Technology Stack table Kernel + Compiler rows defer to state.md for version + size; Pending Cyrius Ports table version column removed; Boot Flow updated for Path-C / gnoboot; Design Decisions section version cells stripped; aegis status bumped (scaffold → Cyrius-native 1.0+, May 2026). Previous refresh 2026-05-09: kernel-layers content inlined; 2026-05-06: initial. |
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `roadmap.md` | 2026-05-17 | ✅ Fresh | **Touched 2026-05-17**: Phase 1-5 status table refreshed from stale "Attempt 30 burn pending" baseline to actual state (Phases 1-3 landed + iron-validated, Phase 3 iron-blocked by silent-absorb arc / 12 hypotheses falsified, Phases 4-5 landed in 1.30.5 + QEMU-validation-surface, Phase 5 closes the typeable gate). Stale "1.30.1 ships when" gate replaced with iron-cadence framing pointing at `iron-nuc-zen-log-mvp.md` § Attempt 54 prep. 1.30.x section header retained as cycle-arc framing. |
| `state.md` | 2026-05-18 | ✅ Fresh | **Refreshed 2026-05-18 (FF→QQ arc + audit-result framing)**: header rewritten three times across the day (NN+OO bundle falsification → vendor-cap dry well → MSI-X audit divergence found → QQ candidate staged). Cycle slot 8 updated for Attempts 56-62 cmd-path arc; QQ + QQ'' MSI-X table programming staged but not yet burned; PP heavy-hammer DMA UC remap as bottoming-out path. Cyrius 5.11.59 active. Previous refresh 2026-05-17: agnos 1.30.4 H1-H4 + 1.30.5 Phase 4/5 + Attempt 54 prep. |
...
| `planning/shared-crates.md` | 2026-05-17 | 🟡 Stale | **Last full refresh 2026-05-09** (12 version bumps + darshana / cyim-lsp added). Touched 2026-05-17 for agnos 1.30.5 row only. Larger pin-cluster drift still pending per `state.md` 2026-05-11 eve snapshot: agnostik 1.2.0→1.2.2, agnosys 1.2.1→1.2.6, sigil 3.1.0→3.1.1, sankoch 2.0.0→2.2.5, libro 2.0.5→2.6.3, sandhi 1.3.0→1.3.4, niyama 1.0.1→1.0.2, aegis 0.8.2→**1.0.0**, cyim 1.6.7→1.7.0, chakshu 0.2.0→0.3.0, darshana 0.2.0→0.3.0, **+ argonaut 1.7.0 + kybernet 1.2.1**. Schedule full sweep next time this doc is touched. |
