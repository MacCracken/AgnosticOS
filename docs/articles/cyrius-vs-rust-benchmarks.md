...
**Pure compute gap**: Branch-heavy operations show 2–42× overhead vs LLVM -O3. The compiler-optimization arc (O1–O6) shipped continuously through v5.6.x → v5.7.x → v5.8.x. O1 (instrumentation + FNV-1a) v5.6.0–v5.6.4; O2 (five peephole categories) v5.6.11; O3a IR instrumentation v5.6.12; linear-scan regalloc default-on v5.6.20–v5.6.24; O4a/b/c regalloc + Poletto-Sarkar linear-scan picker through v5.7.x–v5.8.x. **O5/O6 codebuf compaction (NOP harvest with jump+fixup) referenced through v5.8.x with status sweep deferred** to v5.11.x carry-forward triage. v5.9.x ran a 44-patch catchup + niyama-fold cycle (closed 2026-05-08); **v5.10.x is the REAL TYPE SYSTEM arc** (per-phase compile-time profiling instrumentation + cstring/Result/Option/Tagged vocabulary + call-site type checking — 24 patches in 2 days, in flight at v5.10.24). Bare-metal AGNOS + RISC-V rv64 reservation **moved to v6.x** at the 2026-05-12 tight-close decision (was v5.10 → v5.11 → v5.12 → v6.x as foldin and type-system work compounded; v5.12.x retired). v5.11.x is now the final 5.x minor (close pair at v5.11.68/.69); v6.x opens the platform-expansion arc.
...
**Refreshed 2026-05-15.** Body numbers above are the original snapshots from each port's initial measurement; current per-repo `bench-history.csv` files are authoritative for current values.

- **v5.9.x catchup arc closed 2026-05-08** — 44 patches in 3 days. Niyama fold (5 regex engines, 6,664 lines, 7 modules) shipped at v5.9.0; remainder ran consumer-rollup (agnosys/vyakarana/sandhi/cyim/agnostik/owl pin-lag bands collapsed). aegis graduated 0.1.0 → 0.8.2 (and then to **1.0.0** in v5.10.x); **darshana** (TTY/raw-mode primitives, दर्शन — *viewing*) extracted from cyim's `src/tty.cyr` when chakshu became second consumer.
- **v5.10.x closed 2026-05-11 at .50** — 50 patches in 5 days. Three completed compiler arcs: **typed-simd ABI** (11 phases — value-form f64v2/f64v4 with ABI-aware register routing, the substrate for future Cyrius-native codec work), **REAL TYPE SYSTEM** (5 phases, Phase 2 call-site type checking landed v5.10.24 via per-fn param-type bitmasks; canonical-motivator stdlib fns annotated), **struct-byval ABI** (3 phases, cross-backend return surface). Plus 2.7× compile-time-perf miniarc (.40+.41). cc5 binary 741,048 → **804,472 B** across the cycle.
- **v5.11.x in flight — 55 patches across 3 days** (2026-05-11/12/13). Stdlib annotation arc + consumer-issue closeout. Includes the ELF section-header fix arc (.29/.30/.31 — GRUB's `e_shoff=0` rejection on first iron-boot attempt) and Path-A→Path-C transition support (.43-.55 enabled the sovereign UEFI bootloader). cc5 at **809,200 B** at v5.11.24 baseline (.25 → .55 delta not snapshotted here; pull from `cyrius/cc5` for live size).
- **Path C sovereign UEFI handoff shipped 2026-05-13.** GRUB-multiboot2-EFI hit OVMF 2024+ strict-W^X (self-patching `.text` in `grub_relocator64_efi_boot` — Linux distros use `linuxefi` and don't trip this). Pivoted same-day to **gnoboot** — Cyrius-native PE32+ EFI Application, 80-byte sovereign boot-info struct in RDI. Industry-converged shape (Linux EFI stub / FreeBSD `loader.efi` / OpenBSD `BOOTX64.EFI` / Windows `winload.efi` / Limine all do variants of the same).
- **AGNOS kernel iron-validated 2026-05-15** on NUC AMD Beelink SER — **1.30.0 boots to shell on the framebuffer** through gnoboot 0.2.0. Twenty-nine attempts across three weeks (full arc: [`development/iron-nuc-zen-log-mvp.md`](../development/iron-nuc-zen-log-mvp.md); distilled process pattern: [`development/iron-bring-up-process.md`](../development/iron-bring-up-process.md)). 1.30.1 cycle opened same-session — xHCI Phase 1 (PCIe discovery + capability reads) landed; Phases 2–5 (controller init + port enum + HID boot protocol + interrupt-driven kb_buf feed) are the cycle's substantive work to close MVP gap #3 (USB keyboard input on modern UEFI without SMM PS/2 emulation).
- **v6.x reserved** for bare-metal AGNOS target + RISC-V rv64 backend (was v5.10 → v5.11 → v5.12 → v6.x; v5.12.x retired 2026-05-12 at the tight-close decision). v5.11.x is the FINAL 5.x minor.
- **Optimization arc carry-forward**: O5/O6 codebuf compaction referenced through v5.8.x; status sweep triaged into v5.11.x. Live benchmark deltas vs the v5.6.x → v5.11.x progression should be re-pulled per port from `bench-history.csv` before quoting.
