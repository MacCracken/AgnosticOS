...
The ledger is a **five-volume arc**, then an epilogue. The arc grew from three to five as the work it documents grew: a kernel that now boots on iron, four new native subsystems, an optimization arc that's still in flight rather than closed, and platform-expansion work (v6.x: bare-metal, RISC-V) that's its own measurement frontier. Three volumes would have crammed too much under each header; five volumes give each measurement-shift its own snapshot.

- **Volume 1 — early baseline (this).** Cyrius **v5.5.4**, ten ports, pre-optimization-arc, 17-days-old language. Frozen.
- **Volume 2 — mid-arc state of things ([Volume 2](port-ledger-volume-2.md), 2026-05-15).** Cyrius v5.10/v5.11 era. **Kernel iron-validated** at agnos 1.30.1. Project pin-cluster review across the 5.10.44 bedrock, 5.10.x warm cluster, 5.11.4 post-burst cluster, 5.11.55 leading-edge pair, held-cluster v5.7.48 tail, deep-lag v5.1.x tail. Four native subsystems shipped without Rust antecedents (aegis 1.0.0 graduated, gnoboot 0.2.0, commandress 0.1.0, kriya 0.1.0). **NOT a comprehensive Volume-1-comparison re-measurement** — explicit "state of things, mid-arc" snapshot. Volume 1's "Where Rust Still Wins" gaps are reviewed for *direction of motion* without claiming closure.
- **Volume 3 — end-of-5.x / v6.0 re-measurement.** The full Volume 1 comparison. By this point: the optimization arc has been closed (O5/O6 codebuf compaction sweep done, REAL TYPE SYSTEM matured, all v5.10.x ABI work standing for a full release cycle), v5.11.x has closed at its final patch, and either v6.0 has cut or the end-of-5.x snapshot is the natural endpoint. The four "Where Rust Still Wins" categories from Volume 1 either close, narrow with specific reasons, or persist with named ones. Pin-lag tail collapsed; held-cluster v5.7.48 resolved one way or another.
- **Volume 4 — post-v6.x platform expansion.** RISC-V rv64 backend live, bare-metal target formalized, PIE / closures / language-level async / Class B FFI fold all behind us. New measurement surfaces appear: cross-architecture comparison (x86_64 vs aarch64 vs rv64 same binary path), bare-metal vs hosted comparison, the Cyrius compiler's own performance on its new targets. Ports that picked up new capabilities under v6.x get their post-v6 numbers; new native subsystems that landed in the platform-expansion window get their first measurements.
- **Volume 5 — full ecosystem ledger + synthesis.** Every port with a Rust antecedent closed and measured. The OS Independence story (self-hosting LFS, Public Beta) is in scope. Native subsystems (hadara, takumi if it lands native, aethersafha, mela, agnova, samay, seema) documented alongside. The synthesis of where Cyrius stands relative to the ecosystems it converged with, diverged from, or exceeded.
- **Epilogue — what the five volumes prove.** Retrospective. Where the gaps are that Rust still holds. Where Cyrius has pulled into territory Rust cannot reach without external toolchains (RISC-V, bare-metal, self-hosting from 29 KB). The synthesis of the syntheses.

The deliberate choice across the arc: **each volume locks its measurement era and stays still.** Volume 1 captures 29-KB-seed-week numbers honestly; Volume 2 captures mid-arc state honestly; Volume 3 captures post-arc honestly; Volume 4 captures post-platform-expansion honestly; Volume 5 closes. Anyone reading Volume 1 today gets the baseline; anyone reading Volume 2 gets the state-of-things; anyone reading Volume 3 gets the optimization-arc verdict. No volume rewrites the others — they sit alongside, each a marker. The accreted "Status as of 2026-05-X" footnotes that were drifting through earlier drafts of this body have been removed; they belong in their respective volumes, not in mid-paragraph footnotes.
