...
- **Bare-metal + RISC-V rv64 reservation moved to v6.x** (was v5.10 → v5.11 → v5.12 → v6.x; **v5.12.x retired 2026-05-12** at the tight-close decision). v5.11.x is now the FINAL 5.x minor, closing as a .68/.69 pair (heap-map full reorganization at .68 + optional dep-fold tag at .69). v6.x opens the platform-expansion arc: bare-metal target formalization + RISC-V rv64 + PIE + closures + language-level async + Class B FFI fold. The framing matters: **v5.x = "what the language IS" (frozen feature set); v6.x = "what the language gains" (new capabilities)**.
