...
As of August 2026: the kernel is at v1.56.x (40+ subsystems, base kernel-internals essentially complete, current head in state.md, **MVP gate hit on iron at v1.30.9 — typeable shell on archaemenid Beelink SER AMD Renoir 2026-05-18**; storage arc closed v1.31.6 with real Linux ext4 mounted on NVMe). The iron-validated arc has since grown far past first-shell: networking, ext4 extent allocation, JBD2 crash-safe journaling, exec-from-disk, the userland-shell separation (agnoshi, a ring-3 shell loaded from disk), graphics with **DOOM playable in-game on real hardware**, multi-threading with preemptive scheduling and SMP, HDA audio (**DOOM-with-sound out the analog front jack, 1.52.x**), kernel FP/SIMD (**real f64 in ring 3, 1.53.x**), GPU compute on the Cezanne shader cores with no amdgpu and no ROCm (**f64 matmul bit-identical to the CPU reference, 1.54.x**), display scanout on DCN 2.1 and a sovereign ATOM BIOS interpreter bit-correct on iron, ACPI S5 self-poweroff (**the machine powers itself off, 1.55.x**), and native 2560×1440 scanout with a hardware-panned console (**1.56.36–38**) are all iron-validated on archaemenid. On 2026-08-03, at four CPUs online, the aethersafha compositor hosted two real client windows on archaemenid's panel — 278 frames, keys delivered, clean quit. The compiler is at v6.5.x (cycc self-hosting from a 29 KB seed; multi-platform byte-identical across x86_64 Linux, aarch64 Linux, Apple Silicon Mach-O, Intel-Mac x86_64 Mach-O, Windows PE32+). Forty-plus subsystems have been ported from Rust to Cyrius. The boot pipeline is sovereign Cyrius. The sovereign UEFI loader (gnoboot) replaces GRUB. The floor is real, and it boots — on QEMU and on iron.
...
1. Kernel         — AGNOS kernel (Cyrius-native, 40+ subsystems)
...
The swallow stage is the **inclusion bet**: rather than demanding the user abandon every Windows or Linux binary they rely on, AGNOS becomes capable of hosting them inside a sovereign compat sandbox — **mirshi**, a userland syscall-translation supervisor on the pico-process model, with **mehman** as the compat surface backend. The compat layer is permanent. The kernel never absorbs foreign ABIs. The sandbox is the boundary, and the boundary holds.
