...
- **B4 — honor peer SND.WND.** ✅ **LANDED (agnos 1.35.1) — closes the arc.** Peer window captured from every inbound segment (top of `net_handle_tcp`, so the SYN-ACK's initial window counts); `tcp_send_chunk` sizes each segment `min(remaining, MSS, window)`, zero-window → 1-byte persist probe (RFC 9293 §3.8.6.1) re-driven by the B2 timer. **Validated**: `tcp-smoke.sh` `tcp: wnd PASS` (chunk-sizing math, hermetic) + `tcp-listen-smoke.sh` 2/2 (the full banner ships, which now depends on a non-zero captured window).

**Arc complete (agnos 1.35.1):** B0→B4 landed; the minimal SYN/ACK/FIN machine is now a reliable, flow-controlled byte stream. Deferred items below remain a follow-on.
