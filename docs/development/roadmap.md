...
- **Networking-comms continuation** (user order, 2026-05-27): **TCP hardening is the active 1.35.1 cycle** (retransmit + RX window + MSS on the minimal SYN/ACK/FIN machine — the reliable-`ark`/`nous`-fetch enabler; audit-doc-first), then **NTP/SNTP** time sync. **TLS is out of scope here** — a cyrius-stdlib concern the user drives with the cyrius agent. (HTTP client rides on TLS, so it follows.)
