...
> **Status**: Pre-Beta — Closed Beta targeting **late August 2026**, preceded by a **founder solo-dogfood month** (~July/early-August: founder lives on the system daily + runs the Docker server-sweep to find weak points *before* anyone else is let on) · public beta + GA still trail (see ladder below) | **Last Updated**: 2026-06-29 (beta rescoped to late-August-after-a-solo-month; body refreshed to agnos 1.50.6 — the 1.46→1.50 kernel arcs all iron-validated; today's compositor-backend scaffolds + the kavach↔agnos audit folded in). This roadmap is **forward-facing** — shipped arcs live in [`state.md`](state.md) + the per-repo CHANGELOGs, not re-narrated here. Refer to state.md for current kernel / Cyrius / per-repo versions.
>
> 🔴 **BETA RESCOPED (2026-06-29)**: the whole **summer-2026 arc stays CLOSED beta**, now with a deliberate **founder-solo-month gate in front of it**. **Phase 0 — founder solo-dogfood month (~July / early-August)**: the founder **spends a month living on the system daily** + runs the **Docker AGNOS server-sweep** (BBS / MUD / remote-shell / web / `ark`+`nous` server-side — sweep-test to find the weak points; server workloads need no humans-at-a-screen). This is the "I trust it enough to put others on it" window — confidence + hardening receipts *before* exposure, not a tester phase. **Phase 1 — closed beta opens late August 2026** (was Late June/Early July; pushed to give the solo month room): brings in **external testers** (invite-only / closed, NOT public) — GUI / daily-driver workloads genuinely need humans at the screen, so this is where the cohort lands. **Public beta is DEFERRED to post-summer** (the ~Q4 2026 window holds): formal public enrollment + third-party security audit + community program open only after the closed beta proves out. *(Supersedes the 2026-06-14 rescope, which opened closed beta Late June/Early July with a separate desktop-external-tester phase mid-late summer; collapsed here into one late-August external-tester opening behind the solo month, per user 2026-06-29 — "spend a month myself with the system before letting others on.")*
>
> 🟢 **MVP GATE CLEARED on iron** — Attempt 68 / agnos 1.30.9 / 2026-05-15: kernel + kybernet (PID 1) + agnoshi reach a **typeable shell** on archaemenid (NUC AMD Beelink SER). Since then the kernel shipped the storage stack (NVMe / AHCI / USB-MS / RAM-disk / GPT), the **r8169 networking stack** (iron-COMPLETE — DHCP real lease), **ext2/4 + FAT-family read+write** + the crash-safe FS stack (extent→jbd2→VFS), **exec-from-disk + VFS routing** (1.40), the **shell→userland-agnsh separation** (1.41, iron burn `14115`), **graphics + DOOM** (1.43, in-game on iron, burn `1439`), **multi-threading / preemptive scheduling** (1.44), the **1.45.x TLS → HTTPS → net-tools** arc (ring-3 net/socket syscalls #45-#57 — client + server `sock_listen`/`accept`), **SMP / true preemptive-on-iron** (1.46 — iron-validated; the IF=1 `#GP` was the AMD SYSRET SS-RPL bug, fixed one byte), **fault-resilience + per-process fd tables** (1.47), **FAT/exFAT perf** (1.48), **full RAM + loopback + socket-as-VFS-fd** (1.49 — epoll-able sockets, the event-loop keystone), and **RAM full-usage + boot-CR3→own-PML4** (1.50 — iron-validated 2026-06-29). **Iron: boot-to-shell with a live multi-command keyboard on real Zen; >256 MB RAM + >1 GB mmap arena online.** Kernel head **agnos 1.50.6**. Current kernel + active cycle: [`state.md`](state.md).
>
...
| Stage | Capability content | Status (2026-06-29) | Exit trigger |
|---|---|---|---|
...
| **base** | Kernel solid; ext4 read+write; ark/nous package manager working end-to-end (resolve / fetch / install / remove) **on the *sovereign* path** (zugot→takumi→native `.ark`); AGNOS-side update mechanism; enough soak surface for real-workload exposure without weekly showstoppers. **⚠ ark caveat:** ark 1.0.0 is Cyrius-*ported* but its `SOURCE_SYSTEM` leg still wraps `apt-get` (Linux-host transitional, "a product of the switch") — and apt doesn't exist on agnos, so genuine on-agnos package mgmt ≡ **sovereign ark**, a **v2 long-horizon goal** that's gated on agnos first exposing the surface it binds to (FS-write ✅ / exec ✅ / HTTPS-fetch 1.45.x / the takumi self-host build surface = server-stage). Without that kernel surface, sovereign ark has nothing to run on. The 1.45.x "ark-fetch" milestone = fetch-of-native-`.ark`-over-HTTPS, not full package-mgmt-on-agnos. | **Current.** The base-defining kernel work is **iron-validated**: ext2/4 + FAT-family read+write (1.33.x/1.34.x), the FS-crash-safe stack (extent→jbd2→VFS, 1.37–1.39), and **exec-from-disk** (1.40.x through 1.40.13, iron-validated 2026-05-31 — ring-3 exec + FAT verbs with ext2 at `/`). **Shell separation** (1.41.x — recovery-only kernel REPL, agnsh runs in ring 3 from disk) is **iron-complete** (burn `14115`, 2026-06-06 — agnsh types/echoes/dispatches on archaemenid); the **1.42.x** perf+hardening arc + the **1.43.x** graphics/userland arc (execwait #37 → userland `run`; FB-console ANSI/SGR color; `fbinfo`#38/`blit`#39 framebuffer; `uptime_ms`#40/`sleep_ms`#41 timing — culminating in **DOOM rendering on AGNOS at 1.43.6**, the first real userland app: cyrius-doom 0.28.2 blits the title screen in ring 3) have landed (QEMU-validated via `doom-smoke.sh`, ride the next iron burn), with the first AGNOS-tic userland tools (`bnrmr`/`cmdrs`/`klug`/`anuenue`) on `/bin` and **agnoshi 1.4.5** (`verb_abspath` — `ls`/`ls .` see the FS) ([`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) #tracker-143x-cycle). **1.44.x** (multi-threading / preemptive scheduling — schedulable agnsh `&` jobs, SMP-AP wake+park, per-proc env) is **ARC COMPLETE**, and on iron (2026-06-13/14) AGNOS reached **boot-to-shell with a live multi-command keyboard on real Zen** (xHCI keyboard-ring fix); the **1.45.x** net/TLS arc landed, and the kernel then ran the full **1.46→1.50** sweep (SMP/preemptive-on-iron · fault-resilience + per-process fd tables · FAT perf · full RAM + loopback + **socket-as-VFS-fd** · RAM full-usage + boot-CR3→own-PML4 — all iron-validated by 2026-06-29, head **1.50.6**). **The base-DEFINING kernel-internals are now essentially complete** — the audit lens (planning/kavach-agnos-compat.md) confirms base's *remaining* work is the **ecosystem half**, not the kernel: the **ISO Stage-4 cut**, the **sovereign-ark path** (`ark` v2 — see [`planning/ark-v2-sovereignty-path.md`](planning/ark-v2-sovereignty-path.md)) + an AGNOS-side update mechanism, and soak. Triggers archaemenid dual-boot migration (AGNOS-primary on internal NVMe + Linux on SATA — see [`state.md`](state.md) § *archaemenid migration*). | Native installer + server ecosystem landing. |
| **server** | **agnova native installer** (AGNOS installs itself onto target hardware; install-usb.sh host-side script retires); non-desktop subsystem suite (BBS, MUD, sovereign remote-shell, web server, ark+nous server-side) + the libs they consume. **Most of "Linux's purpose" gets absorbed at this stage** — archaemenid Linux usage is predominantly server-flavored (build hosting, file serving, CLI dev, network services), so server-stage AGNOS replaces it without needing GUI work. Archaemenid Linux eviction lands at **server**, not "swallow." | **Foundation materializing.** 1.32.x server-side TCP primitives (bite A: `tcp_listen`/`tcp_bind`/`tcp_accept`) are the kernel base; **1.45.x** added the ring-3 net/socket syscall surface (#45-#57 — client TCP/UDP/ICMP **and** server `sock_listen#56`/`sock_accept#57`, the kernel inbound-TCP unlock, landed 1.45.5/.6) that userland servers consume, **1.49** added **loopback** (`lo`/127.0.0.1) + **socket-as-VFS-fd** (an accepted conn is now an epoll-able VFS handle — the event-loop keystone); and the server-app tier is already real on the Cyrius side — **`agora`** (telnet BBS, iron-validated) + **`cyrius-yeomans-descent`** (MUD — **runs on agnos over server sockets**, harness `docker/descent-sweep/`) + the network-tools family (`yo`/`dig`/`whirl`/`taar`). **The Docker service-sweep is UNDERWAY**, not "not started": the descent sweep already **found + fixed a real kernel bug** (TCP CLOSE_WAIT listener slot-leak, agnos 1.45.11). What's still NOT started/open: the **`agnova` native installer** (the stage-defining deliverable — AGNOS installs itself, retiring `install-usb.sh`) **— but agnova can't install anything *right* without a sovereign package manager beneath it, so sovereign-ark (below) is a HARD PREREQUISITE on the critical path (`sovereign ark → agnova → server exit`), not a parallel track**; the broader **multi-service sweep matrix** (agora/remote-shell/web beyond descent); and **`ark`/`nous` server-side** = the **sovereign-ark path** now roadmapped in [`planning/ark-v2-sovereignty-path.md`](planning/ark-v2-sovereignty-path.md). | aethersafha + GUI userland landing. |
| **desktop** | aethersafha (Wayland compositor — currently Pending in CLAUDE.md table) + display drivers (mabda + iGPU/dGPU) + user-facing app ports + GUI userland. Absorbs the daily-driver / GUI / browser workloads that server stage couldn't. | **Not started — but the compositor backend seam now exists** (2026-06-29): the sovereign-native [`bhumi`](https://github.com/MacCracken/bhumi) (platform backend — agnodrm/kernel-input/seat, replacing the Linux DRM/libinput/logind trio) + [`mehman`](https://github.com/MacCracken/mehman) (compat/swallow surface backend) scaffolds replace the dead "XWayland" framing aethersafha carried. bhumi is the MVP backend; mehman is swallow-stage. | Compat sandbox + non-native-workload absorption. |
...
**The arc is sequential.** Don't skip stages: desktop work doesn't open before server lands; swallow doesn't open before desktop lands. Stage exits map to release milestones — **rescoped 2026-06-14 to a compressed summer-2026 track** as the kernel arcs accelerated: demo→base ≈ MVP entry (✅ 2026-05-15), **reaching server base work + a founder solo-dogfood month ≈ Closed Beta opening (late August 2026)**, **server→desktop ≈ desktop completeness (late summer 2026, external testers at the late-August beta)**, desktop→swallow ≈ post-desktop horizon. (Was: base→server ≈ Public Beta / server→desktop ≈ v1.0 Q2 2027 — superseded; public beta now slots between the closed-beta cohort and desktop.)
...
**Opening gate** (external testers target **late August 2026** — rescoped 2026-06-29; a **founder solo-dogfood month** (~July/early-August) runs first so the founder trusts the system before anyone else is on it, and so the cohort lands on a server-capable, lived-in base rather than a bare shell):
...
**Phase 0 — founder solo-dogfood month (~July / early-August, server base):** a full month of the founder **living on the system daily** + the Docker server-sweep below — the "do I trust it enough to put others on it" window. Confidence + hardening receipts before exposure.
...
**Phase 1 — external testers (late August, STILL CLOSED):**
- [ ] Initial cohort: friend-network, 5–15 **external** testers — brought in **late August**, after the solo month, where GUI / daily-driver workloads genuinely need humans at the screen (unlike the founder server sweep above)
...
**Cadence dependency**: opening gate is **toolchain-independent** — the kernel already builds and boots against the current Cyrius pin (in [`state.md`](state.md); held deliberately on a known-working version). The dependency reduces to *agnosticos-side work* (ISO Stage-4 cut via `iso.cyr` + first hardware boot session). The earlier framing that gated MVP on the Cyrius v6.0.x bare-metal target was conceptual residue from pre-monolith-extraction days; corrected 2026-05-12. Opening slips by week, not by month. **The summer-long program then runs independently of Cyrius cycles** — it's hardware-and-cohort-paced, not toolchain-paced. Cyrius work continues in parallel (now on the v6.3.x line — v6.0.x delivered the bare-metal target + RISC-V rv64 among its gains; see [`state.md`](state.md) for the live cycle) but does not gate the MVP ship.

### Desktop — late summer 2026 (external testers arrive at the late-August beta, STILL CLOSED) *(rescoped 2026-06-29; folds the old separate mid-late-summer external-tester phase into the late-August beta opening, behind the founder solo month)*
...
- [x] **Deeper page-mapping fix** — landed at **1.42.2** (direction B: `proc_create_address_space` leaves the mmap arena not-present instead of present-supervisor; QEMU repro 0 ring-3 `#PF`/20). The rare SYSCALL-path RBP smash was fixed at **1.42.3** (0 smash/40). → [issue](issues/archive/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md)
...
**Image-assembly tooling — Linux now, sovereign at self-host (decision 2026-06-30).** AGNOS is UEFI-native: the medium is a **GPT disk with a FAT EFI System Partition** holding `BOOTX64.EFI` (gnoboot) + kernel + rootfs — **not** a GRUB ISO9660, so `grub-mkrescue` / `xorriso` / `mksquashfs` are **not** used (those were Linux leftovers, now removed from `scripts/src/boot.cyr`'s `--iso-check`). Today the ESP image is assembled on the Linux host with **`mtools` (`mcopy`) + `dosfstools` (`mkfs.fat`) + `gptfdisk` (`sgdisk`)** (`agnsh-smoke.sh` / `install-usb.sh`). This is acceptable **bootstrap scaffolding** and is *not* on the MVP/closed-beta critical path. **A Cyrius-native sovereign disk-image assembler (name TBD) is a required SERVER-stage component**, gated by the **"agnos building agnos" / self-hosting** milestone: a self-hosting AGNOS cannot shell out to Linux `mtools`/`dd` to build its own boot image, so it needs a native tool that writes the GPT + FAT-ESP and stages gnoboot/kernel/rootfs itself. It is the same sovereignty closure already walked — cyrius replaced gcc, agnos replaced Linux, gnoboot replaced GRUB — this tool replaces `mtools`/`dd`/`xorriso`. Tracked as a planned tool in [`planning/shared-crates.md`](planning/shared-crates.md); scaffold + name when the self-host cut is scheduled. See [[project_sovereign_image_builder_planned]].
...
**Phasing** (full detail in spec):
- **Phase 1 — Foundation** ✅ Substrate (kavach 3.x shipped; `Subsystem<T>` agnostik type to add when active). **Audit refinement (2026-06-29, [`planning/kavach-agnos-compat.md`](planning/kavach-agnos-compat.md))**: on the *bare* agnos kernel kavach today is a scanner + HMAC-audit + policy frontend with **zero kernel-backed confinement** — correct by design, since confinement is the capability layer's job and that layer isn't in the kernel yet. The real Phase-20 kernel deliverable is the **"Native sandbox-confinement primitives"** (capability-scoped FS-reach / syscall-reach / resource-budget / net-reach grants — **capability-first, NOT Landlock/seccomp/namespace ABI emulation**), a **swallow-stage** item already on the agnos roadmap's Deferred table. (Also fixed today: kavach 3.5.3 gated a Linux-syscall-aliasing hazard on the agnos build.)
...
**Subsystem name**: TBD — Sanskrit convention applies when Phase 2 begins. "The compat subsystem" / "linux-compat-env" works as a placeholder. **The compositor-surface slice is already scaffolded** (2026-06-29): [`mehman`](https://github.com/MacCracken/mehman) (Persian *guest*) is the swallow-stage **compat surface backend** for aethersafha — it hosts *foreign-app surfaces* as guests in a kavach sandbox (the legitimate successor to XWayland's actual job; **not** an X.Org port). Post-MVP; pairs with [`bhumi`](https://github.com/MacCracken/bhumi) (the sovereign-native platform backend).
...
### Phase 23 — Foundation Structure (governance meta-defense) — ⏸️ POSTPONED to public-release consideration (user 2026-06-29)

> **Postponed**: the whole Foundation track — including the "do-now" pre-Foundation actions below — is **deferred until we actively consider public release**. Rationale: the closed-beta arc (founder solo month → late-August external testers) is not public adoption, so the time-sensitivity that made trademark/DCO "do before public adoption" hasn't bound yet. Revisit when the public-beta / GA conversation opens. (Until then, treat the items below as a staged checklist, not active work.)
...
**Pre-Foundation actions** (staged — ⏸️ postponed to public-release consideration per the note above; Robert can do these without legal counsel when the track reopens): trademark filings (`AGNOS`, `Cyrius`, key subsystem names); DCO sign-off in contribution docs + CI; CONTRIBUTING.md IP model documentation; quiet outreach to 2-4 candidate founding board members; project bank account separate from personal finances; signing-key custody documentation. These were previously flagged "before public adoption" — that clock starts at the public-release conversation, which the closed summer arc does not trigger.
...
**Kernel-substrate reality (2026-06-29, from [`planning/kavach-agnos-compat.md`](planning/kavach-agnos-compat.md))**: the Layer-4 capability gate is **the design intent but not yet in the bare kernel** — agnos today runs ungated-root (getuid#15 always 0; no in-kernel capability/permission gate), with isolation enforced purely by hardware (per-proc CR3 + ring-3 + SMEP/SMAP). Two small, sovereign **early Layer-4-substrate steps** were queued as agnos 1.50.x items: a **real `proc_get_ppid`** (so `kill#16`'s ownership gate stops being inert) and **bg-job fault teardown**. The full capability-enforcement realization is the swallow-stage **"Native sandbox-confinement primitives"** (Phase 20) — capability-first, never the Unix uid/namespace pattern this phase rejects.
...
| **cmdit** (CLI / argument parsing) | **Scaffolded + built v0.1.0 (2026-06-25)** — review found no standalone CLI *lib*, but a stdlib *parser* `lib/flags.cyr` (9 consumers); kii was only a wrapper. cmdit = flags.cyr **productized as a distlib + extended** (materialize bridge + auto help/version + exit constants; 0.2.0 enum/repeat/required/range/env; 0.3.0 verbs). 26/26 tests green. Design + adoption tiers: [`planning/cmdit.md`](planning/cmdit.md). | ~40 hand-rollers; **kii re-folds first** (seed → first consumer) | Medium |
| **amuzesh** (classical / shallow ML) | The first non-deep-learning ML axis (k-means / GLM / GBDT / Kalman) on `ganita`+`hisab`. Named 2026-06-25, **not scaffolded** — second-consumer-triggered. Map: [`planning/classical-shallow-ml.md`](planning/classical-shallow-ml.md). | phylax/aegis (auditable GBDT), seema, intent routing | Low (research-watch) |
| **Black-box / derivative-free optimization** (GP-BO + acquisition + CMA-ES) | The single unmapped ML gap from the 2026-06-25 product-mining (ifran ships only grid+random). Scoped to `min f(x)` on a scalar objective. | attn11 / tarka hyperparameter sweeps | Low (demand-gated) |

> **Open ML extraction lanes (later review)** — from the 2026-06-25 ifran/secureyeoman product-mining ([`planning/ml-product-mining.md`](planning/ml-product-mining.md)): **QLoRA→Type-3** "Pre-Trained" reference (LoRA + NF4; the most-corroborated demand, on [`planning/generative-paradigms.md`](planning/generative-paradigms.md)), **black-box optimization** (row above), and the **attn11-deferred** items (KD soft-target objective, **speculative decoding** — now a full forward-design map ([`planning/speculative-decoding.md`](planning/speculative-decoding.md)) after DeepSeek **DSpark** (2026-06), homed as an attn11 **decode lane** that reuses its MTP heads (self-speculative draft) + the KD objective (draft training); EDA augmentation — all gated on attn11 reopening for objective/decode work). tarka DPO + RLHF-KL **shipped** (1.1.0, 2026-06-25).
