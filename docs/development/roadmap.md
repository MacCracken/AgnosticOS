...
| **1.32.x** | **Networking on iron** (Realtek r8169 first; e1000/e1000e/igc later) | queued per § 1.32.x below | Already-planned cycle theme. ~1.5-2.5 KLOC for first real NIC. |
...
**Iron-validation coverage after 1.31.5**: NVMe (Crucial P3, Attempt 80) + SATA (WD Blue SA510, Attempts 81+82) + USB MS (Silicon Motion stick, Attempt 87) + 1.31.4 no-regression (Attempt 88). 1.31.5 ext2/ext4 iron burn PENDING at Attempt 89 against archaemenid NVMe Linux-FS partition. RAM-disk is `pmm_alloc`-backed (no hardware variable — see "RAM-disk iron exercise" in the opportunistic block for the optional `RAMDISK_ENABLE=1` cosmetic burn); VirtIO-blk has no bare-metal device. Optical defers to the plug-and-play cycle (pre-1.35.0). **Cycle sequencing pinned post-1.31.x**: 1.32.x = networking (Realtek r8169 first), 1.33.x = WRITE cycle (ext2/ext4 mutation paths).
