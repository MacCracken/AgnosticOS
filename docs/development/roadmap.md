...
- **DNS — stub resolver client** — ✅ **LANDED 2026-05-27 (QEMU-validated; agnos [Unreleased] → 1.35.1).** RFC 1035 A-record stub on the existing UDP transport (the 1.32.x DHCP work already shipped `udp_bind`/`udp_recv_from` + the ingress demux — the old "UDP ingress is the gap" framing was stale). Two bites: (1) capture DHCP **option 6** (was requested but dropped) → `net_dns_server` + gateway fallback; (2) `dns_qname_encode`/`dns_build_query`/`dns_skip_name` (`0xC0` compression-pointer handling)/`dns_parse_answer`/`dns_resolve` + a `dns <host>` shell verb (the **`dig`** substrate). `scripts/dns-smoke.sh` green — hermetic compression-pointer parse (`93.184.216.34`) + opt-6 capture (`10.0.2.3`); live `example.com → 172.66.147.243` through SLIRP. No iron burn (rides the iron-COMPLETE 1.32.9 r8169/DHCP path). Audit-doc-first per [[feedback_redesign_dont_reinvent]]: [`dns-stub-resolver-prior-art.md`](dns-stub-resolver-prior-art.md) (RFC 1035/3596 + musl `res_*` + lwIP/iPXE + Plan 9 `ndb/dns`). Stub scope — no cache / TCP-fallback / EDNS0 / AAAA (deferred, audit § 8). The precondition for name-based `ark`/`nous` fetch + `hoosh` hostnames.
- **ICMP echo / ping** — ✅ **LANDED 2026-05-27 (QEMU-validated; agnos [Unreleased]).** RFC 792 echo in both directions: `net_handle_icmp` wired into the IP demux at proto 1 — type 8 → reply (AGNOS is **pingable**), type 0 → match the outstanding request, types 3/11 logged; `icmp_ping(ip)` + a `ping <host>` shell verb (dotted-quad or DNS-resolved). `scripts/icmp-smoke.sh` green — hermetic checksum self-verify + live gateway round-trip through SLIRP. ICMP checksum reuses `ip_checksum` (no pseudo-header). RFC 792 cited inline (no standalone audit doc — no wire subtleties). The diagnostic pair with `dns`.
- **Networking-comms continuation** (user order, 2026-05-27) — **COMPLETE on the AGNOS side**: **TCP hardening** (1.35.1, B0–B4 — in-order ring + retransmit/RTO + MSS/segmentation + peer-window; [`tcp-hardening-prior-art.md`](tcp-hardening-prior-art.md)) then **NTP/SNTP** ([Unreleased] → 1.35.2 — the kernel's first wall clock; [`ntp-sntp-prior-art.md`](ntp-sntp-prior-art.md)) both landed + QEMU-validated. The remaining destinations — **TLS** (→ HTTP client) and **PIE** (→ full-binary KASLR) — are **cyrius-side** (days-to-weeks; the user drives them with the cyrius agent). The AGNOS network substrate they need (reliable TCP + DNS + wall clock) is now in place.
- **Legacy virtio-net interface** — **BACK-BURNERED (2026-05-27)**. A cap-list-absent BAR0-I/O + `QUEUE_PFN` 0.9.5 fallback would only trigger under non-default `disable-modern=on` (default QEMU `0x1000`/`0x1041` is already covered by the modern driver), and the [`virtio-net-legacy-layout-audit.md`](virtio-net-legacy-layout-audit.md) § "Post-implementation update" documents an **unidentified TX-handler failure** on the legacy path that was never root-caused (the modern rewrite was the workaround). Decision: leave it out of the active cycle — once the rest of 1.35.x is in and the networking surface is exercised more, the legacy path is likely easier to diagnose correctly. Not iron-relevant (archaemenid uses r8169); QEMU-completeness only.
