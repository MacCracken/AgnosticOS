...
> **Status**: Pre-Beta — Closed Beta targeting **early June 2026** | **Last Updated**: 2026-06-04. This roadmap is **forward-facing** — shipped arcs live in [`state.md`](state.md) + the per-repo CHANGELOGs, not re-narrated here. Refer to state.md for current kernel / Cyrius / per-repo versions.
>
...
| **base** | Kernel solid; ext4 read+write; ark/nous package manager working end-to-end (resolve / fetch / install / remove); AGNOS-side update mechanism; enough soak surface for real-workload exposure without weekly showstoppers. | **Current.** The base-defining kernel work is **iron-validated**: ext2/4 + FAT-family read+write (1.33.x/1.34.x), the FS-crash-safe stack (extent→jbd2→VFS, 1.37–1.39), and **exec-from-disk** (1.40.x through 1.40.13, iron-validated 2026-05-31 — ring-3 exec + FAT verbs with ext2 at `/`). The active kernel move is **shell separation** (1.41.x — recovery-only kernel REPL, agnsh runs in ring 3 from disk; a kernel-slimming step) — **software-complete + QEMU-validated, iron burn pending** ([`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) #tracker-141x-cycle). Maturing over the ark/nous client cycles + AGNOS-side update. Triggers archaemenid dual-boot migration (AGNOS-primary on internal NVMe + Linux on SATA — see [`state.md`](state.md) § *archaemenid migration*). | Native installer + server ecosystem landing. |
...
**1.41.x userland-agnsh on iron — burn-pending (PMM fix)**
The 1.41.x shell-separation arc moved the interactive shell to userland `/bin/agnsh` (ring 3); the native in-kernel shell is now a recovery REPL. The `14112` burn surfaced a kernel PMM cross-class-fragmentation bug that `#PF`'d agnsh before its prompt; fixed at **agnos 1.41.12** (`pmm_alloc` top-down — QEMU 0 `#PF`/52 boots). Deploy is de-Linux'd + correct via `install-media.sh --update-all`.
- [ ] **Iron-validate the PMM fix** — burn 1.41.12 kernel + agnsh 1.3.6 on archaemenid; confirm the `[ASSIST] >` prompt renders (was banner-freeze ~50% of boots). The validating burn.
- [ ] **Deeper page-mapping fix** — top-down `pmm_alloc` likely *masks* the underlying supervisor-PDE / `proc_map_page`-under-per-process-CR3 defect rather than curing it (direct fixes failed validation). A proper page-table fix is owed. → [issue](issue/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md)
- [ ] **Syscall-path RBP frame-smash** — rare (~1/52); user RBP not preserved across the kstack switch in `agnos/kernel/arch/x86_64/syscall_hw.cyr`. Separate bite.
- [ ] **USB-HID keyboard input on iron** — the next blocker *after* the prompt renders: does a ring-3 keystroke register (`kbd_read_blocking` / xhci-HID feed). Separate arc; "prompt but no echo" is the milestone, NOT a regression.
...
| C1 | High | De-Linux the boot pipeline (sovereign initramfs / retire cpio) | Boot is now gnoboot → sovereign agnos kernel (no Linux), but GRUB/multiboot2/Linux-initramfs cruft lingers. **Done**: `install-media.sh` no longer ships/requires a Linux `initramfs.cpio.gz` — it purges any stale one off the ESP and ships sovereign `\boot\initramfs` only if present (boot proceeds without; gnoboot makes it optional, the kernel mounts agnos-fs off NVMe directly). **To retire**: (a) `scripts/src/install.cyr` — builds a Linux `cpio.gz` via the `cpio` tool "for the Linux kernel initramfs handler" (lines 3/136/225/323); delete, or rewrite to emit the sovereign INDR `\boot\initramfs` *only if* an initramfs is ever actually needed (likely not). (b) `scripts/test-uefi-qemu.sh` — GRUB `module2 /boot/initramfs.cpio.gz` harness, pre-gnoboot. (c) `scripts/build/initramfs.cpio.gz` — stale artifact, delete. (d) Sweep `scripts/` for other multiboot2/GRUB/cpio Linux-boot-era refs. Surfaced 2026-06-05 (install-media.sh build). |
...
> MVP iron-validated 2026-05-15; the kernel boots to a **typeable** shell on real hardware. The post-MVP hardening arcs — **1.30.x** keyboard, **1.31.x** storage (NVMe / AHCI / USB-MS / RAM-disk / GPT + ext2/4 read), **1.32.x** networking (r8169 + DHCP, iron-COMPLETE), **1.33.x** ext2/4 WRITE, **1.34.x** FAT-family, **1.35.x** comms-substrate (DNS/ICMP/TCP-hardening/NTP/mmap/RTC), **1.36.x** byte-identical refactor, **1.37.x** ext4 extent-allocation, **1.38.x** jbd2 journaling, **1.39.x** VFS generic-write lift, **1.40.x** exec-from-disk + VFS mount routing — are all **shipped + (mostly) iron-validated** (the 1.37–1.40 crash-safe + exec arc iron-validated on the `13810`/`1409`/`14013` burns through 1.40.13). The **1.41.x — shell separation** arc (agnos roadmap § *Shell Separation Arc*) is **software-complete + QEMU-validated** (the kernel shell shrinks to a recovery-only REPL; agnsh runs in ring 3 from disk) but **iron burn pending** — it has not yet booted on real hardware ([`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) #tracker-141x-cycle, A1–A4 rubric; the 1.40.14 teardown closeout folds into the same pending burn). Per-cut detail lives in the agnos CHANGELOG + [`state.md`](state.md) + the iron logs, not here. What remains forward in this queue:
...
*Last Updated: 2026-06-04 (1.41.x shell-separation arc software-complete + QEMU-validated, iron burn pending — see #tracker-141x-cycle; 1.40.x through 1.40.13 iron-validated 2026-05-31; per-cut detail in agnos CHANGELOG/state.md) | Next Review: closed-beta cut (~early June 2026)*
