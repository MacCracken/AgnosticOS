...
| **Legacy SMM PS/2 emulation off** | Modern firmware doesn't emulate PS/2 over XHCI post-`ExitBootServices`. Keyboard input does not reach legacy port 0x60. | Plan for a native XHCI + USB-HID-boot driver. See [`planning/usb-hid-keyboard-driver.md`](planning/usb-hid-keyboard-driver.md) for the 5-phase scope. |
...
- [`planning/usb-hid-keyboard-driver.md`](planning/usb-hid-keyboard-driver.md) — the 5-phase scope for closing MVP gap #3 (USB keyboard input on modern UEFI firmware).
