...
>
> **Fossil notice (2026-04-14, refreshed 2026-05-25):** AGNOS now has its own sovereign kernel written in Cyrius (40+ subsystems, iron-validated on NUC AMD; current version + size in [`development/state.md`](../state.md)). This guide describes Linux kernel module development for the **host bootstrap layer only** — used to build the cross-compiler toolchain on an existing Linux host before AGNOS can self-host. The AGNOS kernel is developed in the `agnos` repo — see `agnos/CLAUDE.md` for current kernel development instructions. Any `userland/…` paths below predate the 2026-04-01 monolith extraction (now standalone repos) — historical, not live.
