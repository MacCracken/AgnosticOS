> **Status**: ⏹ **FROZEN archive — the MVP 2.0 era** (networking + filesystem WRITE), Attempts 69+ / agnos 1.30.10 – 1.34.x. The **active log continues in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md)** (the Base→Server era, 1.37.x+). This file is kept for its per-attempt root-cause history (r8169 unicast-RX, DHCP, the ext2/4 + FAT WRITE burns) and its tracker anchors `#tracker-132x/133x/134x`. **One MVP2.0-era burn is still pending here**: the FAT/exFAT iron touch ([`#tracker-1341-cycle`](#tracker-1341-cycle)).
>
> **Log chain**: [`iron-nuc-zen-log-mvp.md`](iron-nuc-zen-log-mvp.md) (MVP 1.0 — boot-to-shell, Attempts 1 – 68, agnos 1.30.9) → **this file** (MVP 2.0 — networking + write, 1.30.10 – 1.34.x) → [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) (Base→Server — big-write + storage-depth, 1.37.x+).
>
...
>
> **State.md cycle headers link to these anchors via `iron-nuc-zen-log-mvp2.md#tracker-1322-cycle` style.**

### Tracker: 1.33.x cycle (OPEN 2026-05-25 — **ext2/ext4 WRITE arc, the demo→base maturity exit** ([[project_agnos_maturity_arc]]). The networking arc closed at 1.32.9; WRITE makes the box *livable* — state persists across reboots, not just *shown*. **Audit-first per [[feedback_redesign_dont_reinvent]]**: multi-source convergent prior-art doc [`ext2-ext4-write-prior-art.md`](prior-art/ext2-ext4-write-prior-art.md) landed 2026-05-25 (FreeBSD ext2fs primary, Linux ext2, OpenBSD, ext4 spec, e2fsprogs). **Key finding: the block-device WRITE primitives already exist + are iron-validated** (`blk_write` → `nvme_blk_write`/`ahci_blk_write`, proven at USB-MS Attempt 87 + AHCI demos) — so this arc is purely the ext2 metadata-mutation layer, zero driver work. NO iron burn until phase W5; W1-W4 are QEMU `e2fsck -fn`-clean smoke gates, not iron, per [[feedback_iron_burns_block_other_work]].) {#tracker-133-cycle}

**Cycle plan** (phased per the audit § 8; each phase = a QEMU smoke gate; W5 = the iron burn):

| Phase | Scope | Gate |
|---|---|---|
| **W1** | write primitives + `ext2_put_inode` (RMW) + SB/BGDT free-count stores + bitmap read | grp-0 free counts match host `debugfs stats`; image untouched → `e2fsck -fn` clean |
| **W2** | block + inode allocators (bitmap walk + cursor + accounting) | alloc-3-blocks+1-inode then free → free counts return to baseline; `e2fsck -fn` clean |
| **W3** | `ext2_write_at` (alloc holes, indirect blocks, `i_size`/`i_blocks`) + truncate | in-AGNOS readback matches; host `mount`+`cat` shows bytes; `e2fsck -fn` clean |
| **W4** | dirent insert/remove + `create`/`unlink` + VFS `vfs_write`/`vfs_create` arms + shell `touch`/`echo>`/`rm` | `echo x > /f` → reboot QEMU → `cat /f`=x; host sees `/f`; `e2fsck -fn` clean |
| **W5** | `mkdir`/`rmdir` + commit-order + `s_state` dirty/clean + **IRON BURN** | see iron rubric below |

**✅ W1 LANDED 2026-05-25 (QEMU smoke-green, no iron).** `blk_write_on` + `ext2_blk_write`/`ext2_write_block` + `ext2_write_superblock` + `ext2_put_inode` (block-granular RMW) + endian stores + free-count accessors + the `EXT2_WRITE_SELFTEST` self-test + `scripts/ext2-write-smoke.sh`. Production **629,568 B** (+5,752). No regression (`test.sh` 4/4 + `ext2-smoke.sh` 5/5, all backends reach shell). **W1 gate PASS**: `grp0 free_blk` = `sb free_blk` = `bitmap free-bits` = **16068**, byte-identical to host `debugfs stats`; `ext2_write_block` + `ext2_put_inode` identity round-trips byte-clean; **`e2fsck -fn` clean (exit 0)** on the post-boot image. Next: W2 (block + inode allocators).

**✅ W2 LANDED 2026-05-25 (QEMU smoke-green, no iron).** `ext2_alloc_block`/`ext2_free_block` (block-bitmap walk + goal-group hint + group wrap), `ext2_alloc_inode`/`ext2_free_inode` (inode-bitmap mirror + `bg_used_dirs_count`), full per-group + superblock free-count accounting, commit-ordering per doc § 3 (bit+bitmap before use, counts last), BGDT read/write + group-descriptor accessors + bitmap bit helpers + `ext2_bitmap_buf`. Plus the **write-safety gate** (`ext2_write_ok`, doc § 10): mutation refused on `metadata_csum`/`uninit_bg`/`bigalloc`/`64bit` (the real agnos-fs `mkfs.ext4`-default partition refuses write until the csum/64bit sub-cycles; reads stay full). Production **640,848 B**. No regression (`ext2-smoke.sh` 5/5). **W2 gate PASS**: alloc `blk 1084 1085 1086 ino 15` → free all → free-counts return exactly (`16068 → 16068`, `17138 → 17138`); **`e2fsck -fn` clean** — bitmap set/clear + BGDT/SB accounting symmetry proven. **The 1-byte over-read flagged at W1 (`ext2_init` "log_block_size out of range:" len 36→35) is FIXED this bite per user request.** Next: W3 (`ext2_write_at` allocating holes + indirect-block alloc + `i_size`/`i_blocks`; truncate).

**✅ W3 LANDED 2026-05-25 (QEMU smoke-green, no iron).** `ext2_bmap_alloc` (allocating logical→physical — direct + single-indirect; double/triple deferred), `ext2_write_at` (on-demand block alloc, sparse holes, full-block + partial-RMW, `i_size`/`i_blocks` accounting), `ext2_truncate_zero` (free data + indirect blocks → 0), inode field setters. Production **648,016 B**. No regression (`ext2-smoke.sh` 5/5). **W3 gate PASS**: overwrite /hello.txt @0 200 B → readback byte-identical; sparse-write 100 B @8192 /etc/hostname → new direct block, block 1 a hole, `i_blocks` +8; truncate /hello.txt → block freed, reads 0. **`e2fsck -fn` clean**; host `debugfs` confirms on disk (/etc/hostname Size=8292, /hello.txt Size=0). Next: W4 (dirent insert/remove + `create`/`unlink` + VFS write arms + shell `touch`/`echo>`/`rm`).

**✅ W4a LANDED 2026-05-25 (QEMU smoke-green, no iron) — demo→base milestone at the FS layer.** `ext2_dir_insert` (rec_len split / tombstone reuse / new-block append), `ext2_dir_remove` (coalesce into prev / tombstone head), `ext2_create` (alloc+init+insert, rollback on failure), `ext2_unlink` (remove+dec-links+free). Production **654,384 B**. No regression (`ext2-smoke.sh` 5/5). **W4a gate PASS**: create+write /w4keep.txt + create+write+unlink /w4tmp.txt → in-AGNOS lookup/readback + free-count round-trip all clean; **`e2fsck -fn` clean**; **host `debugfs` reads `/w4keep.txt`="AGNOS-W4-WROTE" off the disk image (create+write PERSISTED)** and confirms /w4tmp.txt absent (unlink persisted). **The e2fsck gate caught 2 real unlink bugs** (zero-`i_dtime` "Deleted inode has zero dtime"; then small-`i_dtime` misread as orphan-list next-pointer "corrupted orphan linked list" → fix: stamp a timestamp-shaped `i_dtime` > any inode#) — exactly the doc-§3 "reads-back-but-fsck-screams = fail" discipline. Next: **W4b** (VFS `vfs_write`/`vfs_create` arms + shell `touch`/`echo>`/`rm` — user-facing `echo hello > /f`; interactive/iron-validated, NOT headless-smokeable), then **W5** (mkdir/rmdir + iron burn).

**✅ W5 FS-layer LANDED 2026-05-25 (QEMU smoke-green, no iron) — the ext2 mutation set is COMPLETE at the FS layer.** `ext2_mkdir` (alloc dir inode + `.`/`..` seed + parent link bump + `bg_used_dirs++`), `ext2_rmdir` (empty-check + parent link drop + truncate + free), `ext2_dir_is_empty`. Production **659,408 B**. No regression (`ext2-smoke.sh` 5/5). **W5 gate PASS**: mkdir /w5keep → lookup + dir-type + root-link-bump verified; rmdir round-trip (mkdir → create file → rmdir REFUSED non-empty → unlink → rmdir OK) → counts + parent link + lookup all return. **`e2fsck -fn` clean — Pass 4 validates the mkdir/rmdir link accounting** (the trickiest part); host `debugfs` confirms /w5keep is a `directory` on disk + /w5tmp absent. **Full FS-layer set: create / write (+sparse +truncate) / unlink / mkdir / rmdir, all e2fsck-clean + disk-persisted.** Remaining for the demo→base milestone: the user-facing glue (VFS write arm + shell `touch`/`echo>`/`rm`/`mkdir`/`rmdir`; interactive/iron) + `s_state` dirty/clean + the **W5 iron burn** (create file → power-cycle → persists, the demo→base proof).

**✅ W4b LANDED 2026-05-25 (QEMU smoke-green, no iron) — `echo > /f` works end-to-end.** `vfs_write` EXT2 arm (`vfs.cyr`) + shell verbs `touch`/`rm`/`mkdir`/`rmdir` + `echo TEXT > FILE` redirect (`shell.cyr`) with path helpers `sh_abspath`/`sh_split_parent`/`sh_trim`. The interactive reader is HID-driven (not headless-driveable), so a gated `sh_write_selftest` drives the **real** verb path via `sh_exec` with canned commands. Production **666,840 B**. No regression (`ext2-smoke.sh` 5/5). **W4b gate PASS**: `mkdir /shdir` → `echo SHELL-WROTE-IT > /shdir/keep.txt` → `cat` prints **"SHELL-WROTE-IT"** back (echo-redirect created+wrote via the VFS arm); `touch`/`rm` exercised. **`e2fsck -fn` clean**; host `debugfs` reads `/shdir/keep.txt`="SHELL-WROTE-IT" off the disk image + confirms /shtmp absent. **`echo > /f` → reboot → `cat /f` now works at the QEMU level.** **NEXT = W5 IRON BURN** (per user: burn after QEMU passes). **Burn prereq**: the agnos-fs partition must be re-carved write-friendly (`mkfs` with `^metadata_csum,^64bit,^dir_index,^uninit_bg`) — the current `mkfs.ext4`-default partition refuses write by design (W2 safety gate). Two burn options: (a) burn the `EXT2_WRITE_SELFTEST` build → self-tests run at boot, then mount the partition on Linux + `e2fsck` + check files; (b) burn production + type `echo hi > /f`, power-cycle, `cat /f`.

**🔥 W5 IRON BURN 2026-05-25 — gate CONFIRMED, no re-carve done.** Production 1.33.0 burned against the **unmodified** default-`mkfs.ext4` agnos-fs partition (photos `1330_attempt_no_log_write` / `1330_failed_writes`). Clean boot through the storage trio + GPT + `ext2: mounted (blocksize=4096, inode_size=256, inodes_per_group=8192)` + DHCP lease `.142` + shell `v1.33.0`. The write self-test ran and printed exactly the by-design refusal: `ext2w: grp0 free_blk=23507 …` → `ext2w: bitmap free-bits=23507` → **`ext2w: read-only FS -- write checks skipped`**, then every shell verb failed (`mkdir: failed`, `echo: no such directory`, `touch: failed`, `rm: failed`). **This is the W2 safety gate firing on a `metadata_csum`+`64bit` FS, not a bug** — exactly the posture audit § 10 documents. **User decision: instead of re-carving write-friendly (path a), take on real-`mkfs.ext4` write (path b)** → **1.33.1 sub-cycle opened** (metadata_csum + 64bit write). See [`#tracker-1331-cycle`](#tracker-1331-cycle).

**Hypothesis**: ext2 write is mechanically a mirror of the closed READ arc — every read accessor gets a write-back sibling, every "find a block" walker gets a "create a block" allocator — over an already-working block writer. Crash-consistency without a journal comes from **ordered writes** (audit § 3: write the pointed-to before the pointer, the alloc-bit before the use; a crash then leaks resources fsck reclaims but never dangles a pointer). The risk surface is **accounting drift** (i_blocks-in-512B-sectors trap, SB/BGDT free-count desync) and the **`metadata_csum` trap** (the agnos-fs partition is `mkfs.ext4`-default → almost certainly checksummed + 64bit; writing without CRC32c corrupts it). **Mitigation decided in audit § 12.1**: re-carve agnos-fs write-friendly (`-O ^metadata_csum,^64bit,^dir_index,^uninit_bg,^has_journal`) before W5; csum-write is a deferred sub-cycle. **The QEMU `e2fsck -fn`-clean gate is the canary** — a file that reads back but leaves fsck screaming is a fail, because that's the accounting/ordering bug surfacing.

**Iron falsification rubric** (W5 burn — the demo→base proof; mount-modify the agnos-fs partition write-friendly per [[feedback_prefer_mount_modify_over_reflash]], NOT auto-proposed):

| Signal | demo baseline (read-only) | W5 PASS target | Falsification → meaning |
|---|---|---|---|
| File create + persist | (read-only — no persistence) | `echo agnos > /persist.txt` → **power-cycle** → `cat /persist.txt` = `agnos` | file gone / wrong bytes after reboot → write didn't reach platter (sync/ordering bug) or inode/dirent not committed |
| Host fsck after burn | n/a | pull drive, host `e2fsck -fn /dev/…agnos-fs` → **exit 0, no FIXED** | fsck reports leaks/bad-counts/dangling → accounting or commit-order defect; localize by which structure fsck flags (bitmap vs count vs dir vs inode) |
| `mkdir`/`rmdir` | n/a | `mkdir /d` → `echo x>/d/f` → reboot → tree intact; `rmdir` non-empty refused | parent links_count wrong / `..` missing → dir-init or link accounting bug |
| Storage trio + GPT + ext2 mount + shell | byte-clean | byte-clean | regression impossible — scope is ext2.cyr mutation + VFS arms only |

---

### Tracker: 1.33.1 cycle (OPEN 2026-05-25 — **metadata_csum + 64BIT WRITE: mutate a REAL default-`mkfs.ext4` partition, no re-carve.** The 1.33.0 W5 iron burn confirmed the W2 safety gate refuses write on the user's checksummed+64bit agnos-fs (`ext2w: read-only FS -- write checks skipped`). User chose path (b): implement what a correct writer must maintain so both refusals lift. **Audit-first** — prior-art doc § 14 landed 2026-05-25, deriving CRC32c (Castagnoli `0x82F63B78`) + the per-class checksum recipes from e2fsprogs `lib/ext2fs/csum.c` (the same code `e2fsck` runs) + kernel ext4 + spec. NO iron burn until the final bite; bites 1-6 are QEMU `e2fsck -fn`-clean smoke gates against a **`metadata_csum,64bit` image**, per [[feedback_iron_burns_block_other_work]].) {#tracker-1331-cycle}

**Bite plan** (audit § 14.6; each bite = a QEMU smoke gate on the checksummed image; the last bite = the iron burn):

| # | Bite | Gate |
|---|---|---|
| 1 | ✅ **LANDED 2026-05-25** — **64bit BGDT write** (lift `incompat & 0x80` refusal) + **extent-write safety guard** | **PASS.** `64bit,extent,^metadata_csum` image: `ext2_write_ok=1`, FULL mutation set (alloc/free + write/sparse/truncate + create/unlink + mkdir/rmdir + shell verbs) green, `e2fsck -fn` clean, all files persisted on disk. Confirmed § 14.4 (zero store changes — `_hi` halves preserved by RMW, `desc_size` stride already correct). **Bonus correctness fix**: `ext2_bmap_alloc`/`write_at`/`truncate_zero` had NO `EXTENTS_FL` guard → would corrupt Linux-created extent files on the real partition; now refuse cleanly (extent-alloc deferred, § 11; new files are indirect so `echo hi > /f` unaffected). Build 667,128 B; stripped-image regression still green. |
| 2 | ✅ **LANDED 2026-05-25** — **`ext2_crc32c` (Castagnoli) + `ext2_csum_seed`** | **PASS.** crc32c vector `0x1CF96D7C` (^~0=0xE3069283 iSCSI) self-test OK; mount computes `csum on=1 seed=e25418d4` which **matched the host UUID-derived `crc32c(~0,uuid,16)`** — primitive + seed validated against e2fsck's reference before any checksum is written. No behavior change (write still gated); 64bit-image regression green. Build 667,440 B. |
| 3 | ✅ **LANDED 2026-05-25** — **SB + group-descriptor csum** | **PASS.** `ext2_sb_csum_compute` (seed `~0`, [0,0x3FC)) + `ext2_grp_csum_compute` (group#→[0,0x1E)→zero16→[0x20,desc_size), lo16) + `ext2_bgdt_recsum`; hooked into `write_superblock`/`write_bgdt` (csum_on-guarded). Compute-and-compare on metadata_csum image: `SB csum match` + `grp0 csum match` — reproduce e2fsprogs values exactly. 64bit-image regression green (hooks no-op when csum off). Build 669,072 B. |
| 4 | ✅ **LANDED 2026-05-25** — **bitmap csums** | **PASS.** `ext2_set_block_bitmap_csum` (0x18/0x38, span bpg/8) + `ext2_set_inode_bitmap_csum` (0x1A/0x3A, span **ipg/8** ⚠ not blocksize) hooked into all 4 allocator paths (csum_on-guarded, before write_bgdt). Compute-and-compare: `blk-bitmap csum match` + `ino-bitmap csum match` vs e2fsprogs disk values. 64bit regression green. Build 671,552 B. |
| 5 | ✅ **LANDED 2026-05-25** — **inode csum** | **PASS.** `ext2_inode_csum_calc`/`_stamp` (crc over ino#→i_generation→inode bytes with 0x7C+0x82 zeroed; lo@0x7C, hi@0x82 gated on `i_extra_isize>=4`); hooked into `put_inode` before splice. `inode2 csum match` vs e2fsprogs. All 5 csum classes now compute-clean. 64bit regression green. Build 672,992 B. |
| 6 | ✅ **LANDED 2026-05-25** — **directory-leaf csum + tail** | **PASS.** `ext2_dir_limit` (usable [0,blocksize-12)), `ext2_dir_leaf_csum` (seed→dir_ino→i_generation→[0,blocksize-12)), `ext2_dir_tail_stamp`; walkers (try_insert/dir_remove) stop before the 0xDE tail; tail recomputed on every dir-block write (insert slack+append, remove, mkdir seed). `rootdir csum match` vs e2fsprogs. **All 6 csum classes compute-clean.** 64bit + stripped regression green (dir paths inert when csum off). Build 674,640 B. |
| 7 | ✅ **IRON PASS 2026-05-25** — **lift metadata_csum refusal** | **PASS (QEMU + IRON).** Dropped `0x400` from `ro_danger` → `write_ok=1` on metadata_csum+64bit. Full write smoke on the `metadata_csum,64bit,extent` image (the real-partition profile): WHOLE mutation set through the write path → **`e2fsck -fn` clean**, all files persisted. **`bg_itable_unused` accounting fix** (alloc into the never-used tail must shrink the watermark) cleared the e2fsck cascade the per-class compares couldn't surface. Matrix green (stripped/64bit/csum). Build 675,152 B. **AGNOS writes an unmodified default mkfs.ext4 — no re-carve.** 🔥 **W5 iron burn BURNED 2026-05-25 → PASS** (photo `1331_After_Reboot`, filed as [`1331-agnos-1.33.1-write-w5-persist-txt-survives-reboot-default-mkfs-ext4-shell-ls.jpg`](iron-nuc-zen-photos/1331-agnos-1.33.1-write-w5-persist-txt-survives-reboot-default-mkfs-ext4-shell-ls.jpg)): `echo > /persist.txt` → **power-cycle** → `agnos> ls` returns `./ ../ lost+found/ hello.txt welcome.txt agnos/ persist.txt` — the created file SURVIVED the reboot on the unmodified default partition. Burn audit: [`metadata-csum-write-iron-burn-audit.md`](prior-art/metadata-csum-write-iron-burn-audit.md). |

**Hypothesis**: the 1.33.0 mutation set is correct (proven `e2fsck`-clean on a stripped image); the only thing standing between it and the real partition is **derived metadata** — 64bit `_hi` halves (which are 0 on this <16 TB box, so effectively free per § 14.4) and CRC32c checksums on every metadata block touched. Each checksum class is an independent, locally-verifiable recompute hooked onto a buffer that's *already* about to be written in the existing ordered-write sequence — so adding them rides the § 3 crash-consistency for free and can't reorder anything. **The risk surface is the CRC32c algorithm details** (poly, no-final-XOR, seed source, per-class span/offset/truncation) — every one is a silent `e2fsck` reject if wrong, which is exactly why bites 1-6 each gate on `e2fsck -fn` against the checksummed image *before* bite 7 enables write. The trickiest is the **directory tail** (§ 14.7): the existing rec_len walker would happily reuse the 12-byte tail as a tombstone and corrupt it.

**Iron falsification rubric** (bite-7 burn — supersedes the 1.33.x W5 rubric; the partition is now the **unmodified** default `mkfs.ext4`, NOT re-carved; not auto-proposed):

| Signal | 1.33.0 baseline (this burn) | 1.33.1 bite-7 PASS target | Falsification → meaning |
|---|---|---|---|
| Mount + gate | `ext2w: read-only FS -- write checks skipped` | `ext2_write_ok=1` on the checksummed FS; write self-test runs | gate still 0 → a feature bit beyond metadata_csum/64bit/uninit_bg/bigalloc is set; dump `ro_compat`/`incompat` hex |
| File create + persist | every verb `failed` | `echo agnos > /persist.txt` → **power-cycle** → `cat /persist.txt` = `agnos` | gone/wrong after reboot → write didn't reach platter or csum-rejected on remount |
| Host fsck after burn | n/a (read-only) | pull drive, host `e2fsck -fn /dev/…agnos-fs` → **exit 0, no FIXED** | "checksum invalid" on a structure → that class's CRC32c recipe wrong (localize: SB / group-desc / bitmap / inode / dir) |
| `mkdir`/`rmdir` | n/a | `mkdir /d` → `echo x>/d/f` → reboot → tree intact + `e2fsck` clean | dir-tail csum or link accounting bug |

**🎯 W5 IRON BURN OUTCOME (2026-05-25) — PASS. The demo→base maturity exit is confirmed on iron.** Photo `1331_After_Reboot` (filed [`1331-agnos-1.33.1-write-w5-persist-txt-survives-reboot-default-mkfs-ext4-shell-ls.jpg`](iron-nuc-zen-photos/1331-agnos-1.33.1-write-w5-persist-txt-survives-reboot-default-mkfs-ext4-shell-ls.jpg)): clean boot through the storage trio + GPT (`[0] EFI System AGNOS-BOOT` 255 MiB / `[1] Linux FS agnos-fs` 25600 MiB) → `ext2: probe matched backend=2 partition_lba=524280` → `ext2: mounted (blocksize=4096, inode_size=256, inodes_per_group=8192)` → Heap → SYSCALL/SYSRET → canary → interrupts → `Timer ticks before sched: 6` → `Activating scheduler...` → DHCP lease `.142` (`DISCOVER → OFFER → REQUEST → ACK gw=192.168.1.1`) → `arp: REPLY gw_mac=d4:6a:91:ce:70:60` → `net: L2 OK` → kybernet (`0 processes`, `3462 free pages`) → **`AGNOS shell v1.33.1`**. Rubric rows cleared: **Mount + gate** — write self-test ran (gate lifted, `write_ok=1` on the checksummed+64bit FS); **File create + persist** — after the `echo > /persist.txt` create and a **power-cycle**, `agnos> ls` returns `./ ../ lost+found/ hello.txt welcome.txt agnos/ persist.txt` — `persist.txt` SURVIVED the reboot on the **unmodified default `mkfs.ext4` partition** (no re-carve). The host-`e2fsck -fn` after-burn row (pull-drive verification) is the optional belt-and-suspenders check; the in-system survive-the-reboot evidence is the dispositive PASS. **1.33.1 RELEASED 2026-05-25; the ext2/ext4 WRITE arc clears the demo→base boundary on real NAND.** Next: **1.33.2 RELEASED** as a standalone lockup-hardening cut (bench/serial stability — see [`#tracker-1332-lockup`](#tracker-1332-lockup)); the WRITE follow-ons moved to **1.33.3 OPEN — WRITE-arc-continued** (rename / hardlink / symlink-create + `s_state` dirty/clean + `sync` verb). See [`#tracker-1333-cycle`](#tracker-1333-cycle).

---

### Tracker: 1.34.1 cycle — exFAT (OPEN 2026-05-26 — the FAT-family sibling, split out of 1.34.0 per the 2026-05-26 FS-scope decision. **Lean cycle-open**: VERSION 1.34.0 → 1.34.1, CHANGELOG header, this tracker; no code bites yet.) {#tracker-1341-cycle}

**exFAT is a sibling filesystem, not a FAT mode** (audit `fat-family-prior-art.md` § 4): 12-sector boot region (Main + Backup) with `"EXFAT   "`@3 + a VBR checksum sector; an **allocation bitmap** (1 bit/cluster, found via a root dir-set entry type `0x81`) instead of a FAT free-scan; an **upcase table** (`0x82`) for case-insensitive Unicode; the FAT used **only for fragmented chains** (contiguous files set `NoFatChain` and the FAT is ignored for them); 32-byte **typed dir records** — a file = `0x85` File (attrs/timestamps/`SecondaryCount`/**SetChecksum**) + `0xC0` Stream-Ext (NameLength/NameHash/`FirstCluster`/`DataLength`/`NoFatChain`) + ⌈NameLen/15⌉ `0xC1` File-Name (UTF-16, 15 chars each; no 8.3/LFN). Near-zero shared code with `fatfs` → **new `kernel/core/exfat.cyr`** module + its own MSFT-Basic-Data probe (the `fatfs` probe rejects exFAT: `BytsPerSec`@11 is 0).

**Bite plan** (each code bite = a QEMU smoke gate with **`mkfs.exfat`/`fsck.exfat`** (exfatprogs) as the host oracle — the exFAT analogue of `e2fsck`/`fsck.fat`; **NO iron burn until the final bite**):

1. ✅ **LANDED 2026-05-26 — multi-source exFAT audit** ([`exfat-prior-art.md`](prior-art/exfat-prior-art.md)). Offset-precise: boot region (`"EXFAT   "`@3, FatOffset@80/FatLength@84/ClusterHeapOffset@88/ClusterCount@92/RootCluster@96/BytesPerSectorShift@108/SectorsPerClusterShift@109/NumberOfFats@110), `cluster→sector = ClusterHeapOffset + (c-2)*spc`; FAT 32-bit (EOC 0xFFFFFFFF) **used only for fragmented files** (`NoFatChain` flag → contiguous); allocation **bitmap** (type-`0x81` root entry, LSB-first, bit `i`→cluster `i+2`); upcase (`0x82`; ASCII upcase suffices for ASCII names); dir-**set** = `0x85` File (SecondaryCount + **SetChecksum**) + `0xC0` Stream-Ext (NameLength/NameHash/FirstCluster/DataLength/NoFatChain) + ⌈NameLen/15⌉ `0xC1` File-Name (UTF-16). Pinned the three rotate-add algos: **SetChecksum** (16-bit, skip the set's own bytes 2-3), **NameHash** (16-bit over upcased UTF-16), **boot checksum** (32-bit, skip 106/107/112 — read doesn't need it; write won't touch the boot region). AGNOS mapping = new `exfat.cyr` mirroring the `fatfs`/`ext2` mount + `exfat_blk_{read,write}` routing; MSFT-Basic probe (fatfs already self-excludes exFAT via `BytsPerSec`@11=0).
2. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — exFAT mount + read** (`kernel/core/exfat.cyr` new; `core/agnos.cyr` include after `fatfs.cyr`; `core/main.cyr` `exfat_init()` after `fatfs_init()` + `EXFAT_SELFTEST` block; `scripts/build.sh` flag; `scripts/exfat-smoke.sh` new). Mirrors the `fatfs`/`ext2` mount: `exfat_init` probes whole-disk then GPT ESP/MSFT-Basic on `blk_active`, routing through `exfat_blk_read = blk_read_on(backend, part_lba+sec)`; `exfat_parse_boot` (`"EXFAT   "`@3, geometry @80/88/92/96/108/109/110); `exfat_next_cluster` (32-bit FAT, EOC/bad→0, one-sector cache); `exfat_locate_system_files` records the `0x81`/`0x82` entries; `exfat_find` walks the root as one ordered entry stream with a set-collection state machine (handles a `0x85`+`0xC0`+`0xC1…` set straddling a sector/cluster boundary), ASCII-caseless name match; `exfat_read` contiguous (`NoFatChain`) or chain. **Seeding finding**: exFAT has no `mtools`-equivalent and archaemenid has no non-interactive root / no fuse, so a file can't be auto-seeded. **Validate the read substrate against what `mkfs.exfat` writes into an EMPTY volume instead** — `exfat-smoke.sh` (GPT: FAT32 ESP boot path + `mkfs.exfat -c 512` MSFT-Basic p2) gates **`exfat: mounted backend=2 partition_lba=67584 spc=1 root_clus=47`** (boot parse + MSFT-Basic probe + bitmap@clus2/upcase@clus35/len5836 located) + **`exfatu: upcase-checksum OK`** — reading the 5836-B up-case table back over its **multi-cluster FAT chain** reproduces the `TableChecksum` `mkfs.exfat` baked into the `0x82` entry (independent chain-read oracle, verified in Python first = `0xE619D30D`); `fsck.exfat -n` → clean (reads don't mutate). Build 740,272 → **759,256 B**. The `0x85`/`0xC0`/`0xC1` **file-set read** (name reconstruction + data read) is implemented + compile-clean but **not exercised by the automatic gate** — `exfat-smoke.sh` reads back a seeded `EXFTEST.BIN` (`exfatr: file-read OK`) only when present (needs a privileged `mount -t exfat` seed). Carry-forward: close the file-set gate via a user-driven seed (or a later writer); non-ASCII names need the real upcase table; >512-B logical sectors.
3. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — exFAT write (core mutation set)** (`kernel/core/exfat.cyr` write path; `core/main.cyr` `EXFAT_WRITE_SELFTEST` block; `scripts/build.sh` flag; `scripts/exfat-write-smoke.sh` new). **Verified the bug-prone 16-bit checksums in Python first** (a hand-injected dir-set blessed by `fsck.exfat`: SetChecksum `0x873E` + NameHash `0x6457` on `EXWRITE.BIN`) before the Cyrius port — the doc's pseudocode `X & 0xFFFF + b` is precedence-ambiguous; correct = rotate-right-1, add, mask. **3a dir-set create** — `exfat_emit_set` writes `0x85`(SecondaryCount + SetChecksum + attr) + `0xC0`(flags + NameLength + NameHash + ValidDataLength/FirstCluster/DataLength) + ⌈n/15⌉ `0xC1`(UTF-16) into a free `0x00` run (`exfat_find_free_dir_run`; the trailing `0x00` stays, preserving end-of-dir); `exfat_csum16` is the shared 16-bit ror-add; `exfat_create` = empty file. **3b content write** — bitmap allocator (`exfat_bitmap_get/set` address a cluster's bit **through the bitmap file's own cluster chain** via `exfat_bitmap_load_byte`, dedicated `exfat_bmp_buf`; `exfat_alloc_contiguous` free-run scan); `exfat_write_file` allocs a contiguous run, writes data, **then** publishes the set with `AllocationPossible|NoFatChain` (data-before-dirent → crash leaks bitmap bits, never cross-links). **3c delete + truncate** — `exfat_find` records the set's sector/slot/count; `exfat_delete` clears the InUse bit (0x80) on every set entry then frees; `exfat_truncate_zero` zeroes the `0xC0` alloc fields + recomputes SetChecksum then frees; `exfat_free_clusters` clears bitmap bits (contiguous / chain). **Gate** (`exfat-write-smoke.sh`, no seeding — AGNOS is the writer): boot mutates a fresh `mkfs.exfat -c 512` volume, extract the post-boot partition, **`fsck.exfat -n` → clean, files 3**; plus 3a create rc=0 + find-back, 3b `write rc=0` + multi-cluster (3000 B/6 clusters, `nfc=1`) round-trip byte-exact via AGNOS's own `exfat_read`, 3c delete + truncate rc=0 with no leaked/cross-linked cluster. Build 759,256 → **768,024 B**; `test.sh` 4/4, ext2 + FAT smokes green. **Also closed the bite-2 file-set read gate** — once a `mount -t exfat`-seeded `EXFTEST.BIN` was present, `exfatr: file-read OK` (`got=3000 nfc=1`, byte-exact) confirmed the `0x85`/`0xC0`/`0xC1` read + name reconstruction. **exFAT read + write COMPLETE.**

**1.34.x write-completeness continuation** (organized 2026-05-26 — only FAT/exFAT were the planned minor; the non-verb carry-forwards from 1.34.0 + 1.34.1 land as 4 themed in-arc cuts; **shell verbs stay deferred to the 1.39.x VFS lift**, not bolted on as throwaway `fat*`/`exfat*` verbs). Each cut = a QEMU smoke gate, `fsck.fat -n` / `fsck.exfat -n`-clean, no iron burn until the cap:

1. ✅ **1.34.2 — exFAT write parity — RELEASED 2026-05-26** (VERSION 1.34.1 → 1.34.2 via `version-bump.sh`; kernel banner v1.34.2; CHANGELOG `[1.34.2]`; release-tree smoke all-PASS + `fsck.exfat` clean; build 772,568 B; awaiting user git tag): **overwrite-existing** — `exfat_write_file` allocates+writes new data, repoints the existing dir-set in place (`exfat_set_update_alloc`: rewrite `0xC0` flags/ValidDataLength/FirstCluster/DataLength + recompute SetChecksum), frees old clusters AFTER repointing (crash → old file intact + leaked new, never dangling); **arbitrary-length truncate** — `exfat_truncate(name,newlen)` shrink-only (shrink DataLength + recompute SetChecksum, then free contiguous tail; grow + chained-file-tail are follow-ons); **PercentInUse maintenance** — `exfat_update_percent_in_use` sequential bitmap popcount → `PercentInUse`@112 in the main + backup boot sectors (offset 112 is EXCLUDED from the boot checksum → no recompute; `fsck.exfat -n` clean confirms the boot region survives the write — the one boot-region field a writer may touch; VolumeDirty left clean, no transaction model); **ENOSPC rollback** — alloc returns 0 before setting any bits → write returns -1 with nothing published. Gate: `exfat-write-smoke.sh` overwrite 3000→2000 byte-exact + truncate 2000→1000 + ENOSPC (100 MB on a 67 MB vol) clean, `fsck.exfat -n` clean files 3; build 768,024 → **772,568 B**; `test.sh` 4/4, ext2 + FAT green. NB overwrite/truncate run **in place** on an existing file; creating *new* files past the single-cluster root's 16 entries needs root extension + deleted-slot reuse → folded into the 1.34.4 cross-boundary cut.
2. ✅ **1.34.3 — FAT LFN/truncate completeness — RELEASED 2026-05-26** (VERSION 1.34.2 → 1.34.3 via `version-bump.sh`; kernel banner v1.34.3; CHANGELOG `[1.34.3]`; release-tree `fat-write-smoke.sh` all gates + `fsck.fat -n` clean; build 772,568 → **783,240 B**, `test.sh` 4/4; awaiting user git tag): ✅ **bite 1 LFN-with-content** (`fatfs_write_file_lfn`: alloc+write chain, then LFN set with the 8.3-alias entry pointing at the first cluster + FileSize, vs `fatfs_create_lfn`'s empty cluster-0/size-0; `LongContent12345.bin` 3000 B byte-exact through its long name). ✅ **bite 2 LFN-name overwrite-match LANDED + QEMU-VALIDATED 2026-05-26** — new `fatfs_find_lfn` does the first LFN *read*: reassembles long names from their reverse-ordered `0x0F` runs (13 UTF-16 chars/entry, terminator-aware) + matches LONG name OR 8.3 name + records the 8.3 slot; `fatfs_write_file_lfn` uses it to overwrite a long-named file IN PLACE (repoint cluster+size, free old chain, preserve alias + LFN entries) instead of duplicating under a fresh `~N` alias. Gate: `LfnOver12345.bin` 3000→2000 B by long name → **single** dir entry (no duplicate) + content byte-exact. ✅ **bite 3 grow-truncate** — `fatfs_truncate` was shrink-only; now grows to `newlen` with the grown region reading as zeros (zero-fill old-last-cluster slack + alloc+zero-fill new clusters + link + publish size last; handles grow-from-empty + grow-within-last-cluster; ENOSPC rolls back). Gate: `GROW.BIN` 1000→3000 B → `[0,1000)` data + `[1000,3000)` zeros, `fsck.fat -n` clean. exFAT has no LFN (UTF-16-native), so this is FAT-only. **NEXT in 1.34.x: 1.34.4 cross-boundary dir runs + root extension.** (Delete-by-long-name — clearing the LFN run — is a follow-on; `fatfs_find_lfn` already locates the set.)
3. **1.34.4 — cross-boundary directory runs + root extension** (IN PROGRESS, under CHANGELOG `[Unreleased]`, no bump): ✅ **exFAT side LANDED + QEMU-VALIDATED 2026-05-26** — root extension + cross-sector dir-set unified into one correct **spanning append**: `exfat_fat_set` (FAT-chained root needs FAT writes) + `exfat_root_cluster_for_index` (extend root by fresh zeroed clusters) + `exfat_dir_append_set` (append at the first `0x00`, stream entries across sector/cluster boundaries, extend as needed); `exfat_emit_set` builds into a 640-B scratch then appends. **Bug caught + fixed via the smoke**: the first attempt placed sets one-sector-fit, which stranded a `0x00` before live entries → `fsck.exfat` "0x85 follows unused entry"; the append-at-end-of-dir rewrite is the correct exFAT semantics. Gate: 10 new files (30 dir entries) past the 16-entry root → `fsck.exfat -n` **clean, files 13** + extended-root readback byte-exact (re-enables the 1.34.2-deferred multi-new-file create path). `fsck.exfat -n` clean, files 13. ✅ **FAT side LANDED + QEMU-VALIDATED 2026-05-26** — same spanning-append mechanism mirrored into `fatfs.cyr`: `fat_root_extend` + `fat_root_cluster_for_index` (FAT32 root-chain extend) + `fat_dir_end_index` + `fat_dir_append_set` (append at first `0x00`, span sector/cluster, extend FAT32 root; FAT12/16 fixed root bounded by RootEntryCount); `fatfs_create_lfn`/`fatfs_write_file_lfn` build the LFN set into a shared scratch then append; `fatfs_find_free_root_slot` extends a 100%-full FAT32 root. Gate (`fat-write-smoke.sh`): 40 LFN-named files (`LfNN_longx.dat`, 120 dir entries, distinct `~N` bases) past the 16-entry FAT32 root cluster → `fsck.fat -n` clean, all 40 long names reconstruct (count=40), readback byte-exact. **1.34.4 RELEASED 2026-05-26** (exFAT + FAT; VERSION 1.34.3 → 1.34.4, banner v1.34.4, release-tree both write smokes + `fsck` clean, awaiting user git tag). Build 783,240 → **788,696 B**, `test.sh` 4/4. **NEXT in 1.34.x: 1.34.5 exFAT Unicode names** (last item). (Deleted-slot reuse deferred — directories grow append-only; deleted entries stay fsck-valid holes.)
4. ✅ **1.34.5 — exFAT Unicode names — RELEASED 2026-05-26** (VERSION 1.34.4 → 1.34.5, banner v1.34.5, release-tree exfat-write-smoke + `fsck.exfat` clean, awaiting user git tag): `exfat_load_upcase` loads the `0x82` table into an 8 KB buffer at mount; `exfat_upcase` maps a UTF-16 unit via the RLE-compressed table (`0xFFFF`+count = identity run; verified in Python first against a real table: `a→A`, `é→É` U+00E9→U+00C9, `ÿ→Ÿ` U+00FF→U+0178); `exfat_name_hash` + `exfat_name_eq` now use it (ASCII fallback if the table is absent/oversized). Gate: `exfat-write-smoke.sh` creates `Café.txt` (byte `0xE9`) → `fsck.exfat -n` **clean** (the discriminating oracle — fsck recomputes the NameHash with the volume table; ASCII upcase would mismatch) + find-by-non-ASCII-name + readback. Build 788,696 → **798,648 B**, `test.sh` 4/4. **The 1.34.x write-completeness continuation (1.34.2–1.34.5) is FEATURE-COMPLETE.** Remaining 1.34.x: arc cap only — ESP-write safety guard + user-driven FAT/exFAT iron burn.

**Arc cap.** ✅ **ESP-write safety guard RELEASED 2026-05-26 as 1.34.6** (VERSION 1.34.5 → 1.34.6, banner v1.34.6, CHANGELOG `[1.34.6]`): `fatfs`/`exfat` record `fat_on_esp`/`exfat_on_esp` at mount (set when the matched GPT partition's type GUID is ESP; 0 for MSFT-Basic / whole-disk), and the single write chokepoint `fat_blk_write`/`exfat_blk_write` refuses (`#ifndef FAT_ALLOW_ESP_WRITE`) when on the ESP — so the boot ESP (gnoboot `BOOTX64.EFI` + the kernel) is read-only and a write self-test / write bug can't brick boot. Validated 3 ways: **(A) fires** — `FATFS_WRITE_SELFTEST` with NO override on the ESP image → `fatw: create NEWFILE.TXT rc=-1`, `BOOTX64.EFI` present, NEWFILE absent, `fsck.fat` clean; **(B) override** — `FAT_ALLOW_ESP_WRITE` → ESP writes work (used by `fat-write-smoke.sh`, whose ESP is a throwaway test image); **(C) no false-block** — exFAT on a MSFT-Basic partition with NO override → writes work, `fsck.exfat` clean. (`boot_info` carries no boot-device field, so the guard keys off the ESP **partition-type GUID**, not "the partition we booted from.")

**Iron-burn plan (user-driven, blocks other work per [[feedback_iron_burns_block_other_work]]) — the FAT-family arc's first iron touch:**
1. **Build** `FATFS_WRITE_SELFTEST=1 EXFAT_WRITE_SELFTEST=1 ./scripts/build.sh` — **WITHOUT** `FAT_ALLOW_ESP_WRITE` (the boot ESP stays read-only).
2. **Provide a writable data volume** AGNOS mounts as the FAT/exFAT target — a **Microsoft-Basic-Data** FAT32 (and/or exFAT) partition on the **NVMe** (probed before USB-MS, so it's the first match ahead of the boot USB's ESP), or a whole-disk FAT/exFAT USB stick. `mkfs.vfat` / `mkfs.exfat` it from Linux first (mount-modify per [[feedback_prefer_mount_modify_over_reflash]]).
3. **Burn + read the FB**: the write self-tests run on the mounted data volume → `fatw:`/`exfatw:` `rc=0` + round-trip-OK lines on the framebuffer (no serial on iron per [[feedback_no_serial_on_iron]]).
4. **Validate from Linux**: power off, mount the data volume, `fsck.fat -n` / `fsck.exfat -n` → clean + the test files present + byte-exact.
5. **Safety**: the guard means the boot ESP is never written even if AGNOS mounts it (writes → rc=-1), so the burn **cannot brick boot** regardless of which volume mounts. If the topology makes AGNOS mount the ESP first (so writes are refused and the test doesn't exercise), the follow-on is a small mount-preference (prefer MSFT-Basic > ESP) — **confirm archaemenid's disk topology before the burn**.

Shell verbs (`touch`/`rm`/`mkdir`/`cat`/`ls` on FAT/exFAT) remain out of this arc → 1.39.x VFS generic-write lift. Roadmap: agnos `docs/development/roadmap.md` row 21.

**Falsification rubric**: each read bite byte-exact vs the host `mkfs.exfat`-seeded file; each write bite `fsck.exfat` clean + host confirms persistence; no regression to ext2 / FAT / the boot path. FALSIFIED = `fsck.exfat` flags a structure (→ re-derive from the audit) or a multi-cluster read truncates.

---

### Tracker: 1.34.0 cycle — FAT-family filesystems (FAT write → exFAT) (RELEASED 2026-05-26 — tagged 1.34.0; the FAT12/16/32 read+write arc. exFAT split to 1.34.1 § [`#tracker-1341-cycle`](#tracker-1341-cycle).) {#tracker-1340-cycle}

**Starting-point finding (reframes the arc).** The existing `kernel/core/fatfs.cyr` is **minimal FAT16, READ-ONLY** (249 LOC; header literally `# === FAT16 Filesystem (Read-Only) ===`, "for virtio block device"). Two gaps beyond "no write": (1) **FAT16-only** — the data-start / fixed-root-region math is FAT16-shaped; the real iron targets (the ESP + commodity USB sticks) are **FAT32**, where the root directory lives in the cluster chain, not a fixed region; (2) **single-backend** — it appears to read the VirtIO device directly rather than routing through `blk_read_on(backend)` the way ext2 does, so on the NVMe/AHCI/USB-MS iron it may not even mount. So this arc is **not** "bolt write onto a finished FAT driver" — it is *maturing* the driver: **FAT32 read parity + multi-backend routing first**, **then write**, then **exFAT**. Bite 1's audit confirms/scopes this.

**Strategic anchor.** FAT write is the **first *second* writable filesystem** on AGNOS → the concrete trigger for the **1.39.x VFS generic-write lift** (roadmap row 15 — the `ext2_*`→`vfs_*` abstraction earns itself only when a second writable FS exists). So this arc has architectural leverage beyond the feature itself.

**Bite plan** (refined by the bite-1 audit; each code bite = a QEMU smoke gate with `fsck.fat`/`exfatprogs` as the host oracle — the FAT-family analogue of `e2fsck` for the ext2 arc; **NO iron burn until the final bite** per [[feedback_iron_burns_block_other_work]]):

1. ✅ **LANDED 2026-05-26 — multi-source FAT-family audit** ([`fat-family-prior-art.md`](prior-art/fat-family-prior-art.md), per [[feedback_redesign_dont_reinvent]]; MS FAT spec / EFI FAT32 SPG + exFAT spec + FreeBSD `msdosfs`/`newfs_msdos` + Linux `fs/fat` & `fs/exfat` + ECMA-107; `fsck.fat`/`mtools`/`exfatprogs` oracle). **The diff against `fatfs.cyr` (read in full, not its comments) found 5 gaps, bigger than "no write":** (1) `fatfs_init` is wired **only inside the virtio-blk branch** (`main.cyr:306-315`) → never runs on NVMe/AHCI/USB-MS iron; (2) **whole-disk absolute LBA 0**, not partition-aware → can't mount the ESP/USB FAT partition (`blk_read`, not `blk_read_on(backend, partition_lba+sec)`); (3) **no FAT-chain traversal** — `fatfs_fat_buf` declared-but-**unused**; `fatfs_open`/`fatfs_read` read only the **first cluster, capped 512 B**; (4) **FAT16 layout only** — `FATSz16`@22 + fixed-root-region math; FAT32 (`FATSz32`@36, `RootClus`@44, cluster-chain root — the format of the ESP + most USB sticks) mis-parses; (5) **8.3 only, no LFN, no write.** Also pinned: the **only** correct FAT-type test is `CountOfClus` (<4085 FAT12 / <65525 FAT16 / else FAT32); FAT writes touch **all** FAT copies + maintain `FSInfo`; FAT has no journal (fsck = crash-safety, like ext2); FAT `sync` should call the 1.33.5 `blk_flush_on`. **exFAT is a sibling FS, not a mode flag** (allocation bitmap + typed 32-B dir-set `0x85`/`0xC0`/`0xC1` + upcase + FAT-for-fragmented-only). **Reframes the arc**: gaps 1–4 (mature read: partition-aware multi-backend mount + FAT-chain traversal + FAT32 + LFN read) are the **precondition** for write — a write path on a driver that can't follow a chain or mount a partition is meaningless.
2. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — FAT32 read parity + partition-aware multi-backend mount + cluster-chain traversal** (`fatfs.cyr`, `main.cyr`, `build.sh`, new `fat-smoke.sh`). Closes audit gaps 1–4. `fatfs_init` rewritten to the ext2 mount pattern (`fat_backend` + `fat_partition_first_lba`; every read via new `fat_blk_read` = `blk_read_on(...)`); walks all registered backends, then GPT ESP/MSFT-Basic partitions; **lifted out of the virtio-only branch** to a standalone `main.cyr` call after backends+GPT. `fatfs_parse_bpb` adds FAT32 EBPB (`FATSz32`@36/`RootClus`@44) + the `CountOfClus` type test. New `fat_next_cluster` (FAT12 packed+sector-straddle / 16 / 32 28-bit, one-sector FAT cache) + `fatfs_first_sector_of_cluster`; `fatfs_ls`/`fatfs_find_root`/`fatfs_read` all follow the chain (FAT32 root chain + FAT12/16 fixed region); `fatfs_read` reads a whole multi-cluster file into the caller buffer (was first-cluster-≤512-B). **Gate PASSED** — `fat-smoke.sh` (FATFS_SELFTEST) on a FAT32 ESP seeded with a 3000-B multi-cluster `FATTEST.BIN` (byte[i]=i&0xFF): `fat: mounted FAT32 backend=2 partition_lba=2048` + `fatr: FATTEST.BIN size=3000 got=3000` + `fatr: chain-read OK` (byte-exact at 0/255/256/1000/2999 — reads past the old 512-B cap). Build 712,504 → **722,064 B**; `test.sh` 4/4, `ext2-smoke` 5/5, `ext2-write-smoke` W1–W5 PASS (no regression). **Residual read item**: LFN reassembly (8.3 names work today; the gate file is 8.3 so didn't need it) — small, folds into bite-3 prep.
3. **FAT write** (sub-divided — each sub-bite `fsck.fat -n`-clean + `mtools` persistence; the write smoke writes into the FAT32 **ESP** alongside the real gnoboot+agnos boot files, so fsck-clean = "didn't corrupt the coexisting files," a stringent gate):
   - 3a. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — empty-file create (dirent-write primitive).** `fat_blk_write` (= `blk_write_on(...)`) + `fatfs_build_83` (factored out of `match_name`, shared read/write 8.3 encoder) + `fatfs_create`/`fatfs_emit_dirent` (refuse-exists → find free slot 0x00/0xE5 in the root → write 8.3 dirent, attr=ARCHIVE, cluster 0, size 0; root-extend + LFN-gen are follow-ons). Isolates dirent plumbing from the allocator (ext2-arc discipline: W1 before W4). New `fat-write-smoke.sh` + `FATFS_WRITE_SELFTEST`: `fatw: create NEWFILE.TXT rc=0` + **`fsck.fat -n` clean** on the post-boot ESP + `mdir` shows NEWFILE.TXT. Build 722,064 → **724,048 B**; `test.sh` 4/4, `ext2-smoke` 5/5, FAT read regression intact (`chain-read OK`). (One harness gotcha pinned: `mformat -i img@@1MiB -F` formats the FAT32 to the END of the 128 MiB image, ~127 MiB — not the 32 MiB GPT partition — so fsck must read the full region, not a 32 MiB slice.)
   - 3b. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — cluster allocator + multi-cluster content write.** `fat_get_entry` (raw value, vs `fat_next_cluster`'s EOC→0) + `fat_set_entry` (writes ALL FAT copies, FAT32 preserves top-4 reserved bits; **FAT12 write refused** — reads still work) + `fat_eoc` + `fat_alloc_cluster` (free-scan from 2, mark EOC) + `fat_fsinfo_mark_unknown` (FSInfo `Free_Count`/`Nxt_Free`→0xFFFFFFFF so `fsck.fat -n` skips the stale-count check) + `fatfs_write_file` (alloc chain → link prev→cur → write data zero-padding slack → publish dirent cluster+size → FSInfo; ordered so a crash leaks clusters, never a live dirent→free chain). `fatfs_create` refactored onto shared `fatfs_find_free_root_slot` + generalized `fatfs_emit_dirent` (carries cluster+size, self-contained re-read). **Gate PASSED** — `fat-write-smoke.sh` writes a 3000-B multi-cluster `WTEST.BIN` (byte[i]=i&0xFF, ~6 clusters): `fatw: write WTEST.BIN rc=0` + **`fsck.fat -n` clean** + **`mtype` byte-exact** vs host pattern (`cmp`). Build 724,048 → **729,232 B**; `test.sh` 4/4, `ext2-smoke` 5/5, FAT read regression `chain-read OK`. ENOSPC-rollback + overwrite-existing are follow-ons.
   - 3c. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — delete (unlink) + truncate-to-zero.** `fat_free_chain` (walk chain, free each entry in all FAT copies, read-next-before-free, loop-guarded) + `fatfs_delete` (mark dirent `0xE5` FIRST, then free chain — crash leaves leaked clusters, never a live dirent→free chain) + `fatfs_truncate_zero` (clear dirent cluster+size first, then free chain). `fatfs_find_inner` now records the matched dirent location (`fatfs_slot_sec`/`idx`) for in-place modify. **Gate PASSED** — write+delete `DELME.BIN`, write+truncate `TRUNC.BIN`: `wrc=0 drc=0` / `wrc=0 trc=0` + **`fsck.fat -n` clean after freeing BOTH chains** (no leaked clusters) + `mdir` DELME absent + `mtype` TRUNC 0 bytes. Build 729,232 → **731,072 B**; `test.sh` 4/4, `ext2-smoke` 5/5, FAT read `chain-read OK`. Core mutation set (create/write/delete/truncate) complete + all fsck-clean. Arbitrary-length truncate is a follow-on.
   - 3d. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — LFN (long filename) create.** `fat_lfn_checksum` (8.3-alias rotate-add checksum) + `fatfs_name_fits_83` (8.3 names → existing create) + `fatfs_gen_alias` (`BASIS~N.EXT`, unique via `fatfs_alias_exists`) + `fatfs_find_free_run` (K+1 contiguous slots in one sector) + `fatfs_emit_lfn` (seq/`0x40`-last, 13 UTF-16 chars, 0x0000+0xFFFF pad, checksum) + `fatfs_create_lfn` (K=ceil(namelen/13) reverse-order LFN entries + 8.3 alias, one-sector RMW). **Gate PASSED** — create `LongFileName12345.txt` (2 LFN entries): `fatw: lfn create rc=0` + **`fsck.fat -n` clean** + **`mdir` reconstructs the exact long name** (proves the LFN chain + the 8.3-alias checksum link are correct). Build 731,072 → **738,272 B**; `test.sh` 4/4, `ext2-smoke` 5/5, FAT read `chain-read OK`. **Shell verbs deferred to the 1.39.x VFS lift** (FAT write is its trigger; `fat*`-prefixed verbs now = throwaway). LFN-with-content + LFN refuse-exists + cross-sector runs are follow-ons. **Bite 3 (FAT write) COMPLETE: create / write / delete / truncate / LFN, all fsck-clean.**
   - 3e. ✅ **LANDED + QEMU-VALIDATED 2026-05-26 — write polish (closes bite 3).** Overwrite-existing (`fatfs_write_file` reuses the dirent, clears cluster+size, frees the old chain dirent-first, writes new content; 8.3-match only) + ENOSPC rollback (free the partial chain on `fat_alloc_cluster` exhaustion) + `fatfs_truncate(name, newlen)` (arbitrary shrink: keep ceil(newlen/clus) clusters, cap EOC, free tail; newlen=0 → truncate_zero; grow = follow-on). **Gate PASSED** — `WOVER.BIN` write-1000-then-overwrite-2000 (`mtype` = 2000 B byte-exact, old chain replaced) + `TRUNC2.BIN` write-3000-then-truncate-1000 (`mtype` = first 1000 B byte-exact, tail freed) + **`fsck.fat -n` clean through every freed chain**. Build 738,272 → **740,272 B**; `test.sh` 4/4, `ext2-smoke` 5/5, FAT read intact. ENOSPC-rollback code-complete (not smoke-exercised — can't fill a 127 MiB FS).
   - **Iron-safety note (resolve before the final iron burn):** writing to the **ESP** is dangerous on real hardware (it holds the bootloader). The QEMU smoke writes the ESP safely (fresh image each run, boot already loaded). Before the user iron burn: either prefer a Microsoft-Basic-Data partition for the write mount, or add a write-guard refusing ESP-type partitions.

**Bite 4 — exFAT → moved to a 1.34.1 cut** (FS-scope decision 2026-05-26: finish FAT12/16/32 fully in 1.34.0, exFAT as its own follow-on). Read (boot region + allocation bitmap + upcase + typed dir-set) then write; sibling FS, near-zero shared code.

**🎯 1.34.0 CYCLE QEMU-COMPLETE 2026-05-26** — `fatfs` matured from a toy whole-disk FAT16-RO virtio-only reader into a real **partition-aware multi-backend FAT12/16/32 driver**: full read (FAT32 + cluster-chain, bite 2) + full write (create / multi-cluster content / delete / truncate-to-zero / arbitrary-truncate / LFN-create / overwrite, bites 3a–3e), every mutation **`fsck.fat -n`-clean + `mtype`-verified** on a FAT32 ESP alongside live boot files (`fat-smoke.sh` + `fat-write-smoke.sh`, `FATFS_SELFTEST`/`FATFS_WRITE_SELFTEST` flags). Production build 740,272 B; `test.sh` 4/4, `ext2-smoke` 5/5, `ext2-write-smoke` unaffected. **NEXT = user tag of 1.34.0**, then **1.34.1 exFAT**. Carry-forwards (all documented, none blocking the cut): shell verbs → 1.39.x VFS lift; ESP-write safety guard before any FAT iron burn; LFN-with-content + LFN-name overwrite-match + grow-truncate + cross-sector LFN runs + ENOSPC-rollback-smoke-exercise.

**Falsification rubric**: each write bite = `fsck.fat -n` (FAT) / `fsck.exfat` (exFAT) **clean** after the self-test + host `mtools` (`mdir`/`mtype`) confirms the mutation persisted, exactly as `e2fsck -fn` + `debugfs` gated the ext2 WRITE arc; no regression to the existing FAT16 read path or to ext2. **FALSIFIED / rethink** = a mutation that `fsck.fat` flags (FAT-chain inconsistency, lost cluster, bad dirent) → re-derive ordering from the audit, not by trial.

---

### Tracker: 1.33.5 cycle — `fsync`/FLUSH-CACHE durability barrier (RELEASED 2026-05-26 — the **last** 1.33.x WRITE-arc follow-on; after this the patch line closes and the big own-cycle writes move to 1.37.x–1.39.x. **Lean cycle-open**: VERSION 1.33.4 → 1.33.5, CHANGELOG header, this tracker; no code bites yet.) {#tracker-1335-cycle}

**Premise (what 1.33.3 left honest-but-incomplete).** The 1.33.3 `sync` verb closes the *filesystem* contract: it flips `s_state` back to `EXT2_VALID_FS` and AGNOS writes synchronously through `blk_write`, which **completes** the NVMe/AHCI command before returning ([`ext2-ext4-write-prior-art.md`](prior-art/ext2-ext4-write-prior-art.md) § 2.3) — so kernel step-ordering *is* on-disk ordering, there's no AGNOS writeback cache to reorder behind us. What it does **not** cover: the **drive's own volatile write cache**. A controller can ACK a write command while the data still sits in the drive's DRAM/SLC cache, not yet on NAND/platter. A power-cycle in that window loses acknowledged data. `fsync`/FLUSH-CACHE is the barrier that forces the drive to commit — making `sync` an *honest durability* boundary, not just an FS-state flip.

**This is a HARDENING item, not a correctness fix — scope honestly.** The prior-art doc (§ 3, § 14 item 3) already records: *"the drives we burn on honor write-completion, and the smoke gate proves it"* — the W5 iron burn persisted `persist.txt` across a real power-cycle on archaemenid's Crucial P3 with **no** explicit flush. So there is **no reproducible data-loss bug to fix**; the value is (1) contract correctness — `sync` should *mean* durable across every backend, incl. drives that *do* buffer aggressively — and (2) removing a silent assumption. Do **not** over-invest chasing an observable "before" failure; the rubric is *"the flush command is issued and the device acknowledges it,"* not *"data that was being lost is now retained."*

**Hypothesis.** A `blk_flush()` dispatch mirroring the existing `blk_write`/`blk_read` fan-out (`core/block.cyr`), issuing the per-backend cache-flush command and waiting for completion, then called from `ext2_sync()`, makes `sync` a true end-to-end durability barrier with zero regression to the write path.

**Bite plan** (each bite = a QEMU smoke gate; the audit informs the final per-backend decomposition; **NO iron burn until the final bite** per [[feedback_iron_burns_block_other_work]], and even then user-driven):

1. ✅ **LANDED 2026-05-26 — multi-source prior-art audit** ([`flush-cache-prior-art.md`](prior-art/flush-cache-prior-art.md), per [[feedback_redesign_dont_reinvent]] — Linux is one source of many). Converged the flush command shape across **NVM Express** FLUSH (opcode 0x00, NSID-scoped), **ATA8-ACS** FLUSH CACHE EXT (0xEA, 48-bit; vs legacy 0xE7) as an AHCI non-data H2D FIS, **SBC-3** SYNCHRONIZE CACHE(10) (0x35) over USB-MS BBB, **VirtIO 1.x** block FLUSH (`VIRTIO_BLK_T_FLUSH`=4 gated on `VIRTIO_BLK_F_FLUSH`), cross-ref **Linux** `blkdev_issue_flush`/`nvme_setup_flush`/`sd_sync_cache` + **FreeBSD** `BIO_FLUSH` — then **diffed each against the existing AGNOS command path**: NVMe reuses `nvme_io_submit`/`nvme_io_poll` as-is (the non-data degenerate case); AHCI needs a non-data `ahci_issue_nodata` sibling (PRDTL=0) since `ahci_issue_rw` is data-only; USB-MS rides `msc_scsi_exec` with a no-data CDB; VirtIO needs `VIRTIO_BLK_F_FLUSH` negotiation + a 2-descriptor no-data `vblk_do_flush`; RAM-disk is an honest no-op. Dispatch = `blk_flush()`/`blk_flush_on(tag)` mirroring `blk_write_on`; FS hook = `blk_flush_on(ext2_backend)` appended to `ext2_sync()` after the SB write. **One real gotcha surfaced** (audit § 7): a cache flush can take *seconds* (ATA permits ~30 s; NVMe similar) — the transfer-tuned poll ceilings are too tight, so bite 2 needs a dedicated flush timeout, not the reused transfer timeout.
2. ✅ **LANDED 2026-05-26** — `blk_flush()`/`blk_flush_on(tag)` dispatch (`core/block.cyr`, mirrors `blk_write_on`) + **`nvme_blk_flush`** (FLUSH 0x00 via the generic `nvme_io_submit`; `nvme_io_poll` refactored to `nvme_io_poll_n(cid, max_iters)` + default wrapper so the transfer path is byte-unchanged, flush uses a 300M ceiling) + **`ahci_blk_flush`** via new non-data **`ahci_issue_nodata`** (FLUSH CACHE EXT 0xEA, PRDTL=0, reuses `ahci_build_rw_fis`; `AHCI_FLUSH_TIMEOUT_SPINS`=30M) + `ext2_sync()` → `blk_flush_on(ext2_backend)` after the SB write. **Verified**: build 707,896 → **710,328 B**; `test.sh` 4/4; `ext2-smoke` 5/5; `ext2-write-smoke` **PASS on both** `metadata_csum,64bit,extent` **and** the default stripped profile — **`Wsync state`** (the `ext2_sync`→NVMe-FLUSH path) OK, **`e2fsck -fn` clean**, no `nvme: I/O poll timeout`/CID-mismatch in the boot log, full W1–W5+rename/ln/sym regression green. NVMe flush is QEMU-validated end-to-end; AHCI flush is code-complete + structurally identical to the proven `ahci_issue_rw` (not exercised by the NVMe-backed smoke). VirtIO/USB-MS/RAM-disk are a documented no-op default until bite 3.
3. ✅ **LANDED 2026-05-26 — closes the cycle.** **VirtIO** (`virtio_blk.cyr`): `VIRTIO_BLK_F_FLUSH` (bit 9) opportunistic negotiation + `vblk_flush_supported` latch + **`vblk_do_flush`** (2-descriptor no-data chain — header `T_FLUSH`=4/sector=0 + status, vs the 3-desc R/W chain) + `vblk_blk_flush` (no-op when feature absent). **USB-MS** (`msc.cyr`): `msc_blk_flush` = SYNCHRONIZE CACHE(10) 0x35 no-data CDB via the proven `msc_scsi_exec`. **RAM-disk** (`ramdisk.cyr`): `ramdisk_blk_flush` honest no-op. `blk_flush_on`'s three bite-2 no-op arms promoted to real calls (dispatch fully populated). **Verified**: build 710,328 → **712,504 B**; `test.sh` 4/4; `ext2-smoke` 5/5; NVMe write-smoke regression PASS (both profiles). **VirtIO flush validated end-to-end** — a `virtio-blk-pci`-backed boot mounts ext2 on **`backend=1` (VIRTIO)**, `nvme: no controller found`, so `Wsync` drives `ext2_sync`→`vblk_blk_flush`→`vblk_do_flush` (F_FLUSH negotiated under QEMU writeback): `Wsync state` OK, `e2fsck -fn` clean, **QEMU's strict virtqueue validation raised zero descriptor complaints on stderr** (a malformed 2-desc chain would warn). USB-MS `msc_blk_flush` + RAM-disk no-op + AHCI flush (bite 2) are code-complete + structurally identical to proven paths.

**🎯 1.33.5 CYCLE QEMU-COMPLETE 2026-05-26** — all five block backends carry a cache-flush, `blk_flush_on` is fully populated, and `ext2_sync()`/`sync` is an end-to-end durability barrier (FS `s_state` clean **+** drive cache committed). NVMe + VirtIO validated end-to-end in QEMU; AHCI + USB-MS code-complete (mirror the proven `ahci_issue_rw` / `msc_scsi_exec`); RAM-disk no-op. The 1.33.x ext2/ext4 WRITE patch line **closes here** — big own-cycle writes (ext4 extent alloc / jbd2 / VFS generic-write) are 1.37.x–1.39.x (roadmap row 15). **NEXT = user tag of 1.33.5.** Optional iron burn (`echo > /f; sync` on the real NVMe partition) is user-driven, NOT auto-proposed per [[feedback_iron_burns_block_other_work]] — and per the audit § 0 the flush is observable on iron only as no-error (archaemenid's Crucial P3 already honors write-completion; this is hardening, not a bug fix).

**Falsification rubric (QEMU smoke is the oracle; the harness IS the test, no iron burn needed to prove the mechanism):**
- **PASS** = on each backend, `blk_flush` issues the device's flush command and the device returns success (NVMe CQE status 0 / AHCI PxCI clear with no PxTFD error / USB-MS good CSW / VirtIO used-ring OK); `ext2_sync()` calls it; `ext2-write-smoke.sh` stays `e2fsck -fn` clean with no W1–W5 regression; build within the 800 KB `test.sh` ceiling.
- **FALSIFIED / rethink** = a backend's flush command hangs or returns an error in QEMU (→ wrong opcode/FIS shape or missing feature-gate — re-derive from the audit, not by trial); or the flush perturbs the write path (e2fsck regression); or it can't be expressed without a writeback-cache layer AGNOS doesn't have (→ the synchronous-completion model already satisfies the contract and the per-backend flush is genuinely a no-op — document and close, don't manufacture machinery).
- **Caveat (no instrumentation creep per [[feedback_no_instrumentation_means_no_instrumentation]])**: any flush-completion confirmation is a single bounded smoke-only print, never an iron diagnostic; archaemenid has no serial ([[feedback_no_serial_on_iron]]).

---

### Tracker: 1.33.4 cycle — interactive-lockup root-cause **+ the planned ext2/ext4 FS items** (RELEASED 2026-05-26 — tagged 1.33.4; bite 1 = lockup fix; bites 2-3 = uninit_bg materialization + symlink resolution, folded back in after the lockup detour; fsync moved to 1.33.5 § [`#tracker-1335-cycle`](#tracker-1335-cycle)) {#tracker-1334-lockup}
...
**🎯 REPRODUCED OFF-IRON 2026-05-26 — first time this hang is reproducible in QEMU.** New harness [`agnos/scripts/lockup-repro.sh`](../../../agnos/scripts/lockup-repro.sh) + [`lockup-driver.py`](../../../agnos/scripts/lockup-driver.py): boots the **production** kernel via gnoboot+OVMF with `-device qemu-xhci -device usb-kbd`, then types commands one keystroke at a time over QMP `send-key`, using each `uptime`'s live `timer_ticks` as a liveness canary. The shell froze mid-word at `agnos> u` (tick ~600–630) in **two independent runs** — and **`query-status` reported `running: True`**: the vCPU never hard-halted. The CPU stays alive (timer ISR + `hlt`-wake loop running); what dies is **keyboard input** — `hid_poll()` stops delivering events, so the read loop (`shell.cyr:864`) `hlt`s forever waiting for a keystroke that never arrives. From the user's seat that is indistinguishable from a freeze.

**Two findings, in order of importance:**

1. **The context-switch path is INERT in production builds — this RETIRES 1.33.2's "prime suspect."** The two test processes + the scheduler-exercise loop are gated behind `#ifdef KTEST` (`main.cyr:473-599`). A production build never raises `proc_count` to 2, so `do_context_switch` (`sched.cyr:133`) early-returns on `proc_count < 2` every tick and never touches the register-save / `proc_restore_context` code that the 1.33.2 tracker (and `pic.cyr:48-54`) named as the leading lockup suspect. That path cannot be the cause of a production-build hang. (`sched_active=1` is set, but the `proc_count < 2` guard fires first.) Do not re-chase the scheduler for this symptom.

2. **Root cause: the HID keyboard transfer ring has NO Link TRB.** `hid_kbd_configure` (`hid.cyr`) zeroes the 256-TRB transfer-ring page and arms Normal TRBs; `hid_arm_xfer_trb` re-arms one TRB per processed report and wrapped `hid_kbd_xfer_idx` `255 → 0` **with no Link TRB at the end of the ring.** xHCI transfer rings MUST end in a Link TRB (xHCI 1.2 §4.9.2.2) — the controller only returns to slot 0 when it reads one. Without it, after the controller consumes ~255 TRBs it advances past the page end into whatever PMM page follows, the interrupt-IN endpoint stalls, no more Transfer Events are posted, and keyboard input wedges. The freeze point (~tick 600 ≈ ~256 HID reports ≈ one report per transfer TRB) matches the 256-TRB ring depth exactly. The **command ring got this right** (Link TRB at slot 255 — `xhci_ring.cyr:203-206` — with cycle re-align on wrap in `xhci_cmd_submit` `xhci_cmd.cyr:112-130`); the kbd transfer ring was simply built without the equivalent. MSC bulk transfers never tripped it (a handful of TRBs per command, never 256 in a session); the continuously-polled keyboard exhausts the ring within seconds of typing.

**Fix (bite 1, applied 2026-05-26):** mirror the command-ring pattern on the kbd transfer ring — write a Link TRB at slot 255 (base→slot 0, TC=1, initial C=0) in `hid_kbd_configure`, and in `hid_arm_xfer_trb` wrap at `idx == 255`: align the Link TRB's cycle bit to the current producer cycle, flip the cycle, reset idx to 0 (slot 255 reserved, never a Normal TRB). Build 692,112 → **692,384 B**, fmt clean, `test.sh`/`ext2-smoke` unaffected (no FS change).

**Falsification rubric (QEMU harness — the repro IS the oracle here, no iron burn needed per [[feedback_iron_burns_block_other_work]]):**
- Pre-fix: harness freezes at `agnos> u`, tick ~600, within ~18 commands. ✅ confirmed (two runs).
- Post-fix **PASS** = harness runs the full duration with `timer_ticks` rising monotonically into the thousands across **many** ring wraps (a 300 s run types thousands of keys ≫ 256), serial log keeps growing, no stall. → the Link TRB was the cause.
...
### Tracker: 1.33.3 cycle (OPEN 2026-05-25 — **ext2/ext4 WRITE arc continued: the dirent-mutation follow-ons, now that the indirect-write path is iron-proven** ([[project_agnos_maturity_arc]]). These were originally scoped under 1.33.2, but 1.33.2 shipped as a standalone lockup-hardening cut (§ [`#tracker-1332-lockup`](#tracker-1332-lockup)); the WRITE work moved here. 1.33.1's W5 iron burn (above) confirmed `echo > /persist.txt` survives a power-cycle on the **unmodified** default `mkfs.ext4` partition — metadata_csum + 64bit write landed and reaches the platter. So the operations queued in [`ext2-ext4-write-prior-art.md`](prior-art/ext2-ext4-write-prior-art.md) § 4 become cheap follow-ons: they ride the dirent insert/remove + inode primitives W4 already built. **Lean cycle-open** — VERSION 1.33.2 → 1.33.3, CHANGELOG header, tracking surface; no code bites yet. Same discipline as 1.33.1: each bite is a QEMU `e2fsck -fn`-clean smoke gate on the `metadata_csum,64bit` image; **NO iron burn until the final bite**, per [[feedback_iron_burns_block_other_work]]. Remaining 1.33.x follow-ons: 1.33.4 (uninit_bg materialization + symlink resolution) / 1.33.5 (fsync barrier). The big own-cycle writes (ext4 extent allocation, jbd2 journaling) land at 1.37.x–1.39.x — see agnos roadmap row 15.) {#tracker-1333-cycle}

**Bite plan** (each bite = a QEMU smoke gate on the checksummed image; the last bite = the iron burn):

| # | Bite | Gate |
|---|---|---|
| 1 | **`rename`/`mv`** — dirent-insert at the new parent + dirent-remove at the old (both primitives exist from W4); update `..` if the parent changes; `i_links_count` care for directory moves | ✅ **LANDED 2026-05-25** — `ext2_rename` + `ext2_ftype_from_mode`/`ext2_dir_set_dotdot`/`ext2_is_ancestor` + `mv` shell verb. Smoke on the **`metadata_csum,64bit,extent`** profile: `Wren file` (same-dir file) + `Wren xdir` (cross-parent dir move, `..`-repoint + parent link −1/+1) + `Wren refuse` (dst-exists) all OK; **`e2fsck -fn` clean incl. Pass-4**; `debugfs` confirms `/rndst.txt` present + `/rnsrc.txt` gone + `/rnp/rnd` is a dir + `/rnd` gone from root. No regression (W1–W5 + W4b green). Build 675,472 → 682,952 B. Base-stage limits: refuses dst-exists / self-move / dir-into-descendant. |
| 2 | **hardlink/`ln`** — dirent-insert pointing at an existing inode + `i_links_count` bump; refuse cross-device + dir hardlinks | ✅ **LANDED 2026-05-25** — `ext2_link` (link-count bumped FIRST then dirent, rollback on fail; refuses dir hardlinks + dst-exists) + `ln` shell verb. Smoke on `metadata_csum,64bit,extent`: `Whard link` (link → unlink one name → other survives, nlink 2→1) + `Whard refuse-dir` OK; **`e2fsck -fn` clean incl. Pass-4**; `debugfs` confirms `/hl_b.txt`="HL" survives unlink of `/hl_a.txt` + `/hl_d2` absent. No regression. Build 682,952 → 686,576 B. |
| 3 | **symlink-create/`ln -s`** — allocate inode with `S_IFLNK`; fast symlink (target ≤ 59 B in `i_block`) vs slow symlink (target in a data block); `ln -s` shell verb | ✅ **LANDED 2026-05-25** — `ext2_symlink` (fast inline <60 B / slow data-block; refuses dst-exists; rollback never `truncate_zero`s a fast symlink's text `i_block`) + `ln -s` flag in the shared `ln` verb. Smoke on `metadata_csum,64bit,extent`: `Wsym fast` + `Wsym slow` OK; **`e2fsck -fn` clean** (both formats); `debugfs` confirms `/sl_f` symlink w/ fast target `/hl_b.txt` + `/sl_s` slow symlink. No regression. Build 686,576 → 690,816 B. Resolution-on-traversal deferred to 1.33.4. |
| 4 | **`s_state` dirty/clean + `sync` verb** — clear `EXT2_VALID_FS` in `s_state` on first write, set it on a `sync`; flush superblock | ✅ **LANDED 2026-05-25 — closes 1.33.3** — `ext2_mark_dirty` (clears VALID_FS on first write, in-memory latch, hooked into `write_block`; no recursion since `write_superblock` uses `blk_write` direct) + `ext2_sync` + `sync` verb. Smoke: `Wsync state` (DIRTY after writes → CLEAN after sync) OK; `debugfs show_super_stats` = **Filesystem state: clean**; `e2fsck -fn` clean. No regression. Build 690,816 → 692,112 B. |

**🎯 1.33.3 CYCLE CLOSED (QEMU-complete) 2026-05-25** — all four dirent-mutation follow-ons (rename/`mv` · hardlink/`ln` · symlink-create/`ln -s` · `s_state`/`sync`) land `e2fsck -fn`-clean (incl. Pass-4 link accounting + cross-parent `..`-repoint) on the real default-`mkfs.ext4` profile (`metadata_csum,64bit,extent`), full regression green. Iron burn (`mv`/`ln`/`ln -s` on the real partition → power-cycle → persists + host `e2fsck`) available but **user-driven, not auto-proposed** per [[feedback_iron_burns_block_other_work]]. Next: 1.33.4 (uninit_bg materialization + symlink *resolution*).
| N | **iron burn** (final bite) — user-driven, NOT auto-proposed | PENDING — `mv`/`ln`/`ln -s` on the real default partition → power-cycle → tree intact; host `e2fsck -fn` exit 0 |

**Hypothesis**: rename/link/symlink are *recompositions* of primitives that are already iron-proven (dirent insert/remove from W4, inode alloc + `i_links_count` writes, the metadata_csum + 64bit ordered-write machinery from 1.33.1). No new on-disk-format surface, no new checksum class — each operation is a specific ordered sequence of writes the WRITE arc already performs individually. **Risk surface is link-count accounting** (e2fsck Pass-4 catches every off-by-one: a hardlink that doesn't bump `i_links_count`, a rename that double-counts a moved directory's `..`, an unlink-after-link that frees a still-referenced inode) — which is exactly why each bite gates on `e2fsck -fn` (Pass-4 included) before the next.

**Iron falsification rubric** (final bite — not auto-proposed):

| Signal | PASS target | Falsification → meaning |
|---|---|---|
| `mv /a /b` + remount | `/b` reads, `/a` gone | dirent-insert/remove ordering bug → localize to rename |
| `ln /a /b`; `rm /a`; remount | `/b` still reads (inode survived the unlink) | link-count not bumped → unlink freed a live inode |
| `ln -s target /s`; remount | `/s` inode is `S_IFLNK`, target intact | symlink store wrong (fast/slow boundary) |
| host `e2fsck -fn` after burn | exit 0, no FIXED | Pass-4 link-count or dir-`..` accounting bug |

---

### Tracker: 1.33.2 cycle — lockup-hardening (RELEASED 2026-05-25 — bench/serial stability, cut standalone from the WRITE arc) {#tracker-1332-lockup}

**Symptom (user-reported, iron):** the box locks up at the **idle shell, a few seconds–to–a-minute *after* `bench` completes** — NOT during the bench. Delayed, post-bench, at idle.

**Fixes landed** (the three small unbounded/unguarded spots in the bench + serial path, found by code review):
- **Bounded `serial_putc` THR-empty poll** (`arch/x86_64/serial.cyr`) — was an uncapped `jz` spin (`in al,0x3FD; test al,0x20; jz back`); rewritten over `inb`/`outb`, cap 100k then drop the char. Serial output must never wedge the box (archaemenid has no cable, [[feedback_no_serial_on_iron]]).
- **`bench` memwrite guards `pmm_alloc()==0`** (`user/shell.cyr`) — unguarded `store64(mem_pg+j,…)` would clobber physical 0–4095 (IVT / low page tables / kernel structures) → a corruption that faults *later* at idle.
- **`bench` memfile loop guards the `vfs_create_memfile` fd** — `-1` (full `vfs_table`) → `vfs_read(-1,…)` reads 32 bytes before the table (OOB). Now gated on `fd >= 0`.

Build 675,152 → **675,472 B**, `test.sh` 4/4, `ext2-smoke` 5/5 (serial output runtime-verified — every variant reached `AGNOS shell v`).

**Hypothesis:** the delayed-idle lockup is corruption `bench` leaves behind that the per-tick scheduler (`timer_handler`→`do_context_switch`, runs ~100×/s at idle) trips when it next touches the corrupted region. The three fixes remove the concrete corruption sources identifiable by code review.

**Falsification criteria** (next user-driven burn — NOT auto-proposed per [[feedback_iron_burns_block_other_work]]; escape plan written BEFORE the burn per [[feedback_stop_letter_laddering]]):
- Run `bench`, return to idle shell, wait ≥2 min → **PASS** if no lockup (the hardening was the cause).
- **If it still locks up** → root cause is NOT in the bench/serial path. Prime suspect: the context-switch path itself — `pic.cyr:48-54` documents a *prior* delayed-idle lockup from a register-save bug "when the kernel idle proc was re-selected." That path needs iron observability (CMOS-stamp the proc save/restore, or bisect by gating `do_context_switch` off at idle).

---

### Tracker: 1.32.9 cycle (🎯 **OUTCOME (BURNED 2026-05-25 ~17:50 PDT): DHCP LEASE ON IRON — every rubric row PASSED, hypothesis CONFIRMED.** FB ([`1329-agnos-1.32.9-dhcp-re-enabled-discover-offer-request-ack-lease-142-gw-mac-hex-l2-ok-shell.jpg`](iron-nuc-zen-photos/1329-agnos-1.32.9-dhcp-re-enabled-discover-offer-request-ack-lease-142-gw-mac-hex-l2-ok-shell.jpg), user phone-label `1329_DHCP`): clean storage tail (NVMe secondary block_dev → GPT 2-active `[0] EFI System AGNOS-BOOT` 255 MiB / `[1] Linux FS agnos-fs` 25600 MiB → `ext2: probe matched backend=2 partition_lba=524288` → `ext2: mounted blocksize=4096 inode_size=256` → Heap → canary → interrupts → `Timer ticks before sched: 6` → `Activating scheduler...`) then the full exchange: **`dhcp: DISCOVER` → `dhcp: OFFER ip=192.168.1.142` → `dhcp: REQUEST` → `dhcp: ACK ip=192.168.1.142 gw=192.168.1.1 mask=255.255.255.0`** → `arp: request -> gateway` → **`arp: REPLY gw_mac=d4:6a:91:ce:70:60`** (HEX — logging fix confirmed) → `net: L2 OK -- gateway MAC cached` → kybernet (`0 processes`, `3462 free pages`, `launching shell`) → **`AGNOS shell v1.32.9`**. The leased address is `.142` — a **real lease**, not the `.222` static fallback — so the server's unicast OFFER + ACK both reached AGNOS, exactly the physical-match class the 16-deep ring overflowed pre-1.32.7. The 1.32.3 `OFFER timeout` WAS the ring-depth drop, not a DHCP-protocol bug. **The 1.32.x networking arc is complete: L2 → unicast TCP (1.32.7) → DHCP lease (1.32.9).** Next major arc: 1.33.x WRITE (demo→base). Original OPEN framing follows for traceability. — **DHCP re-enablement**, now that the 1.32.7 ring-depth fix delivers unicast frames on iron. The 1.32.3 STATIC bypass in `main.cyr` is removed and `dhcp_init()` re-attempted, with STATIC `.222` seeded as the iron fallback so a DHCP miss still leaves a usable address. Bundled net-path logging fix: MAC octets + the r8169 MMIO base + chip-rev byte now print **hex** not decimal (new fb-visible `kprint_byte`/`kprint_hex`). NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]]; hypothesis + falsification rubric below. 1.32.8 was a no-iron cleanup cut — no tracker.) {#tracker-1329-cycle}

**Hypothesis**: the 1.32.3 `dhcp: OFFER timeout` was the **same unicast-RX delivery drop** that the 1.32.x arc chased and the 1.32.7 ring-depth fix (16 → 64) cleared — not a DHCP-protocol bug. A DHCP server unicasts its OFFER (and ACK) at L2 to the client's hardware MAC (the client has no lease yet, broadcast flag unset), so those frames are exactly the physical-match unicast class the 16-deep ring was overflowing under LAN chatter (`missed=176` → `0` at bite-5). With unicast RX proven on iron (both on-LAN and off-LAN TCP handshakes at 1.32.7), `dhcp_init()`'s DISCOVER → OFFER → REQUEST → ACK loop should now complete. The DHCP code itself (`net.cyr` `dhcp_init`) is unchanged from the 1.32.0 RFC 2131 implementation; this cycle only re-enables the call site. **Build-verified**: production **623,816 B**, `test.sh` 4/4 + `ext2-smoke.sh` 5/5 (QEMU smokes carry no NIC → they skip the net-probe; iron is the real test), multiboot2 ELF64 OK, `build/agnos` reflects HEAD. **Burn-ready — NOT auto-proposed.**

**Iron falsification rubric** (1.32.9 DHCP burn — FB watch from the boot net-probe; the `arp: REPLY gw_mac=` line should now read hex, e.g. `d4:6a:91:ce:70:60`, confirming the logging fix in passing):

| Signal | 1.32.7 baseline (STATIC bypass) | 1.32.9 PASS target | Falsification → meaning |
|---|---|---|---|
| DHCP exchange | (bypassed — `net: STATIC ip=192.168.1.222`) | `dhcp: DISCOVER` → **`dhcp: OFFER ip=…`** → `dhcp: REQUEST` → **`dhcp: ACK ip=… gw=… mask=…`** | `dhcp: OFFER timeout` → the OFFER drop is NOT (only) the unicast-RX issue the arc closed — re-investigate the OFFER frame class (broadcast-flag / server unicast-vs-broadcast) or DHCP-server reachability. **Box stays networkable** — `net: DHCP failed -- using static fallback` keeps `.222` and the ARP/L2 path runs. |
| ACK half | (n/a) | reaches `dhcp: ACK` (above) | `dhcp: ACK timeout` after a good OFFER → REQUEST egress or server-side issue; OFFER-delivery hypothesis still CONFIRMED (unicast RX delivered the OFFER), narrower fault below it |
| ARP / L2 (post-DHCP) | `arp: REPLY gw_mac=…` → `net: L2 OK -- gateway MAC cached` | unchanged — now targets the **leased** gateway (or `.222` fallback) | L2 breaks only if a DHCP-leased gateway is unreachable; fallback path is the 1.32.7-proven one |
| MAC hex (logging fix) | `gw_mac=212:106:145:206:112:96` (decimal) | `gw_mac=d4:6a:91:ce:70:60` (hex) | still decimal → `kprint_byte` not wired (build regression — impossible given 4/4+5/5, but the on-iron confirm is free) |
| Storage trio + GPT + ext2 + shell | byte-clean | byte-clean | regression impossible — scope is the `main.cyr` net-probe call site + net-path log formatters only |

---

### Tracker: 1.32.7 cycle (RELEASED 2026-05-25. 🎯 **OUTCOME (bite-5, BURNED 2026-05-25): CONNECTED — the r8169 unicast-RX defect was RING DELIVERY CAPACITY.** Deepening the RX ring 16 → 64 collapsed `missed` 176 → **0** and both handshakes read on iron — `net: LAN-TCP OK` (on-LAN MBP-peer SYN+ACK) AND `net: L3+TCP OK` (off-LAN 1.1.1.1 through the gateway), trailing `r8169: … missed=0 … rx_uc=10`. **The entire 1.32.x r8169 unicast-RX arc (1.32.0 → 1.32.7, opened 2026-05-22) is CLOSED — MVP on-iron networking blocker cleared.** The split was resolved across three bites: bite-3 exonerated the filter (silicon `rx_uc`>0), bite-4 falsified the drain-only fix (`missed` 158 → 176 — a budget can't exceed 16 physical descriptors), bite-5 fixed capacity. **1.32.8 = cleanup + remaining 1.32.x items.** Original OPEN framing follows for traceability. — **r8169 RX of UNICAST frames on iron, take 2: the IDR physical-match write moved past `CR.RE`**. 1.32.6 shipped gateway L2 reachability (RFC-826 snoop) but the unicast TCP SYN+ACK still never lands. NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]]; hypothesis + falsification rubric below.) {#tracker-1327-cycle}
>
> **Why this is not another letter on the ladder** (per [[feedback_stop_letter_laddering]] + [[feedback_audit_re_derive_dont_validate_comments]]): the prior burns that "exonerated" the accept filter, AAP, CPlusCmd, and the IDR were **all confounded by the same defect** — they wrote filter-config registers *before* `CR.RE`, and bite-7 (1.32.5) proved this VER_46 stepping silently drops filter-config writes issued while RX is disabled (Linux commit `05212ba8132b`). The accept nibble was rescued by re-asserting it post-`CR.RE` (broadcast + multicast then delivered). The **IDR0-5 physical-match write is the lone filter register still left only pre-`CR.RE`** — bite-2 reordered it but never moved it past enable. So its "exonerated" verdict is invalid, and broadcast/multicast delivery proves nothing about it because those ride the **separate MAR hash register** (latched fine pre-enable). The IDR physical-match path has never delivered a frame on iron across the entire arc.

**Hypothesis**: AGNOS's unicast (APM) physical-match filter compares inbound dst-MACs against IDR0-5, but IDR is programmed only pre-`CR.RE` (step 2), where this stepping drops it — so the filter holds a stale/garbage MAC and every unicast frame (gateway ARP reply, TCP SYN+ACK) fails the match, while broadcast + multicast pass via the MAR all-1s hash. **Fix (landed, build-verified)**: re-assert IDR0/IDR4 *after* `CR.RE` (`r8169.cyr` step 11c), mirroring the proven step-11b accept-nibble re-assert — Cfg9346-unlock, high half + readback, low half (latches the 6-byte filter) + readback, lock; reuses `mac_lo`/`mac_hi` from step 2. Build 622,560 → **622,656 B** (+96 B); `test.sh` 4/4 + `ext2-smoke.sh` 5/5; multiboot2 ELF64 OK; `build/agnos` reflects HEAD. **Burn-ready** — NOT auto-proposed.

| # | Bite | Iron? | Status |
|---|------|-------|--------|
| 1 | **IDR physical-match filter re-asserted post-`CR.RE`** (`r8169.cyr` step 11c) — the one filter register bite-2's confounded reorder never moved past enable. | code → burned | 🔥 **BURNED 2026-05-25 → FALSIFIED** ([`1327-agnos-1.32.7-bite1-idr-post-cr-re-reassert-l2-ok-gateway-cached-still-no-synack-unicast-drop-survives.jpg`](iron-nuc-zen-photos/1327-agnos-1.32.7-bite1-idr-post-cr-re-reassert-l2-ok-gateway-cached-still-no-synack-unicast-drop-survives.jpg), phone-label `1327_TCP_Failure`). FB byte-identical to the bite-6 burn: `arp: REPLY gw_mac=212:106:145:206:112:96` → `net: L2 OK -- gateway MAC cached` → `tcp: connect 1.1.1.1:80` → `net: L3+TCP FAIL -- SYN sent but no SYN+ACK`. **The rubric's L3 falsification branch fired**: the post-enable IDR write is NOT the dropped lever → the accept+filter layer (accept nibble, AAP, CPlusCmd Normal_mode, **and now the IDR physical-match write**) is **genuinely exonerated** — every L2-filter lever has been re-asserted post-`CR.RE` and unicast still drops. The unicast-RX defect is **below the L2 accept/filter layer**. |
| 2 | **On-LAN unicast-TCP discriminator** (user direction 2026-05-25) — instead of jumping straight to the `rx_ucasts` tally instrumentation the bite-1 rubric named, run a *behavioral* discriminator that splits **raw unicast RX** from **gateway/off-LAN routing**. AGNOS auto-discovers an on-LAN peer (the `mbp-arp-rx-probe.sh` harness on `.121`, snooped from its broadcast who-has — ZERO unicast RX), connects to `peer:80` on the **same L2 segment** (no gateway, no NAT), and prints `net: LAN-TCP OK`/`FAIL` **before** the 1.1.1.1 test. The peer's SYN+ACK is the first unicast frame AGNOS must RX, sent by an endpoint we own and log. `net.cyr`: `net_peer_*` slots + capture in `net_handle_arp` (non-gateway who-has-for-us) + `route_next_hop_mac` on-LAN peer branch. `main.cyr`: ~2s peer-wait + on-LAN `tcp_connect` before 1.1.1.1. | code → BURNED | 🔥 **BURNED 2026-05-25 → unicast-RX drop CONFIRMED at an endpoint we own + log** ([`1327-agnos-1.32.7-bite2-onlan-tcp-discriminator-lan-tcp-and-l3-both-fail-mbp-pcap-synack-out-no-ack-unicast-rx-drop-confirmed.jpg`](iron-nuc-zen-photos/1327-agnos-1.32.7-bite2-onlan-tcp-discriminator-lan-tcp-and-l3-both-fail-mbp-pcap-synack-out-no-ack-unicast-rx-drop-confirmed.jpg), phone-label `1327_dual_tcp_tests_failing`; MBP `tcpdump` log [`1327-mbp-onlan-tcp-probe-tcpdump-log-2026-05-25.txt`](iron-nuc-zen-photos/1327-mbp-onlan-tcp-probe-tcpdump-log-2026-05-25.txt)). FB: `tcp: connect on-LAN 192.168.1.121:80` → `net: LAN-TCP FAIL -- on-LAN SYN sent but no SYN+ACK`, THEN `tcp: connect 1.1.1.1:80` → `net: L3+TCP FAIL -- SYN sent but no SYN+ACK`. **The rubric's LAN-TCP FAIL branch fired (line 54).** The discriminator's payoff is the MBP-side proof of the full chain: AGNOS auto-discovered the `.121` peer (the MBP's broadcast `who-has 192.168.1.222` → AGNOS snooped sender + replied — `[ok] broadcast RX + ARP responder live`), AGNOS TX'd the on-LAN SYN (`192.168.1.222.49153 > 192.168.1.121.80 Flags [S]` @ 14:05:47.998264 — `[!!] it discovered us + TX'd the SYN`), the MBP kernel SYN+ACK'd it (`192.168.1.121.80 > 192.168.1.222.49153 Flags [S.]` @ 14:05:47.998638) **and RETRANSMITTED the SYN+ACK** (@ 14:05:48.999407) — **AGNOS never ACK'd**. The SYN+ACK is a unicast frame from an endpoint we own and log, on the **same L2 segment with NO gateway / NAT / Cloudflare in the path**, and it never reached AGNOS. ⇒ **the alternative branch (LAN-TCP OK → re-scope to gateway/off-LAN routing) is RULED OUT; the unicast-RX drop is GENERAL, not gateway-specific.** On-LAN and off-LAN SYN+ACK (same IDR-physical-match class) drop identically → the defect is **r8169 RX delivery of unicast-class frames, below the L2 accept/filter layer** (accept nibble + AAP + CPlusCmd Normal_mode + IDR physical-match all re-asserted post-`CR.RE` and falsified across 1.32.5→1.32.7). **Convergent behavioral levers are now exhausted → the `rx_ucasts` hardware tally readback (reg `0x28`) is JUSTIFIED** per [[feedback_no_instrumentation_means_no_instrumentation]] — it localizes whether unicast frames reach the MAC at all (L2-filter drop vs DMA/ring-delivery drop). _Pre-burn:_ ✅ LANDED + BUILD-VERIFIED, build 622,656 → **623,928 B** (+1,272 B), `test.sh` 4/4 + `ext2-smoke.sh` 5/5 (zero regression — r8169 iron-only, QEMU uses virtio_net), multiboot2 ELF64 OK; MBP harness `scripts/wire-probe/mbp-arp-rx-probe.sh` rewritten into the logging on-LAN TCP webservice + capture + verdict. |
| 3 | **r8169 hardware accept-counter readback** (`main.cyr` boot net-probe, after both unicast handshakes) — wire in the *already-written, never-called* `r8169_print_stats`/`r8169_dump_stats` (`r8169.cyr:843-918`, FreeBSD `re_sysctl_stats` / RTL8168 §6.8.4 tally-counter DMA). Reads the chip's OWN silicon counters split by class: `rx_uc` (dump offset 0x28, unicast accepted via physical-match), `rx_bc` (0x30, broadcast), `rx_mc` (0x38, multicast). Settles the filter-vs-delivery question every inference-only burn could not — the chip itself reports whether it accepted any unicast frame. r8169-only (guarded on `r8169_present`; QEMU virtio_net has no RTL8168 to count). | code → 🔥 BURNED | ✅ **LANDED + BUILD-VERIFIED 2026-05-25.** main.cyr boot net-probe now calls `r8169_print_stats()` after the on-LAN + 1.1.1.1 tests, guarded `if (r8169_present == 1)`. **NO new instrumentation authored** — the dump function was already complete + prior-art-correct, just never wired in. Build 623,928 → **623,976 B** (+48 B), `test.sh` 4/4 + `ext2-smoke.sh` 5/5 (zero regression — the dump fires only on the iron path; QEMU `r8169_present==0` → output byte-identical to bite-2), multiboot2 ELF64 OK, `build/agnos` reflects HEAD+edit. 🔥 **BURNED 2026-05-25 → `rx_uc=2` — THE FILTER-VS-DELIVERY SPLIT IS RESOLVED: the filter ACCEPTS unicast, the RING drops it** ([`1327-agnos-1.32.7-bite3-silicon-counter-readback-rx-uc-2-filter-accepts-unicast-ring-drops-it-missed-158.jpg`](iron-nuc-zen-photos/1327-agnos-1.32.7-bite3-silicon-counter-readback-rx-uc-2-filter-accepts-unicast-ring-drops-it-missed-158.jpg), phone-label `1327_r8169_line`). FB trailing line: `r8169: tx_ok=5 rx_ok=140 tx_err=0 rx_err=0 missed=158 align=0 rx_uc=2 rx_bc=82 rx_mc=64`. **`rx_uc=2` > 0 → the `rx_uc>0` rubric branch fired (line 61): the chip's OWN silicon counted unicast frames ACCEPTED via physical-match — the MAC filter is innocent. The ENTIRE L2 accept/filter arc (accept nibble + AAP + CPlusCmd Normal_mode + IDR physical-match) is now CLOSED by the chip's counter, not by inference.** The blocker is **RX ring/poll delivery**: `missed=158` (RxMissed = RX-FIFO overflow, no host descriptor free) with `rx_err=0 align=0` (frames arrive CLEAN) = the ring drops > half of received traffic for lack of a free descriptor; the gateway/SYN+ACK unicast frames die there, not at the filter. `r8169_poll` (`r8169.cyr:761`) returns after ONE good frame per call (`return pkt_len` @ `:804`) — the ring backs up under the bcast/mcast flood between polls; FreeBSD `re_rxeof` + iPXE `realtek_poll` drain the WHOLE ring per call → that is the convergent next bite. _(The on-LAN MBP peer was running but AGNOS printed `net: no on-LAN peer` — the burn disrupted the MBP's stimulus before AGNOS snooped its who-has, so the on-LAN test fell through to 1.1.1.1; immaterial to the verdict — `missed=158` indicts the ring on its own evidence.)_ |
| 4 | **Whole-ring drain per poll call** (`net.cyr` `net_poll`) — the bite-3-named fix. `net_poll` pulled ONE frame per call (`nic_poll` once → dispatch → return) and `tcp_connect` polls `net_poll(); arch_wait();` ×200, so one frame drained per `hlt` tick; a LAN-chatter burst (82 bcast + 64 mcast in the probe window) overran the 16-deep ring between ticks → FIFO overflow (`missed=158`) → the unicast SYN+ACK dropped before it was read. Convergent prior art: FreeBSD `re_rxeof` + iPXE `realtek_poll` loop until the next NIC-owned descriptor, draining the whole ring per call. **Read path verified sound first** (`tcp_find_conn` 4-tuple cross-match correct; `net_handle_tcp` SYN+ACK ack-validation correct, no false-drop) → this is purely a delivery fix, not a parse fix. `net_poll` rewritten into a bounded drain loop (`budget=64`): pull→dispatch each frame, malformed frames skip dispatch but keep draining (the next frame may be the SYN+ACK), stop when the ring is empty (`nic_poll < 14`). No driver/filter/ring-size change — RX servicing rate only. | code → 🔥 BURNED | ✅ LANDED + BUILD-VERIFIED 2026-05-25 (commit `4c8e972`). Build 623,976 → **624,056 B** (+80 B), `test.sh` 4/4 + `ext2-smoke.sh` 5/5. 🔥 **BURNED 2026-05-25 → FALSIFIED** (`1327_TCP_FAIL_Again.jpg`, with the on-LAN MBP peer up this time + the user's pcap/server-log verdict). FB: `arp: REPLY gw_mac=212:106:145:206:112:96` → `net: L2 OK` → `tcp: connect on-LAN 192.168.1.121:80` → `net: LAN-TCP FAIL` → `tcp: connect 1.1.1.1:80` → `net: L3+TCP FAIL`; trailing `r8169: tx_ok=9 rx_ok=144 tx_err=0 rx_err=0 missed=176 align=0 rx_uc=5 rx_bc=74 rx_mc=65`. **The rubric's "`missed` stays high" branch fired — `missed` went 158 → 176 (UP, not toward 0).** The drain loop alone could not relieve starvation: a 64-frame drain budget can only pull the **16 descriptors that physically exist**, so a burst between `hlt`-spaced polls still overflows. The user's controlled probe (pcap `agnos-lan-probe-20260525-155754` + server log) is dispositive — at an endpoint **we own and log**, no gateway/NAT/Cloudflare: AGNOS sent the SYN, our kernel sent SYN+ACK to AGNOS's MAC, AGNOS never ACK'd (`UNICAST RX of SYN+ACK : 0`). ⇒ the pre-committed **deepen-the-ring** lever (bite-5) is the fix. |
| 5 | **Deepen the RX ring 16 → 64** (`r8169.cyr`) — the bite-4-rubric pre-committed lever. The whole-ring drain (bite-4) proved the bottleneck is **ring capacity, not servicing logic**: 16 descriptors is an outlier vs all prior art (Linux `NUM_RX_DESC=256`, FreeBSD/OpenBSD 256) and a LAN-chatter burst (74 bcast + 65 mcast in the probe window) overruns it within one `hlt`-spaced poll gap, dropping the clean unicast SYN+ACK for lack of a free descriptor (`missed=176`, `rx_err=0 align=0`; silicon `rx_uc=5` already proved the filter accepts it). `R8169_RX_RING_SIZE` 16 → **64** (4× burst headroom; 64×16 = 1024 B still fits one 4 KB ring page); shared `R8169_RING_MASK` (0x0F) split into `R8169_RX_RING_MASK` (0x3F) + `R8169_TX_RING_MASK` (0x0F) since the rings now differ in depth; `r8169_rx_bufs[16]` → `[64]`; `init_rx` loop bound + EOR-slot keyed to `R8169_RX_RING_SIZE`; `r8169_poll` error-skip walk budget 16 → `R8169_RX_RING_SIZE`. **TX untouched** (tx_err=0, never overflowed) — stays 16. No filter/driver-config change — RX buffering depth only; keeps the deepen-ring result unconfounded against the (now-landed) drain loop. | code → 🔥 BURNED | 🎯 **BURNED 2026-05-25 → CONNECTED. THE 1.32.x r8169 UNICAST-RX ARC IS CLOSED** ([`1327-agnos-1.32.7-bite5-rx-ring-16-to-64-connected-lan-tcp-ok-l3-tcp-ok-missed-0-rx-uc-10.jpg`](iron-nuc-zen-photos/1327-agnos-1.32.7-bite5-rx-ring-16-to-64-connected-lan-tcp-ok-l3-tcp-ok-missed-0-rx-uc-10.jpg), phone-label `1327_CONNECTED`). **The bite-5 rubric's win branch fired — `missed` collapsed 176 → 0.** FB: `net: STATIC ip=192.168.1.222 gw=192.168.1.1` → `arp: REPLY gw_mac=212:106:145:206:112:96` → `net: L2 OK -- gateway MAC cached` → `tcp: connect on-LAN 192.168.1.121:80` → **`net: LAN-TCP OK -- on-LAN unicast handshake established`** → `tcp: connect 1.1.1.1:80` → **`net: L3+TCP OK -- outbound TCP handshake established`** → trailing `r8169: tx_ok=21 rx_ok=103 tx_err=0 rx_err=0 missed=0 align=0 rx_uc=10 rx_bc=47 rx_mc=46` → `AGNOS shell v1.32.7`. **Both** the on-LAN MBP-peer SYN+ACK (the first unicast frame from an endpoint we own + log) **and** the off-LAN 1.1.1.1 handshake through the gateway read cleanly — `missed=0` proves the 64-deep ring absorbed the LAN-chatter burst that overran the 16-deep ring (bite-4 `missed=176`). The unicast-RX defect was **ring DELIVERY CAPACITY all along** — bite-3 had already exonerated the filter (`rx_uc`>0) and bite-4 isolated it to capacity (drain budget can't exceed 16 physical descriptors); 16 was the outlier vs all prior art (Linux/FreeBSD/OpenBSD 256). The verdict the user's earlier `163049` probe pre-stated ("re-scope away from raw unicast RX") is now MOOT — both raw on-LAN unicast RX *and* off-LAN routing work; nothing left to re-scope. **MVP on-iron networking blocker CLEARED.** _Pre-burn:_ ✅ LANDED + BUILD-VERIFIED 2026-05-25, build 624,056 → **624,488 B** (+432 B), multiboot2 ELF64 OK, entry `0x1000a8`, `build/agnos` reflected HEAD+edit, VERSION untouched. `scripts/test.sh` **4/4** + `ext2-smoke.sh` **5/5** (zero regression). |

**Line audit** (1.32.7 bite-3 — instrumentation, audited before any burn per [[feedback_iron_burns_block_other_work]]):
- **`main.cyr` insertion** (boot net-probe, after the L3+TCP block, inside the `net: L2 OK` branch): a single guarded call `if (r8169_present == 1) { r8169_print_stats(); }`. No behavioral change to RX/TX/filter — read-only counter DMA. Placed AFTER both handshakes so the counters reflect every unicast frame the chip saw during the probe window.
- **`r8169_dump_stats`** (`r8169.cyr:869`): lazily `pmm_alloc`s a 64-byte buffer, zeroes it, writes phys-hi→`0x14` / phys-lo→`0x10` / phys-lo\|START→`0x10`, polls bit 3 of `0x10` (≤10000 spins ≈ <10 ms; chip clears on done). Pure MMIO + DMA read, **no chip-state mutation**. Returns -1 on timeout (prints `stats dump timeout`, harmless).
- **`r8169_print_stats`** (`r8169.cyr:898`): prints `tx_ok / rx_ok / tx_err / rx_err / missed / align / rx_uc / rx_bc / rx_mc` from the DMA'd block (FreeBSD `struct rl_stats` offsets). FB-visible (`kprint`/`kprint_num`), no serial per [[feedback_no_serial_on_iron]].
- **Iron-only**: QEMU runs virtio_net, `r8169_present==0` → the guard skips the call → QEMU output + behavior byte-identical to bite-2 (confirmed: `test.sh` 4/4 + `ext2-smoke` 5/5).

**Iron falsification rubric** (1.32.7 bite-3 burn — hardware accept-counter readback; same FB watch as bite-2 plus the new trailing `r8169: … rx_uc=N rx_bc=N rx_mc=N` line):

| Signal | Meaning |
|---|---|
| `rx_bc`/`rx_mc` > 0 | sanity — the tally-counter DMA works and the chip is counting (we already know bcast+mcast deliver) |
| **`rx_uc=0`** (with `rx_bc`/`rx_mc` > 0) | **the MAC silicon accepted ZERO unicast frames → the physical-match filter rejects every frame to `b0:…` → IDR/APM still not engaging on this VER_46 despite the post-`CR.RE` re-assert. Bug is in the MAC address filter, definitively NOT the ring.** Next: re-derive the VER_46 unicast-filter enable — the reverted Linux force-allmulti pair (`efa5f1311c49` / `6a26310273c3`) + `rtl_set_rx_mode` + datasheet §5/§6 IDR/APM latch ordering; the IDR write may need a different latch sequence or APM a companion bit this stepping requires |
| **`rx_uc>0`** 🔥 **FIRED — `rx_uc=2`** | **the MAC IS accepting unicast → frames die between MAC-accept and `r8169_poll`/the ring → bug is in our descriptor/ring delivery, NOT the filter.** Corroborated by `missed=158` (RxMissed FIFO-overflow) with `rx_err=0 align=0`: clean frames dropped for no free descriptor. `r8169_poll` drains one frame per call (`r8169.cyr:804`) → the 16-deep ring backs up under the bcast/mcast flood → the SYN+ACK is missed. **Next bite: whole-ring drain per poll call** (FreeBSD `re_rxeof` / iPXE `realtek_poll` loop until the next NIC-owned descriptor). |

This is the decisive split — one burn names which of the two layers owns the bug, the thing no prior burn could distinguish.

**Iron falsification rubric** (1.32.7 bite-2 burn — on-LAN unicast-TCP discriminator; run with `sudo ./scripts/wire-probe/mbp-arp-rx-probe.sh` on the `.121` MBP during the burn):

| Signal | bite-1 baseline | Outcome → meaning |
|---|---|---|
| `tcp: connect on-LAN 192.168.1.121:80` printed | (new) | absent → peer discovery/on-LAN route didn't fire; check the MBP harness was broadcasting in-window |
| `net: LAN-TCP OK` (+ MBP `ACCEPTED connection` log line) | (new) | **on-LAN unicast handshake completed → raw unicast RX WORKS** → the 1.1.1.1 failure is **gateway/off-LAN-specific** (routing/relay), NOT raw unicast RX → major re-scope away from the r8169 filter |
| `net: LAN-TCP FAIL` (MBP pcap shows SYN-in + SYN+ACK-out, no AGNOS ACK) | (new) | **unicast-RX drop CONFIRMED at an endpoint we own and log** — no gateway/NAT/Cloudflare in the path. Both on-LAN and off-LAN unicast SYN+ACK (same IDR-physical-match class) drop identically → the defect is in r8169 RX delivery of unicast-class frames, below the L2 accept/filter layer → next escape is the `rx_ucasts` hardware tally (reg `0x28`) instrumentation, now justified |
| `net: L3+TCP FAIL -- SYN sent but no SYN+ACK` (1.1.1.1) | same | unchanged from bite-1; the on-LAN verdict above is the discriminating signal this burn |

**Superseded — Iron falsification rubric (1.32.7 bite-1 burn — IDR post-`CR.RE` re-assert; FALSIFIED above):**

| Signal | bite-6 baseline | PASS target | Falsification → meaning |
|---|---|---|---|
| ARP / L2 | `arp: REPLY gw_mac=212:106:145:206:112:96` → `net: L2 OK` | unchanged (L2 already proven via broadcast snoop) | L2 breaks → the post-enable IDR write disturbed the chip; back out, isolate |
| L3 gate (`tcp: connect 1.1.1.1:80`) | `net: L3+TCP FAIL -- SYN sent but no SYN+ACK` | **`net: L3+TCP OK -- outbound TCP handshake established`** | still FAIL → pre-enable IDR write was NOT the dropped lever; the accept+filter layer is **genuinely** exonerated (not confounded this time) and the unicast drop is below it → next escape is the hardware `rx_ucasts` tally counter (reg `0x28`) to localize whether unicast frames reach the MAC at all — **instrumentation, deferred** per [[feedback_no_instrumentation_means_no_instrumentation]] until this last behavioral lever is spent |
| Storage trio + GPT + ext2 + shell | byte-clean | byte-clean | regression impossible — scope is the `r8169.cyr` post-enable IDR write only |

---

### Tracker: 1.32.6 cycle (CLOSED 2026-05-25 — cut, awaiting user tag. **Outcome: gateway L2 reachability PROVEN on iron** via the RFC-826 ARP sender-snoop (bite 4) — `arp: REPLY gw_mac=d4:6a:91:ce:70:60` → `net: L2 OK -- gateway MAC cached`, first gateway reachability in the whole 1.32.x arc, MAC learned with ZERO unicast RX. CPlusCmd `Normal_mode` restored (bite 6, the proven-Linux value) but FALSIFIED as the unicast gate. **Unicast-class RX residual** — gateway unicast ARP reply / TCP SYN+ACK still undelivered — **carries to [1.32.7](#tracker-1327-cycle)** (IDR physical-match re-assert post-`CR.RE`). Original OPEN framing follows for traceability. — **r8169 RX delivery of UNICAST frames on iron**. 1.32.5 proved broadcast + multicast RX (bite-7 + the FB self-test); the gateway's unicast ARP reply stays unseen. **The user-confirmed "IDR0-5 write-back re-derive" FALSIFIED its own premise** — reading the code per [[feedback_audit_re_derive_dont_validate_comments]] shows the IDR0-5 write-back is ALREADY present + correct (`r8169.cyr:520-534`, Cfg9346-wrapped, == Linux `rtl_rar_set`), so it was in the burned 1.32.5 build. NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]]; bite agenda + falsification rubric below.) {#tracker-1326-cycle}
>
> **🎯 STATUS 2026-05-25 — bite-4 (ARP RFC-826 sender-snoop) IRON-PROVEN, L2 reachability achieved.** `arp: REPLY gw_mac=d4:6a:91:ce:70:60` → `net: L2 OK -- gateway MAC cached` on the framebuffer (`1326_Gateway_Connected`), gateway MAC learned with zero unicast RX exactly as designed. The boot now walks into L3: `net: L3+TCP FAIL -- SYN sent but no SYN+ACK`. The SYN+ACK is a **unicast** frame → the parked APM/unicast-RX gap is now the sole live blocker (or, weaker, gateway source-guard on the static `.222`). **bite 5** added below to track the L3 disambiguation (`tcp_syn_probe`, zero-burn).

**🔑 REFRAME 2026-05-25 — the bug was at the ARP protocol layer, NOT the driver** (user direction: *"the driver works — it's ARP and DHCP"*). Bites 1/2/3 below + the (deleted) VFIO userland-driver probe all chased *r8169 unicast-class RX delivery* — the WRONG LAYER. The driver is proven (TX + broadcast + multicast on iron). The real defect: `net_handle_arp` (`net.cyr:551`) learned the gateway MAC ONLY from a solicited unicast reply (`oper==2` matching the pending request) — a violation of **RFC 826**, which snoops the sender IP→MAC from EVERY ARP frame. **bite 4 fixes it and is Linux-validated WITHOUT a burn** (the gateway broadcasts its own `who-has`, sender=`.1`, which AGNOS already receives as broadcast → gateway MAC learnable with ZERO unicast RX). **VFIO probe is DEAD** — binding `enp1s0` (the sole link) kills the very connection the test runs over; user deleted the scripts. The unicast-RX driver question (bite-2 verdict) is now MOOT for ARP/DHCP reachability — parked, not blocking.
...
| # | Bite | Iron? | Status |
|---|------|-------|--------|
| 1 | **ARP request retransmit** (RFC 1122 §2.3.2.1) — re-send the gateway ARP ~1/sec across the 5 s window while `arp_pending` holds (`main.cyr:684-713`); 4 retransmits; no per-retx print. | code → next burn | ✅ **BURNED 2026-05-25 → [`1326-agnos-1.32.6-arp-retransmit-rx15-arpin11-arpans2-unicast-drop-systematic.jpg`](iron-nuc-zen-photos/1326-agnos-1.32.6-arp-retransmit-rx15-arpin11-arpans2-unicast-drop-systematic.jpg)** (user phone-label `1326_LX2_Still_No_Gate`; catalogued 2026-05-25). FB: `net: L2 RX ALIVE rx=15 arp_in=11 arp_ans=2 -- gateway unicast reply pending`. Retransmit egressed (`arp_ans=2`), ring delivered 11 broadcast ARP, **gateway unicast reply STILL unseen** → rubric's SYSTEMATIC branch fired (transient-miss RULED OUT) → bite 2 unblocked. Shell v1.32.6 byte-clean, no regression. |
| 3 | **Descriptor-OWN handoff re-derive → RUNT-drop probe (Linux-first)**. `r8169_poll` (`r8169.cyr:737`) drops any descriptor with status bit `0x00100000` set, comment "any RX error". On the RTL8168 RX descriptor that bit is **RUNT** (frame < 64 B on the wire), not the error *summary* (bit 21 / `0x00200000`). Hypothesis: the Araknis gateway emits its unicast ARP reply UNPADDED (42 B) → chip flags RUNT → poll drops it; broadcast ARP from other hosts arrives padded ≥ 60 B → passes. A runt-drop is a clean frame-class discriminator; nothing in the accept filter is. **Tested on Linux first** (not header-archaeology — BSD `if_rlreg.h` actually agrees with AGNOS's `0x00100000`, so the answer is empirical, not in the enums) via `scripts/dhcp-probe/src/arp_reply_len.cyr` (AF_PACKET, AGNOS-shape request, measures gateway-reply wire length vs broadcast lengths; Linux strips FCS so its length == AGNOS `pkt_len`, < 60 == RUNT-flagged == poll would drop). | **Linux probe → THEN iron** | ❌ **RUNT THEORY FALSIFIED ON LINUX 2026-05-25** (`build/arp-reply-len enp1s0`). Every gateway unicast reply = **60 bytes** (not < 60), `dst=unicast`, to AGNOS's MAC `b0:41:6f:0c:e4:25` — NOT a runt; the `0x00100000` gate is not load-bearing. **But the probe proved the decisive thing: the gateway sends unicast replies on the wire and Linux's r8169 on the SAME chip (`enp1s0`) receives every one** → wire + chip + gateway all GOOD; unicast-RX failure is 100% inside AGNOS's r8169 *config*, no hardware mystery to instrument on iron. Signature sharpened: AGNOS sees only bcast/mcast (which pass via `MAR=all-1s` hash) and ZERO unicast even with **AAP (promiscuous) set** — if AAP engaged, AGNOS would see all the LAN's `.108→.1` unicast chatter; it sees none → **the whole accept nibble (APM+AAP) is dead despite the write; AB/AM only leak through the mcast hash.** Next: drive AGNOS's actual r8169 register sequence against the real chip from **userland on Linux (VFIO)** with full register-readback visibility — the definitive no-iron isolate of why the accept nibble won't latch. |
| 2 | **Deep driver APM re-derive (multi-source)** — FreeBSD/OpenBSD/NetBSD `re_rxeof`/`re_iff` + iPXE `realtek_poll` + RTL8168h datasheet §6 + Linux VER_46 erratum; derive unicast-accept + descriptor-OWN handoff from sources first, then diff. | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-25** (bite-1 systematic branch unblocked it). 3 parallel re-derives (BSD triangle / iPXE+Haiku+U-Boot / Linux+datasheet+erratum-git) converged; reconciled vs actual code → two protocol fixes: **(1) IDR write-back reordered to Linux `rtl_rar_set` (high-half MAC4 first + `rtl_pci_commit` readback, then MAC0 + readback)** — low-word write latches the 6-byte filter so high bytes must be resident first; readback blocks posted-write reorder from torn-latching the unicast filter (torn filter drops gateway unicast while bcast/mcast pass — exact signature). **(2) Post-`CR.RE` RxConfig: RMW → one clean full write** of `RXCFG_DEFAULTS\|accept` + commit (Linux `05212ba8132b`: RxConfig writes while RX-disabled are dropped → pre-enable write was no-op; RMW may never have landed the profile word). Reframe: `MAR=all-1s` passes bcast via the mcast hash so bcast/mcast delivery does NOT prove the accept nibble engages; Linux force-allmulti VER_46 patch `efa5f1311c49` was REVERTED `6a26310273c3` (correct-init VER_46 RXes unicast w/o allmulti) → init-protocol bug, not a missing workaround. + stale CPlusCmd comment fixed. Build 622,560 → **622,544 B** (−16 B); `test.sh` 4/4 + `ext2-smoke.sh` 5/5; multiboot2 OK; `build/agnos` reflects working tree. NOT auto-proposed per [[feedback_iron_burns_block_other_work]]. **🔥 BURNED 2026-05-25 (no photo — user-reported FB line) → FALSIFIED.** FB: `net: L2 RX ALIVE rx=15 arp_in=7 arp_ans=1 -- gateway unicast reply pending` — still `reply pending` with `rx=15 > 0`. **The bite-2 rubric's falsification branch fired**: `rtl_rar_set` IDR write-back ordering + the clean post-`CR.RE` RxConfig full write are **NOT load-bearing** for the unicast drop. (`arp_in=7 arp_ans=1` < bite-1's `11`/`2` is LAN broadcast-ARP traffic variance across the 5 s window, not a regression — `rx=15` total held identical, and the only load-bearing signal is `reply pending` persisting.) **Unicast (APM-class) RX drop is now isolated BELOW the accept+IDR-filter layer** — both the L2 accept nibble (bite-7) and the IDR physical-match filter (this bite) are EXONERATED. Next escape per rubric: hardware tally-counter `rx_ucasts` readback at reg `0x28` to localize whether unicast frames reach the MAC at all — **instrumentation, DEFERRED** per [[feedback_no_instrumentation_means_no_instrumentation]] until convergent behavioral levers are exhausted; the prior-art-convergent behavioral lever to pull first is the `r8169_init_rx`/`r8169_poll` descriptor-OWN handoff re-derive (the one source-set divergence not yet code-burned both ways). **NOTE (post-bite-4): MOOT for ARP/DHCP** — bite 4 resolves gateway-MAC learning at the ARP layer with zero unicast RX; this driver-side unicast question is parked, not on the MVP path. |
| 4 | **ARP RFC 826 sender-snoop** (`net.cyr:551`) — learn the sender IP→MAC from EVERY ARP frame (request OR reply), gated on `arp_pending_ip` (security posture unchanged), not only from a solicited unicast reply. The Araknis gateway broadcasts its own `who-has` (sender=`.1`, mac=`d4:6a:91:ce:70:60`) which AGNOS already receives as BROADCAST (the FB `arp_in=N` count), so the gateway MAC is learnable with **ZERO unicast RX** — the entire bite-1/2/3 + VFIO unicast-RX chase was the wrong layer. | Linux-validated → BURNED | ✅✅ **LANDED + LINUX-VALIDATED + 🎯 IRON-PROVEN 2026-05-25** ([`1326-agnos-1.32.6-arp-snoop-l2-ok-gateway-cached-tcp-syn-no-synack.jpg`](iron-nuc-zen-photos/1326-agnos-1.32.6-arp-snoop-l2-ok-gateway-cached-tcp-syn-no-synack.jpg), user phone-label `1326_Gateway_Connected`). FB read `arp: REPLY gw_mac=212:106:145:206:112:96` (= `d4:6a:91:ce:70:60`, the Araknis gateway) → `net: L2 OK -- gateway MAC cached` — **bite-4 PASS rubric met exactly; first gateway reachability on iron in the whole arc.** The gateway MAC was learned with ZERO unicast RX (snooped from the gateway's own broadcast `who-has`), proving the RFC-826 snoop mechanism on iron, not just on Linux. _Zero-burn pre-proof_ via `scripts/dhcp-probe/src/arp_snoop_check.cyr` (AF_PACKET — NIC stayed bound to Linux): 15 s capture saw gateway `.1` as ARP sender **10× broadcast + 5× unicast**; the EXACT new snoop logic cleared `arp_pending` from the broadcast frames the OLD reply-only code discarded → CONFIRMED load-bearing. Build **622,496 B**, `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK, VERSION untouched (1.32.6). **New wall surfaced same burn → bite 5 (L3).** |
| 5 | **L3+TCP outbound — `net: L3+TCP FAIL -- SYN sent but no SYN+ACK`** (boot probe `main.cyr:733`, `tcp: connect 1.1.1.1:80`). The handshake's SYN+ACK is a **unicast frame to AGNOS's MAC** — the boot probe's own comment (`main.cyr:729-732`) lists "r8169 RX of unicast reply" as one of the four things it exercises. With L2 now reached via the broadcast snoop, the boot walks far enough to **isolate the parked APM/unicast-RX gap (bite-2/bite-3 verdict) as the sole remaining blocker**: TX proven 4×, routing resolves 1.1.1.1 → cached gateway MAC, SYN egresses, SYN+ACK (unicast) never delivered up the ring → `tcp_connect` (`net.cyr:852-861`) times out its 200-poll wait. Timeout budget is NOT the issue: `arch_wait()` = `hlt` (`io.cyr:131`), timer-bounded at 100 Hz → ~2 s wall-clock, ample for a Cloudflare RTT. **Honest alternative**: gateway source-guarding the static `.222` (no DHCP lease) — disambiguated zero-burn by a `tcp_syn_probe` AF_PACKET test sourced from `.222`. User prior: static leases on this Araknis have worked before → gateway-policy is the weaker hypothesis. | Linux probe → THEN iron | ✅ **DISAMBIGUATED ON LINUX 2026-05-25 (no burn) → AGNOS unicast RX CONFIRMED as the blocker.** `scripts/dhcp-probe/src/tcp_syn_probe.cyr` (AF_PACKET, sends a BYTE-FAITHFUL AGNOS SYN — `ip_build`/`ip_checksum`/`tcp_build_header`/`tcp_checksum_compute` ported verbatim from `net.cyr` — to `1.1.1.1:80` from `.222`+AGNOS MAC over Linux's r8169, no VFIO, box stayed reachable): gateway MAC self-resolved to `d4:6a:91:ce:70:60`; **SYN+ACK received from `1.1.1.1:80` flags=`0x12`**; `who-has 192.168.1.222 answered 0×` (gateway snooped `.222`→our-MAC off our outbound SYN, RFC-826-style, never had to ARP). ⇒ **causes (2) gateway-source-guard and (3) malformed-SYN both RULED OUT** — the byte-exact AGNOS SYN is well-formed, the gateway accepts the static `.222`, NATs it, and `1.1.1.1`'s unicast reply returns to the wire. The ONLY thing that differs on iron is unicast-class RX delivery → the parked **APM/unicast-RX gap is the blocker**, and unlike the ARP layer there is NO broadcast-snoop route-around (TCP requires unicast RX). User priors corroborate (1 of 3): static IPs run fine on this Araknis (garage switch pinned `.244`), network unchanged but for the upstream provider. **Next: crack "why the RxConfig accept nibble (APM+AAP) won't latch on iron despite the write" — bite-3's VFIO-readback plan is DEAD (unbinds the sole link), so the clean zero-burn replacement is an `ethtool -d enp1s0` register dump of the LIVE WORKING Linux r8169 on this exact chip, diffed against AGNOS's `r8169.cyr` write sequence; parallel track is a behavioral multi-source re-derive of accept-filter engagement timing.** |
| 6 | **CPlusCmd `Normal_mode` (0x2000) — the unicast-RX gate** (`r8169.cyr:538`). The 2026-05-25 `ethtool -d enp1s0` dump of the LIVE WORKING Linux r8169 on this exact chip ([`r8169-live-linux-register-dump-2026-05-25.md`](prior-art/r8169-live-linux-register-dump-2026-05-25.md)) gave the proven config: CPlusCmd=`0x2061`, RxConfig=`0x0002CF0E`. AGNOS wrote a bare CPlusCmd `0x0008` that ZEROED the chip's power-on `Normal_mode` bit — the lone source-confirmed, never-burned divergence after the L2 accept filter was exonerated. 3-source agent audit (Linux `r8169_main.c` + FreeBSD `if_rlreg.h` + RTL8168 datasheet) **RULED OUT bit-17/RMW** (Linux never authors it; FreeBSD blind-writes without it and RXes unicast fine) and found the prior CPlusCmd/profile A/B burns were **CONFOUNDED** — every one ran with `Normal_mode` cleared, so no profile/accept word could deliver unicast regardless. | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-25.** CPlusCmd `0x0008`→`0x2061` (Normal_mode\|RxVlan\|RxChkSum\|INTT0, matched verbatim to the working dump); RxConfig profile `0xEF00`→`0xCF00` (drop spurious RXFTH bit 13) + accept `0x0F`→`0x0E` (drop AAP — falsified on iron, never set by the working config). All now-stale comments rewritten per [[feedback_audit_re_derive_dont_validate_comments]]. Build **622,560 B**, `test.sh` 4/4 + `ext2-smoke.sh` 5/5 (zero regression), multiboot2 ELF64 OK, `build/agnos` reflects working tree (12:48 rebuild). **🔥 BURNED 2026-05-25 → FALSIFIED for unicast** ([`1326-agnos-1.32.6-bite6-cplus-normal-mode-burn-l2-ok-gateway-cached-still-no-synack-unicast-drop-survives.jpg`](iron-nuc-zen-photos/1326-agnos-1.32.6-bite6-cplus-normal-mode-burn-l2-ok-gateway-cached-still-no-synack-unicast-drop-survives.jpg), user phone-label `1326_still_ack_issue`). FB byte-identical to the bite-4 burn: `arp: REPLY gw_mac=212:106:145:206:112:96` → `net: L2 OK -- gateway MAC cached` → `tcp: connect 1.1.1.1:80` → `net: L3+TCP FAIL -- SYN sent but no SYN+ACK`. **The rubric's L3 falsification branch fired**: Normal_mode is NOT the unicast gate (it's now correct + retained — the proven-Linux value — but not load-bearing for the SYN+ACK). Gateway L2 reachability held (bite-4 win intact). **Branch retired** → CPlusCmd Normal_mode + AAP-drop both confirmed correct-but-not-the-fix; unicast drop survives with the entire accept+CPlusCmd layer now matching the working `ethtool -d` dump. The one filter register STILL written only pre-`CR.RE` is the IDR physical-match write — bite-2's reorder never moved it past enable (CONFOUNDED exoneration). → **1.32.7 bite: IDR post-`CR.RE` re-assert** (see [`#tracker-1327-cycle`](#tracker-1327-cycle)). |

**Iron falsification rubric** (bite-6 burn — CPlusCmd Normal_mode + RxConfig matched to the working dump):

| Signal | bite-4/5 baseline | Bite-6 PASS target | Falsification → meaning |
|---|---|---|---|
| ARP / L2 | `arp: REPLY gw_mac=d4:6a:91:ce:70:60` → `net: L2 OK` | unchanged (L2 already proven) | L2 breaks → CPlusCmd offload bits disturbed dispatch; back out RxChkSum/RxVlan, keep Normal_mode only |
| L3 gate (`tcp_connect 1.1.1.1:80`) | `net: L3+TCP FAIL -- SYN sent but no SYN+ACK` | **`net: L3+TCP OK -- outbound TCP handshake established`** | still FAIL → Normal_mode is NOT the gate; unicast RX drop is below CPlusCmd → escalate to the RX descriptor OWN/DMA path with the ethtool-confirmed working register set as the new baseline |
| Storage trio + GPT + ext2 + shell | byte-clean | byte-clean | regression impossible — scope is r8169 CPlusCmd + RxConfig only |
...
**Iron falsification rubric** (bite-4 burn — ARP RFC 826 sender-snoop, the load-bearing fix):

| Signal | bite-2 baseline | Bite-4 PASS target | Falsification → meaning |
|---|---|---|---|
| ARP outcome | `L2 RX ALIVE rx=15 … -- gateway unicast reply pending` | `arp: REPLY gw_mac=d4:6a:91:ce:70:60` → `net: L2 OK -- gateway MAC cached` | Still `reply pending` with rx>0 → the gateway's BROADCAST `who-has` frames (Linux saw 10 in a 15 s window) aren't being dispatched/snooped on iron → re-examine `net_poll` broadcast-ARP dispatch, NOT the driver accept filter |
| L3 gate (`tcp_connect 1.1.1.1:80`) | skipped behind failed ARP | reached + PASS/FAIL print | ARP passes but L3 fails → routing/TCP issue distinct from ARP |
| Storage trio + GPT + ext2 + shell | byte-clean | byte-clean | regression impossible — scope is the `net.cyr` ARP handler only |

> **Zero-burn validation receipt (bite 4)**: `arp-snoop-check enp1s0` 2026-05-25 — 24 ARP frames in 15 s; gateway `192.168.1.1` (`d4:6a:91:ce:70:60`) seen as sender **10× broadcast + 5× unicast**; new snoop clears `arp_pending` from broadcast frames the reply-only code ignored → CONFIRMED. NIC stayed bound to Linux throughout (no link drop, no VFIO). Probe source: `scripts/dhcp-probe/src/arp_snoop_check.cyr`.
...
| 5 | **RxMaxSize field-overflow fix** — `r8169.cyr:557` RMS `0x4000` → `0x05F3`. | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-24 20:41**. Post-Attempt-104 full-path re-audit (probe order / init_rx / bring-up envelope / poll / rearm / dispatch all confirmed prior-art-correct) surfaced ONE divergence: RxMaxSize (0xDA) is a 13-14 bit field on the 8168; `0x4000` sets bit 14 OUTSIDE it → reads as RMS=0 → "max RX packet size 0" → **NIC rejects all inbound frames; TX never consults RMS** ⇒ matches the exact iron signature (TX egresses per 1324/1325, RX silent). NO prior-art driver writes `0x4000` (Linux `RX_BUF_SIZE=0x05F3`, FreeBSD = buffer length). Malformed value was introduced at Attempt 97 while the FS\|LS gate + wrong RxConfig profile were still masking it; those are now fixed, so it surfaces as a live load-bearing candidate. Build **621,704 B** (constant swap, no size change), `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK, `build/agnos` reflects HEAD. This is a genuine behavioral fix (not instrumentation) — the next burn TESTS it, doesn't gather logs. **⚠️ BURNED 2026-05-24 23:42 (`1325_pcap_attempt_2.pcapng`) → FALSIFIED** — "no change apparent" on iron; ARP still times out. Pcap = TX re-proof #3 from the same wrong-vantage switched port (capture host `42:c2:df:db:ee:78` = .121, holding a live SSH session into archaemenid's Linux .141/`b0` lease — proof it's an ordinary port, blind to the gateway→`b0` unicast reply). RMS hypothesis was also internally inconsistent with the multicast-passing of Attempts 97–99. **RMS candidate CLOSED**; `0x05F3` retained as correct-but-not-the-bug. |
| 6 | **RX-engine bring-up fix — RXDV-gate settle + CPlusCmd** (multi-source re-derived). | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-25**. Two parallel research agents re-derived the 8168H RX-engine bring-up (Linux/FreeBSD/OpenBSD/NetBSD/U-Boot/iPXE). **(a) RXDV-gate settle (PRIMARY)**: AGNOS clears `RXDV_GATED_EN` (MISC 0xF0 bit 19) but enabled `CR.RE` µs later; Linux pairs the clear with `fsleep(2000)` (~2 ms) — without the settle the RX validator opens *after* RX starts → RX silent while TX (never consults the gate) egresses fine = the exact iron signature. U-Boot clears this bit specifically for *"DHCP failures after kernel reboots"* = archaemenid's warm-boot-from-Linux scenario. Added a ~4–8 ms non-posted-MMIO spin (`r8169.cyr:546` region) before CR.RE. **(b) CPlusCmd correction (SECONDARY)**: `0x000B → 0x0008` (PCIMulRW only) — `0x0001\|0x0002` are the legacy-8139C+ "C+ RX/TX enable" map; on the gigabit 8168 those bits are the INTT interrupt-moderation field (Linux `INTT_MASK`), FreeBSD sets them only on `!MACSTAT` (not the 8168H). Inert under IMR=0 but unambiguously wrong — the "nine-burn CPlusCmd trap" per [[feedback_audit_re_derive_dont_validate_comments]]. Research also CONFIRMED bite H was right to delete the Linux ephy/ERI/MAC-OCP/firmware clone (BSDs drive this stepping with none of it). Build **621,768 B** (+64 B); `test.sh` 4/4 + `ext2-smoke.sh` 5/5; multiboot2 OK; `build/agnos` reflects HEAD. **⚠️ BURNED 2026-05-25 ~00:03 PDT (`1325_pcap_attempt_3.pcapng`) → FALSIFIED** — RX still silent. This pcap is the **cleanest RX vantage of the arc**: capture host `.121` (`42:c2:df:db:ee:78`) is on AGNOS's L2 segment and probed AGNOS directly (1 unicast ICMP echo to `b0`'s exact MAC + **63 broadcast `who-has 192.168.1.222`** queries). AGNOS — which has a working ARP responder (`net.cyr:560-593`) and proven TX — answered **none** of them (its only frame was the one outbound ARP request). RX-drop is now proven DIRECTLY, not inferred from the vantage-blind missing-gateway-reply. **Both RXDV-settle and CPlusCmd FALSIFIED.** Next: from-scratch multi-source re-derive of the whole RX path (RxConfig write-effectiveness + ring + poll + rearm), NOT another targeted register bite. See § *agnos 1.32.5 — RX-engine bring-up fix* body. |
| 7 | **Post-RX-enable accept-filter re-assert** (`r8169.cyr:639-664`) — read-modify-write the RxConfig accept nibble AFTER `CR.RE`, mirroring OpenBSD `re_iff` / Linux `rtl_set_rx_mode`. | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-25**. From-scratch 3-agent re-derive (BSD triangle / iPXE / RTL8168H spec). **The conflict the agents surfaced — iPXE says set CPRX\|CPTX, spec agent says omit them — was resolved by git history, NOT by picking a source**: `782822ae` *added* CPRX\|CPTX (`0x000B`, burned Attempts 104 + bite-5 → FALSIFIED), bite-6 *removed* them (`0x0008`, burned `_3` → FALSIFIED) ⇒ CPlusCmd tested BOTH ways, closed. Likewise RxConfig PROFILE `0xCF00` (Attempt 98) AND `0xEF00` (current) both burned, closed. **The one untried, four-source-convergent lever**: every reference except the single FreeBSD branch AGNOS copied programs the accept filter AT/AFTER RX-enable — OpenBSD `re_iff` (RMW post-CR.RE), Linux `rtl_set_rx_mode` (post-`hw_start`), Linux stable patch ("RxConfig writes ignored before TX/RX enable"), iPXE (writes RCR after CR.RE\|TE). AGNOS wrote the accept nibble ONLY pre-enable, on every burn. Mechanism fit: multicast is programmed via the SEPARATE MAR register (lands) while bcast/ucast/AAP live in the RxConfig accept nibble (ignored-before-enable) → exactly the multicast-passes/bcast-ucast-drop split. Fix = post-CR.RE RMW (clear accept byte `0x3F`, re-OR `AAP\|AB\|AM\|APM`) preserving any profile bits the chip latched at the RE edge. Build **621,896 B** (+128 B); `test.sh` 4/4 + `ext2-smoke.sh` 5/5; multiboot2 OK; `build/agnos` reflects HEAD. NOT auto-proposed per [[feedback_iron_burns_block_other_work]] — next burn TESTS it. |
| 8 | **Honest L2 RX self-test** (`main.cyr:684-721` + `net.cyr:592`) — boot net probe counts frames `net_poll()` delivered + ARP replies the responder TX'd during the gateway-probe wait; on timeout prints `net: L2 RX ALIVE rx=N arp_in=M arp_ans=K -- gateway unicast reply pending` (or `... SILENT`) instead of the flat `L1/L2 FAILED`. No driver change — reads `net_poll()`'s existing return code. | code → burn | ✅ **LANDED + BURNED 2026-05-25 ~02:00 PDT** (`1325-agnos-1.32.5-honest-l2-rx-selftest-rx-alive-on-fb.jpg`) → **RX ALIVE ON THE FB: `rx=10 arp_in=7 arp_ans=1`**. Bite-7 broadcast breakthrough now visible without a pcap; the "FB lied for 4 days" defect retired. Broadcast + multicast proven on the FB; gateway unicast (APM) reply still pending. Leading next bite: restore the post-reset IDR0-5 write-back bite H removed (broadcast AB delivers / unicast APM does not = zeroed-IDR split). See § honest L2 RX self-test body. |

---

### Tracker: 1.32.4 cycle (OPEN 2026-05-23 PM — 10-bundle from [`dhcp-offer-downstream-audit.md`](prior-art/dhcp-offer-downstream-audit.md) closes the 1.32.3 carry-forward DHCP OFFER-downstream-of-r8169_poll residual. Items: (1) Bite 1 tcpdump user-side capture documented in next-burn prep; (2) CMOS instrumentation `[0x88..0x8B]` ethertype/proto/port stamps; (3) Fix A BOOTP `op==2` gate in OFFER matcher; (4) Fix B magic-cookie validation in OFFER matcher; (5) Fix C xid byte-order AUDITED OK no-code-change; (6) Fix D options-walker invariants AUDITED OK no-code-change; (7) listener-state stamp `[0x8C]`; (8) `DHCP_FRAME_DUMP` compile-gated 64-byte full-frame dump to `[0x90..0xCF]`; (9) `DHCP_STATIC_IP` compile-gated fallback; (10) ACK-matcher mirror of Fix A + Fix B. NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]] — user authorizes when ready. Pre-burn rubric pinned at § "Attempt 101 prep" below.) {#tracker-1324-cycle}
...
**Linked docs**: [`dhcp-offer-downstream-audit.md`](prior-art/dhcp-offer-downstream-audit.md) (this cycle's full audit + § 7 escape plan); [`dhcp-end-to-end-audit.md`](prior-art/dhcp-end-to-end-audit.md) (1.32.2-era predecessor); `agnos/CHANGELOG.md` § [1.32.4].
...
**Driving audit**: [`dhcp-and-outbound-l3-audit-2026-05-24.md`](prior-art/dhcp-and-outbound-l3-audit-2026-05-24.md) — consolidated DHCP review against RFCs + multi-source prior art (Linux / lwIP / iPXE / *BSD / U-Boot / Plan 9), identifies the outbound L3 next-hop-MAC gap as the fresh finding, lays out the test ladder.
...
**Linked docs**: [`dhcp-and-outbound-l3-audit-2026-05-24.md`](prior-art/dhcp-and-outbound-l3-audit-2026-05-24.md) (this prep's anchor); [`dhcp-offer-downstream-audit.md`](prior-art/dhcp-offer-downstream-audit.md) (10-bundle still landed for T7); [`r8169-chip-init-audit.md`](prior-art/r8169-chip-init-audit.md) (chip-init prior-art convergence for T1/T2).
...
**Linked docs**: `agnosticos/docs/development/prior-art/virtio-net-legacy-layout-audit.md` + `r8169-rx-path-audit.md` + `r8169-chip-init-audit.md § BSD + iPXE convergence (2026-05-23)`; iron-nuc-zen-log.md § Attempts 98 / 99 / 100; state.md § *Last refresh*.
...
**Linked docs**: [`dhcp-end-to-end-audit.md` § 10](prior-art/dhcp-end-to-end-audit.md) (FINDINGS #7-#10 multi-source convergent writeup — § 10.2 hypothesis FALSIFIED, next audit pending CMOS readback); state.md § *1.32.2 cycle*.
...
**Linked docs**: [`dhcp-end-to-end-audit.md`](prior-art/dhcp-end-to-end-audit.md) (original 6-FIX audit + post-95 § 10 extension); state.md § *1.32.1 cycle*.
...
**Linked docs**: [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md); [`network-arc-prior-art.md`](prior-art/network-arc-prior-art.md); state.md § *1.32.0 cycle*.
...
**Linked docs**: [`ext2-ext4-extents-prior-art.md`](prior-art/ext2-ext4-extents-prior-art.md); [`ext4-64bit-prior-art.md`](prior-art/ext4-64bit-prior-art.md); [`ext2-iron-burn-audit.md`](prior-art/ext2-iron-burn-audit.md); state.md § *1.31.x cycles*.
...
- Local: `docs/development/uefi-boot-prior-art.md`, `docs/development/path-c-sovereign-uefi.md`, `docs/development/prior-art/true-font-swap-plan.md`.
...
**Carry-forward:** four-patch Phase 2.6 fix stack, single burn, audited against Linux's `xhci_endpoint_reset` + `xhci_handle_cmd_set_deq` + USB MSC BBB §6.7.3 + xHCI 1.2 §4.6.8 / §4.6.10 / §4.10.2.1. Detail in [`msc-reset-recovery-prior-art.md`](prior-art/msc-reset-recovery-prior-art.md). Per `feedback_redesign_dont_reinvent`: Linux's sequence is canonical; no first-principles diagnostic letters; stack all four patches into one burn.
...
- [`msc-reset-recovery-prior-art.md` § 9](prior-art/msc-reset-recovery-prior-art.md) — four-source convergent audit (FreeBSD + OpenBSD + EDK2 + Linux).
...
- [`msc-reset-recovery-prior-art.md` § 9](prior-art/msc-reset-recovery-prior-art.md) — four-source convergent audit (FreeBSD + OpenBSD + EDK2 + Linux) that informed Phase 2.7 + 2.8.
...
- [`ext2-iron-burn-audit.md`](prior-art/ext2-iron-burn-audit.md) (bite E pre-burn audit, success rubric § 7).
...
- [`ext2-iron-burn-audit.md`](prior-art/ext2-iron-burn-audit.md) (reused — no new iron-validation surface vs Attempt 90).
- [`ext4-64bit-prior-art.md`](prior-art/ext4-64bit-prior-art.md) (bite A multi-source convergent audit).
...
- [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md) (pre-burn — 9 hypotheses, did not anticipate gate-predicate bug).
...
- [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md) § 5 (H1 / H7 / H8 — now-reachable hypothesis surface; next audit cycle extends with discriminator instrumentation).
...
First iron burn of agnos 1.32.1. Bites B (CMOS-bank discriminator stamps at slots 0x58-0x5F, ~50 LOC in `r8169.cyr` + ~30 LOC in `read-boot-log.cyr`) and C (`r8169_phy_init` MDIO + autoneg poll, ~85 LOC in `r8169.cyr`, claimed OpenBSD-converged) landed in commit `b12e25a` ("audit, instrumentation, and attempted fix"; 1.32.0 → 1.32.1 version bump in the same commit). Build size 601,392 → 603,784 B (`TCP_LISTEN_SMOKE=1`, +2,392 B, ~80 LOC effective). Audit doc bite A (§ 10.1-10.8 of [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md)) also landed in the same commit. `scripts/test.sh` 4/4 PASS pre-burn.
...
- [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md) § 10.1-10.8 (bite A audit extension — Attempt 92+93 evidence re-rank + multi-source convergent line-by-line vs current code + per-hypothesis corrective-patch shape + CMOS slot range plan).
...
**Cross-references:**
- [`dhcp-end-to-end-audit.md` § 10](prior-art/dhcp-end-to-end-audit.md) — FINDING #7/#8/#9 multi-source convergent writeup with Linux/OpenBSD/FreeBSD/NetBSD/Haiku cross-validation.
- Attempt 94 entry above — CMOS readback evidence pattern that 1.32.2 FIXes target.
- [`r8169-iron-burn-audit.md`](prior-art/r8169-iron-burn-audit.md) § 10 — superseded by `dhcp-end-to-end-audit.md` § 10 for OFFER-timeout root cause.
...
**Cross-references:**
- [`dhcp-end-to-end-audit.md` § 10](prior-art/dhcp-end-to-end-audit.md) — FIX #7-#10 multi-source convergent writeup that drove this burn.
...
**Cross-references:**
- `agnosticos/docs/development/prior-art/r8169-rx-path-audit.md` — the multi-source convergent audit + post-implementation update.
...
- Audit doc: [`r8169-chip-init-audit.md § BSD + iPXE convergence (2026-05-23)`](prior-art/r8169-chip-init-audit.md) — four-source citation matrix.
...
**Cross-references:**
- `agnosticos/docs/development/prior-art/r8169-chip-init-audit.md § BSD + iPXE convergence (2026-05-23)` — four-source citation matrix + memory follow-up.
...
- 1.32.4 10-bundle still landed but never exercised at Attempt 101: [`dhcp-offer-downstream-audit.md`](prior-art/dhcp-offer-downstream-audit.md).
...
- Construction / chip-init audit threads CLOSED by Linux proof: [`r8169-chip-init-audit.md`](prior-art/r8169-chip-init-audit.md).
...
- Construction / chip-init audit threads CLOSED: [`r8169-chip-init-audit.md`](prior-art/r8169-chip-init-audit.md).
...
- Audit receipt: [`r8169-rx-path-audit.md`](prior-art/r8169-rx-path-audit.md) (§ 1.32.5 addendum landed the AAP fix; needs a post-Attempt-104 closing note that AAP was falsified).

---

### agnos 1.32.5 — RX-engine bring-up fix (RXDV-gate settle + CPlusCmd correction) 2026-05-25 ~00:03 PDT → FALSIFIED (RX still silent; but `1325_pcap_attempt_3.pcapng` is the cleanest RX vantage of the entire arc — an on-segment host probed AGNOS directly and AGNOS's own ARP responder stayed silent)

**No photo** (user: "result was the same as before" — `arp: TIMEOUT` / `net: L1/L2 FAILED`, shell v1.32.5 byte-clean). Evidence this round is the wire capture, not the FB.

**Build under test**: `build/agnos` **621,768 B** (HEAD `6dcb3f5 "more audits and repair fixes - rx"`, mtime 2026-05-25 00:03), bite-6 RXDV-gate settle (`r8169.cyr:546` region, ~4–8 ms non-posted-MMIO spin between RXDV_GATED clear and CR.RE) + CPlusCmd `0x000B→0x0008`. cyrius 6.0.1 + gnoboot 0.4.2 unchanged. `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK.

**Wire evidence — `1325_pcap_attempt_3.pcapng`** (capture host `42:c2:df:db:ee:78` = `.121`, 00:15:26 → 00:17:02, 829 frames). **This vantage is qualitatively different from Attempts 101–104**: `.121` is on AGNOS's L2 segment AND it actively probed AGNOS, so AGNOS's *reply* (if any) lands at the capturer by construction — there is no "unicast not flooded to me" blind spot for the inbound-to-AGNOS direction.

| Observation | Evidence | Conclusion |
|---|---|---|
| AGNOS ARP request on the wire | `00:16:00.908195 b0:41:6f:0c:e4:25 > ff:ff:ff:ff:ff:ff Request who-has 192.168.1.1 tell 192.168.1.222` — the **sole** AGNOS-originated frame in the whole 96 s capture | **TX RE-PROVEN** (now 4× across 1324 / 1325_test / _2 / _3). bite-6 did not regress TX. |
| `.121` directly probed AGNOS — unicast | `00:16:00.908292` (97 µs after AGNOS's ARP) `42:c2:df:db:ee:78 > b0:41:6f:0c:e4:25 192.168.1.121 > 192.168.1.222 ICMP echo request`. `.121` learned `b0`'s MAC from the sender fields of AGNOS's broadcast ARP request, then unicast-pinged it. | A frame addressed to AGNOS's **exact hardware MAC** was put on AGNOS's port. |
| `.121` directly probed AGNOS — broadcast | **63×** `42:c2:df:db:ee:78 > ff:ff:ff:ff:ff:ff Request who-has 192.168.1.222 tell 192.168.1.121` (00:15:54 → 00:17:02) | 63 broadcast frames each demanding an ARP reply from AGNOS. |
| AGNOS replied to **none** of it | zero `b0:41:…` frames other than the one outbound ARP request; zero ARP replies, zero ICMP echo replies | **DECISIVE.** AGNOS implements a working ARP responder (`net.cyr:560-593` — on `oper==1` with `target_ip == net_ip` it `nic_send`s `192.168.1.222 is-at b0:41:6f:0c:e4:25`), and TX is proven. If a single one of the 63 directed broadcast queries had reached `net_handle_arp`, AGNOS would have egressed a reply and the capturer would hold it. None exists. |

**Verdict — bite-6 (RXDV-gate settle + CPlusCmd) FALSIFIED; RX-drop now proven DIRECTLY, not inferred.** Every prior attempt isolated the bug by the *absence of the gateway's unicast reply*, which was always vantage-blind (the reply is switched straight to AGNOS's port, never flooded to the sniffer). This capture sidesteps that entirely: the sniffer **is** the prober. AGNOS received 63 directed broadcast ARP queries + 1 directed unicast ICMP echo at its exact MAC and delivered **none** of them up to a net stack that demonstrably would have answered. The r8169 RX path drops broadcast **and** unicast before they reach `r8169_poll`'s consumer — confirming, with no inference gap, the Attempt-104 isolation (descriptor OWN / DMA / ring delivery, downstream of the now-exonerated L2 accept mask).

> Honest caveat: an absent ICMP echo reply alone would be inconclusive (AGNOS has no ICMP responder yet — that's future `yo`). The conclusive signal is the **63 unanswered ARP requests**, because the ARP responder is present and TX is proven. The gateway's unicast ARP reply to `b0` is *still* invisible at this vantage — but it is no longer needed.

**Ladder reckoning** (per [[feedback_stop_letter_laddering]]): the "RX silent for broadcast+unicast" symptom has now survived Attempts 97, 98, 99, 103, 104, bite-5 (`_2`), bite-6 (`_3`) — 7 burns, every targeted register/timing fix falsified (RxConfig profile, RX_EARLY_OFF, RXDV-gate clear, RXDV-gate settle, RMS field-overflow, CPlusCmd, AAP promiscuous, FS|LS-gate removal). **The escape is NOT bite-7-another-register.** Two unexamined contradictions should drive a from-scratch multi-source re-derive of the *whole* RX path (RxConfig write-effectiveness + ring + poll + rearm), treating every existing comment as suspect per [[feedback_audit_re_derive_dont_validate_comments]]:
> 1. **AAP-promiscuous-but-still-dropping.** With AAP set (`0xEF0F`) the L2 filter is wide open, yet broadcast+unicast still don't arrive while multicast did (Attempts 97–99 CMOS `[0x5E]=0x01`). A single 32-bit RxConfig write sets AAP/APM/AM/AB in one low nibble — if AM is honored but AB/APM/AAP are not, either the write isn't landing (clobbered after CR.RE? wrong latch edge?) or the "multicast passes" framing is stale on post-Attempt-99 builds. **Unverifiable without a photo/CMOS this round** — flag for the next burn's readback.
> 2. **Solicited-vs-unsolicited.** The frames that passed were unsolicited periodic multicast (mDNS/SSDP); the frames that drop are solicited responses (DHCP OFFER, ARP reply). At L2 that distinction shouldn't exist — which points back at ring/poll timing or RxConfig effectiveness, not the accept mask.

**Escalation (NOT auto-proposed per [[feedback_iron_burns_block_other_work]]; NO instrumentation per [[feedback_no_instrumentation_means_no_instrumentation]])**: from-scratch re-derive `r8169_init_rx` + `r8169_poll` + `r8169_rx_rearm` + the RxConfig write site against FreeBSD/OpenBSD/NetBSD `re_rxeof`/`re_init_locked` + iPXE `realtek_poll` + RTL8168h datasheet §6 — derive the descriptor OWN/EOR handoff and the RxConfig latch timing from sources *first*, then diff against AGNOS. A behavioral re-derive is a code bite (allowed); the next burn TESTS it. A TRUE SPAN/mirror port on a managed switch is still owed only if we later need to byte-confirm the gateway→`b0` reply lands on AGNOS's port — but `1325_pcap_attempt_3` shows we can already prove RX-drop without it.

**Cross-references**:
- Predecessors: bite-5 RMS field-overflow FALSIFIED (`1325_pcap_attempt_2`); Attempt 104 AAP-bisector FALSIFIED (L2 accept-mask exonerated).
- RX path under the spotlight: `agnos/kernel/core/r8169.cyr` (`r8169_init_rx` 417, `r8169_poll` 665, `r8169_rx_rearm` 658, RxConfig write 624-625, CR.RE late-enable 636-637).
- ARP responder that proves the silence: `agnos/kernel/core/net.cyr:560-593`.
- Audit receipt: [`r8169-rx-path-audit.md`](prior-art/r8169-rx-path-audit.md) (still owes a closing note that AAP + bite-5 + bite-6 are all falsified and RX-drop is now wire-proven directly).
...
**Landed same session — honest L2 RX self-test** (build-verified; **BURNED 2026-05-25 ~02:00 PDT → RX ALIVE on the FB — see entry below**): the boot net probe (`main.cyr:684-721`) now counts frames `net_poll()` delivered + ARP replies the responder TX'd (`net_arp_replies_sent`, `net.cyr:592`) during the gateway-probe wait, and prints `net: L2 RX ALIVE rx=N arp_in=M arp_ans=K -- gateway unicast reply pending` (or `net: L2 RX SILENT -- 0 frames in ~5s` when truly dead) instead of the flat `net: L1/L2 FAILED`. Build **622,408 B** (+512 B), `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK. **The next burn DID show it: `rx=10 arp_in=7 arp_ans=1` on the framebuffer (entry below).** NOT auto-proposed per [[feedback_iron_burns_block_other_work]].
...
### agnos 1.32.5 — honest L2 RX self-test on the framebuffer 2026-05-25 ~02:00 PDT → RX ALIVE CONFIRMED ON THE FB (rx=10 arp_in=7 arp_ans=1; broadcast + multicast RX proven without a pcap; gateway unicast reply still pending)

**Photo**: [`1325-agnos-1.32.5-honest-l2-rx-selftest-rx-alive-on-fb.jpg`](iron-nuc-zen-photos/1325-agnos-1.32.5-honest-l2-rx-selftest-rx-alive-on-fb.jpg) (filed from agnosticos top-level `1325_FB_LX2_repond.jpg`, iPhone, mtime 2026-05-25 02:03 PDT). Boot tail byte-clean — AHCI WD Blue SA510 enumerated → GPT 2 active / 128 reserved (`AGNOS-BOOT` ESP + `agnos-fs`) → VFS → `ext2: mounted (blocksize=4096, inode_size=256, inodes_per_group=8192)` → Heap → SYSCALL/SYSRET → stack canary → interrupts → `Timer ticks before sched: 6` → `Activating scheduler.` — then the net block:

```
net: STATIC ip=192.168.1.222 gw=192.168.1.1
arp: request -> 192.168.1.1
arp: gateway reply not seen within ~5s
net: L2 RX ALIVE rx=10 arp_in=7 arp_ans=1 -- gateway unicast reply pending
Launching kybernet...
kybernet: starting init
kybernet: 0 processes
kybernet: 3511 free pages
kybernet: launching shell
AGNOS shell v1.32.5 (type 'help')
agnos>
```

**Build under test**: `build/agnos` **622,408 B** (HEAD `52a66f8 "answered; now fixing fb logs"` — the honest-self-test build that was PENDING at the bite-7 entry above). cyrius 6.0.1 + gnoboot 0.4.2 unchanged. `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK.

**Verdict — the bite-7 breakthrough SHOWN on the framebuffer, no pcap required.** The three counters decode cleanly:
- **`rx=10`** — `net_poll()` returned >0 ten times during the ~5 s gateway-ARP wait ⇒ the RX ring delivered 10 frames up to the consumer. RX is unambiguously ALIVE on iron, **on the FB**.
- **`arp_in=7`** — 7 of those were inbound ARP frames (broadcast `who-has` queries off the segment).
- **`arp_ans=1`** — AGNOS's responder TX'd 1 reply: the full inbound chain (r8169 RX → `r8169_poll` → `net_poll` → `net_handle_arp` matched `target_ip == net_ip` → reply TX) fired on iron, reproducing the `1325_pcap_attempt_4` result **on the FB instead of in a capture**.
- **`rx=10 > arp_in=7`** — 3 non-ARP frames were also delivered (other ethertypes, e.g. IPv4 multicast) ⇒ broadcast **and** multicast classes both pass.

**This retires the "FB lied for 4 days" defect** the bite-7 entry called out: during the exact 5 s the gateway gate times out, the screen now reports the live NIC instead of flat `net: L1/L2 FAILED`. A partial-RX NIC no longer reads identically to a dead one — the single most expensive diagnostic trap of the 1.32.x arc is closed.

**Remaining gap (unchanged from bite-7, now FB-visible):** `arp: gateway reply not seen within ~5s` — AGNOS's own `who-has 192.168.1.1` got no matched reply (`arp_pending_ip` never cleared), so **unicast (APM-class) RX delivery is still unconfirmed**. Broadcast working (AB) does not guarantee unicast-to-our-MAC (APM).

**Leading next bite** (NOT auto-proposed per [[feedback_iron_burns_block_other_work]]): the broadcast-passes / unicast-pends split now strongly implicates the **post-reset IDR0-5 write-back that bite H (1.32.3) removed** — bite H trusted datasheet §2.3 ("`CR_RST` preserves IDR0-5, EEPROM-autoloaded"). If the RTL8168h does NOT autoload IDR on this warm-from-Linux path, the APM unicast filter matches against a zeroed IDR ⇒ every unicast (incl. the gateway's ARP reply) is dropped while broadcast (AB, IDR-independent) passes — *exactly* the observed split (tracker hypothesis 2, now sharpened by the broadcast-confirmed result). The gateway is PROVEN to unicast-reply to this exact MAC+IP frame on Linux (Attempt 102 `arping`/`arp-probe-raw` 3/3), so gateway-silence is largely ruled out; the suspect is squarely the chip's unicast accept path. A behavioral re-derive of the IDR writeback is a code bite (allowed); the next burn would watch `arp: reply <- 192.168.1.1` appear and `rx`/`arp_in` climb past the delivered unicast reply.

**Cross-references**:
- Predecessor: bite-7 (post-RX-enable accept-filter re-assert) — broadcast RX proven on the wire (`1325_pcap_attempt_4`); this burn shows the same capability on the FB without a capture.
- Honest self-test code: `agnos/kernel/core/main.cyr:684-721` + `net.cyr:592` (`net_arp_replies_sent`). No driver change — counts `net_poll()`'s existing return code.
- Next-bite target: `agnos/kernel/core/r8169.cyr` post-reset IDR0-5 write-back (removed in bite H, 1.32.3); tracker hypothesis 2 at [`#tracker-1325-cycle`](#tracker-1325-cycle).
