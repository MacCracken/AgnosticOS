...
| # | Bite | Iron? | Status |
|---|------|-------|--------|
| 1 | **IDR physical-match filter re-asserted post-`CR.RE`** (`r8169.cyr` step 11c) — the one filter register bite-2's confounded reorder never moved past enable. | code → next burn | ✅ **LANDED + BUILD-VERIFIED 2026-05-25.** 622,656 B, `test.sh` 4/4 + `ext2-smoke.sh` 5/5, multiboot2 OK. Burn-ready. |

**Iron falsification rubric** (1.32.7 bite-1 burn — IDR post-`CR.RE` re-assert):
