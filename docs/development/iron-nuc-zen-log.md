...
**Fix (1.41.12, behavioral).** `serial_dev_write` → `kprint` (serial+fb mirror, `core/devs.cyr`). agnsh-smoke **PASS**, banner retained on serial, build **1,070,704 B**. **Next-burn A1 is now OBSERVABLE** — expect `agnoshi 1.3.5` on the **FB**, then agnsh blocks on `read(fd 0)`→`kbd_read_blocking` (the documented empty USB-HID kbd ring, `boot_finish.cyr` Attempt 29) → banner+prompt visible but no keystrokes. **Next blocker = xhci USB-HID input on iron — a separate arc, not bundled here.**
