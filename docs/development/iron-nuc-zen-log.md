...
**Arc cap (user-driven, blocks other work per [[feedback_iron_burns_block_other_work]]):** land the **ESP-write safety guard** first, then a final FAT + exFAT write iron burn on real removable media (USB stick). Shell verbs (`touch`/`rm`/`mkdir`/`cat`/`ls` on FAT/exFAT) remain out of this arc → 1.39.x VFS generic-write lift. Roadmap: agnos `docs/development/roadmap.md` row 21.
