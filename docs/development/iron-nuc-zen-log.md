...
2. **1.34.3 — FAT LFN/truncate completeness** (IN PROGRESS, under CHANGELOG `[Unreleased]`, no bump): ✅ **bite 1 LFN-with-content** (`fatfs_write_file_lfn`: alloc+write chain, then LFN set with the 8.3-alias entry pointing at the first cluster + FileSize, vs `fatfs_create_lfn`'s empty cluster-0/size-0; `LongContent12345.bin` 3000 B byte-exact through its long name). ✅ **bite 2 LFN-name overwrite-match LANDED + QEMU-VALIDATED 2026-05-26** — new `fatfs_find_lfn` does the first LFN *read*: reassembles long names from their reverse-ordered `0x0F` runs (13 UTF-16 chars/entry, terminator-aware) + matches LONG name OR 8.3 name + records the 8.3 slot; `fatfs_write_file_lfn` uses it to overwrite a long-named file IN PLACE (repoint cluster+size, free old chain, preserve alias + LFN entries) instead of duplicating under a fresh `~N` alias. Gate: `LfnOver12345.bin` 3000→2000 B by long name → **single** dir entry (no duplicate) + content byte-exact, `fsck.fat -n` clean. Build 775,240 → **781,048 B**; `test.sh` 4/4. **Remaining: grow-truncate** (`fatfs_truncate` is shrink-only). exFAT has no LFN (UTF-16-native), so this is FAT-only. (Delete-by-long-name — clearing the LFN run — is a follow-on; `fatfs_find_lfn` already locates the set.)
3. **1.34.4 — cross-boundary directory runs + root extension**: FAT cross-sector LFN runs (`fatfs_find_free_run` is one-sector) + exFAT cross-sector dir-set (`exfat_find_free_dir_run` is one-sector) — the shared "entry-set spans a sector/cluster boundary" gap; both refuse such a run today. **Also: root-directory extension + deleted-slot reuse** — exFAT new-file create fails once the single-cluster root's 16 entries fill (1.34.2 confirmed: overwrite/truncate had to run in place; FAT has the same one-cluster-root ceiling). When this lands, **re-enable the deferred multi-new-file create test** (the EXOVER/EXTR new-file path 1.34.2 punted to in-place ops) — per the 2026-05-26 user note, those test blockers should clear here.
