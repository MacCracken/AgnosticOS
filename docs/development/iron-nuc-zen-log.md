...
| 3 | **symlink-create/`ln -s`** — allocate inode with `S_IFLNK`; fast symlink (target ≤ 59 B in `i_block`) vs slow symlink (target in a data block); `ln -s` shell verb | PENDING — `e2fsck -fn` clean; symlink inode type + target read back correctly (resolution itself is a separate read-path concern, prior-art § notes Phase-3 deferred symlink *resolution*) |
