...
3. **FAT write** (sub-divided — each sub-bite `fsck.fat -n`-clean + `mtools` persistence; the write smoke writes into the FAT32 **ESP** alongside the real gnoboot+agnos boot files, so fsck-clean = "didn't corrupt the coexisting files," a stringent gate):
   ...
   - 3b. **Cluster allocator + content write** — find free cluster (FAT scan) → mark EOC in **all** FAT copies → write data → update dirent cluster+size → maintain `FSINFO`. Gate: `fsck.fat -n` clean + `mtype` reads back the content byte-exact.
