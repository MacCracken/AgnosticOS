...
1. **1.34.2 — exFAT write parity** (bring exFAT to FAT's bite-3e level): overwrite-existing (`exfat_write_file` refuses an existing name today) + arbitrary-length truncate (to-zero only today) + `PercentInUse`/VolumeDirty maintenance (cheap correctness; fsck tolerates stale) + **ENOSPC-rollback smoke** exercising the rollback path on both FS (FAT code exists but was never smoke-triggered).
2. **1.34.3 — FAT LFN/truncate completeness**: LFN-with-content (`fatfs_create_lfn` is empty-file only) + LFN-name overwrite-match (`fatfs_find_root` is 8.3-only, so overwrite/delete by long name misses) + grow-truncate (FAT truncate is shrink-only). exFAT has no LFN (UTF-16-native), so this is FAT-only.
3. **1.34.4 — cross-boundary directory runs**: FAT cross-sector LFN runs (`fatfs_find_free_run` is one-sector) + exFAT cross-sector dir-set (`exfat_find_free_dir_run` is one-sector) — the shared "entry-set spans a sector/cluster boundary" gap; both refuse such a run today.
