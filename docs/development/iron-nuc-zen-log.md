...
### Tracker: 1.32.7 cycle (OPEN 2026-05-25 — **OUTCOME (bite-3, BURNED 2026-05-25): the filter-vs-delivery split is RESOLVED.** `rx_uc=2` from the chip's own silicon accept-counter proves the MAC physical-match filter ACCEPTS unicast → the entire L2 accept/filter arc is CLOSED. The blocker is **RX ring/poll DELIVERY** (`missed=158` RxMissed FIFO-overflow, `rx_err=0 align=0` clean frames dropped for no descriptor). **Next: whole-ring drain in `r8169_poll`** (FreeBSD `re_rxeof` / iPXE `realtek_poll`). Original OPEN framing follows for traceability. — **r8169 RX of UNICAST frames on iron, take 2: the IDR physical-match write moved past `CR.RE`**. 1.32.6 shipped gateway L2 reachability (RFC-826 snoop) but the unicast TCP SYN+ACK still never lands. NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]]; hypothesis + falsification rubric below.) {#tracker-1327-cycle}
>
...
| 5 | **Deepen the RX ring 16 → 64** (`r8169.cyr`) — the bite-4-rubric pre-committed lever. The whole-ring drain (bite-4) proved the bottleneck is **ring capacity, not servicing logic**: 16 descriptors is an outlier vs all prior art (Linux `NUM_RX_DESC=256`, FreeBSD/OpenBSD 256) and a LAN-chatter burst (74 bcast + 65 mcast in the probe window) overruns it within one `hlt`-spaced poll gap, dropping the clean unicast SYN+ACK for lack of a free descriptor (`missed=176`, `rx_err=0 align=0`; silicon `rx_uc=5` already proved the filter accepts it). `R8169_RX_RING_SIZE` 16 → **64** (4× burst headroom; 64×16 = 1024 B still fits one 4 KB ring page); shared `R8169_RING_MASK` (0x0F) split into `R8169_RX_RING_MASK` (0x3F) + `R8169_TX_RING_MASK` (0x0F) since the rings now differ in depth; `r8169_rx_bufs[16]` → `[64]`; `init_rx` loop bound + EOR-slot keyed to `R8169_RX_RING_SIZE`; `r8169_poll` error-skip walk budget 16 → `R8169_RX_RING_SIZE`. **TX untouched** (tx_err=0, never overflowed) — stays 16. No filter/driver-config change — RX buffering depth only; keeps the deepen-ring result unconfounded against the (now-landed) drain loop. | code → BURN-READY | ✅ **LANDED + BUILD-VERIFIED 2026-05-25.** Build 624,056 → **624,488 B** (+432 B), multiboot2 ELF64 OK, entry `0x1000a8`, `build/agnos` reflects HEAD+edit, VERSION untouched (1.32.7 open). `scripts/test.sh` **4/4** + `ext2-smoke.sh` **5/5** (zero regression — all 5 backends reach shell; r8169 path is iron-only, QEMU uses virtio_net). **Burn-ready** — NOT auto-proposed per [[feedback_iron_burns_block_other_work]]. **Rubric**: trailing `r8169: … missed=N …` line is the discriminator — `missed` collapses toward 0 ⇒ the deeper ring absorbed the burst → if the on-LAN MBP peer is up, `net: LAN-TCP OK` (SYN+ACK finally read) is the win. If `missed` stays high with a 64-deep ring ⇒ the misses are NOT in the TCP-connect poll window (they accrue during non-polling boot stretches / at the idle shell prompt) and the SYN+ACK drop is **not** an overflow at all → re-baseline: the SYN+ACK either never reaches a descriptor (chip-side, below the ring) or the `tcp_connect` poll cadence misses its arrival window (tighten the post-SYN poll loop, drop the `arch_wait` gap). |
