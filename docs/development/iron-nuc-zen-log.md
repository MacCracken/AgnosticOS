...
The 1.42.x hardening engineering is software-complete + QEMU-validated and **rides whatever the next archaemenid burn is** (kernel **1.42.12**, build 1,096,464 B — was 1.42.4/1,074,304 B when this tracker opened; since landed: perf **1.42.7–9** memory-core/fs-read/fb-blit, sysinfo syscalls **1.42.10** `uname`#34/`sysinfo`#35, **klug** kernel-log ring buffer **1.42.11–12** `klog`#36). Nothing here demands a dedicated burn — it folds into the next one the user runs. The agnos-fs `/bin` now also carries the first **AGNOS-tic userland tools** (`bnrmr`/`cmdrs`, staged via `stage-tools.sh`) alongside agnsh. What the next burn validates / probes:
