...
| 7 | ✅ **QEMU-COMPLETE 2026-05-25** — **lift metadata_csum refusal** (iron burn pending) | **PASS.** Dropped `0x400` from `ro_danger` → `write_ok=1` on metadata_csum+64bit. Full write smoke on the `metadata_csum,64bit,extent` image (the real-partition profile): WHOLE mutation set through the write path → **`e2fsck -fn` clean**, all files persisted. **`bg_itable_unused` accounting fix** (alloc into the never-used tail must shrink the watermark) cleared the e2fsck cascade the per-class compares couldn't surface. Matrix green (stripped/64bit/csum). Build 675,152 B. **AGNOS writes an unmodified default mkfs.ext4 — no re-carve.** W5 iron burn = user-driven, audit at [`metadata-csum-write-iron-burn-audit.md`](metadata-csum-write-iron-burn-audit.md). |
