...
### Tracker: 1.33.5 cycle — `fsync`/FLUSH-CACHE durability barrier (OPEN 2026-05-26 — the **last** 1.33.x WRITE-arc follow-on; after this the patch line closes and the big own-cycle writes move to 1.37.x–1.39.x. **Lean cycle-open**: VERSION 1.33.4 → 1.33.5, CHANGELOG header, this tracker; no code bites yet.) {#tracker-1335-cycle}
...
3. **Remaining backends + close** — **`msc_blk_flush`** (SYNCHRONIZE CACHE over USB-MS) + **`vblk_blk_flush`** (VirtIO FLUSH if feature negotiated, else success no-op) + **`ramdisk_blk_flush`** (no-op, returns 0). QEMU smoke against `usb-storage` + `virtio-blk`. Final-bite optional iron burn is user-driven (`echo > /f; sync` on the real NVMe partition; flush ACK'd — observable only as no-error, since the drive already honored completion).
