...
| 2 | **hardlink/`ln`** — dirent-insert pointing at an existing inode + `i_links_count` bump; refuse cross-device + dir hardlinks | PENDING — `e2fsck -fn` clean (Pass-4 link accounting is the oracle); `ln /a /b` → both names resolve to one inode, unlink one → other survives |
