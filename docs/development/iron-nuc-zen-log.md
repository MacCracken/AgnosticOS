...
| # | Bite | Gate |
|---|---|---|
| 1 | **`rename`/`mv`** — dirent-insert at the new parent + dirent-remove at the old (both primitives exist from W4); update `..` if the parent changes; `i_links_count` care for directory moves | PENDING — `e2fsck -fn` clean after a file move + a dir move on the `metadata_csum,64bit` image; `mv /a /b` then reboot-equivalent remount reads `/b` |
