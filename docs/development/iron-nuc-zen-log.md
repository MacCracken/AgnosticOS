...
2+. **FAT32 read parity + multi-backend routing** (if the audit confirms the gap) — mount the real ESP/USB-stick FAT32 through `blk_read_on(backend)`.
3+. **FAT write** — cluster allocator + dual FAT-table write → dirent create/delete (8.3 + LFN) → file write/truncate/append → shell verbs. Each bite `fsck.fat`-clean on a QEMU FAT image.
