...
### Tracker: 1.32.7 cycle (OPEN 2026-05-25 — **r8169 RX of UNICAST frames on iron, take 2: the IDR physical-match write moved past `CR.RE`**. 1.32.6 shipped gateway L2 reachability (RFC-826 snoop) but the unicast TCP SYN+ACK still never lands. NO iron burn auto-proposed per [[feedback_iron_burns_block_other_work]]; hypothesis + falsification rubric below.) {#tracker-1327-cycle}
>
...
| 3 | **r8169 hardware accept-counter readback** (`main.cyr` boot net-probe, after both unicast handshakes) — wire in the *already-written, never-called* `r8169_print_stats`/`r8169_dump_stats` (`r8169.cyr:843-918`, FreeBSD `re_sysctl_stats` / RTL8168 §6.8.4 tally-counter DMA). Reads the chip's OWN silicon counters split by class: `rx_uc` (dump offset 0x28, unicast accepted via physical-match), `rx_bc` (0x30, broadcast), `rx_mc` (0x38, multicast). Settles the filter-vs-delivery question every inference-only burn could not — the chip itself reports whether it accepted any unicast frame. r8169-only (guarded on `r8169_present`; QEMU virtio_net has no RTL8168 to count). | code → BURN-READY | ✅ **LANDED + BUILD-VERIFIED 2026-05-25.** main.cyr boot net-probe now calls `r8169_print_stats()` after the on-LAN + 1.1.1.1 tests, guarded `if (r8169_present == 1)`. **NO new instrumentation authored** — the dump function was already complete + prior-art-correct, just never wired in. Build 623,928 → **623,976 B** (+48 B), `test.sh` 4/4 + `ext2-smoke.sh` 5/5 (zero regression — the dump fires only on the iron path; QEMU `r8169_present==0` → output byte-identical to bite-2), multiboot2 ELF64 OK, `build/agnos` reflects HEAD+edit. **Burn-ready** — NOT auto-proposed per [[feedback_iron_burns_block_other_work]]; line audit + rubric below. |
...
| **`rx_uc>0`** | **the MAC IS accepting unicast → frames die between MAC-accept and `r8169_poll`/the ring → bug is in our descriptor/ring delivery, NOT the filter. Investigate why broadcast survives the same ring** (buffer size, descriptor-OWN handoff, or a class-dependent path in `r8169_poll`) |
