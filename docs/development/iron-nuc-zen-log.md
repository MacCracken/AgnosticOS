...
| 4 | **`s_state` dirty/clean + `sync` verb** — clear `EXT2_VALID_FS` in `s_state` on first write, set it on a `sync`; flush superblock | PENDING — host `e2fsck` after a yanked (no-`sync`) burn shows dirty (expected); after `sync` shows clean. Honest no-journal story per prior-art § (was "Add to W5"). |
