...
- **1.37.2 — multi-leaf (depth-1 sibling split). ✅ DONE (awaiting tag).** `ext2_extent_add_sibling_leaf`: leaf full → new sibling leaf (csum-stamped) + a 2nd/3rd/4th index entry in the inline root, so a fragmented file spans up to 4 leaves (≈ 4 × eh_max extents) without depth 2. The depth-1 `append_block` path decides extend / insert / sibling per-append. Smoke drives a 2-leaf depth-1 tree (`eh_depth==1`, `eh_entries==2`) → e2fsck-clean.
- **1.37.3 — depth-2 (index-block grow). ✅ DONE (awaiting tag).** `ext2_extent_grow_indepth2` spills the inline root's 4 index entries into an INDEX block (`eh_depth=1`, csum-stamped), root → `eh_depth=2` with one index entry. `ext2_extent_add_leaf_to_idxblk` adds sibling leaves under the index block (held in `ext2_indirect_buf_l1` while the leaf builds in `ext2_extent_buf`); a new depth-2 descend in `append_block` walks root → index block → leaf. Completes the on-demand grow ladder **0→1→2** (sibling index blocks / depth-3 deferred — unreachable: ~115 k extents). Smoke drives `eh_depth==2` (`final depth=2 size=11.1 MB`, ~1360 extents) → e2fsck-clean, proving both grows + leaf AND index-node checksums + i_blocks. **Remaining in 1.37.3: the arc's iron burn** (user-driven — extent allocation's first real-hardware touch; depth-0/1 covers every realistic file).

(Boundaries collapsed/split as the work landed: 1.37.1 = depth-1 grow, 1.37.2 = multi-leaf sibling split, 1.37.3 = depth-2 + iron burn — the tree-growth split into clean per-depth bites.)
