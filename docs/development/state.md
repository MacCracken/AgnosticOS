...
> **Last refresh**: 2026-05-17 — **agnos 1.30.5 staging absorbed Phase 4 + Phase 5 (USB HID-boot keyboard driver, ~600 LOC across 2 new files + 4 extended).** Build-verified clean (350,272 B → 364,736 B); fmt-clean. Phase 4 (`hid_kbd_configure`: Get Configuration Descriptor + walk for HID-boot-keyboard interface + Configure Endpoint TRB type 12 + SET_PROTOCOL=boot + interrupt-IN transfer ring) + Phase 5 (`hid_poll` event-ring drain + HID-usage→PS/2-set-1 translation table + report differ + `kb_buf` writer through the existing `scancode_to_ascii` path). 1.30.4 closed 2026-05-17 with xHCI Linux-diff hardening (H1-H4: PAGESIZE assert / IMAN.IP clear / IMOD=250µs / USBCMD.HSEE) as spec-discipline closeout. **Iron status**: Attempt 52 closed silent-absorb arc as "non-spec gate, parallel-track only" — Phase 4/5 dormant on archaemenid (Phase 3 enumeration doesn't complete due to silent-absorb on USB2 port reset). **Next iron burn (Attempt 54) externally gated on kriya 0.3.0 ship** (M2 file-operations milestone) — bundles non-regression 1.30.5 confirmation with kriya M2's installer-pipeline surface. Per-attempt + Attempt 54 prep matrix in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md). Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md). **Cyrius toolchain**: 5.11.55 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
