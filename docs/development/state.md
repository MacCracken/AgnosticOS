...
> **Last refresh**: 2026-05-23 (**1.32.3 OPEN — Iron-side root cause IDENTIFIED via wire + QEMU diagnostics (NO additional burn); fix landed; Attempt 98 PENDING USER BURN. RxConfig `0xE700` → `0xCF00`: chip-rev-specific profile alignment matching Linux v7.0 `r8169_main.c:2589-2614 rtl_init_rxcfg` for archaemenid's confirmed `RTL8168h XID 541` = `RTL_GIGA_MAC_VER_46`. The pre-fix legacy profile left `RX_EARLY_OFF` (bit 11) CLEAR; on G/H/M silicon Early-RX ENABLED drops broadcast frames mid-DMA above ~256-512 B threshold — DHCP OFFER (~590 B) in the kill zone, multicast frames (IGMP/NDP/mDNS ~46-120 B) below it. Exactly matches Attempt 97 evidence (16 frames consumed, all multicast)**). Highlights:
...
| D | **Chip-rev RxConfig fix** — `R8169_RXCFG_DEFAULTS = 0xE700` → `0xCF00`. ONE 16-bit constant. Source comment expanded with Linux line-cite + per-mac_version table. | Iron Attempt 98 | 1 LOC change + ~17 LOC comment | ✅ **LANDED 2026-05-23 15:31 PDT** — `agnos/kernel/core/r8169.cyr:109`. Build 617,000 B same size (constant swap doesn't grow bytecode; comments stripped). `test.sh` 4/4 + `ext2-smoke.sh` 5/5 + 5/5 regression + `tcp-listen-smoke.sh` 1/2 baseline. md5 `1b5e7dd70ba765cd4ad9ba56dd0f0f1d`. |
| E | **Iron Attempt 98** — validates the chip-rev profile alignment against Attempt 97 evidence base. Target: full DHCP cycle on archaemenid. | Iron-only | n/a | ⏳ **PENDING USER BURN** — pre-burn rubric pinned at iron-nuc-zen-log § Attempt 98 prep. Expected on PASS: `dhcp: DISCOVER → OFFER ip=192.168.1.X → REQUEST → ACK gw=192.168.1.1 mask=255.255.255.0`; CMOS 0x5A 0x02 → 0x03+, 0x5E 0x01 → 0xff or 0xb0. |
| F | **Cycle-close sweep + Attempt 98 transcript** | n/a (doc-roll) | n/a | ⏳ pending burn outcome |
