...
> **Last refresh**: 2026-05-17 — **Attempt 56 root-caused Phase 4 `events_seen=0`: Repair (FF) IMAN.IE was 0 on archaemenid AMD FCH 1022:1639, blocking event posting to the ring.** 1.30.6 cut applies the one-line fix at `kernel/arch/x86_64/usb/xhci.cyr:541` — `xhci_rt_write32(ir0 + XHCI_IR_IMAN, 0x1)` → `0x3` (IP=W1C clear + IE=set). Linux `xhci-mem.c` sets IE=1 unconditionally; AGNOS had a deliberate "IE=0 in poll mode" comment that the FB instrumentation falsified empirically. Same shape as Repair (EE): defensible-in-isolation spec reading falsified by AMD FCH silicon, surfaced by prior-art diff, not per-spec audit. **Iron evidence (Attempt 56, instrumentation burn at 1.30.5 + Attempt-56 instrumentation in `xhci_cmd_wait` + enable_slot entry)**: FB prints `xhci: enable_slot entry idx=1 cycle=1` followed by `xhci: cmd completion timeout, final_idx=1 cycle=1 events_seen=0` after the Enable Slot doorbell — the controller posts zero events to the ring over the full `XHCI_CMD_TIMEOUT_SPINS` (~250 ms) window. Pre-PR `xhci: drained 0 events` confirms the ring was empty before R/S advance, so reset PSC events also never landed (despite `Reset events bitmap=63` suggesting they fired at PORTSC level). Event ring infrastructure (ERSTSZ=1, ERSTBA, ERDP, CRCR + RCS=1 ordering) all clean per audit — the gate is at the interrupter. **Iron status**: Phase 3 still UNBLOCKED on USB2 bank port 3 (Repair EE intact). Phase 4 Enable Slot gate has a proposed unblock awaiting Attempt 57 iron burn. Phase 4/5 code surface unchanged from 1.30.5. CMOS `[0x86]=0x5A` / `[0x87]=0xA5` extended-CMOS-offset-≥6 alias quirk reproduced (firmware-managed, not a code regression). Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempt 56. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § 1.30.6. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
