...
> **Last refresh**: 2026-05-21 evening (post-1.31.5 cut + **Attempt 89 interim no-regression burn PASS**; **agnos 1.31.6 cleanup-cycle OPEN**).
>
> **Current scope** — 1.31.x storage arc is ext2/4-filesystem complete; 1.31.6 cleanup-cycle is the disciplined closeout (8 bites, see § 1.31.6 below). **Filesystem layer in place**: `kernel/core/ext2.cyr` ~870 LOC Phase 1-4 (superblock + BGDT + inode + direct-block / single-double-triple-indirect tree / dirent walk + path resolution + `VFS_EXT2_FILE` FD / ext4 extents header + leaf walker), QEMU 5/5 green on both ext2 and ext4 images; iron Attempt 89 confirmed no-regression on archaemenid (full storage trio re-validated + new NVMe `agnos-fs` partition enumerated cleanly through GPT Phase 3); `ext2: mounted` line absent as predicted (gated on 1.31.6 bite (G) multi-backend probe). Photos: [`attempt-89-…-pt1-xhci-usb-ms-nvme.jpg`](iron-nuc-zen-photos/attempt-89-agnos-1.31.5-ext2-no-regression-pre-bite-g-pt1-xhci-usb-ms-nvme.jpg) + [`pt2-ahci-gpt-3parts-vfs-kybernet.jpg`](iron-nuc-zen-photos/attempt-89-agnos-1.31.5-ext2-no-regression-pre-bite-g-pt2-ahci-gpt-3parts-vfs-kybernet.jpg). Build trajectory through 1.31.x storage arc: 421,912 B (1.31.0 cycle-open lean) → 568,960 B (1.31.5 ext2/4 Phase 1-4) — +147 KB / ~6,200 LOC across NVMe + AHCI + GPT + USB MS + RAM-disk + VirtIO modern + ext2/4. MVP gate stays green on iron (typeable shell from Attempt 68 / 1.30.9; storage trio iron-debut at Attempts 80/81/87; no-regression burns at Attempts 82/88/89). gnoboot stays at 0.4.2.
>
...
> - **1.31.6 OPEN** — Eight-bite cleanup / hardening / audit cycle. See § *1.31.6 cleanup cycle* below.
>
...
### 1.31.6 cleanup cycle (OPEN 2026-05-21 evening)

Disciplined post-greenfield closeout for the 1.31.x storage + filesystem arc. Eight bites per agnos roadmap § 7a (~270 LOC + ~200 audit prose). Item (G) is the GATING work for iron Attempt 90 (ext4 victory lap against the new NVMe `agnos-fs` carve + the SATA `agnos-fs` carve).

| # | Bite | Status | Notes |
|---|------|--------|-------|
| A | ext2 input validation sweep (~80 LOC) | planned | superblock-field bounds, inode-num range, PATH_MAX cap, blocksize sanity (dirent rec_len already validated 1.31.5 Phase 3) |
| B | fatfs BPB validation sweep (~30 LOC) | planned | num_fats / sectors_per_cluster / root_entry sanity. 1.31.5 added bytes_per_sector + sectors_per_cluster guards; this extends. |
| C | drop ext2 boot-time smoke hook from main.cyr (-40 LOC) | planned | `ls`/`cat` shell verbs are the real consumers now |
| D | Cyrius `var X[N]` byte-vs-u64 gotcha → feedback memory + CLAUDE.md note | planned | module-global `var X[N]` allocates N×u64 (8N bytes) — surfaced during ext2 indirect-buffer sizing |
| E | `agnosticos/docs/development/ext2-iron-burn-audit.md` (~200 prose) | planned | modeled on nvme-iron-burn-audit / ahci-iron-burn-audit / usb-ms-iron-burn-audit / ramdisk-virtio-modern-prior-art |
| F | state.md / roadmap / iron-log sweep for 1.31.5 closeout | **in progress** | this refresh |
| **G** | **multi-backend ext2 probe + `blk_read_on` helper (~80 LOC) — GATING for Attempt 90** | planned | `ext2_init` walks all registered backends (NVMe / AHCI / USB-MS / VirtIO / RAMDISK) reading LBA 2-3 for `0xEF53` magic, mounting first match. New `blk_read_on(backend_tag, sector, buf)` for explicit per-backend dispatch. |
| H | partition-aware mount via GPT consumption (~50-100 LOC) | planned | new `ext2_init_partition(first_lba)` for cases where the test FS lives inside a partition rather than at LBA 0. Lands after G. |

Per `feedback_iron_burns_block_other_work` — no iron burn proposed for the cycle without a written audit first. Attempt 90 (ext4 victory lap) is gated on bite (G) landing + ext2-iron-burn-audit doc (E) landing. Build target trajectory: 568,960 B (1.31.5) → 1.31.6 close (-40 LOC bite C; +~200 LOC A+B+G+H net). QEMU smoke after each code bite; full storage-trio smoke after G + H.
...
**Order through 1.31.x**: ~~GPT~~ ✅ → ~~AHCI/SATA~~ ✅ → ~~**USB MS + Optical via USB MS**~~ ✅ (USB MS closed at agnos 1.31.3 / Attempt 87 PASS; USB optical pending) → ~~**RAM-disk + VirtIO-blk modern**~~ ✅ (1.31.4 / Attempt 88 PASS as no-regression) → ~~**ext2 read-only**~~ ✅ (1.31.5 Phase 1-4 / Attempt 89 PASS as no-regression; ext2-mount-on-iron gated on 1.31.6 bite G) → **cleanup cycle** (1.31.6 OPEN — 8 bites, see § *1.31.6 cleanup cycle* above).

**Iron-validation coverage at each cut**: 1.31.0 → NVMe iron debut (Crucial P3 / Attempt 80). 1.31.1 → NVMe + SATA iron debut (WD Blue SA510 / Attempt 81 PASS-WITH-CAVEAT). 1.31.2 → AHCI carry-forward triplet iron-PASS (Attempt 82) + USB MS iron progression (Attempts 83/84 PARTIAL → 85/86 FALSIFIED). **1.31.3 → USB MS iron debut PASS (Phase 2.8 eight-bug stack / Attempt 87 full INQUIRY+TUR+RC10 on Silicon Motion silicon).** 1.31.4 → RAM-disk + VirtIO (QEMU only) + iron no-regression burn (Attempt 88). **1.31.5 → ext2/4 Phase 1-4 (QEMU 5/5 green) + iron no-regression burn (Attempt 89; new NVMe `agnos-fs` carve enumerated through GPT Phase 3; ext2 mount silently misses as predicted, gated on 1.31.6 bite G).** 1.31.6 → iron Attempt 90 PENDING (ext4 victory lap on dual carves, post-bite-G).
...
**Storage arc post-1.31.5 status:** ~~1.31.3~~ ✅ (USB MS Phase 2.8 / Attempt 87 PASS) → ~~1.31.4~~ ✅ (RAM-disk + VirtIO modern / Attempt 88 PASS no-regression) → ~~1.31.5~~ ✅ (ext2/4 Phase 1-4 / QEMU 5/5 green + Attempt 89 PASS no-regression). Active cycle: **1.31.6 cleanup** (see § *1.31.6 cleanup cycle* above for the eight-bite punch list).
...
| **agnos** | **1.31.6** | **6.0.1** | **Kernel rolled through the 1.31.x storage + filesystem arc on cycc 6.0.1.** 1.31.2 (AHCI carry-forward triplet + USB MS Phase 1-4 + 2.5 + 2.6 + 2.7) → 1.31.3 (USB MS Phase 2.8 eight-bug repair stack / Attempt 87 iron PASS) → 1.31.4 (RAM-disk + VirtIO 1.x modern / Attempt 88 iron PASS no-regression) → 1.31.5 (ext2/4 Phase 1-4 / Attempt 89 iron PASS no-regression) → **1.31.6 cleanup-cycle OPEN 2026-05-21 evening** (8 bites — see § *1.31.6 cleanup cycle*). Pin stable on cycc 6.0.1 since the mid-1.31.x graduation. |
