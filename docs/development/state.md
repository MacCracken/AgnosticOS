...
| Merge `0.2 → main` | [ ] Queued for next gnoboot touch |
| Bump `VERSION` 0.1.0 → 0.2.0 | [ ] Bundle with merge |
| Rename `[Unreleased]` → `[0.2.0] - 2026-05-15` in `CHANGELOG.md` | [ ] Bundle with merge |
| Tag `v0.2.0` | [ ] Bundle with merge |

Per `feedback_bootloader_kernel_ownership` Claude owns gnoboot end-to-end during iron-boot bring-up; the explicit hold here is so the merge + version bump happens in a single user-driven update rather than racing the kernel-side cleanup. No code change required at merge time — branch state is the release.
...
*Refreshed 2026-05-15 (header + iron-boot row spot-updated for **Attempt 29 burn outcome + post-Attempt-29 cleanup pass**: ✅ shell visible on iron (Repair P confirmed); new blocker = USB-keyboard scancodes not reaching `kb_buf` (UEFI legacy SMM PS/2-emulation not active on archaemenid). MVP gap #3 falsified — primary triage is BIOS knob / port swap, real-answer fallback is native XHCI + USB-HID-boot driver. Cleanup pass: all 19 cp_fb() call lines stripped from main.cyr (CMOS port-I/O stamps preserved as post-mortem channel); 85 serial_print/serial_println → kprint/kprintln (fixes scrambled-digits fb-output bug); FB_CONSOLE_Y0 80→8 (boot_shim canary preserved); read-boot-log.cyr verdict text refreshed for post-cleanup kernel + Attempt 29 ground truth. cp_fb() fn + color palette + boot-shim canary preserved (logging infrastructure intact for future regression bisection). Kernel 266,712 → 266,312 bytes. gnoboot 0.2 branch (CMOS-removal track) ready for main merge.**. Earlier same-day Repair (O) entry: mem-iso test block deleted from main.cyr after re-reading uefi-boot-prior-art.md confirmed it was post-MVP work breaking pre-MVP boot. Kernel: 255,048 → 253,496 bytes (pre-Repair-P baseline). Earlier same-day spot-updates for Attempts 21–27 + Repairs (I)/(J)/(K)/(L)/(M)/(N)/(O) summarized in the iron-boot-row narrative — full per-attempt records live in [`iron-boot-testing-log.md`](iron-boot-testing-log.md).
