...
> **Last refresh**: 2026-05-18 — **Attempt 62 burned + FALSIFIED (NN+OO bundled — Tier 1 + Tier 2 convergent-prior-art ineffective; `events_seen=0` survives ERDP-before-ERSTBA + CRCR-after-IMOD + USBSTS-clear + IMAN.IE-post-R/S + mfence + TRB-readback).** 9-letter ladder closed at OO (FF → GG → HH → JJ → KK → LL → MM → NN → OO on the same `events_seen=0` after Enable Slot doorbell). Per `feedback_stop_letter_laddering` the escape plan is now load-bearing. **Vendor-cap research completed (0 burns) — DRY WELL** (Linux applies exactly one `1022:1639`-gated quirk, irrelevant). **MSI-X audit + BAR memtype audit completed (0 burns) — MSI-X DIVERGENCE FOUND, BAR MEMTYPE CLEAN**. AGNOS's `pci_enable_msix_unmasked` (`pci.cyr:216-241`) sets MSI-X Enable + clears Function Mask, then stops — **never reads MSI-X Table Offset/BIR from cap+0x04, never writes Message Address / Message Data / Vector Control to any table entry**. Every vector's table is literally all zeros (Address=0, Data=0, Vector Control=1 by reset per PCI 3.0 §6.8.2.5.3). Linux's `msix_capability_init` (`drivers/pci/msi/msi.c`) writes Address/Data for every vector between Enable+MaskAll and clear-MaskAll. Mechanism story: AMD FCH controller's interrupter state machine may speculatively read its own table to gate event posting; sees Address=0 + mask=1 and silently absorbs CCE deposits. PSCE may post via a different interrupter path that doesn't gate on this. BAR memtype is strict UC (`vmm.cyr:59-82` writes PWT=1+PCD=1+PAT=0 → PAT entry 3 under firmware PAT MSR `0x0007040600070406`); matches or exceeds Linux `ioremap_uc()`. **Repair (QQ) staged in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempt 63 prep**: extend `pci_enable_msix_unmasked` to program vector 0 with Message Address Lo=0xFEE00000 (BSP LAPIC base, dest CPU 0, physical mode), Address Hi=0, Data=0x40 (vector 0x40, Fixed delivery, Edge trigger), Vector Control=1 (mask preserved — AGNOS polls; no ISR plumbing needed); + readback flush. ~9 LOC delta in `pci.cyr`. Risk: low-to-medium (controller-side speculative-table-read hypothesis; failure mode is same `events_seen=0` plus possible PCI error log). Confidence: medium-high — first repair in arc with a direct, named, Linux-implicit divergence (every prior was spec-path reorder or defensive flush). **Pending user per-action burn approval.** **No burn proposed; user decides.** USBSTS-side boot evidence is loud (HCH/HSE/EINT/PCD/CNR/HCE/SRE all 0x00, USBCMD = R/S|HCRST|INTE|HSEE) — controller is healthy and silent on CCEs only. Bootstrap artifact: [`xhci-prior-art-audit.md`](xhci-prior-art-audit.md). Reader: [`scripts/read-boot-log.sh`](../../scripts/read-boot-log.sh) default prints the focused current-sweep CMOS view; `--verbose` recovers the full slot listing. User directive 2026-05-18 ("I really don't care what fixes it I want it fixed... hardening and cleanup can always be done later") remains in effect — single-repair-per-burn discipline suspended for the cmd-path arc; instrumentation discipline (`feedback_no_instrumentation_means_no_instrumentation`) remains in force. CMOS sentinel note: bytes 0x86/0x87 read 0x5a/0xa5 in the OO boot log instead of expected 0xcc/0xdd — those slots sit above the archaemenid virgin-scratch zone (0x50-0x7F); AA (0x81)/BB (0x84) intact (0x80-0x84 band confirmed reliable). No VERSION bump through NN/OO — agnos 1.30.6 banner retained. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § [Unreleased]. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
