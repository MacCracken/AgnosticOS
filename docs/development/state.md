...
> **Last refresh**: 2026-05-22 (**1.32.1 cycle continued via DHCP wiring repair — Attempt 95 ready to burn**). Earlier 2026-05-22: bites A+B+C landed in commit `b12e25a` (audit doc § 10.1-10.8 + CMOS 0x58-0x5F instrumentation + `r8169_phy_init` MDIO/autoneg poll); Iron Attempt 94 PARTIAL — build verified on iron via shell `v1.32.1`, bite-B CMOS readback FALSIFIED the audit § 10 H1/H7/H8 framing in-place (0x5B=0x30 H7 falsified, 0x5E=0x01 H8 falsified, 0x59=2 traced to bite-C BMSR-latching-low + busy-wait-100×-too-short bug). r8169 functional on iron; OFFER-timeout root cause moved UPSTREAM of NIC. End-of-day 2026-05-22: full [`dhcp-end-to-end-audit.md`](dhcp-end-to-end-audit.md) landed → user direction "fix all the issues" + "all still in 1.32.1 cycle work, most likely close after burn either way" → **all 6 FIXes landed same-day in 1.32.1** (no version bump per [[feedback_no_unprompted_version_bumps]]): FIX #1 `nic_mac()` backend-agnostic kernel-MAC accessor (PRIMARY: pre-fix every net.cyr egress site used `vnet_mac` which was uninitialized zero on iron without virtio → DHCP DISCOVER chaddr was 00:00:00:00:00:00, server dropped per RFC 2131 §4.1); FIX #2 `net_init(0,0,0)` on iron path; FIX #3 `r8169_phy_init` non-blocking kick; FIX #4 OFFER+ACK chaddr validation; FIX #5 timeout 200→800 iter; FIX #6 RxConfig named-constants + datasheet citation. Build 603,784 → 604,096 B prod / 604,904 B TCP_LISTEN_SMOKE. `scripts/test.sh` 4/4 + `ext2-smoke.sh` 5/5 + 5/5 regression + `tcp-listen-smoke.sh` 1/2 (pre-existing iron-only). **Iron Attempt 95 PENDING** — target: full DHCP cycle on archaemenid; closes 1.32.1. Storage trio + GPT + ext4 mount + scheduler + tcp_listen + kybernet + shell all byte-clean. Carry-forward continues for i225-V driver bite C (Intel iron pending) + BBS/MUD userland (out-of-cycle).
>
> **Current scope** — **1.32.1 PROGRESSED 2026-05-22 through Attempt 94 PARTIAL** (networking-arc continuation; cycle theme: **r8169 driver-level OFFER-timeout debug**). Bites A (audit), B (instrumentation), C (PHY init) all landed; bite D Attempt 94 burned PARTIAL (build-on-iron verified, OFFER timeout persists, H1/H7/H8 readback pending); bite E cycle-close transcript IN-FLIGHT (this update). **Cycle bites** (see § *1.32.1 cycle* below): bite A = OFFER-timeout audit (§5b extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) — line-by-line H1/H7/H8 vs current `r8169.cyr` + multi-source prior art); bite B = discriminator instrumentation (CMOS-bank stamps at `r8169_send` entry/exit + `r8169_poll` invocation count + post-DISCOVER TX OWN-bit readback + post-timeout RX ring head/tail); bite C = driver-level corrective patches per audit findings (PHY init most likely candidate per Linux's `__rtl8169_set_phy_reg_bit` sequence); bite D = Iron Attempt 94+ re-burn (target outcome: full DHCP cycle on archaemenid); bite E = cycle-close sweep + Attempt 94+ transcript. **Out-of-cycle carry-forward continues**: i225-V driver bite C (Intel-NIC iron pending, post-archaemenid-migration); BBS + MUD userland (separate standalone repos). The 1.31.x storage + filesystem arc (storage devices NVMe/AHCI/USB-MS/RAM-disk/VirtIO-blk + filesystem ext2/ext4 Phase 1-5 + GPT + multi-backend probe + partition-aware mount + shell-UX `cd`/`pwd`/CWD/bare-name `cat`/`ls -la`) closed at 1.31.7 (Iron Attempt 91 PASS); **1.32.0 networking arc closed at 1.32.0** (TCP server primitives + UDP server primitives + DHCP client + r8169 Phases 1-4 + DHCP gate fix; Attempts 92 + 93 PARTIAL — see § *1.32.0 cycle* below for full receipt). MVP gate stays green on iron from Attempt 68 / 1.30.9 and **still green at Attempt 94**; gnoboot stays at 0.4.2; cyrius pin stays on 6.0.1.
>
...
> - **1.32.1 IN-FLIGHT 2026-05-22 (Attempt 95 PRE-BURN, post-6-FIX DHCP repair)** — Cycle reshaped mid-stream via audit-driven repair. Pre-Attempt-94 theme: r8169 driver-level OFFER-timeout debug. Bites A+B+C landed in commit `b12e25a`; Iron Attempt 94 PARTIAL surfaced that the audit-§ 10 framing was wrong (r8169 functional; OFFER-timeout root cause UPSTREAM of NIC). Same-day audit ([`dhcp-end-to-end-audit.md`](dhcp-end-to-end-audit.md)) found six wiring bugs; user direction "fix all the issues" + "all still in 1.32.1 cycle work" → all 6 FIXes landed same-day in the existing cycle window (no version bump). PRIMARY fix: `nic_mac()` backend-agnostic kernel-MAC accessor — replaces 7 sites in net.cyr that read uninitialized `vnet_mac` (= zero on iron without virtio). Build 603,784 → 604,096 B prod / 604,904 B TCP_LISTEN_SMOKE. `scripts/test.sh` 4/4 + `ext2-smoke.sh` 5/5 + 5/5 regression + `tcp-listen-smoke.sh` 1/2 (matches pre-fix baseline). **Iron Attempt 95 PENDING — closes 1.32.1**. Storage trio + GPT + ext4 mount + scheduler + tcp_listen + kybernet + shell expected byte-clean. cyrius 6.0.1 + gnoboot 0.4.2 unchanged. See § *1.32.1 cycle* below + [`iron-nuc-zen-log.md` § Attempt 94](iron-nuc-zen-log.md) (Attempt 95 entry pending burn).
>
...
### 1.32.1 cycle (OPEN 2026-05-22 — networking-arc-continued; cycle-open lean: VERSION bump + CHANGELOG header + this tracking surface, no code touches yet)
