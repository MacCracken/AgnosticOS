...
> **Last refresh**: 2026-05-23 (**1.32.3 OPEN — Attempts 98 + 99 both FALSIFIED (CMOS byte-identical to Attempt 97: 16 multicast frames consumed, OFFER timeout); post-Attempt-99 bundle LANDED — full `rtl_hw_start_8168h_1` port + Linux-exact `rtl_hw_start` envelope + audit items 6/7/9/10/11 stacked. Next iron burn PENDING USER per [[feedback_iron_burns_block_other_work]]. Build 617,000 → 622,616 B (+5,616 B / ~250 LOC across `r8169.cyr`)**). Highlights:
...
### 1.32.3 cycle (OPEN — chip-rev RxConfig fix LANDED; Iron Attempt 98 PENDING USER BURN)
...
| F | **Post-Attempt-99 bundle — full `rtl_hw_start_8168h_1` body + Linux-exact `rtl_hw_start` envelope (items 6+7+9+10+11 stacked)**. (1) MAC-OCP helpers (read/write/modify via OCPDR @0xB0); (2) EPHY helpers (read/write/modify via EPHYAR @0x80 with completion polling); (3) ERI set/clear-bits helpers; (4) PCIe-LNKCTL ASPM/CLKREQ disable helper (walks PCI cap, clears bit 8 in cap+0x10); (5) `r8169_pcie_state_l2l3_disable` (clear `Config3.Rdy_to_L23`); (6) **`r8169_hw_start_8168h_1`** body — 6-entry EPHY table init + FIFO size + pause thresholds + ASPM entry-latency PCI write + ERI 0xDC/0x5F0/0xC0/0xB8/0x1B0 writes + RXDV disable + DLLPR PFM_EN+TX_10M_PS clears + MISC_1 PFM_D3COLD clear + PCIe L2/L3 disable + 4 mac_ocp_modify + 4 mac_ocp_write; (7) `r8169_probe` restructured to mirror Linux envelope (Cfg9346 unlock → ASPM disable → CPlusCmd write → `hw_start_8168h_1` → 32-bit MAC writes → MAR all-1s → Cfg9346 lock); (8) `r8169_init_rx` + `r8169_init_tx` restructured to mirror Linux's CR=TE\|RE single-write shape (item 8): `init_rx` removes RxConfig + CR.RE writes, swaps RDSAR HI-before-LO (item 9); `init_tx` swaps TNPDS HI-before-LO (item 9), does CR=TE\|RE in ONE write, then RxConfig profile + TxConfig + RxConfig RMW with AB\|AM\|APM accept bits — ALL post-enable per Linux line 4124-4128. | Iron Attempt 100 PENDING | ~250 LOC in `r8169.cyr` | ✅ **LANDED 2026-05-23 ~18:10 PDT** — Build 617,000 → **622,616 B** production. `test.sh` 4/4 PASS + `ext2-smoke.sh` 5/5 PASS + 5/5 regression cross-check + `tcp-listen-smoke.sh` 1/2 (matches pre-fix baseline; scenario 1 SLIRP-RX gap iron-only). md5 `e613735f1a4294e0b2047a52acbff373`. Per `feedback_iron_burns_block_other_work` Attempt 100 NOT auto-proposed — user decides when to burn. |
