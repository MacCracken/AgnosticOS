...
> **Last refresh**: 2026-05-20. Closed-beta MVP gate green on iron, both BIOS paths: VGA-spec at Attempt 68 (1.30.9), Quiet Boot at Attempt 76 (1.30.11). 1.30.x FB hardening sweep closes at .11. Active scope: **true-font swap** for 1.30.12 (replace hand-drawn 8×8 CGA bitmap with a real bitmap font source — legibility issue, not paint/cache/scanout).
> **Current iron state**: Attempt 76 PASS on archaemenid Quiet Boot — no lockup, keyboard live, refresh live, glyphs visually scaled at 24-px cell but still illegible as letters (8×8 source bitmap is the bottleneck). Three of four bars cleared in one burn; the fourth (legibility) is the true-font work. Build 422,048 B, gnoboot 0.4.1. See [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempt 76.
