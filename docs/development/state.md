...
> **Last refresh**: 2026-05-22 (post-1.31.7 release + **agnos 1.32.0 networking-arc cycle OPEN-AND-CLOSED same-day, feature-complete cut per user direction**: bites A/F/G + r8169 Phases 1-4 + DHCP gate fix all landed; **Iron Attempts 92 + 93 burned on archaemenid** — Attempt 92 PARTIAL surfaced `main.cyr:655` gate predicate bug (gate keyed on `vnet_active` only, so r8169-only iron path skipped `dhcp_init`); fix LANDED same-day with explicit-OR generic abstraction `if (vnet_active != 0 || nic_ready() != 0)`; **Attempt 93 PARTIAL VERIFIED the gate fix on iron** — `dhcp: DISCOVER` line egresses through r8169 path for the first time, new failure mode is `dhcp: OFFER timeout` (DISCOVER → wire → no OFFER back); carry-forward to 1.32.1: driver-level OFFER-timeout debug (H1/H7/H8 audit hypothesis surface now reachable) + i225-V driver bite C + BBS/MUD userland; build 600,520 → 601,392 B + 4/4 test.sh PASS; user tags 1.32.0 per [[feedback_no_unprompted_version_bumps]]).
>
> **Current scope** — **1.32.x networking arc CLOSED 2026-05-22 at 1.32.0** (feature-complete cut). What landed: kernel server-side TCP primitives (bite A — `tcp_listen`/`tcp_bind`/`tcp_accept` + passive-open SYN handler + SYN_RCVD state + ARP REQUEST handler, ~188 LOC); UDP server-side primitives (bite F — `udp_bind`/`udp_recv_from`/`udp_send_from` + 8-listener table, ~95 LOC); DHCP client (bite G — RFC 2131 DISCOVER → OFFER → REQUEST → ACK with full BOOTP header + option parsing, ~260 LOC); r8169 driver Phases 1-4 (bite B — PCI probe + MAC read + soft reset + RX descriptor ring + TX descriptor ring + NIC dispatcher `nic_ready`/`nic_send`/`nic_poll`, ~400 LOC); iron debut Attempts 92 + 93 on archaemenid (bite D); DHCP gate predicate fix at `main.cyr:655`; pre-burn audit doc [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md); cycle-close sweep (bite E). **1.32.1 cycle pending** — opens after user tag of 1.32.0 with VERSION bump per [[feedback_no_unprompted_version_bumps]]. Carry-forward to 1.32.1: **r8169 driver-level OFFER-timeout debug** (H1 PHY-not-configured / H7 TX OWN stuck / H8 RX OWN stuck — the originally-anticipated audit hypothesis surface, now reachable post-gate-fix; needs new audit cycle + discriminator instrumentation via CMOS-bank stamps per the no-serial-on-iron constraint); **i225-V driver port** (bite C, pending dedicated Intel-NIC iron); **BBS + MUD userland consumer apps** (separate standalone repos, out-of-cycle by design). The 1.31.x storage + filesystem arc (storage devices NVMe/AHCI/USB-MS/RAM-disk/VirtIO-blk + filesystem ext2/ext4 Phase 1-5 + GPT + multi-backend probe + partition-aware mount + shell-UX `cd`/`pwd`/CWD/bare-name `cat`/`ls -la`) closed at 1.31.7 (Iron Attempt 91 PASS — see § *1.31.7 cycle* below for receipt); MVP gate stays green on iron from Attempt 68 / 1.30.9 and **still green at Attempt 93**; gnoboot stays at 0.4.2; cyrius pin stays on 6.0.1.
>
...
### 1.32.x cycle (CLOSED 2026-05-22 at 1.32.0 — networking arc feature-complete; Iron Attempts 92 + 93 burned; carry-forward to 1.32.1 below)
...
### 1.32.1 carry-forward (pending — opens after user tag of 1.32.0)

Three items move from 1.32.0 to 1.32.1 per the cycle-close decision (user direction 2026-05-22):

1. **r8169 driver-level OFFER-timeout debug** — primary 1.32.1 scope. Attempt 93 confirmed DISCOVER egresses on iron through the post-fix gate, but no OFFER returns within the timeout window. This puts the originally-anticipated H1/H7/H8 audit hypothesis surface back on the table as the now-reachable failure mode:
   - **H1 — PHY not configured / no link**: top candidate. DISCOVER frame built + handed to `r8169_send` + queued into TX desc 0, but no electrical link → bits go into the void. Linux's `r8169_main.c` does explicit PHY init (RTL8211 register sequence) post-reset; current `r8169.cyr` Phase 1 does soft-reset + descriptor-ring setup but skips PHY-side init per the audit doc's "no PHY init" pre-burn discipline. Probable smallest fix: port the Linux `__rtl8169_set_phy_reg_bit` sequence as a Phase 5 patch.
   - **H7 — TX OWN bit stuck**: DISCOVER may be sitting in TX descriptor 0 with OWN=1 (DMA-handed-to-NIC) forever. Currently no observability — TX completion path doesn't poll back. Discriminator: CMOS-bank stamp at `r8169_send` entry/exit + post-poll TX OWN read.
   - **H8 — RX OWN bit stuck**: OFFER may be arriving on the wire and landing in an RX buffer whose OWN bit we're not polling correctly. Discriminator: CMOS-bank stamp of RX ring head/tail + `r8169_poll` invocation count.
   - **Audit doc to land at cycle-open**: extends `r8169-iron-burn-audit.md` § 5 with the now-reachable H1/H7/H8 sub-section. No code touches before the audit lands per [[feedback_iron_burns_block_other_work]].

2. **bite C / i225-V driver** — Intel 2.5GbE family port (~700-1100 LOC). Stays PENDING until dedicated Intel-NIC iron is available — likely post-archaemenid-migration when the i9 gains the Intel NIC surface. Mirror Phase 1-3 of r8169.

3. **BBS + MUD userland consumer apps** — separate standalone repos (out-of-cycle by design). Arrive when wire-end-to-end has DHCP ACK + TCP accept-success on iron. Not gated by 1.32.1 itself; can move in parallel.

The 1.32.1 cycle-open will VERSION-bump per [[feedback_no_unprompted_version_bumps]] (bump on cycle OPEN, user tags on cycle CLOSE) and write the first row of the 1.32.1 bite table — driver-level audit doc as the lean opening move.
...
| **agnos** | **1.32.0** | **6.0.1** | **1.31.x storage + FS arc CLOSED 2026-05-22 / 1.32.x networking arc CLOSED 2026-05-22 at 1.32.0 (feature-complete).** 1.31.2 (AHCI carry-forward triplet + USB MS Phase 1-4 + 2.5 + 2.6 + 2.7) → 1.31.3 (USB MS Phase 2.8 eight-bug repair stack / Attempt 87 iron PASS) → 1.31.4 (RAM-disk + VirtIO 1.x modern / Attempt 88 iron PASS no-regression) → 1.31.5 (ext2/4 Phase 1-4 / Attempt 89 iron PASS no-regression) → 1.31.6 cleanup cycle (8 bites + 2 smoke-surfaced fixes + Iron Attempt 90 ext4 victory lap PASS on NVMe `agnos-fs` p3 — see § *1.31.6 cleanup cycle*) → **1.31.7 filesystem follow-ups + shell UX CLOSED 2026-05-22** (ext4 64BIT Phase 5 + `cd`/`pwd`/CWD scoping + bare-name `cat` + `ls -la` flag dispatch + Iron Attempt 91 PASS — see § *1.31.7 cycle*) → **1.32.0 networking arc CLOSED 2026-05-22**: TCP server primitives bite A (~188 LOC) + UDP server primitives bite F (~95 LOC) + DHCP client RFC 2131 bite G (~260 LOC) + r8169 driver Phases 1-4 bite B (~400 LOC) + iron Attempts 92 + 93 on archaemenid (bite D) + DHCP gate predicate fix at `main.cyr:655` + cycle-close sweep (bite E). **Iron evidence**: Attempt 92 PARTIAL surfaced gate predicate bug → same-day fix `if (vnet_active != 0 || nic_ready() != 0)` → **Attempt 93 PARTIAL VERIFIED the fix on iron** (`dhcp: DISCOVER` egresses through r8169 path for the first time; new failure mode `dhcp: OFFER timeout` = 1.32.1 carry-forward). MVP gate still green at Attempt 93. **Carry-forward to 1.32.1**: r8169 driver-level OFFER-timeout debug (H1 PHY-not-configured / H7 TX OWN / H8 RX OWN — now-reachable post-gate-fix); i225-V driver bite C (pending Intel-NIC iron); BBS + MUD userland (out-of-cycle, separate repos). Strategic destination: AGNOS installable-state on archaemenid → user's planned **dual-boot migration** (refined 2026-05-22): 1TB ex-USB drive becomes Beelink internal NVMe + hosts AGNOS-primary; WD Blue SA510 SATA stays in chassis as Linux daily-driver root; 2TB Crucial P3 NVMe migrates to i9. See § *archaemenid migration* above and [[project_archaemenid_install_plan]]. Multi-cycle path: 1.32.x networking → 1.33.x ext4 WRITE → installable-state cycle slot TBD. See § *1.32.x cycle* + § *1.32.1 carry-forward*. Pin stable on cycc 6.0.1. Build trajectory across the cycle: 578,432 B (1.31.7 close) → **601,392 B (1.32.0 close, `TCP_LISTEN_SMOKE=1`)** = +22,960 B / ~1,000 LOC across `net.cyr` + `r8169.cyr` + `main.cyr`. |
