...
> **Last refresh**: 2026-05-17 — **Attempt 54 closed Row 1: 1.30.5 iron-clean baseline on archaemenid; silent-absorb persists per Attempt 52 (thirteenth observation).** Both external gates (kriya 0.3.0 + Cyrius cycle) released ahead of the burn; non-regression + Phase 4/5 init confirmation landed in one iron exposure. FB rendered `hid: keyboard layer initialized` cleanly (`hid_kbd_init()` runs without faulting; `hid_poll()` shell-tick call from `kb_has_key()` structurally inert under no-slot-addressed condition). CMOS `[0x86]=0x5A` / `[0x87]=0xA5` reproduced the FCH 1022:1639 extended-CMOS-offset-≥6 alias quirk from Attempt 52 — confirmed firmware-managed, not a code regression. **agnos 1.30.5 staging absorbed Phase 4 + Phase 5 (USB HID-boot keyboard driver, ~600 LOC across 2 new files + 4 extended)** — kernel 350,272 B → 364,736 B, fmt-clean. Phase 4 (`hid_kbd_configure`: Get Configuration Descriptor + walk for HID-boot-keyboard interface + Configure Endpoint TRB type 12 + SET_PROTOCOL=boot + interrupt-IN transfer ring) + Phase 5 (`hid_poll` event-ring drain + HID-usage→PS/2-set-1 translation table + report differ + `kb_buf` writer through the existing `scancode_to_ascii` path). 1.30.4 closed 2026-05-17 with xHCI Linux-diff hardening (H1-H4: PAGESIZE assert / IMAN.IP clear / IMOD=250µs / USBCMD.HSEE) as spec-discipline closeout. **Iron status**: silent-absorb arc remains "non-spec gate, parallel-track only" — no Attempt 55 *code* burn without (a) real silent-absorb unblock hypothesis or (b) substantive new Linux-diff finding. Phase 4/5 validation continues on QEMU xhci-pci. **Cycle-burn queued (no code change, no USB reflash)**: USB3 256 GB thumb drive on a blue port to test whether silent-absorb is USB2-bank-specific or controller-wide — current 1.30.5 binary already stamps the needed CMOS slots (`[0x63]` CCS, `[0x64]` reset-OK, `[0x65-0x67]` proto map). Three outcomes: USB3 reset succeeds → USB2-bank-specific (major narrowing); USB3 reset also absorbs → controller-wide gate; thumb drive doesn't enumerate → separate-controller-path hypothesis (mirrors the boot USB-C drive's absence from Attempt 54 CCS bitmap). Cheaper than a code burn — single device-swap reboot. **Post-burn pin sweep cleared to proceed**: agnos + agnosticos/scripts `cyrius = "5.11.55"` → `"5.11.59"` absorbed in one bump now that 1.30.5 baseline is iron-confirmed. Per-attempt detail (incl. Attempt 54 outcome) in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md). Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md). **Cyrius toolchain**: 5.11.55 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
...
| 8 | **Iron-boot bring-up (NUC AMD archaemenid)** | 🔄 1.30.5 staging — Phase 4/5 landed, Attempt 54 prep pending kriya 0.3.0 | Closed-beta gate is boot-to-shell on iron with typeable keyboard. **xHCI silent-absorb arc closed 2026-05-17 Attempt 52** as "non-spec gate, parallel-track only" — 12 hypotheses falsified, post-Attempt-52 handoff/AMD-quirk audit confirmed no Renoir-specific cold-boot workarounds AGNOS misses. **1.30.4 closeout (cut 2026-05-17)**: H1-H4 xHCI Linux-diff hardening (PAGESIZE assert / IMAN.IP clear / IMOD=250µs / USBCMD.HSEE bit 3) — kernel 350,008 → 350,272 B, no iron burn. **1.30.5 staging (active)**: Phase 4 (`hid_kbd_configure` Get Config Descriptor + interface walk + Configure Endpoint TRB type 12 + SET_PROTOCOL=boot + interrupt-IN transfer ring) + Phase 5 (`hid_poll` event-ring drain + HID→PS/2 translation table + report differ + `kb_buf` writer) — kernel 350,272 → 364,736 B, no iron burn, fmt-clean. Code surface complete; QEMU xhci-pci is the active validation surface. Phase 4/5 dormant on archaemenid until silent-absorb resolves. **Attempt 54 prep**: non-regression confirmation + opportunistic re-probe of silent-absorb under post-Phase-4/5 binary; **externally gated on TWO upstream completions** — (1) kriya 0.3.0 M2 ship + (2) Cyrius agent's current work cycle finishing. Burn fires only after the second gate closes. Pre-bound outcome matrix + iron protocol in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempt 54 prep. **Post-burn pin sweep planned**: agnos + agnosticos/scripts 5.11.55 → 5.11.59 (cycle patches .56-.59 expected to fix papercuts surfaced during previous builds; absorbed in one bump after burn confirms baseline against current pin so any post-bump regression is attributable cleanly). |
