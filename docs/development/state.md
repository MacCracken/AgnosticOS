...
> **Last refresh**: 2026-05-14 night (captures iron-boot Attempts 4–15 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 15 = sub-case 3b ruled OUT, sub-case 2 confirmed**: iron stalled at CP 0x23 again despite repairs A+B, visual CP ladder confirmed the new kernel ran on iron (cells through 0x10 painted YELLOW/GREEN/CYAN), so the sched-policy diagnosis was incomplete. Actual bug: **`kernel/arch/x86_64/pic.cyr` `timer_isr_build()` only pushes the 9 caller-saved GPRs** despite a stale comment claiming otherwise — the 6 callee-saved regs (rbx/rbp/r12-r15) leak between procs across every context switch, so when proc 0 finally re-runs, its `[rbp - N]` access to `idle_count` reads test_proc_b's stack instead of the kernel stack. **Repair (C) landed 2026-05-14 night**: ISR now pushes/pops all 15 GPRs; `proc_save_context`/`proc_restore_context` updated for the 6 new slot offsets + hw-frame offsets shifted +48. Kernel **252,480 → 253,712 bytes** (+1,232). `timer_isr[]` buffer at 63/64 bytes — 1 byte headroom only. **QEMU pre-flight permanently blocked for this kernel** (path A W^X-blocked on OVMF; `qemu -kernel` rejects multiboot2 ELF64; gnoboot QEMU+OVMF launch script not yet wired) → iron is the only test surface. **Closed-beta gate is CP `0x11` MAGENTA on Attempt 16.** Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
