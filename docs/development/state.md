...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–22 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)** ; Attempts 17–22 landed on the next-bug ladder. **Attempt 18** found CR4.SMAP=0 on iron — Path C boot shim deferred CR4 tuning to a non-existent "kernel re-tune". **Repair (F)** mirrored the CPUID-gated SMEP+SMAP enable into Path C shim. **Attempt 20 (2026-05-15)** confirmed cp_fb-under-AS1 hung: kcp=0x18 stuck because GOP framebuffer phys sits above the 0..4 GB range `proc_create_address_space` mirrors. **Repair (H)** dropped the four under-AS1 cp_fb calls (Option A; long-term fix B — pre-map FB phys into AS1/AS2 — deferred until mem-iso proves clean). **Attempt 21 (2026-05-15)** result: kcp=0x1D — Repair (H) landed cleanly; AS1 round-trip fully complete; death in 21-line block beyond with zero kcp resolution. **Repair (I) for Attempt 22 landed 2026-05-15**: 9 port-I/O bisector stamps (kcp=0x61..0x69) inserted at every step of AS2 block + second AS1 round-trip + kernel-CR3 restore. **Attempt 22 (2026-05-15)** result: kcp=0x68, [0x54]=0x30, [0x55]=0x30 — Repair (I) landed cleanly; second AS1 round-trip fully complete (kcp=0x66/0x67/0x68 all clean); **kernel-CR3 restore at main.cyr:575-577 (`mov rax, 0x1000 / mov cr3, rax`) #PFs** — pre-bound "kernel PML4 corrupt" branch hit. SMEP+SMAP healthy throughout. Code-path audit identified two suspects: (1) PMM allocates a kernel-PT page in [0x1000..0x3FFF] → `proc_create_address_space`'s `memset(addr, 0, 4096)` zeros kernel PML4 in place (high confidence, binary-decidable); (2) KPTI tail write at `proc.cyr:212` lands in kernel PD if `new_pd` is mis-set. **Repair (J) for Attempt 23 landed 2026-05-15**: 12 single-byte CMOS stamps in `proc_create_address_space` after each of the 6 pmm_alloc calls, distinguishing AS1 (slots 0x56..0x5B) from AS2 (slots 0x5C..0x61) via static call counter. Each stamp writes `(addr >> 16) & 0xFF` — byte 2 of phys addr. Any slot < 0x20 = smoking gun #1 confirmed (PMM handed out a kernel-PT page). Uses existing `outb` helper from `arch/x86_64/io.cyr`; no new helpers. **QEMU pre-flight still permanently blocked** → iron is the only test surface. **Next gate (Attempt 23)**: scan CMOS [0x56..0x61] for any < 0x20 reading; binary smoking-gun confirmation. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break (multiboot2 → sovereign boot-info struct); v1.30.1 candidate carries Repair F + H + I + J. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
