...
> **Last refresh**: 2026-05-17 — Attempt 51 burn outcome captured; 1.30.4 staging absorbs CC + DD repairs ahead of Attempt 52 burn. **Cycle status**: agnos 1.30.4 staging — open-ended; absorbs all silent-absorb hypotheses (BB / CC / DD / …) + Phase 4/5 code surface until coherent ship state. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md). Per-attempt iron outcomes + Attempt 52 prep matrix in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md). **Cyrius toolchain**: 5.11.55 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
...
| 8 | **Iron-boot bring-up (NUC AMD archaemenid)** | 🔄 Active — Attempt 52 prep | Closed-beta gate is boot-to-shell on iron with typeable keyboard. Per-attempt outcomes + repair ladder + Attempt 52 prep matrix all in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md). Current blocker: xHCI silent-absorb on USB2 reset (port 1+3, AMD FCH 1022:1639) — 11 hypotheses falsified; CC + DD bundled for the last just-testing burn before Phase 4/5 pivot per decoupling-decision. xHCI Linux-diff hardening backlog in [`agnos/docs/development/roadmap.md` § 1.30.x xHCI hardening](https://github.com/MacCracken/agnos/blob/main/docs/development/roadmap.md). |
