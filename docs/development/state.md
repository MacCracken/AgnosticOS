...
> **Cyrius toolchain**: 5.11.64 | **Cycle**: v5.11.x — stdlib annotation arc + consumer-issue closeout (active; **59 patches landed across 3 days** 2026-05-11/12/13 — 24 + 18 + 17; then 5 more across 2026-05-14/18 → .60 / .61 / .62 / .63 / **.64**)
> **Last refresh**: 2026-05-18 — **TYPEABLE SHELL ON QEMU REACHED. End-to-end xHCI bring-up working: USB HID keystroke → interrupt-IN transfer event → HID translation → kb_buf → agnoshi shell echo. `agnos> help` works on virtual hardware.** Root cause of the entire 10-letter Phase 3 cmd-path silent-absorb arc (FF→QQ+QQ2) was **a Cyrius compiler bug, NOT silicon**: `var X = INT_LITERAL ;` at module top would read as 0 before the init block ran (kmode==1 init-order: top-level asm → PARSE_PROG → EMIT_GVAR_INITS, with kernel's main body in PARSE_PROG never returning). `XHCI_CMD_TIMEOUT_SPINS = 10000000` read as 0 → spin loop never executed → `events_seen=0` always. `XHCI_EVT_RING_SEGMENT_SIZE = 256` read as 0 → ERST entry planted with 0-sized segment → controller had no event ring slot. **Fixed in cyrius v5.11.64** (issue [`2026-05-18-gvar-init-order-zero-reads.md`](https://github.com/MacCracken/cyrius/blob/main/docs/development/issues/2026-05-18-gvar-init-order-zero-reads.md), Option 1 — image-static init for literal-RHS gvars at file scope, every backend covered: ELF32/64-kernel/user/shared/obj, aarch64, MachO x86_64/ARM64, PE-EXEC). agnos pin bumped 5.11.59 → **5.11.64** (`agnos/cyrius.cyml`). **Beyond the cyrius fix, three agnos-side bugs surfaced via the QEMU lane**: (1) `vmm_remap_uc_2mb` only handled PML4[0]; QEMU's qemu-xhci BAR at 768GB needed PML4[N>0] support — extended in `kernel/core/vmm.cyr` (+280 B). (2) `xhci_alloc_input_ctx` hardcoded CSZ=1 (64-byte context) offsets; QEMU's qemu-xhci has CSZ=0 (32-byte contexts) — added `xhci_ctx_size()` / `xhci_slot_ctx_off()` / `xhci_ep0_ctx_off()` / `xhci_ep_ctx_off()` helpers in `xhci_ctx.cyr` and substituted all hardcoded offsets across `xhci_alloc_input_ctx`, `xhci_input_ctx_add_interrupt_in`, and one read-back site in `xhci.cyr:1036`. (3) `xhci_input_ctx_add_interrupt_in` OR'd new Add Flags onto stale A1 (EP0) from `xhci_alloc_input_ctx` — Configure Endpoint with A1=1 told HW to reload EP0 from stale Input Context, ccode=5 TRB Error. Fixed to set Add Flags = A0 | A_dci only, matching Linux `xhci_init_input_control_ctx` convention. **Iron implications**: iron has CSZ=1 → CSZ-aware code computes the same 64-byte offsets that were previously hardcoded (no regression); Add Flags fix is universal and likely unblocks iron Configure Endpoint if it was failing for the same reason; gvar fix is compiler-level and universal. **Iron burn pending** (your decision; previous Attempt 63 had visual MVP on iron but cmd-path silent — the same gvar bug bit it). Build delta: `build/agnos` 410,840 B (Attempt 63) → 411,280 B (post-CSZ + Add Flags fix). The cyrius bug had a pre-existing suspicion comment at `xhci_cmd.cyr:107-115` ("Suspected gvar-init-order: `XHCI_DIAG_SUBMIT_MAX` reading 0 at first call time") that turned out to be exactly right; the user's previous version-bump.sh workaround documented the kmode==1 constraint but missed the load-bearing-register case. **Letter-ladder retrospective** (FF→GG→HH→JJ→KK→LL→MM→NN→OO→QQ+QQ2 — ten falsified silicon-quirk hypotheses across Attempts 57-63): all were red herrings; none of them could have been correct because the bug was compile-time, not runtime. Per `feedback_known_knowledge_first` and `feedback_stop_letter_laddering`, the lesson is that compiler-class bugs need to be on the suspect list earlier when symptom + spec + prior-art-diff all conflict. The QEMU lane was the unlock — same symptom on a completely different controller proved silicon couldn't be the cause. **Pre-resolution context** (kept for historical reading): **Attempt 63 burned + FALSIFIED (QQ + QQ2 bundled — MSI-X table vector-0 programming + Enable→MaskAll→table→clear-MaskAll ordering ineffective; `events_seen=0` survives), AND VISUAL BOOT-TO-SHELL ON IRON ACHIEVED.** FB renders `agnoshi shell v1.30.7 (type 'help')` on archaemenid — kernel proceeds past xhci cmd-path failure into scheduler → userland → kybernet → agnoshi. Closed-beta MVP's visual half hit; functional half (typeable keyboard via xhci) still gated on the cmd-path silent-absorb. Letter ladder now 10 deep (FF → GG → HH → JJ → KK → LL → MM → NN → OO → QQ+QQ2); per `feedback_stop_letter_laddering` the escape plan is load-bearing. agnos VERSION bumped 1.30.6 → **1.30.7** in `agnos@3da2c28`; QQ + QQ2 landed in `agnos@677ff2a`. **Attempt 62 falsification (NN + OO bundled — Tier 1 + Tier 2 convergent-prior-art ineffective; ERDP-before-ERSTBA + CRCR-after-IMOD + USBSTS-clear + IMAN.IE-post-R/S + mfence + TRB-readback).** 10-letter ladder closed at QQ+QQ2 (FF → GG → HH → JJ → KK → LL → MM → NN → OO → QQ+QQ2 on the same `events_seen=0` after Enable Slot doorbell). Per `feedback_stop_letter_laddering` the escape plan is now load-bearing. **Vendor-cap research completed (0 burns) — DRY WELL** (Linux applies exactly one `1022:1639`-gated quirk, irrelevant). **MSI-X audit + BAR memtype audit completed (0 burns) — MSI-X DIVERGENCE FOUND, BAR MEMTYPE CLEAN**. AGNOS's `pci_enable_msix_unmasked` (`pci.cyr:216-241`) sets MSI-X Enable + clears Function Mask, then stops — **never reads MSI-X Table Offset/BIR from cap+0x04, never writes Message Address / Message Data / Vector Control to any table entry**. Every vector's table is literally all zeros (Address=0, Data=0, Vector Control=1 by reset per PCI 3.0 §6.8.2.5.3). Linux's `msix_capability_init` (`drivers/pci/msi/msi.c`) writes Address/Data for every vector between Enable+MaskAll and clear-MaskAll. Mechanism hypothesis at the time: AMD FCH controller's interrupter state machine speculatively reads its own table to gate event posting; sees Address=0 + mask=1 and silently absorbs CCE deposits. **Repair (QQ + QQ2) burned Attempt 63 (2026-05-18) and FALSIFIED**: extended `pci_enable_msix_unmasked` to program vector 0 with Message Address Lo=0xFEE00000 (BSP LAPIC base, dest CPU 0, physical mode), Address Hi=0, Data=0x40 (vector 0x40, Fixed delivery, Edge trigger), Vector Control=1; and wrapped the table-write in the Linux-canonical Enable+MaskAll → table-write → clear-MaskAll ordering. `events_seen=0` survives both. Speculative-table-read mechanism is now disproven; if MSI-X is a gate, it's gated on a different vector index or a different field. BAR memtype is strict UC (`vmm.cyr:59-82` writes PWT=1+PCD=1+PAT=0 → PAT entry 3 under firmware PAT MSR `0x0007040600070406`); matches or exceeds Linux `ioremap_uc()`. **VISUAL BOOT-TO-SHELL MILESTONE**: kernel no longer fatal-blocks on `events_seen=0` after xhci init failure; FB renders the full init sequence including `agnoshi shell v1.30.7 (type 'help')` prompt on archaemenid. Visual MVP gate hit; typeable keyboard still gated on cmd-path silent-absorb (no Enable Slot CCE → no HID enumeration). USBSTS-side boot evidence remains loud (HCH/HSE/EINT/PCD/CNR/HCE/SRE all 0x00, USBCMD = R/S|INTE|HSEE) — controller is healthy and silent on CCEs only. Next-move options on the table (no burn proposed, user decides): (PP) UC-remap DMA regions; (QQ3) Linux MSI-X per-vector replay across N vectors; (Phase-4/5-software) xHCI 1.2 §4.6 audit on whether Enable Slot is normatively required for keyboard enumeration; (decouple) Phase 4/5 to QEMU. Bootstrap artifact: [`xhci-prior-art-audit.md`](xhci-prior-art-audit.md). Reader: [`scripts/read-boot-log.sh`](../../scripts/read-boot-log.sh) default prints the focused current-sweep CMOS view; `--verbose` recovers the full slot listing. Reader header text still references "Repair (OO)" — cosmetic lag, flagged not touched. User directive 2026-05-18 ("I really don't care what fixes it I want it fixed... hardening and cleanup can always be done later") remains in effect — single-repair-per-burn discipline suspended for the cmd-path arc; instrumentation discipline (`feedback_no_instrumentation_means_no_instrumentation`) remains in force. CMOS sentinel note: bytes 0x86/0x87 read 0x5a/0xa5 in the QQ boot log instead of expected 0xcc/0xdd — those slots sit above the archaemenid virgin-scratch zone (0x50-0x7F); AA (0x81)/BB (0x84) intact (0x80-0x84 band confirmed reliable). agnos VERSION bumped 1.30.6 → **1.30.7** in `agnos@3da2c28` accompanying the QQ + QQ2 code-land in `agnos@677ff2a`. **Iron Attempts 65 / 66 / 67 (2026-05-18 same-day sequential burns)** — HID enumeration fully cleared on iron at Attempt 67; new blocker is Phase-5 interrupt-IN keystroke delivery. **Attempt 65** (~19:07 PDT) burned the post-.64 binary (cyrius 5.11.64 gvar fix + CSZ helpers + Add-Flags A0|A_new); Phase-3 silent-absorb cleared cleanly on archaemenid (Enable Slot CCE, slot 1, Address Device, GDD-8, GDD-18 all succeed; iron keyboard `VID=1452 PID=591`) but the first `xhci_get_config_descriptor(slot_id, 0, 9)` inside `hid_kbd_configure` timed out — iron-only divergence vs QEMU's typeable end-to-end. **Attempt 66** (~20:08 PDT) burned **Repair (RR) — Linux-canonical EP0 control-transfer hardening** per `xhci_queue_ctrl_tx` (drivers/usb/host/xhci-ring.c v6.13): RR.A sets ISP on Data Stage TRB; RR.B applies deferred-cycle Setup write (`giveback_first_trb` pattern); RR.C propagates full 64-bit `buf_phys` via `p_hi`. Bundled into both `xhci_control_in` and `xhci_control_no_data`; new helper `xhci_ep0_enqueue_raw`. Build 412,080 B. **Falsified** — GCD-9 still times out; EP0-ring-conventions diagnosis disproven. **Attempt 67** (~20:58 PDT) burned **EP0 MPS reconciliation** per xHCI 1.2 §4.6.7 / Linux `xhci_check_maxpacket` (agnos@`d671db3`, +48 LOC in `xhci.cyr`): new `xhci_evaluate_context(slot_id, input_ctx_phys)` issuing TRB type 13; post-GDD-8 reconciliation block in `xhci_enumerate_port` compares real `bMaxPacketSize0` (from `xhci_desc_buf_phys + 7`) vs the speed-safe MPS programmed pre-Address-Device, patches EP0 dw1 bits [31:16] preserving CErr/EPType/MaxBurst, fires Evaluate Context, updates the tracked slot MPS. Build: `build/agnos` 412,832 B. **Result**: every xhci-side command now completes on iron (Enable Slot ✓ Address Device ✓ Evaluate Context ✓ Configure Endpoint ✓), HID enumeration ends with `hid: keyboard configured, boot protocol on, EP=129, polling 8-byte reports`, FB renders `agnoshi shell v1.30.8 (type 'help')` on archaemenid. **New blocker — Phase 5 interrupt-IN silent**: keypresses on the iron keyboard produce no characters at `agnos>`. USBSTS clean (no HSE/HCE/SRE), USBCMD = R/S|INTE|HSEE, no `transfer event timeout` printed — controller isn't erroring, it's just not posting Transfer Events for the interrupt-IN ring on keypress. QEMU symmetric end-to-end with same binary (sendkey → echoed input), so iron-specific divergence is in interrupt-IN event-posting layer (or, less likely, in `xhci_handle_transfer_event` decoding / `kb_buf` enqueue). **No iron burn pending** — next move under user review. Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempts 65–67; photos at `iron-nuc-zen-photos/attempt-{65,66,67}-*.jpg`. agnos CHANGELOG § [1.30.8] closeout landed (three-burn arc, RR + MPS reconciliation, build sizes, Phase-5 framing); § [1.30.9] is a one-line placeholder pending the next landed movement — CHANGELOG entries capture movement, not diagnostic plans. **Phase-5 interrupt-IN keystroke delivery is the open MVP gate** — audit closed 2026-05-18 (0 burns). **Primary finding: SET_CONFIGURATION (USB 2.0 §9.4.7, bmRequestType=0x00 bRequest=0x09) is never sent during enumeration** — verified by grep across `kernel/`. The iron HID keyboard sits in Address state forever; per USB 2.0 §9.1.1.5 non-EP0 endpoints aren't operational until SET_CONFIGURATION transitions to Configured. QEMU's `usb-kbd` ignores this requirement (ships reports anyway), so QEMU-symmetric behavior hid the bug. Two secondary defensive findings: hardcoded `Interval=3` for FS keyboards (Linux uses `fls(8*bInterval)-1` = 6 for bInterval=10), and ISP bit missing on the interrupt-IN Normal TRB. Proposed bundle (1 burn, behavioral-diff-vs-Linux per `feedback_redesign_dont_reinvent`): SET_CONFIGURATION + canonical FS interval + ISP. Full audit at [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § "Phase-5 audit — 2026-05-18". 1.30.x remains the closed-beta MVP arc; 1.31.x stays reserved for post-MVP (networking or storage). **Cyrius toolchain**: 5.11.64 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
...
| 8 | **Iron-boot bring-up (NUC AMD archaemenid)** | 🔄 Phase 3 unblocked Attempt 55 (Repair EE); Phase 4 cmd-path silent-absorb open after 6 falsified arcs (FF/GG/HH/JJ/KK/LL/MM Attempts 57-61); Attempt 62 staged with Repair (NN); Attempt 63 pre-staged with Repair (OO) bundle | Closed-beta gate is boot-to-shell on iron with typeable keyboard. **Phase-3 silent-absorb arc closed 2026-05-17 Attempt 55** — root cause was `xhci_portsc_write` inner re-mask `& XHCI_PORTSC_NEUTRAL` stripping the RW1S PR bit; one-line fix in agnos@41ee6dc. 13 falsified hypotheses chasing a homegrown bug. **Phase-4 cmd-path silent-absorb open**: after 6 Linux-prior-art behavioral repairs (Attempts 56-61), Enable Slot's Command Completion Event still never posts to the event ring — `events_seen=0` survives FF (IMAN.IE=1) + GG (AMD-Vi disable) + HH (doorbell readback flush) + JJ (universal readback flush) + KK (CNR poll) + LL (Link TRB cycle init fix) + MM (MSI-X Function Mask=0). **Letter-laddering exhausted; pivoted 2026-05-18 to multi-source convergent-prior-art audit** ([`xhci-prior-art-audit.md`](xhci-prior-art-audit.md)). **Repair (NN) staged Attempt 62 (2026-05-18)**: two-LOC reorder in `xhci.cyr` — (NN.A) ERSTSZ → ERDP → ERSTBA (was ERSTSZ → ERSTBA → ERDP) per xHCI 1.2 §5.5.2.3.3 + 3-of-4 prior art (FreeBSD `xhci.c:1505-9`, Haiku `xhci.cpp:1744-9`, EDK2 `XhciSched.c:2651-9`); (NN.B) CRCR write moved from before ERST to after IMOD per 2-of-4 prior art (FreeBSD `xhci.c:1517-23`, Haiku `xhci.cpp:1756-7`). Build verified: `build/agnos` **368,472 B** (identical — reorder-only). **Repair (OO) pre-staged Attempt 63 (conditional on NN falsification)**: bundled four convergent-prior-art behavioral repairs — OO.A USBSTS RW1C-clear at `xhci_start` entry (FreeBSD `xhci.c:1463-66`), OO.B IMAN.IE write moved to AFTER R/S=1 (Linux `xhci.c:1145-7` convention; reverses Repair FF's pre-R/S placement), OO.C explicit `mfence` before doorbell write (defensive), OO.D cmd-ring TRB readback flush (defensive, JJ-pattern extended to RAM-side). User directive 2026-05-18: single-repair-per-burn discipline suspended for the cmd-path arc — bundle aggressively. agnos cyrius pin remains 5.11.59. **kriya 1.0.0** graduated to v1.0+ on 2026-05-18 (M5 grep/find/xargs closeout at 0.6.0 on 2026-05-17, then 1.0.0 cut once dispatcher + sovereign-boundary surface stabilized; cyrius pin 5.11.61). **commandress 1.0.0** also graduated to v1.0+ on 2026-05-18 (segment renderer + config layer stabilized after the 2026-05-15 scaffold; cyrius pin 5.11.64; adapter-based for agnoshi/bash/zsh prompt-hook). Joins the v1.0+ binaries cohort (agnos, agnoshi, argonaut, commandress, cyim, cyim-lsp, kriya, kybernet, nous, owl) — now **10 v1.0+ binaries**. |
...
CYML format — LEADING-EDGE 5.11.x post-burst cluster:
  v5.11.55: agnos (1.30.8), agnosticos/scripts (2026.5.13)
