...
> **Last refresh**: 2026-05-22 (post-1.31.7 close + **Iron Attempt 91 PASS — ext4 64BIT Phase 5 + shell-UX victory lap on archaemenid**; 1.31.x extended FS cycle complete).
>
> **Current scope** — **1.31.7 CLOSED 2026-05-22 with Iron Attempt 91 PASS** — five-bite filesystem-follow-ups + shell-UX cycle complete, see § *1.31.7 cycle* below. Six iron debuts / no-regression / victory-lap burns across the 1.31.x arc (NVMe @ 80, SATA @ 81, AHCI carry-forward @ 82, USB MS @ 87, RAM-disk+VirtIO no-regression @ 88, ext2/4 Phase 1-4 no-regression @ 89, ext4 victory lap @ 90, **ext4 64BIT + shell-UX victory lap @ 91**). **Filesystem layer iron-validated through Phase 5**: `kernel/core/ext2.cyr` ~995 LOC Phase 1-5 + cleanup-cycle hardening (superblock + BGDT + inode + direct-block / single-double-triple-indirect tree / dirent walk + path resolution + `VFS_EXT2_FILE` FD / ext4 extents header + leaf walker / **ext4 64BIT-aware BGDT stride + bg_inode_table_hi guard + supported_incompat mask 0x67C6** / multi-backend probe / partition-aware mount via GPT consumption / input validation sweep). Shell layer extended with `cd` + `pwd` + CWD scoping + bare-name `cat`/`ls` consuming CWD + `ls -la` flag-aware dispatch (~235 LOC in `kernel/user/shell.cyr`). QEMU 5/5 green on ext2 + ext4 + 64BIT-flagged ext4 images. **Iron Attempt 91 mounted user's re-carved (default `mkfs.ext4`, 64BIT-enabled) NVMe `agnos-fs` p3 cleanly**; `agnos> cd lost+found` traversed, `agnos> cat hello.txt` returned `agnos 1.31.7 iron Attempt 91   ext4 64BIT validated   2026-05-22` byte-exact from real ext4 on NVMe NAND. Photos: [`attempt-91-…-pt1-xhci-usb-ms-nvme-ahci.jpg`](iron-nuc-zen-photos/attempt-91-agnos-1.31.7-ext4-64bit-shell-ux-pt1-xhci-usb-ms-nvme-ahci.jpg) + [`pt2-ahci-gpt-ext4-mounted.jpg`](iron-nuc-zen-photos/attempt-91-agnos-1.31.7-ext4-64bit-shell-ux-pt2-ahci-gpt-ext4-mounted.jpg) + [`pt3-shell-cd-ls.jpg`](iron-nuc-zen-photos/attempt-91-agnos-1.31.7-ext4-64bit-shell-ux-pt3-shell-cd-ls.jpg) + [`pt4-cat-validated.jpg`](iron-nuc-zen-photos/attempt-91-agnos-1.31.7-ext4-64bit-shell-ux-pt4-cat-validated.jpg). Build trajectory across the full 1.31.x arc: 421,912 B (1.31.0 cycle-open lean) → 571,296 B (1.31.6 close) → **578,432 B (1.31.7 close)** = +156 KB / ~6,760 LOC for NVMe + AHCI + GPT + USB MS + RAM-disk + VirtIO modern + ext2/4 Phase 1-5 + cleanup hardening + shell UX. MVP gate stays green on iron (typeable shell from Attempt 68 / 1.30.9). gnoboot stays at 0.4.2. cyrius pin stayed on 6.0.1 throughout. **Next cycle: 1.32.x scope TBD per user direction.**
>
...
**Storage arc status — CLOSED 2026-05-22; 1.31.x extended FS cycle CLOSED 2026-05-22:** ~~1.31.3~~ ✅ (USB MS Phase 2.8 / Attempt 87 PASS) → ~~1.31.4~~ ✅ (RAM-disk + VirtIO modern / Attempt 88 PASS no-regression) → ~~1.31.5~~ ✅ (ext2/4 Phase 1-4 / QEMU 5/5 green + Attempt 89 PASS no-regression) → ~~1.31.6~~ ✅ (cleanup + multi-backend probe + partition-aware mount / Attempt 90 PASS — ext4 mounted from NVMe `agnos-fs` p3 on iron) → ~~1.31.7~~ ✅ (ext4 64BIT Phase 5 + shell UX `cd`/`pwd`/bare-name `cat`/`ls -la` / Attempt 91 PASS — Phase 5 BGDT stride iron-validated on user's re-carved 64BIT-enabled NVMe `agnos-fs` p3). **Next cycle: 1.32.x scope TBD per user direction.**
...
| **agnos** | **1.31.7** | **6.0.1** | **Kernel rolled through the 1.31.x storage + filesystem arc on cycc 6.0.1, extended FS cycle CLOSED 2026-05-22.** 1.31.2 (AHCI carry-forward triplet + USB MS Phase 1-4 + 2.5 + 2.6 + 2.7) → 1.31.3 (USB MS Phase 2.8 eight-bug repair stack / Attempt 87 iron PASS) → 1.31.4 (RAM-disk + VirtIO 1.x modern / Attempt 88 iron PASS no-regression) → 1.31.5 (ext2/4 Phase 1-4 / Attempt 89 iron PASS no-regression) → 1.31.6 cleanup cycle (8 bites + 2 smoke-surfaced fixes + Iron Attempt 90 ext4 victory lap PASS on NVMe `agnos-fs` p3 — see § *1.31.6 cleanup cycle*) → **1.31.7 filesystem follow-ups + shell UX CLOSED 2026-05-22** (ext4 64BIT Phase 5 + `cd`/`pwd`/CWD scoping + bare-name `cat` + `ls -la` flag dispatch + Iron Attempt 91 PASS — Phase 5 BGDT stride iron-validated on user's re-carved 64BIT-enabled `agnos-fs` p3, `agnos> cat hello.txt` returned seed content byte-exact — see § *1.31.7 cycle*). Pin stable on cycc 6.0.1 since the mid-1.31.x graduation. Build trajectory across the arc: 421,912 B (1.31.0 lean) → 571,296 B (1.31.6 close) → 578,432 B (1.31.7 close) = +156 KB / ~6,760 LOC. |
