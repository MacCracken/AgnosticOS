...
> **Last refresh**: 2026-05-14 eve (captures iron-boot Attempts 4–14 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 14 = sub-case 3b confirmed**: ELF load, multiboot2/Path-C handoff, GDT/TSS/IDT/PIC/LAPIC, timer ISR, ring-0 user procs, serial-from-user-procs, PIC EOI, and TWO context-switch round-trips all work; `sched_next()` round-robin starves proc 0 after its initial context-switch out. **Repairs for Attempt 15 landed 2026-05-14 eve**: (A) unconditional state-ready post-save in `do_context_switch`, (B) `sched_next` falls back to proc 0 (kernel idle) when no other ready proc found. Plus color-coded visual CP ladder (`fb.cyr` + 19 sites in main.cyr) so on-screen iron readout doesn't require reboot+CMOS-mount. **Closed-beta gate is CP `0x11` MAGENTA on next iron burn.** Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
...
| 8 | **Iron-boot Attempts 4–14 (NUC AMD) → Attempt 14 lands sub-case 3b; repairs (A)+(B) landed 2026-05-14 eve; Attempt 15 pending iron burn** | 🔄 Attempt 15 pending | Target: NUC AMD (archaemenid, Zen-class, primary AND user's daily-driver — every attempt costs the dev machine). Intel/Skytech queued post-AMD-proof. **Path A (GRUB MB2-EFI) abandoned** — OVMF 2024+ strict-W^X faults in `grub_relocator64_efi_boot` (writes to its own .text patching `_efi_start`'s register-state immediates); Multiboot2 + GRUB-EFI is genuinely under-tested vs Linux's `linuxefi` path. NOT a cyrius bug. **Path C (sovereign UEFI / `gnoboot`)** is MVP — `gnoboot` is a new repo emitting a UEFI application that hands off via AGNOS sovereign boot-info struct. **agnos 1.30.0** ships the kernel-side ABI break (entry contract: multiboot2 → sovereign boot-info struct). **Attempt 14 hit CP `0x23`** — confirmed in order: kernel→ring-0 user proc context switch, serial from user procs, `hlt`→timer-ISR→`iretq` round-trip, AND proc_a→proc_b switch via `sched_next`. Stall is proc_b's busy-loop never yielding back to proc 0 → **sub-case 3b: `sched_next()` round-robin starves proc 0 after its initial `save_context`**. **Repairs landed 2026-05-14 eve** (both per per-action-consent step-through): **(A)** unconditional `proc_set_state(old, 1)` post-save in `do_context_switch` (drops the `state==2` guard whose conditional read may be the unaccounted iron variable); **(B)** `sched_next()` falls back to `return 0` instead of `return proc_current` when no ready proc found (kernel-idle always wins when busy-loop tries to monopolize). Belt-and-suspenders — either alone defends sub-case 3b. Plus visual on-screen diagnostic: new `kernel/arch/x86_64/fb.cyr` with `cp_fb(idx, color)` + 19 call sites in main.cyr; color-coded ladder (YELLOW=arch, GREEN=subsystems, CYAN=sched-arm + in-hlt-loop, MAGENTA=post-scheduler) means iron Attempt 15 readout doesn't require reboot+CMOS-mount. Net kernel size delta: +368 bytes (252,112 → 252,480). Collateral: `kernel/lib/` → `kernel/klib/` rename eliminates cyrius wrapper's `./lib/` shadow-collision class. Next CP after fix takes: `0x11` MAGENTA = closed-beta gate (full 50-hlt scheduler cycle on iron). See [`iron-boot-testing-log.md`](iron-boot-testing-log.md) § "Repairs landed for Attempt 15" + Attempt-15-plan table. |
...
*Refreshed 2026-05-14 eve (header + iron-boot row spot-updated for sub-case-3b repairs landed: (A) unconditional state-ready post-save in `do_context_switch`, (B) `sched_next` falls back to proc 0 when no other ready proc found, both in `agnos/kernel/core/sched.cyr`. Plus visual on-screen CP ladder via new `kernel/arch/x86_64/fb.cyr` + 19 sites in main.cyr — YELLOW/GREEN/CYAN/MAGENTA color-coded by layer. Collateral: `kernel/lib/` → `kernel/klib/` rename eliminates cyrius wrapper's `./lib/` shadow-collision. Net kernel size delta +368 bytes (252,112 → 252,480). Iron Attempt 15 burn pending — closed-beta gate is CP `0x11` MAGENTA. Pin-lag spectrum body still reflects 2026-05-11 eve snapshot except where called out.) Rewrite-in-place as state changes. v5.10.x history captured here is closeout-context only — Cyrius CHANGELOG is the receipt.*
