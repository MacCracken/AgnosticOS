...
CYML — 6.0.x LEADING EDGE (the bulk of the ecosystem, ~32 repos):
  ...
  v6.0.53:  libro (2.7.1)
  v6.0.56:  agnos (1.41.13), agnoshi (1.4.0), kybernet (1.3.3),
            argonaut (1.8.2)
  v6.0.61:  sigil (3.7.1)
...
| **agnos** | **1.41.13** | **6.0.56** | Kernel. The 1.41.x shell-separation arc is software-complete (full ladder in the header above); pin moved 6.0.14 → 6.0.56 at 1.41.4 (`CYRIUS_TARGET_AGNOS`). Iron burns: `14111` → serial-only ring-3 stdout fix (1.41.12); `14112` → PMM `#PF` fix (1.41.12) then **reached the prompt, no typing → `read(0)` ran interrupts-masked, fixed via polled i8042 at 1.41.13** (QEMU type-test PASS, iron-pending). build 1,070,976 B → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). |
| **agnoshi** | **1.3.5** | **6.0.56** | AI shell — now the userland interactive shell exec'd from disk in ring 3 (the 1.41.x shell-separation arc). Was 1.3.3 / pin 6.0.1 in last refresh; the agnos-target build (`agnsh_agnos`) needed cyrius 6.0.56's `CYRIUS_TARGET_AGNOS` peer, so it graduated with the boot-path core. |
