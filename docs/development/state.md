...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–20 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)** ; Attempts 17–20 landed on the next-bug ladder. **Attempt 18** found CR4.SMAP=0 on iron — Path C (`#ifdef ELF64_KERNEL`) boot shim deferred CR4 tuning to a non-existent "kernel re-tune"; the legacy ELF32 shim's CPUID-gated SMEP/SMAP enable only ran under `#ifndef ELF64_KERNEL`. Without CR4.SMAP=1, `stac` was `#UD` → `#DF` → triple-fault. **Repair (F)** mirrored the CPUID-gated SMEP+SMAP enable into Path C shim post-boot_info_capture_rdi + added a second CR4 byte-2 dump at CMOS[0x55] right before stac. **Attempt 19 (2026-05-15)** confirmed Repair (F) landed cleanly ([0x54]=0x30) but the pre-bound "CR4 cleared between cr3_load and stac" interpretation of [0x55]=0x00 was wrong — the only thing in that window is cp_fb(0x18), which doesn't touch CR4. **Reinterpretation**: [0x55]=0x00 = dump never reached because cp_fb under AS1's CR3 #PFs (FB MMIO not in AS1's PT mirror). **Repair (G)** for Attempt 20 inserted an 8-byte `kcp=0x60` port-I/O disambiguator between cp_fb(0x18) and the [0x55] dump. **Attempt 20 (2026-05-15)** confirmed cp_fb-under-AS1 hung: kcp=0x18 + [0x55]=0x00 — kcp=0x60 stamp never ran. Root cause: `proc_create_address_space` (proc.cyr:154) only mirrors 0..4 GB RAM via PD copy + PDPT[1..3] 1-GB huge pages; Zen UEFI GOP framebuffer phys is above that range, so cp_fb's `store32` under AS1 → #PF → cascade → triple-fault → reset ~2-3 s later. **Repair (H) for Attempt 21 landed 2026-05-15**: drop the four under-AS1 cp_fb calls at main.cyr:479/517/522/527 (Option A — minimum-risk move to advance past CP 0x18); long-term fix B (pre-map FB phys into AS1/AS2 at construction time) intentionally deferred until mem-iso block proves clean. Kernel **254,000 → 253,936 bytes** (−64). read-boot-log gains kcp=0x60 verdict + refreshed kcp=0x18 verdict + post-Repair-F decode-hint baseline. **QEMU pre-flight still permanently blocked** → iron is the only test surface. **Next gate (Attempt 21)**: kcp advances past 0x18 to CP 0x1B/0x1C/0x1D under AS1 SMAP-bracketed access, then through AS2 round-trip, then cp_fb(0x19) MAGENTA paints after kernel-CR3 restore. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct; v1.30.1 candidate carries Path C CR4 enable (Repair F) + cp_fb-under-AS1 drop (Repair H). Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
...
*Refreshed 2026-05-14 night (header + iron-boot row spot-updated for Attempt 15 result + Repair (C) landed. Attempt 15 stalled at CP 0x23 despite repairs (A)+(B), but visual CP ladder confirmed the new kernel ran on iron — eliminated "USB stale" hypothesis. Static analysis showed repairs A+B *should* let proc 0 re-run; therefore policy isn't the bug. Investigation of pic.cyr `timer_isr_build()` revealed it only pushes 9 caller-saved GPRs despite stale comment claiming all 15 — rbx/rbp/r12-r15 leak between procs, corrupting proc 0's frame-pointer-relative locals when it finally re-runs. Repair (C) = single coupled edit (pic.cyr ISR full-15-GPR push/pop + sched.cyr save/restore extended with 6 new slot pairs + hw-frame offsets shifted +48). Kernel **252,480 → 253,712 bytes** (+1,232). `timer_isr[]` buffer at 63/64 bytes (1 byte headroom). QEMU pre-flight permanently blocked for this kernel — iron is the only test surface, every Attempt costs a reboot of the user's daily-driver. Iron Attempt 16 burn pending — closed-beta gate still CP `0x11` MAGENTA. Pin-lag spectrum body still reflects 2026-05-11 eve snapshot except where called out.) Rewrite-in-place as state changes. v5.10.x history captured here is closeout-context only — Cyrius CHANGELOG is the receipt.*
