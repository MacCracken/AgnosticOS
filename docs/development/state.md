...
>
> **Open cycle — 1.44.x userland coreutils delegation.** agnoshi **1.5.0** removed its in-process file verbs (+ `src/verbs.cyr`); agnsh is now a pure launcher. `cp`/`mv`/`rm`/`ls`/`mkdir`/`rmdir`/`touch`/`echo`/`wc`/`find`/`grep` delegate to the staged **kriya** dispatcher (11 `/bin/<verb>` → `kriya` symlinks) and `cat` to **owl**, both on the agnos-fs `/bin` (agnos `scripts/stage-tools.sh`). Kernel needs no new syscall; `symlink`#43 (kriya `ln -s`) deferred to backlog. **End-to-end QEMU-GREEN** (`agnos/scripts/agnsh-delegation-test.py`: bareword `mkdir`/`cp`/`ls` → kriya, `owl -p` cat, cat→owl nudge — all PASS). The smoke surfaced + fixed two AGNOS-only consumer bugs: **kriya 1.1.2** (cyrius pin 6.0.56 → 6.1.14 + re-vendor — the stale `fnptr.cyr` lacked the agnos fncall branch, so `vec_push` wedged `ls`) and **owl 1.3.8** (same fnptr fix + `var r = main()` → bare-call entry so `argc` is non-zero — owl was printing help). Iron-burn pending on archaemenid → [`#tracker-144x-cycle`](iron-nuc-zen-log.md#tracker-144x-cycle).
>
