...
> **Last refresh**: 2026-05-22 (post-1.31.7 release + **agnos 1.32.0 networking-arc cycle OPEN same-day** + **Iron Attempt 92 PARTIAL — r8169 Phase 1-4 lit clean on iron, DHCP silent due to `main.cyr:655` gate keying on `vnet_active` only; fix LANDED same-day: `if (vnet_active != 0 || nic_ready() != 0)` — generic abstraction on RHS so future NIC backends (i225-V queued) don't force another gate edit; build 600,520 → 601,392 B + 4/4 test.sh PASS; Attempt 93 PENDING IRON**; lean cycle-open per "track items we still need to write work for, wait until on that particular iron").
>
> **Current scope** — **1.32.x networking arc OPEN 2026-05-22** with cycle theme **kernel server-side TCP primitives + real-iron NIC drivers** (r8169 family first, i225-V queued); userland consumer apps in parallel out-of-cycle: **BBS + MUD** (early-90s revival aesthetic) as wire-end-to-end proof of life. **Cycle-open is intentionally lean** per user discipline — VERSION bump 1.31.7 → 1.32.0 + multi-source convergent prior-art audit at [`network-arc-prior-art.md`](network-arc-prior-art.md) + this state.md and roadmap row 16 tracking surface. **No code touches yet** — every bite in § *1.32.x cycle* below is marked PENDING IRON. **Existing networking baseline**: `kernel/core/net.cyr` (532 LOC) has TCP client + UDP egress + full SYN/ACK/FIN state machine + checksum; **server-side TCP (`tcp_listen`/`bind`/`accept`) is the first concrete gap** the BBS/MUD apps surface. `kernel/core/virtio_net.cyr` (148 LOC) covers QEMU; no real-iron NIC driver yet. The 1.31.x storage + filesystem arc (storage devices NVMe/AHCI/USB-MS/RAM-disk/VirtIO-blk + filesystem ext2/ext4 Phase 1-5 + GPT + multi-backend probe + partition-aware mount + shell-UX `cd`/`pwd`/CWD/bare-name `cat`/`ls -la`) closed at 1.31.7 (Iron Attempt 91 PASS — see § *1.31.7 cycle* below for receipt); MVP gate stays green on iron from Attempt 68 / 1.30.9; gnoboot stays at 0.4.2; cyrius pin stays on 6.0.1.
>
...
> - **1.32.0 OPEN 2026-05-22** — **Networking arc cycle-open** (lean: VERSION bump + multi-source prior-art audit, no code touches). Theme: kernel server-side TCP primitives + real-iron NIC drivers (r8169 family first, i225-V queued); userland consumers in parallel out-of-cycle = **BBS + MUD apps** (early-90s revival aesthetic). New audit doc at [`network-arc-prior-art.md`](network-arc-prior-art.md) (multi-source convergent per `feedback_redesign_dont_reinvent`: Linux + FreeBSD + OpenBSD + NetBSD + Haiku + RFC 854 Telnet + datasheets). Cycle-open per user discipline "track items we still need to write work for, wait until on that particular iron" — every bite marked PENDING IRON. Existing baseline: `net.cyr` 532 LOC has TCP client + UDP egress; server-side TCP (`tcp_listen`/`bind`/`accept`) is the first concrete gap. See § *1.32.x cycle* below.
>
...
### 1.32.x cycle (OPEN 2026-05-22 — networking arc cycle-open, lean: VERSION bump + audit doc, no code touches yet)
...
| D | **Iron debut burn** — first AGNOS LAN packet on archaemenid (r8169 driver Phases 1-4 + retroactive validation of bites A/F/G). **Pre-burn audit landed** at [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) (9 hypotheses ranked; H1 = PHY-not-configured is highest-risk; success rubric defined; iron-burn checklist for user). | **Iron-only** (Attempt 92 burned 2026-05-22). | ~50 LOC test surface (already in driver; no new code) + ~200 LOC audit doc | ⚠️ **Attempt 92 PARTIAL** — r8169 Phases 1-4 all six lines lit clean on archaemenid (`found at 4243603456` = BAR2 0xFCF04000 byte-match lspci; `MAC=176:65:111:12:228:37` = `b0:41:6f:0c:e4:25` byte-match lspci; `chip-rev byte=0x87`; `reset OK; Phase 1 complete`; `RX ring up (16 desc)`; `TX ring up (16 desc)`). Storage trio byte-matched Attempt 91 — clean no-regression. **DHCP bite G silent on iron** — root cause: `agnos/kernel/core/main.cyr:655` gates `dhcp_init()` on `if (vnet_active != 0)` (virtio-net flag only). On iron `vnet_active == 0` permanently (set inside `virtio_net_init`, never reached on archaemenid), so gate never fires `dhcp_init()` → no DISCOVER egress → no `dhcp:` lines. NOT a wire-level / PHY / RX-walk issue (H1/H7/H8 audit hypotheses all blocked upstream by the gate). Fix LANDED 2026-05-22 per user direction: explicit OR (not hard predicate-swap) AND generic abstraction on RHS (not NIC-specific coupling like `r8169_present` — would have forced another gate edit when i225-V or any other NIC backend lands) — `if (vnet_active != 0 || nic_ready() != 0)` so both backends are visible at the call site and the right-hand `nic_ready()` extends in-place as new NICs join the abstraction in `r8169.cyr:425-431`. `dhcp_init`'s internal `nic_ready()` guard at `net.cyr:287` still catches the transient "PCI-discovered but rings not yet up" window. Build 600,520 → 601,392 B + 4/4 test.sh PASS. **Attempt 93 PENDING IRON** for validation of the corrected gate against archaemenid r8169. See iron-nuc-zen-log § Attempt 92. |
| E | **Cycle-close sweep + Attempt 92 transcript** — state/roadmap/iron-log sweep + photo capture. | n/a (doc-roll). | n/a | ✅ **LANDED 2026-05-22** — two photos catalogued (`attempt-92-…-dhcp-silent-pt1`/`pt2.jpg`); Attempt 92 receipt in iron-nuc-zen-log with verbatim chain, root-cause audit, proposed fix; state.md refresh marker + this row + bite D updated. **Follow-up bite F (DHCP gate fix + Attempt 93 re-burn) pending user direction.** |
...
Build delta at cycle-open: zero structural change to `build/agnos` (only `version_str` literals shifted, within byte-counter precision). cyrius pin stays on 6.0.1. gnoboot stays at 0.4.2.
...
**Storage + FS arc status — CLOSED 2026-05-22; 1.32.x networking arc OPEN 2026-05-22:** ~~1.31.3~~ ✅ (USB MS Phase 2.8 / Attempt 87 PASS) → ~~1.31.4~~ ✅ (RAM-disk + VirtIO modern / Attempt 88 PASS no-regression) → ~~1.31.5~~ ✅ (ext2/4 Phase 1-4 / QEMU 5/5 green + Attempt 89 PASS no-regression) → ~~1.31.6~~ ✅ (cleanup + multi-backend probe + partition-aware mount / Attempt 90 PASS — ext4 mounted from NVMe `agnos-fs` p3 on iron) → ~~1.31.7~~ ✅ (ext4 64BIT Phase 5 + shell UX `cd`/`pwd`/bare-name `cat`/`ls -la` / Attempt 91 PASS — Phase 5 BGDT stride iron-validated on user's re-carved 64BIT-enabled NVMe `agnos-fs` p3) → **1.32.0 OPEN (networking arc cycle-open, lean: VERSION bump + audit doc; bites A-E PENDING IRON per user discipline)**.
...
| **agnos** | **1.32.0** | **6.0.1** | **1.31.x storage + FS arc CLOSED 2026-05-22 / 1.32.x networking arc OPEN 2026-05-22.** 1.31.2 (AHCI carry-forward triplet + USB MS Phase 1-4 + 2.5 + 2.6 + 2.7) → 1.31.3 (USB MS Phase 2.8 eight-bug repair stack / Attempt 87 iron PASS) → 1.31.4 (RAM-disk + VirtIO 1.x modern / Attempt 88 iron PASS no-regression) → 1.31.5 (ext2/4 Phase 1-4 / Attempt 89 iron PASS no-regression) → 1.31.6 cleanup cycle (8 bites + 2 smoke-surfaced fixes + Iron Attempt 90 ext4 victory lap PASS on NVMe `agnos-fs` p3 — see § *1.31.6 cleanup cycle*) → **1.31.7 filesystem follow-ups + shell UX CLOSED 2026-05-22** (ext4 64BIT Phase 5 + `cd`/`pwd`/CWD scoping + bare-name `cat` + `ls -la` flag dispatch + Iron Attempt 91 PASS — Phase 5 BGDT stride iron-validated on user's re-carved 64BIT-enabled `agnos-fs` p3, `agnos> cat hello.txt` returned seed content byte-exact — see § *1.31.7 cycle*) → **1.32.0 OPEN** networking arc cycle-open: lean (VERSION bump + multi-source prior-art audit at `network-arc-prior-art.md`, no code touches). Theme: kernel server-side TCP primitives (`tcp_listen`/`bind`/`accept`; first concrete net.cyr gap) + r8169-family NIC driver (presumed on archaemenid) + i225-V driver queued; userland consumers BBS + MUD in parallel out-of-cycle. Strategic destination: AGNOS installable-state on archaemenid → user's planned **dual-boot migration** (refined 2026-05-22): 1TB ex-USB drive becomes Beelink internal NVMe + hosts AGNOS-primary; WD Blue SA510 SATA stays in chassis as Linux daily-driver root; 2TB Crucial P3 NVMe migrates to i9 (joining existing 1TB Gen5 NVMe + 2TB Windows-holdout drive). See full migration shape in § *archaemenid migration* above and [[project_archaemenid_install_plan]]. Multi-cycle path: 1.32.x networking → 1.33.x ext4 WRITE → installable-state cycle slot TBD. See § *1.32.x cycle*. Pin stable on cycc 6.0.1. Build trajectory across the 1.31.x arc: 421,912 B (1.31.0 lean) → 571,296 B (1.31.6 close) → 578,432 B (1.31.7 close) = +156 KB / ~6,760 LOC; 1.32.0 cycle-open zero-structural-delta (only `version_str` literals shifted). |
