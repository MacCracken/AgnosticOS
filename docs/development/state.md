...
> **Last refresh**: 2026-05-26. **agnos kernel: 1.34.3 RELEASED** · gnoboot 0.4.2. Active arc = the **1.34.x FAT-family write-completeness continuation** (FS roadmap row 21). Done + `fsck`-clean in QEMU: **1.34.0** FAT12/16/32 read+write, **1.34.1** exFAT read+write, **1.34.2** exFAT write parity, **1.34.3** FAT LFN/truncate completeness (LFN-with-content + LFN-name overwrite-match + grow-truncate) — all RELEASED; production build **783,240 B**. **1.34.4** (directory growth — root extension + cross-boundary dir-set append, exFAT + FAT via a spanning append) feature-complete + QEMU/`fsck`-validated, under CHANGELOG `[Unreleased]` awaiting cut. Queued: **1.34.5** exFAT Unicode names (last 1.34.x item); arc cap = ESP-write guard + user-driven FAT/exFAT iron burn. **NEXT = cut 1.34.4 (user-driven), then 1.34.5.** Plan → [`#tracker-1341-cycle`](iron-nuc-zen-log.md#tracker-1341-cycle) + agnos roadmap row 21; per-cut detail → CHANGELOG `[1.34.0]`–`[1.34.3]`.
>
