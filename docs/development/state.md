...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–24 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)**; Attempts 17–24 worked the next-bug ladder. **Attempt 22 (2026-05-15)** result: kcp=0x68 — second AS1 round-trip clean, kernel-CR3 restore at main.cyr:629-631 (`mov rax, 0x1000 / mov cr3, rax`) #PFs. **Attempt 23 (2026-05-15)** with Repair (J) PMM stamps: all 12 slots [0x56..0x61] read 0xaf (phys ≈ 11 MB, well above 2-MB watermark) → **PMM ruled out**. Suspect #1 (PMM hands out kernel-PT page → `memset` zeros PML4 in place) falsified. **Repair (K) for Attempt 24 landed 2026-05-15**: 7 PML4-health CMOS stamps at slots [0x62..0x68] across mem-iso, each reading byte 0 of phys 0x1000. **Attempt 24 (2026-05-15)** result: kcp=0x68, all 7 PML4 stamps read **0x04** — third (unenumerated) bucket: persistent flag-tear value present at the very first stamp ("pre-AS work"), meaning **PML4[0] was wrong from boot, not corrupted during mem-iso**. Bug pinned to `paging.cyr:pt_init`'s OR-in-US pattern: `var pml4e = load64(0x1000); store64(0x1000, pml4e | 0x04)` (and matching PDPT[0] write). Pattern was correct under Path A (boot_shim pre-seeded `[0x1000]=0x2003`) but Path C / gnoboot inherits UEFI's identity map and skips boot_shim's PT setup — phys 0x1000/0x2000 stay cold, the OR yields `0 | 0x04 = 0x04` (US set, P=0, no PDPT address), `mov cr3, 0x1000` walks PML4[0] with P=0 → #PF. **Repair (L) landed 2026-05-15**: 4-line replacement in `paging.cyr:30-33` — explicit `store64(0x1000, 0x2007); store64(0x2000, 0x3007)` instead of OR-into-existing. Self-sufficient under both boot paths. Kernel rebuilds clean: **254,848 → 254,800 bytes** (-48; two literal stores compile smaller than load+OR+store). **Next gate (Attempt 25)**: visual CP 0x19 MAGENTA paint + kcp ≥ 0x69 = Repair (L) sufficient, mem-iso block complete, ladder advances to userland-exec / kybernet sites at CP 0x14 / 0x15. **QEMU pre-flight still permanently blocked** → iron is the only test surface. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break (multiboot2 → sovereign boot-info struct); v1.30.1 candidate carries Repair F + H + I + J + K + L. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
