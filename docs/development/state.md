...
> **Last refresh**: 2026-05-29. **agnos kernel: 1.40.8** · gnoboot 0.4.2. **The 1.40.x exec-from-disk arc is functionally COMPLETE** (incl. multi-`run` + argv) (closes on the user-driven combined VFS+exec iron burn). **The 1.39.x VFS lift + 1.40.x exec = base-maturity's two exit legs (FS-crash-safe + exec-from-disk) — both QEMU-validated; one iron burn away from base-stage close.** (the second base-maturity exit leg: base = FS-crash-safe ✓ **+ exec-from-disk**). The 1.39.x VFS generic-write lift is functionally COMPLETE (verb surface + subdirs; its iron burn rides with the exec burn at end of 1.40.x). 1.37–1.38 big-write arcs closed + iron-validated. Arc ladder since the last refresh:
