...
> **Last refresh**: 2026-05-23 (**1.32.3 OPEN — virtio-net modern rewrite + r8169 RX-path audit + fix all landed; QEMU DHCP works END-TO-END for the first time; r8169 fix iron-burn-validation PENDING (Attempt 97 not yet authorized)**). Highlights:
...
> - **Iron Attempt 96 ground-truth correction**: branch (a1) wire/server FALSIFIED on archaemenid same-day — pulled `ip -br link` from the current Linux session and confirmed enp1s0 (r8169 chip, MAC b0:41:6f:0c:e4:25) is actively leased at 192.168.1.124/24 from 192.168.1.1. Wire + server + chip all confirmed working under Linux. Iron OFFER timeout MUST be a code bug — branch (a2-r8169-RX).
> - **`agnosticos/docs/development/r8169-rx-path-audit.md`** — multi-source convergent audit (Linux v6.6 rtl_rx + OpenBSD re_rxeof + FreeBSD re_rxeof + RTL8168 datasheet). Verdict: LOAD-BEARING bug is `r8169_poll` returning after a single descriptor check. CMOS 0x5E=0x01 was an IPv4 multicast (`01:00:5e:…`) first byte, not the OFFER's `0xb0` unicast — chip is healthy, we just never look at the right slot.
> - **`agnos/kernel/core/r8169.cyr` 5-part bundle** (~50 LOC net): Part A multi-frame budget loop in r8169_poll; Part B `RES` (0x00200000) + `FS|LS` (0x30000000) gating constants; Part C `r8169_rx_rearm` helper with EOR read-preserve mirroring Linux `rtl8169_mark_to_asic`; **Part D CMOS-stamp hot-path elimination** (audit S-1, removes ~8 µs per-poll tax — stamps now fire only on state transitions); **Part E RxMaxSize aligned with Linux convergent** (audit S-2, 0x05F3 → 0x4000 effective-disable). Stacked into one bite so Attempt 97 validates the full bundle (no piecemeal iron-burn ladder, per [[feedback_iron_burns_block_other_work]] + the [[project_agnos_empire_defense_layers]] sweep-hardening pattern). Build 616,744 → **617,000 B (+256 B net)**. `test.sh` 4/4 + `ext2-smoke.sh` 5/5 + tcp-listen-smoke QEMU DHCP cycle all clean — zero regression.
> - **Legacy virtio-net** still deferred to 1.34.x per user direction "switch to modern now."
> - **Iron Attempt 97** USER-AUTHORIZED 2026-05-23 ~14:10 PDT. **Pre-burn prep landed at iron-nuc-zen-log.md § Attempt 97 prep** with full PASS / PARTIAL / FALSIFIED rubric per [[feedback_iron_log_tracker_pattern]]. Build verified fresh: `build/agnos` 617,000 B at 12:44:03 PDT, `r8169.cyr` mtime 12:44:01, version-files at 12:01:21 (1.32.3 banner), agnos commit `a065f45` post-bundle clean. Pre-burn checklist: `sudo ./scripts/install-usb.sh --update` → boot archaemenid → photo at `agnos>` shell → `sudo ./scripts/read-boot-log.sh --verbose` from Linux. Expected on PASS: `dhcp: DISCOVER → OFFER ip=192.168.1.X → REQUEST → ACK gw=192.168.1.1 mask=255.255.255.0`; CMOS 0x5A flips 0x02 → 0x03+, 0x5E flips 0x01 → 0xb0 (unicast OFFER) or 0xff (broadcast OFFER), 0x5C reflects state-transition consumed-frames semantics post-Part-D.
> - **`agnosticos` ground-truth confirmed today**: `ip -br link` on archaemenid Linux session shows `enp1s0 UP b0:41:6f:0c:e4:25 192.168.1.124/24 default via 192.168.1.1 proto dhcp` — same r8169 chip, same wire, same MAC currently leased by Linux. Branch (a1) wire/server falsified ground-truth; Attempt 97 isolates branch (a2-r8169-RX) by changing only the AGNOS-side RX-path code.
> - MVP gate green on iron since Attempt 68 / 1.30.9; will stay green at Attempt 97 (boot-to-shell is independent of NIC).
>
