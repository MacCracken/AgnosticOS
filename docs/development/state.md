...
>
> **Last refresh**: 2026-07-06. **Kernel head: agnos 1.53.5** (HDMI-audio arc cut) — the single tracked dev head; per-cut detail lives in the agnos CHANGELOG + [`agnos/docs/development/state.md`](https://github.com/MacCracken/agnos/blob/main/docs/development/state.md), **NOT here** (this file is current-truth + pointers, per the ⚠-banner above). **▷ HDMI-audio arc (`04:00.1`) — BACKLOGGED behind GPU work (2026-07-06):** bites 1-3 (multi-instance HDA driver + 2nd-controller probe/enum + unified HDMI/DP digital route) are cut **1.53.5** and IRON-PROVEN to the codec (probe/enumerate/route/DigEn/stream all fire on real AMD HDMI silicon), but audio **egress is GPU-gated** — even broadcasting the tone to ALL four live digital pins was silent, because the AMD HDMI/DP link needs amdgpu (DCN mode + ACR/CTS timing) which agnos has none of. The sovereign HDA driver is correct; HDMI audio resumes when GPU work lands. Bite 4 (analog↔HDMI sink-select) is moot until egress works; the (working, iron-validated) analog path ships. The `AUDIO_AGNOS_TARGET` latency trim (256→~60-90 ms) is app-side (cyrius-doom); kernel already latency-ready. → agnos CHANGELOG `[1.53.5]`. **Just-closed + iron-validated** (detail → CHANGELOG): **1.53.x FP/SIMD** (2026-07-06 — real f64 + naad DSP in ring 3 on real Zen, `fpex`→84 / `naadex`→88) · **1.52.x audio** (2026-07-05 — DOOM-with-sound out the archaemenid front jack; the iron echo was an uncalibrated LAPIC timebase, fixed 1.52.8 boot PIT-ch0 calibration; cyrius `vani` agnos backend).
>
> **✅ owl→sit agnos port UNBLOCKED (2026-07-06):** the "sit needs `chdir`, agnos has none by design" blocker was OUR port debt, not a wall ([[feedback_genuine_blocker_taxonomy]]). Fixed by making **sit fully repo-root-relative** — a single `_repo_root`/`sit_abs()` prefix threaded through its FS-helper layer, all 3 `chdir`s removed (Linux byte-identical: 273 unit + 58 integration + fuzz all green). **`owl --agnos` now builds with the VCS gutter LIVE** — walks up from the file's abs path to the repo root, sit reads resolve absolutely (owl `src/vcs.cyr`; Linux path unchanged). **Tail:** runtime QEMU validation of the gutter on agnos + owl cyrius-pin bump (6.2.37→6.4.x, clears stale-lib warnings) + a sit release/tag (owl uses a local `path=../sit` dep meanwhile). sit + owl uncommitted (user's git).
>
> **Open carry-forward (parked):** thorough per-project net-tool validation (`yo`/`dig`/`whirl`/`taar`) deferred — they already run at decent capacity on iron.
>
