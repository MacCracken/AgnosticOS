...
> **Last refresh**: 2026-05-22 (**1.32.1 CLOSED 2026-05-22 — TAGGED at HEAD post-6-FIX DHCP repair; Attempt 95 iron-validation DEFERRED to a future cycle**). Per user direction *"all still in 1.32.1 cycle work, most likely close after burn either way"* + *"tag was going to happen regardless of result"* — the 6-FIX DHCP wiring repair landed in code (commits `b12e25a` audit/instrumentation/PHY init, `d061508` repairs+closeout, `3513a16` format fix); tag `1.32.1` written by user without an Attempt 95 burn. Cycle close shape: **audit-driven repair, iron-validation deferred** — Attempt 95 becomes the FIRST burn of whatever cycle opens next, validating the 1.32.1 repairs retroactively against the Attempt 94 evidence base. Build trajectory: 601,392 B (1.32.0 close) → 603,784 B (post-bites-A+B+C, Attempt 94) → **604,096 B production / 604,904 B TCP_LISTEN_SMOKE (1.32.1 close, post-6-FIX)**. `scripts/test.sh` 4/4 + `ext2-smoke.sh` 5/5 + 5/5 regression + `tcp-listen-smoke.sh` 1/2 (pre-existing iron-only SLIRP-RX gap). Storage trio + GPT + ext4 mount + scheduler + tcp_listen + kybernet + shell all byte-clean. Carry-forward continues for i225-V driver bite C (Intel iron pending) + BBS/MUD userland (out-of-cycle) + **Attempt 95 burn of the 1.32.1 repairs** (now first-burn-of-next-cycle scope, no cycle open until user direction).
>
> **Current scope** — **1.32.1 CLOSED 2026-05-22, TAGGED by user**. No cycle currently open; awaiting user direction. Attempt 95 is the natural first-burn target whenever the next cycle opens (validates the 1.32.1 6-FIX DHCP repair on iron — see § *1.32.1 cycle* below for the expected outcome rubric). **Cycle bites** (see § *1.32.1 cycle* below): bite A = OFFER-timeout audit (§5b extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) — line-by-line H1/H7/H8 vs current `r8169.cyr` + multi-source prior art); bite B = discriminator instrumentation (CMOS-bank stamps at `r8169_send` entry/exit + `r8169_poll` invocation count + post-DISCOVER TX OWN-bit readback + post-timeout RX ring head/tail); bite C = driver-level corrective patches per audit findings (PHY init most likely candidate per Linux's `__rtl8169_set_phy_reg_bit` sequence); bite D = Iron Attempt 94+ re-burn (target outcome: full DHCP cycle on archaemenid); bite E = cycle-close sweep + Attempt 94+ transcript. **Out-of-cycle carry-forward continues**: i225-V driver bite C (Intel-NIC iron pending, post-archaemenid-migration); BBS + MUD userland (separate standalone repos). The 1.31.x storage + filesystem arc (storage devices NVMe/AHCI/USB-MS/RAM-disk/VirtIO-blk + filesystem ext2/ext4 Phase 1-5 + GPT + multi-backend probe + partition-aware mount + shell-UX `cd`/`pwd`/CWD/bare-name `cat`/`ls -la`) closed at 1.31.7 (Iron Attempt 91 PASS); **1.32.0 networking arc closed at 1.32.0** (TCP server primitives + UDP server primitives + DHCP client + r8169 Phases 1-4 + DHCP gate fix; Attempts 92 + 93 PARTIAL — see § *1.32.0 cycle* below for full receipt). MVP gate stays green on iron from Attempt 68 / 1.30.9 and **still green at Attempt 94**; gnoboot stays at 0.4.2; cyrius pin stays on 6.0.1.
>
