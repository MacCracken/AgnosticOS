...
> **Last refresh**: 2026-05-22 (post-1.31.6 close + **Iron Attempt 90 PASS — ext4 victory lap on NVMe `agnos-fs` partition; closes 1.31.x storage arc**; **agnos 1.31.7 filesystem-follow-ups + shell-UX cycle OPEN same-day**).
>
> **Current scope** — **1.31.x storage arc CLOSED 2026-05-22 with Iron Attempt 90 PASS; 1.31.7 OPEN same-day** — five-bite filesystem-follow-ups + shell-UX cycle, see § *1.31.7 cycle* below. Five iron debuts across the cycle (NVMe @ Attempt 80 / SATA @ 81 / USB MS @ 87 / RAM-disk+VirtIO @ 88 / ext4 @ 90) plus four no-regression burns (82/88/89/90's storage-trio check). **Filesystem layer iron-validated**: `kernel/core/ext2.cyr` ~970 LOC Phase 1-4 + cleanup-cycle hardening (superblock + BGDT + inode + direct-block / single-double-triple-indirect tree / dirent walk + path resolution + `VFS_EXT2_FILE` FD / ext4 extents header + leaf walker / multi-backend probe / partition-aware mount via GPT consumption / input validation sweep), QEMU 5/5 green on both ext2 and ext4 images, **iron Attempt 90 mounted ext4 from real NVMe partition with `blocksize=4096 inode_size=256 inodes_per_group=8192`** and walked `ls /` against real ext4 dirent table on iron NAND. Photos: [`attempt-90-…-pt1-xhci-usb-ms-nvme.jpg`](iron-nuc-zen-photos/attempt-90-agnos-1.31.6-ext4-victory-lap-pt1-xhci-usb-ms-nvme.jpg) + [`pt2-ahci-gpt-ext4-mounted.jpg`](iron-nuc-zen-photos/attempt-90-agnos-1.31.6-ext4-victory-lap-pt2-ahci-gpt-ext4-mounted.jpg) + [`pt3-shell-ls-cat-help.jpg`](iron-nuc-zen-photos/attempt-90-agnos-1.31.6-ext4-victory-lap-pt3-shell-ls-cat-help.jpg). Build trajectory across the storage arc: 421,912 B (1.31.0 cycle-open lean) → 571,296 B (1.31.6 close) = +149 KB / ~6,500 LOC for NVMe + AHCI + GPT + USB MS + RAM-disk + VirtIO modern + ext2/4 + cleanup hardening. MVP gate stays green on iron (typeable shell from Attempt 68 / 1.30.9). gnoboot stays at 0.4.2. **Next cycle: 1.31.7 filesystem follow-ups + shell-UX (OPEN same day as 1.31.6 close, 4/5 code bites landed, Attempt 91 PENDING).**
>
...
> - **1.31.7 OPEN 2026-05-22 (4/5 bites landed)** — Filesystem follow-ups + shell-UX cycle. ✅ (D) `ls -la` flag-aware dispatch (~25 LOC), ✅ (B) bare-name `cat` ext2 fall-through (~20 LOC), ✅ (C) `cd` + `pwd` + CWD scoping (~190 LOC), ✅ (A) ext4 64BIT Phase 5 (~25 LOC + audit doc + smoke 5). Bite (E) sweep landed; **Attempt 91 PENDING IRON BURN** — full storage trio re-validation + ext4 mount unchanged + new shell verbs (`cd` / `pwd` / `ls -la`) exercised on archaemenid. Build 571,296 → 578,432 B (+7,136 B). `scripts/test.sh` 4/4 + `scripts/ext2-smoke.sh` 5/5 (smoke 5 = new 64BIT-flagged ext4) + 5/5 regression cross-check.
>
...
### 1.31.7 cycle (OPEN 2026-05-22 — filesystem follow-ups + shell UX; 4 of 5 bites landed, awaiting iron Attempt 91)

Closes the ext4 64BIT pin from row 7b of the agnos roadmap + tightens the Phase-3 shell-UX papercuts surfaced in Attempt 90's transcript (bare-name `cat hello.txt` falling to initrd; `ls -la` failing with `ls: not found`). Five bites in smallest-first order to bank wins quickly; one cycle-close iron burn (Attempt 91) for no-regression validation. No new iron-validation surface — [`ext2-iron-burn-audit.md`](ext2-iron-burn-audit.md) (bite E of 1.31.6) covers Attempt 91 as well.

| # | Bite | Status | Notes |
|---|------|--------|-------|
| D | `ls -la` long-form dispatch (~25 LOC net) | ✅ landed | `sh_cmd_ls` parses flag tokens before the path; Phase-3 listing format unchanged (flags accepted syntactically). Same-touch cleanup: removed dead duplicate `ls`/`fatfs_ls` handler from the dispatcher. Build 571,296 → 571,728 B. |
| B | bare-name `cat` ext2 fall-through (~20 LOC net) | ✅ landed | `sh_cmd_cat` now prefixes bare names with `/` (later: `sh_cwd_path`) and tries ext2 before falling back to initrd. Build 571,728 → 572,080 B. |
| C | `cd` + `pwd` + CWD scoping (~190 LOC net) | ✅ landed | New globals (`sh_cwd_inode`/`sh_cwd_path`/`sh_cwd_len`) + helpers (`sh_cwd_init`, `sh_cwd_parent`, `sh_is_dir`) + verbs (`sh_cmd_cd` with `..`/`.`/`/`/absolute/relative + type-check; `sh_cmd_pwd`). `sh_cmd_cat` + `sh_cmd_ls` rewired to consume CWD. Multi-component relative paths (`cd a/b/c`) work transparently via existing path walker. Build 572,080 → 577,776 B. |
| A | ext4 64BIT support (Phase 5, ~25 LOC + new audit doc + smoke-5) | ✅ landed | Closes row 7b. New `ext2_desc_size` module global + `ext2_sb_desc_size` accessor + `s_desc_size` parse in `ext2_init` (gated on rev≥1 + 64BIT bit) + supported-incompat mask 0x6746 → 0x67C6 + `bg_inode_table_hi` guard in `ext2_get_inode` (refuses with "Phase 6 unlock" if non-zero). Multi-source prior-art doc landed at [`ext4-64bit-prior-art.md`](ext4-64bit-prior-art.md). Build 577,776 → 578,432 B. |
| E | cycle-close sweep + Attempt 91 no-regression burn | **in progress** (sweep ✅, iron PENDING) | `scripts/ext2-smoke.sh` extended 4/4 → 5/5 (new `build_esp_plus_ext4_64bit_partition` builder + smoke 5; all five PASS + 5/5 regression cross-check). state.md / roadmap / iron-log sweep landed. Attempt 91 PENDING IRON BURN — full storage-trio re-validation + ext4 mount unchanged + new shell verbs (`cd` / `pwd` / `ls -la`) exercised on archaemenid. |

Build trajectory across the cycle: 571,296 B (1.31.6 close) → **578,432 B** (post-bite-A, +7,136 B / ~260 LOC net). All four code-bite milestones run `scripts/test.sh` 4/4 PASS and `scripts/ext2-smoke.sh` 5/5 PASS + 5/5 regression cross-check.
...
**Storage arc status — CLOSED 2026-05-22:** ~~1.31.3~~ ✅ (USB MS Phase 2.8 / Attempt 87 PASS) → ~~1.31.4~~ ✅ (RAM-disk + VirtIO modern / Attempt 88 PASS no-regression) → ~~1.31.5~~ ✅ (ext2/4 Phase 1-4 / QEMU 5/5 green + Attempt 89 PASS no-regression) → ~~1.31.6~~ ✅ (cleanup + multi-backend probe + partition-aware mount / Attempt 90 PASS — ext4 mounted from NVMe `agnos-fs` p3 on iron). Next cycle theme TBD per user direction.
...
CYML format — LEADING-EDGE 6.0.x cluster (post-v5.11.x graduations):
  v6.0.1:  agnos (1.31.6), agnoshi (1.3.3), mihi (1.0.0), iam (1.0.0),
...
- **6.0.1 leading-edge cluster** (2026-05-20 → -21, 8 repos): **agnos (1.31.6)** + **agnoshi (1.3.3)** + mihi (1.0.0), iam (1.0.0), chakshu (0.6.0), bannermanor (1.0.0), darshana (0.3.5), hapi (0.5.0). The morning brought sys-info substrate (mihi/iam/chakshu) and afternoon brought terminal-aesthetics (bannermanor / hapi / darshana). The 2026-05-20 evening MVP-path graduation pair (agnos + agnoshi) onto cycc 6.0.1 was the load-bearing change; agnos has since rolled through the storage arc and ext2/4 cycle on the same pin: 1.31.2 → 1.31.3 (USB MS Phase 2.8) → 1.31.4 (RAM-disk + VirtIO modern) → 1.31.5 (ext2/4 Phase 1-4) → **1.31.6** (cleanup-cycle open). The darshana 0.3.0 → 0.3.5 bump was demand-driven (bannermanor needed color escape sequences) — same shared-lib-evolves-to-second-consumer pattern as mihi's cyim → chakshu extraction.
...
| **agnos** | **1.31.6** | **6.0.1** | **Kernel rolled through the 1.31.x storage + filesystem arc on cycc 6.0.1, arc CLOSED 2026-05-22.** 1.31.2 (AHCI carry-forward triplet + USB MS Phase 1-4 + 2.5 + 2.6 + 2.7) → 1.31.3 (USB MS Phase 2.8 eight-bug repair stack / Attempt 87 iron PASS) → 1.31.4 (RAM-disk + VirtIO 1.x modern / Attempt 88 iron PASS no-regression) → 1.31.5 (ext2/4 Phase 1-4 / Attempt 89 iron PASS no-regression) → **1.31.6 cleanup cycle CLOSED 2026-05-22** (8 bites + 2 smoke-surfaced fixes + Iron Attempt 90 ext4 victory lap PASS on NVMe `agnos-fs` p3 — see § *1.31.6 cleanup cycle*). Pin stable on cycc 6.0.1 since the mid-1.31.x graduation. Build trajectory across the storage arc: 421,912 B (1.31.0 lean) → 571,296 B (1.31.6 close) = +149 KB / ~6,500 LOC. |
