...
> **Last refresh**: 2026-05-22 (1.32.0 tagged + **agnos 1.32.1 networking-arc-continued cycle OPEN same-day, lean cycle-open**: VERSION bump 1.32.0 → 1.32.1 via canonical `scripts/version-bump.sh` + CHANGELOG `[1.32.1]` header with cycle scope + this state.md and roadmap.md tracking surface. **No code touches yet** — every bite in § *1.32.1 cycle* below is marked PENDING IRON. Cycle theme: **r8169 driver-level OFFER-timeout debug** (H1 PHY-not-configured / H7 TX OWN stuck / H8 RX OWN stuck — the now-reachable audit hypothesis surface from Attempt 93). Opening move: audit-doc extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) § 5b per [[feedback_iron_burns_block_other_work]] + [[feedback_redesign_dont_reinvent]]; convergent multi-source against current `r8169.cyr` (Linux `r8169_main.c` + FreeBSD `if_re.c` + OpenBSD `re.c` + NetBSD `re.c` + Haiku + RTL8168 datasheet). Carry-forward continues for i225-V driver bite C (Intel iron pending) + BBS/MUD userland (out-of-cycle).
>
> **Current scope** — **1.32.1 OPEN 2026-05-22** (networking-arc continuation; cycle theme: **r8169 driver-level OFFER-timeout debug**). Cycle-open is **intentionally lean** per user discipline — VERSION bump 1.32.0 → 1.32.1 via canonical `scripts/version-bump.sh` + CHANGELOG `[1.32.1]` header + this state.md and roadmap.md tracking surface. **No code touches yet** — every bite below is PENDING IRON. **Cycle bites** (see § *1.32.1 cycle* below): bite A = OFFER-timeout audit (§5b extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) — line-by-line H1/H7/H8 vs current `r8169.cyr` + multi-source prior art); bite B = discriminator instrumentation (CMOS-bank stamps at `r8169_send` entry/exit + `r8169_poll` invocation count + post-DISCOVER TX OWN-bit readback + post-timeout RX ring head/tail); bite C = driver-level corrective patches per audit findings (PHY init most likely candidate per Linux's `__rtl8169_set_phy_reg_bit` sequence); bite D = Iron Attempt 94+ re-burn (target outcome: full DHCP cycle on archaemenid); bite E = cycle-close sweep + Attempt 94+ transcript. **Out-of-cycle carry-forward continues**: i225-V driver bite C (Intel-NIC iron pending, post-archaemenid-migration); BBS + MUD userland (separate standalone repos). The 1.31.x storage + filesystem arc (storage devices NVMe/AHCI/USB-MS/RAM-disk/VirtIO-blk + filesystem ext2/ext4 Phase 1-5 + GPT + multi-backend probe + partition-aware mount + shell-UX `cd`/`pwd`/CWD/bare-name `cat`/`ls -la`) closed at 1.31.7 (Iron Attempt 91 PASS); **1.32.0 networking arc closed at 1.32.0** (TCP server primitives + UDP server primitives + DHCP client + r8169 Phases 1-4 + DHCP gate fix; Attempts 92 + 93 PARTIAL — see § *1.32.0 cycle* below for full receipt). MVP gate stays green on iron from Attempt 68 / 1.30.9 and **still green at Attempt 93**; gnoboot stays at 0.4.2; cyrius pin stays on 6.0.1.
>
...
> - **1.32.1 OPEN 2026-05-22** — **Networking-arc-continued cycle-open** (lean: VERSION bump 1.32.0 → 1.32.1 via canonical `scripts/version-bump.sh` + CHANGELOG `[1.32.1]` header + state.md/roadmap.md tracking surface, no code touches yet). Theme: r8169 driver-level OFFER-timeout debug — H1 PHY-not-configured / H7 TX OWN stuck / H8 RX OWN stuck (the audit hypothesis surface from Attempt 93, now reachable post-gate-fix). Opening move: §5b extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md) (line-by-line H1/H7/H8 vs current `r8169.cyr` + multi-source prior art) per [[feedback_iron_burns_block_other_work]] + [[feedback_redesign_dont_reinvent]]. Every bite PENDING IRON until audit lands. See § *1.32.1 cycle* below.
>
...
| D | **Iron Attempt 94+** — re-burn with bite B+C bundle. Target outcome: full DHCP cycle on archaemenid (`dhcp: DISCOVER` → `dhcp: OFFER ip=<lan-IP>` → `dhcp: REQUEST` → `dhcp: ACK ip=<lan-IP>` four lines in sequence, replacing the current `OFFER timeout`). New 7-line r8169 boot block expected: `found at ... / MAC=... / chip-rev byte=... / reset OK / link up / Phase 1 complete / RX ring up / TX ring up`. Post-burn `sudo ./scripts/read-boot-log.sh` reads CMOS 0x58-0x5F for per-hypothesis discriminator: stamp 0x59 = 1 → H1 fix worked; stamp 0x59 = 2 → autoneg timeout (cable/switch issue); stamp 0x59 = 3/4 → MDIO interface broken (deeper datasheet read needed). Iron-burn checklist: build is current at `agnos/build/agnos` 603,784 B (TCP_LISTEN_SMOKE=1) or production-build re-run; cable connected; capture boot-log photo at shell prompt; then run read-boot-log under sudo for CMOS decode. | Iron-only (archaemenid r8169). | n/a (no new code in this row; bites B+C carry the surface). | **PENDING IRON BURN.** |
| E | **Cycle-close sweep + Attempt 94+ transcript** — state/roadmap/iron-log sweep + photo capture + CHANGELOG cycle-close summary. | n/a (doc-roll). | n/a | **PENDING (gated on bite D's outcome).** |
