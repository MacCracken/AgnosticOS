...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–21 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)** ; Attempts 17–21 landed on the next-bug ladder. **Attempt 18** found CR4.SMAP=0 on iron — Path C boot shim deferred CR4 tuning to a non-existent "kernel re-tune". **Repair (F)** mirrored the CPUID-gated SMEP+SMAP enable into Path C shim. **Attempt 20 (2026-05-15)** confirmed cp_fb-under-AS1 hung: kcp=0x18 stuck because GOP framebuffer phys sits above the 0..4 GB range `proc_create_address_space` mirrors. **Repair (H)** dropped the four under-AS1 cp_fb calls at main.cyr:479/517/522/527 (Option A; long-term fix B — pre-map FB phys into AS1/AS2 — deferred until mem-iso proves clean). **Attempt 21 (2026-05-15)** result: kcp=0x1D, [0x54]=0x30, [0x55]=0x30 — Repair (H) landed cleanly; AS1 SMAP-bracketed round-trip fully complete (stac→store→load→clac all OK); SMEP+SMAP healthy through both dump sites; visual stays at column 0x17 as expected. Death is in main.cyr:538–559 (AS2 stac/store/load/clac + second AS1 round-trip + kernel-CR3 restore) — that 21-line block had **zero kcp stamps**, so the read-boot-log verdict's "cr3_load(as2)" guess couldn't actually distinguish 9 candidate sites. **Repair (I) for Attempt 22 landed 2026-05-15**: 9 port-I/O bisector stamps (kcp=0x61..0x69) inserted at every step of the AS2 block + second AS1 round-trip + kernel-CR3 restore (main.cyr lines 537-580). Pure port-I/O, no memory access — can't fault on a missing PT mirror. Kernel **253,936 → 254,000 bytes** (+64; 72-byte asm growth, 8 bytes absorbed by alignment padding). read-boot-log gains 9 new verdicts (kcp=97..105) + revised kcp=29 verdict reflecting post-Repair-I baseline; binary **32,376 → 36,280 bytes**. **QEMU pre-flight still permanently blocked** → iron is the only test surface. **Next gate (Attempt 22)**: kcp lands somewhere in 0x61..0x69 (or stays at 0x1D, which means the very first new stamp didn't run = as2's CR3=0 from a silent proc_create_address_space failure, OR AS2 PT mirror corrupt — highest-suspicion outcome). Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct; v1.30.1 candidate carries Path C CR4 enable (Repair F) + cp_fb-under-AS1 drop (Repair H) + AS2-block bisector (Repair I). Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
