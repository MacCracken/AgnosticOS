...
> **Last refresh**: 2026-05-17 — **Attempts 57/58 narrowed Phase 4 from "no events" to "no CMD_COMPLETION for Enable Slot."** Repair (FF) IMAN.IE=set landed in 1.30.6 cut. Attempt 57 iron test (clean FF) showed `events_seen=0` survived IMAN.IE=1 (FF *14th falsified hypothesis*). Attempt 58 burned `agnos@0e3d01a` — **bundled Repair (GG) AMD-Vi global disable + Edit A (CRCR/ERSTSZ/IMAN/ERDP readback) + Edit B (cmd_submit# trb_phys + dw3 readback)** — atop the same 1.30.6 banner. Iron FB delivered the breakthrough: `xhci: drained 1 events` (was 0 in Attempts 56/57) + `ERDP_lo=0x569008` (EHB bit set by HW) prove **event-posting infrastructure works** — either FF or GG was the unblock. But `events_seen=0` persists after the Enable Slot doorbell specifically. The gate is now narrow: cmd-ring-side (CRCR write absorbed / TRB cycle bit / doorbell address). GG falsified as silent-absorb root cause (15th hypothesis) — AMD-Vi was firmware-enabled on archaemenid (`en=1`), disable wrote cleanly (`ctrl_rb=0`), events still didn't post for Enable Slot. **Edit A readback**: CRCR.CRR=0 (pre-doorbell; spec-ambiguous, could mean ring-not-latched OR idle-pending-cmd), ERSTSZ=1 ✓, IMAN=2 ✓ (IE=1, IP=W1C-cleared — FF stuck), ERDP_lo=0x569008 (page-aligned base + EHB). **Edit B readback MISSING from FB** — root-caused as stale USB build (commit `0e3d01a` landed at 20:21, USB had been flashed pre-commit). Build verified fresh post-correction (`build/agnos` 367,608 B, mtime 20:53, contains both `xhci: cmd_submit#` and `xhci: CRCR.CRR=` literals). **Iron status**: Phase 3 still UNBLOCKED on USB2 bank port 3 (Repair EE intact across Attempts 55-58). Attempt 59 = re-burn of *same* `0e3d01a` binary on fresh USB; expected delivery = the `cmd_submit#1 trb_phys=… dw3=…` line that disambiguates class 2 (cycle bit) vs class 10a (CRCR latching). No code change; same VERSION 1.30.6 banner. CMOS `[0x86]=0x5A` / `[0x87]=0xA5` extended-CMOS-offset-≥6 alias quirk reproduced (firmware-managed). Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) §§ Attempts 56-58 (+ Attempt 59 expectations). Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § 1.30.6 + [Unreleased]. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
