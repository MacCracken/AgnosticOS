...
> **agnos 1.31.1 cycle OPEN + GPT Phase 1-2 + AHCI Phase 1-4 all landed** 2026-05-20 (CHANGELOG `[Unreleased]`). VERSION bumped 1.31.0 → 1.31.1 per user's "open with version bump — close with tagged release" pattern. Cycle scope: GPT parser + AHCI/SATA (1+2 from the storage-target ranking); USB MS = 1.31.2, ext2 = 1.31.3.
...
Cycle theme stays **storage** through 1.31.x. archaemenid has **two** physical block surfaces — the M.2 NVMe Crucial P3 2 TB (just iron-validated as `nvme0`) AND a 2.5" SATA 1.8 TB SSD on `sda` — so AHCI/SATA is now iron-validatable too, contrary to the initial recommendation (corrected 2026-05-20). With NVMe Phase 1-5 + iron-debut closed under [1.31.0], the next moves down the device list — ranked by archaemenid iron-validatability + LOC cost:

| # | Target | Iron-validatable on archaemenid? | LOC estimate | Notes |
|---|--------|-----------------------------------|--------------|-------|
| 1 | **GPT parser** | Yes (both `nvme0` and `sda`) | ~150 | No new device driver — pure consumer of existing `blk_read`. Decodes LBA 1 header + LBA 2-33 entries. Unlocks partition-aware addressing across all block backends. Smallest interstitial; ship first. |
| 2 | **AHCI/SATA driver** | **Yes** (`sda` 1.8 TB SATA SSD on this box) | ~800-1200 | Second block-device class iron-validatable on archaemenid. Linux `drivers/ata/libahci.c` is the reference impl. Structurally similar shape to NVMe (PCI class probe → BAR remap → port enumeration → command list / FIS-receive structures → port command issue). Gives AGNOS two iron-validated block backends + the `blk_active` dispatch wrapper's "second backend exists, route to whichever registers" pattern gets exercised on iron (today archaemenid only registers NVMe). |
| 3 | **USB Mass Storage (BBB + SCSI)** | Yes (any USB stick) | ~600-800 | Extends the xHCI investment. SCSI READ(10) + WRITE(10) over Bulk-Only Transport. Linux `drivers/usb/storage/usb.c` reference; structurally simpler than xHCI itself (just bulk endpoints + a command opcode set). Third iron-validatable block backend. |
| 4 | **ext2 read-only** | Yes (filesystem layer; consumes GPT + block) | ~1000+ | Filesystem class, not device. FAT16 read-only is the floor; ext2 buys real Linux disk semantics (inodes, indirect blocks). Bigger lift; was previously slotted at agnos roadmap #5. |

**Recommended order** (iron-first): GPT parser → AHCI/SATA → USB Mass Storage → ext2. Reasoning: GPT is trivial and unlocks downstream partition-aware work; AHCI/SATA is the natural second device-class (iron on archaemenid's `sda`, exercises the dispatch wrapper's two-backend path); USB Mass Storage is a third iron-validatable backend that compounds the xHCI investment; ext2 turns the block stack into a real filesystem. Adjust by user preference — none gate the others structurally except GPT before any partition-aware consumer.
