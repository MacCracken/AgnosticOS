...
| # | Bite | Iron-validatable? | LOC estimate | Status |
|---|------|-------------------|--------------|--------|
| A | **OFFER-timeout audit doc** — §5b extension to [`r8169-iron-burn-audit.md`](r8169-iron-burn-audit.md): line-by-line examination of H1 (PHY not configured / no link) / H7 (TX OWN bit stuck) / H8 (RX OWN bit stuck) against current `r8169.cyr` code + multi-source prior art (Linux `r8169_main.c` + FreeBSD `if_re.c` + OpenBSD `re.c` + NetBSD `re.c` + Haiku + RTL8168 datasheet). Ranks the three hypotheses by current-code evidence + proposes smallest discriminator-instrumentation patch (CMOS-bank stamps per [[feedback_no_serial_on_iron]]) + identifies most-likely corrective patch per hypothesis. | n/a (doc-only). | 0 LOC code; ~150-300 LOC audit prose. | **PENDING — opening move, no iron needed.** |
| B | **Discriminator instrumentation** — per bite A's recommendation. CMOS-bank stamps at `r8169_send` entry/exit + `r8169_poll` invocation count + post-DISCOVER TX descriptor OWN-bit readback + post-timeout RX ring head/tail. Stacks into Attempt 94's burn with bite C per the never-bundle-instrumentation-alone rule per [[feedback_iron_burns_block_other_work]] + [[feedback_no_instrumentation_means_no_instrumentation]]. | Iron-validatable (Attempt 94 reads CMOS bank to disambiguate). | ~30-60 LOC in `r8169.cyr` + ~10 LOC CMOS-stamp helper extension. | **PENDING IRON (Attempt 94+).** |
| C | **Driver-level corrective patches** — per audit findings. **Most likely candidate**: port Linux's `__rtl8169_set_phy_reg_bit` PHY init sequence (RTL8211 register-write series post-reset) as a Phase 5 patch — addresses H1, currently the top-ranked hypothesis. May stack with H7-side `r8169_send` post-write TX OWN poll-back and/or H8-side `r8169_poll` RX descriptor walk fix if discriminator instrumentation pinpoints multiple causes. Multi-source convergent per audit doc. | Iron-validatable (Attempt 94 reads outcome via full DHCP-cycle log + CMOS stamps). | ~80-200 LOC in `r8169.cyr` depending on which hypothesis fires. | **PENDING IRON (Attempt 94+).** |
| D | **Iron Attempt 94+** — re-burn with bite B instrumentation + bite C corrective patches. Target outcome: full DHCP cycle on archaemenid (`dhcp: DISCOVER` → `dhcp: OFFER ip=<lan-IP>` → `dhcp: REQUEST` → `dhcp: ACK ip=<lan-IP>` four lines in sequence, replacing the current `OFFER timeout`). Discriminator: which of H1/H7/H8 fires (or stacks) per CMOS stamps; corrective-patch validation per DHCP-cycle completion. Pre-burn audit doc (bite A) must be landed first per [[feedback_iron_burns_block_other_work]]. | Iron-only (archaemenid r8169). | ~50 LOC test-surface (instrumentation already in bite B; no new code in this row) + Attempt 94 audit-extension prose. | **PENDING IRON.** |
...
**Build at cycle-open**: zero structural delta to `build/agnos` (only `version_str` literals shifted across the 1.32.0 → 1.32.1 bump via `scripts/version-bump.sh` regeneration of `kernel/version.cyr`). cyrius pin stays on 6.0.1. gnoboot stays on 0.4.2.
