...
QEMU end-to-end validated with byte-exact disk persistence: `nvme: registered as block_dev (32768 LBAs x 512B)` + `dd if=nvme0.img skip=5 | xxd` returns `CYRIUS!!` written by the kernel through the dispatch wrapper. Build 421,912 → 438,416 → **441,056 B** (+19,144 over 1.31.0 baseline across all five phases). The natural next move is an iron burn — install on archaemenid, verify the kernel sees the real NVMe SSD's IDENTIFY + LBA 0 GPT header; per `feedback_iron_burns_block_other_work`, that needs a written audit before scheduling (separate proposal).
