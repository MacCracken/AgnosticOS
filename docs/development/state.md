...
> **Last refresh**: 2026-05-17 — **Attempt 60 staged with NO instrumentation, four behavioral repairs sourced from Linux xhci prior art, stacked into one burn.** Pivot after user reset discipline mid-staging: "three days for a driver — stop instrumenting, ship fixes." Repair (II) timeout-path readback STRIPPED. Stack now: **HH** = doorbell post-write `load32` flush (matches Linux `xhci_ring_cmd_db`'s `writel+readl` — AMD-FCH missed-doorbell documented symptom); **JJ** = `writel+readl` universal pattern on EVERY operational + runtime register write via `xhci_op_write32/64` + `xhci_rt_write32/64` (Linux xhci-mem.c / xhci.c convention — host bridge holds posted writes >100µs without readback even with UC mapping; CRCR / DCBAAP / ERSTBA / ERSTSZ / ERDP / IMAN / USBCMD / CONFIG all benefit); **KK** = CNR (USBSTS bit 11) poll before any operational write in `xhci_start` (Linux's `xhci_handshake(STS_CNR, 0, …)` — closes post-reset CNR re-assert window where HCRST=0 but CNR=1 silently absorbs writes); **LL** = Link TRB initial cycle bit fix `| 0x1` → no `| 0x1` at xhci_ring.cyr:191 (xHCI 1.2 §4.9.3.1 + Linux `xhci_alloc_segment` precedent — defensive correctness, unlikely to be the first-Enable-Slot bug but worth shipping in same burn). Build verified: `build/agnos` **367,944 B** mtime ~22:50, `xhci: CNR never cleared` literal present (KK), `xhci: timeout state CRCR_lo=` literal ABSENT (II stripped). Decoder `read-boot-log.cyr` reverted to pre-II shape. Attempt 59 outcome retained: classes 2 + 4 closed via `dw3=9217` readback. **Iron status**: Phase 3 reset on port 3 still UNBLOCKED (Repair EE intact across Attempts 55-59). No VERSION bump — 1.30.6 banner retained through HH/JJ/KK/LL cycle. **Attempt 60 awaits user-side USB flash + per-action burn approval.** Stack is decoupled: 4 fixes in one burn, attribution lost intentionally on success (acceptable per user's velocity directive). On failure, the residual hypothesis space is narrow — MSI-X function-mask disable / vendor PCI cap audit / event-ring DMA path probe. Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) §§ Attempts 56-60. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § 1.30.6 + [Unreleased]. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
