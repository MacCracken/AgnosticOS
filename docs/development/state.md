...
CYML — 6.0.x LEADING EDGE (the bulk of the ecosystem, ~32 repos):
  ...
  v6.0.43:  mabda (3.0.1)
  v6.0.52:  agnosys (1.3.2)
  v6.0.53:  libro (2.7.1)
  v6.0.56:  agnos (1.41.15), agnoshi (1.4.1), kybernet (1.3.3),
            argonaut (1.8.2)
  v6.0.61:  sigil (3.7.1)
...
| **agnos** | **1.41.15** | **6.0.56** | Kernel. The 1.41.x shell-separation arc is software-complete (full ladder in the header above); pin moved 6.0.14 → 6.0.56 at 1.41.4 (`CYRIUS_TARGET_AGNOS`). Iron: `14111`→stdout fix (1.41.12); `14112`→PMM `#PF` fix (1.41.12); 1.41.13 i8042-poll falsified → 1.41.14 IRQ1+`sti`; **`14114`→typing READ but collapsed to stuck `Command: D` (no echo) → 1.41.15 = continuous-IF line-disciplined `read(0)` + kernel echo (mirrors in-kernel shell), ABI O1 RAW→canonical-lite** (QEMU 7/7 + type-test echo/dispatch correct, iron-pending). build 1,070,960 B → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). |
| **agnoshi** | **1.4.1** | **6.0.56** | AI shell — the userland interactive shell exec'd from disk in ring 3 (the 1.41.x shell-separation arc). 1.4.1: `read_line` returns one line per call via a carry buffer, pairing with agnos 1.41.15's line-disciplined `read(fd 0)` (host pipes still split correctly — host smoke 59/0). Was 1.3.3 / pin 6.0.1 two refreshes ago; the agnos-target build (`agnsh_agnos`) needed cyrius 6.0.56's `CYRIUS_TARGET_AGNOS` peer, so it graduated with the boot-path core. |
