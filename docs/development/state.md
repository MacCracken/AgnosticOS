...
> **Last refresh**: 2026-05-25. **agnos kernel — 1.32.x networking arc COMPLETE (1.32.9 DHCP lease iron-verified); 1.33.0 ext2/ext4 WRITE arc OPEN — W1 LANDED, QEMU smoke-green** (the demo→base maturity exit — state persists across reboots). Audit-first doc [`ext2-ext4-write-prior-art.md`](ext2-ext4-write-prior-art.md) + version bump (1.32.9 → 1.33.0 via `version-bump.sh`) done. Phased W1-W5 (W1-W4 = QEMU `e2fsck -fn`-clean smokes, W5 = iron). **W1** (write primitives + `ext2_put_inode` RMW + bitmap/free-count reads) + **W2** (block + inode allocators + write-safety gate refusing csum/64bit FSes) both LANDED + QEMU smoke-green via `scripts/ext2-write-smoke.sh`: W1 free-counts triple-agree with host `debugfs` (16068) + identity write-back byte-clean; W2 alloc/free round-trip returns free-counts exactly (`16068→16068`, `17138→17138`); **W3** (`ext2_write_at` + sparse holes + truncate) + **W4a** (dirent insert/remove + `create`/`unlink`) both LANDED smoke-green — **demo→base FS-layer milestone**: create+write+persist+delete all readback-clean, `e2fsck -fn` clean, and host `debugfs` reads `/w4keep.txt`="AGNOS-W4-WROTE" off the disk image (create PERSISTED) + confirms unlinked file absent. The e2fsck gate caught 2 real unlink bugs (zero/small `i_dtime`). No regression (`test.sh` 4/4 + `ext2-smoke.sh` 5/5). Build 654,384 B. **W4b (VFS `vfs_write`/`vfs_create` arms + shell `touch`/`echo>`/`rm` — the user-facing `echo > f`; interactive/iron-validated) then W5 (mkdir/rmdir + iron burn) = next.** (1.32.7 RELEASED: r8169 unicast RX SOLVED; 1.32.8 RELEASED: net-diagnostic cleanup; 1.32.9 RELEASED: DHCP re-enable + MAC hex-print, iron-verified.) · cyrius 6.0.1 · gnoboot 0.4.2.
>
