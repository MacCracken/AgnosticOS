...
> **Last refresh**: 2026-05-18 — **Attempt 61 burned + FALSIFIED (MM MSI-X Function-Mask clear ineffective); Attempt 62 staged with Repair (NN) — convergent-prior-art Tier 1 bundle: ERDP-before-ERSTBA + CRCR-after-IMOD. Attempt 63 pre-staged with Repair (OO) Tier 2 bundle conditional on NN falsification.** Pivot: letter-laddering (FF/GG/HH/JJ/KK/LL/MM) closed — the missing artifact all along was a multi-source convergent-prior-art baseline diff (Linux + FreeBSD + Haiku + EDK2 vs AGNOS), not another symptom-dictionary letter. New bootstrap artifact: [`xhci-prior-art-audit.md`](xhci-prior-art-audit.md). Per-attempt detail: [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) §§ Attempts 61-63. Reader: [`scripts/read-boot-log.sh`](../../scripts/read-boot-log.sh) default prints the focused current-sweep CMOS view (decoded sentinels + ✓/⚠ marks); `--verbose` recovers the full slot listing + interpretation chapters + verdict layers for falsified-hypothesis triage. **Repair (NN)** = two-LOC reorder in `xhci.cyr`: (NN.A) `xhci_start` interrupter-setup writes ERDP before ERSTBA per xHCI 1.2 §5.5.2.3.3 + 3-of-4 prior-art convergence; (NN.B) CRCR moved to after IMOD per 2-of-4 prior-art convergence. Build verified: `build/agnos` **368,472 B** (identical to Attempt 61 — reorder-only). User directive 2026-05-18: "I really don't care what fixes it I want it fixed... hardening and cleanup can always be done later" → single-repair-per-burn discipline suspended for the cmd-path arc; future burns may bundle multiple convergent-prior-art behavioral repairs as long as each carries its own line-by-line audit. Instrumentation discipline (`feedback_no_instrumentation_means_no_instrumentation`) remains in force. **Repair (OO) pre-staged bundle** (conditional on Attempt 62 falsification): OO.A USBSTS RW1C-clear at `xhci_start` entry (FreeBSD pattern); OO.B IMAN.IE write moved to AFTER R/S=1 (Linux convention; reverses Repair FF's pre-R/S placement); OO.C explicit `mfence` before doorbell write (defensive); OO.D cmd-ring TRB readback flush (defensive, JJ-pattern extended to RAM-side). No VERSION bump — 1.30.6 banner retained through NN/OO. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § [Unreleased]. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
