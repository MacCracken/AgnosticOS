...
> **Last refresh**: 2026-05-25. **agnos kernel 1.33.1 — real-`mkfs.ext4` WRITE (metadata_csum + 64bit) QEMU-COMPLETE; W5 iron burn pending (user-driven)** (the demo→base maturity exit: state persists across reboots — now on an UNMODIFIED default partition, no re-carve). Full mutation set — create / write (sparse + truncate) / unlink / mkdir / rmdir + the VFS `vfs_write` arm + shell `touch`/`rm`/`mkdir`/`rmdir`/`echo>` verbs — all `e2fsck -fn`-clean (incl. Pass-4 link accounting) and disk-persistence-verified via host `debugfs` (`scripts/ext2-write-smoke.sh` **22/22 PASS**). **`echo > /f` → reboot → `cat /f` works in QEMU.** Audit-first per [`ext2-ext4-write-prior-art.md`](ext2-ext4-write-prior-art.md); per-phase detail (W1-W5 + W4b, incl. 2 e2fsck-caught unlink bugs) in [`iron-nuc-zen-log.md#tracker-133-cycle`](iron-nuc-zen-log.md#tracker-133-cycle). Release-cleanup: 5 `kprint` byte-length fixes. Build **666,840 B**; `test.sh` 4/4 + `ext2-smoke.sh` 5/5, no regression. cyrius 6.0.1 · gnoboot 0.4.2. **NEXT = W5 iron burn (user-driven, NOT auto-proposed)** — the 1.33.0 burn (photo `1330_failed_writes`) confirmed the W2 gate refusing write on the default-`mkfs.ext4` partition; rather than re-carve, **1.33.1 implemented real metadata_csum + 64bit write** (7 bites, all `e2fsck -fn`-clean in QEMU on the exact `metadata_csum,64bit,extent` profile). Burn readiness audit: [`metadata-csum-write-iron-burn-audit.md`](metadata-csum-write-iron-burn-audit.md) (Option A: `echo > /persist.txt` → power-cycle → `cat`; then host `e2fsck`). `build/agnos` production 675,152 B reflects HEAD. (Prior: 1.32.7/1.32.8/1.32.9 RELEASED — networking arc complete, DHCP lease iron-verified.)
>
