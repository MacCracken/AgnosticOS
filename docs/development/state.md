...
> **Last refresh**: 2026-05-27. **agnos kernel: 1.35.0 RELEASED · 1.35.1 OPEN — TCP hardening** · gnoboot 0.4.2. **1.35.0** was the catchup-tidbits cut: full docs sweep + **DNS stub resolver** (`dns` verb; `dns-smoke.sh` green, live `example.com` via SLIRP; audit [`dns-stub-resolver-prior-art.md`](dns-stub-resolver-prior-art.md)) + **ICMP echo / `ping`** (pingable + `icmp_ping`; `icmp-smoke.sh` green). **1.35.1** hardens the minimal TCP state machine (retransmit + RX window + MSS — the reliable-`ark`/`nous`-fetch enabler; audit-doc-first). Then **NTP/SNTP**; TLS stays with the cyrius agent. Legacy virtio-net back-burnered; plug-and-play remains. The **1.34.x FAT-family write-completeness continuation** (FS roadmap row 21) is **COMPLETE**, all RELEASED + `fsck`-clean in QEMU: **1.34.0** FAT12/16/32 read+write · **1.34.1** exFAT read+write · **1.34.2** exFAT write parity · **1.34.3** FAT LFN/truncate completeness · **1.34.4** directory growth (root extension + spanning dir-set append, exFAT + FAT) · **1.34.5** exFAT Unicode names (real up-case table) · **1.34.6** ESP-write safety guard (the arc-cap code — `fatfs`/`exfat` refuse writes to an ESP-type partition so the boot ESP can't be clobbered; `FAT_ALLOW_ESP_WRITE` overrides for the QEMU ESP test image). Production build **~799 KB**; `test.sh` 4/4. **Only the user-driven FAT/exFAT iron burn remains** — the FAT-family arc's first iron touch (the guard makes it brick-safe); plan in [`#tracker-1341-cycle`](iron-nuc-zen-log-mvp2.md#tracker-1341-cycle). Per-cut detail → CHANGELOG `[1.34.0]`–`[1.34.6]`. **NEXT = user git tag of 1.34.6; then the user-driven iron burn closes 1.34.x, OR open the next arc (1.35.x catchup / NTFS+squashfs / ext4 extent-alloc).**
>
> **Closed arcs (history lives in CHANGELOG + iron-log, not here):** storage backends + GPT + ext2/ext4 read, 1.31.x ([`#tracker-133-cycle`](iron-nuc-zen-log-mvp2.md#tracker-133-cycle) for the FS detail); networking, 1.32.x (see Networking line below); ext2/ext4 WRITE incl. the W5 demo→base iron burn + the 1.33.5 fsync/FLUSH-CACHE barrier, 1.33.x ([`#tracker-1335-cycle`](iron-nuc-zen-log-mvp2.md#tracker-1335-cycle)).
>
