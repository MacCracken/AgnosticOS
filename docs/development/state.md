...
**Iron-validation coverage at each cut**: 1.31.0 → NVMe (Crucial P3). 1.31.1 → NVMe + SATA (WD Blue SA510). 1.31.2 → NVMe + SATA + USB MS (PARTIAL Attempt 83 — TUR-pass gate stops Phase 3+4; Phase 2.5 next-touch) + USB optical (planned). 1.31.3 → same + RAM-disk + VirtIO (QEMU). 1.31.4 → same + ext2 filesystem.
...
#### USB MS iron carry-forward — Attempt 83 surfaced; Phase 2.5 TUR-hardening pending

Third storage-class iron debut (after NVMe at Attempt 80 + SATA at Attempt 81). Real-vendor USB 2.0 stick at xhci port 3 (`VID=0x0936 PID=0x13E8`) enumerated as MSC-BBB on slot 2; bulk EP pair captured (`0x82` IN / `0x01` OUT, MPS 512/512), Configure Endpoint succeeded, CBW transport landed on real silicon. Phase 2 TUR returned NOT_READY / CSW status != 0 — the spec-anticipated cold-insertion behavior (`usb-ms-iron-burn-audit.md` § 3 hyp 2) — and the existing TUR-pass gate at `msc.cyr:317-381` stops Phase 3 INQUIRY / RC10 / tertiary registration from running. Boot walked to shell clean through NVMe primary.

**Code-read finding (post-burn)**: the `(CSW status != 0)` line is the generic else-branch label in `msc_probe_slot:377-380`; the actual failure is **Step 4 CSW receive timeout in `msc_bbb_exec:659-663`** — no CSW was ever decoded, `transport_failed` sticky is SET. Audit § 3 hyp 2 (NOT_READY) remains *unverified*, not confirmed.

| Item | Resolution | Mechanism |
|---|---|---|
| **Bulk-IN transport wedged: Step 4 CSW receive timeout** (confirmed by code-read; `transport_failed=1` after burn) | 🔄 Pending — Phase 2.5 **primary** | Add `msc_reset_recovery` per USB MSC BBB §6.7.3: three control requests via `xhci_control_no_data` (Bulk-Only Mass Storage Reset `0x21/0xFF/0/intf/0` + CLEAR_FEATURE(ENDPOINT_HALT) on bulk-IN + same on bulk-OUT) + host-side ring re-zero / Link-TRB rewrite / cycle+idx reset / clear sticky. |
| No TUR retry — one shot then bail | 🔄 Pending — Phase 2.5 | Linux `usb_stor_TUR` + `sd_spinup_disk` pattern: 3-retry loop in `msc_probe_slot`; call `msc_reset_recovery` between failed attempts when `transport_failed=1`. |
| Phase 3 (INQUIRY) sits inside TUR-pass branch — INQUIRY shouldn't be gated on Ready per SPC-4 §6.6 | 🔄 Pending — Phase 2.5 | Hoist `msc_inquiry` call out of the TUR-pass branch in `msc_probe_slot`; keep `msc_read_capacity` inside (RC10 legitimately requires Ready per SBC-3 §5.10). |
| No REQUEST SENSE path to decode CSW Failed (once transport recovers) | 🔄 Pending — Phase 2.5 | Add `msc_request_sense` (SPC-4 §6.27) helper; decode key/ASC/ASCQ; distinguishes "becoming ready" (02/04/01 → retry) / "no medium" (02/3A/00 → graceful proceed) / "media changed" (02/28/00 → re-init). |
| Device-side NOT_READY response shape | ❓ Unverified | Never received a CSW on this burn → audit § 3 hyp 2's predicted shape (`bCSWStatus=1`, sense `02/04/01`) is unverified. Once Phase 2.5 fixes the transport, this surfaces in REQUEST SENSE output. |
| Controller-side EP recovery (Reset Endpoint + Set TR Dequeue Pointer xHCI commands, TRB types 14 + 16) | 🅿️ Held — Phase 2.6 if needed | Only if Phase 2.5 device-side recovery isn't enough (e.g., `xhci: transfer event timeout` repeats on the retried CBW). Adds new TRB type constants + helpers in xhci_cmd.cyr / xhci_regs.cyr. |

Detail in [`iron-nuc-zen-log.md` § Attempt 83](iron-nuc-zen-log.md). Phase 2.5 lands the four-patch device-side stack (Reset Recovery + TUR retry + INQUIRY hoist + REQUEST SENSE) in next USB MS touch — single-burn fix per `feedback_redesign_dont_reinvent` (Linux precedent canonical; no first-principles diagnostics).
