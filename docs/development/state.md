...
#### USB MS iron carry-forward — Phases 2.5 ✅ / 2.6 FALSIFIED@85 / 2.7 LANDED@86 (Reset Recovery worked, post-recovery TUR still failed) / **Phase 2.8 eight-bug repair stack in agnos 1.31.3 — Attempt 87 burn pending**
...
**Attempt 87 success rubric** (Phase 2.8 eight-bug stack): (a) full — `xhci: bulk transfer event timeout` absent; INQUIRY succeeds first try or with single retry; TUR Pass; RC10 prints last_lba + blk; tertiary registration line; LBA-0 readback. (b) partial — Reset Recovery still fires but at least one round succeeds. (c) failure — same "CSW signature mismatch" loop = new bug class beyond the eight-bug audit.

**MVP gate posture**: unaffected — `msc_probe_slot` already returns 1 even on transport failure, so MVP boot-to-shell stays green regardless of Attempt 87 outcome. USB MS arc is opportunistic; closed beta depends on kybernet + agnoshi + kernel-on-iron, not on a specific block backend.

**1.31.3 close gating**: agnos VERSION at 1.31.3 with `[1.31.3]` complete Phase 2.8 audit in CHANGELOG. **QEMU validation 4/4 green** (`AGNOS shell v1.31.3` in transcript). **Close decision waits on Attempt 87 iron evidence** — full-pass cuts; partial folds into next attempt; failure triggers Phase 2.9 deeper audit.
