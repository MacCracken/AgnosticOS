...
> **Cyrius toolchain**: agnos pins **6.0.56** (`cyrius.cyml`; 6.0.1 → 6.0.3 at the 1.35.5 cycle-open, → 6.0.14 at the 1.39.0 cycle-open, → **6.0.56 at the 1.41.4 shell-separation bite** — the `CYRIUS_TARGET_AGNOS` leap that landed `args_agnos`/`io`/`process_agnos` for the agnos target so `/bin/agnsh` runs in ring 3; cyrius latest released **6.0.88** — build agnos-target binaries with the **pinned 6.0.56**, the default `cyrius` wrapper uses latest and only warns about pin drift). The agnosticos `scripts/cyrius.cyml` boot-pipeline pin **graduated 5.11.59 → 6.0.14 on 2026-05-30** (the long-deferred boot-side sweep landed: rebuilding `read-boot-log` for the 1.40.9 exec-iron CMOS read was its next boot-side touch; compiled clean on 6.0.14, one harmless `vec_get` 6.0.x stdlib-drift warning in a DCE'd path, left unchased per user). v6.0.0 cycle opened 2026-05-19; v5.11.x closed at **5.11.69**.
> **Last refresh**: 2026-06-08. **agnos kernel: 1.43.6 · agnoshi 1.4.6 · anuenue 1.1.1 · cyrius-doom 0.28.2** — **1.42.x perf + hardening + userland cycle: `bench.sh` reconstructed (1.42.1) + the three carry-forward hardening bites landed (1.42.2 page-map direction-B / 1.42.3 SYSCALL RBP smash / 1.42.4 reap leftovers) + heap-zeroing perf −50% (1.42.5) + Track-B userland FS verbs (1.42.6 / agnoshi 1.4.2 — `ls`/`cat`/`cp`/`mv`/`rm`/… as builtins, verb→ext2 roundtrip iron-path-validated) + memory-core perf (1.42.7 — `pmm_alloc`/`pmm_free` bitmap-op inline + the single-core PMM spinlock retired to a user-authorized no-op + word-wide `memset`/`memcpy`; `pmm_alloc_free` 789→~330-400 ≈ −50%, sweep 7/7) + fs read-path perf (1.42.8 — `ext2_read_block` collapses 8 single-LBA NVMe commands into 1 multi-LBA per 4 KB block + `ext2_dir_lookup` inode-refetch hoist; `block_read_amp` bench proves multi=1 vs single=8 submits, sweep 7/7) + fb_console glyph-blit perf (1.42.9 — per-pixel address multiply strength-reduced to add-stride + `fb_scale` inlined; iron path, byte-identical verified by FB screendump, sweep 7/7). **Track-A perf high-confidence list now closed** (heap / memory-core / fs-read / fb_console); remaining perf candidates (scheduler/context-switch, syscall-entry ladder) are marginal vs the KPTI CR3 switches and deferred; `fb_scroll` shadow-buffer rejected (eats the 2 MB-region budget). All QEMU-validated (sweep 7/7 + repro-loops 0 #PF/20, 0 smash/40, reap-stable; iron rides the next burn). **Sovereign sysinfo syscalls SHIPPED at 1.42.10** — `uname`(#34, 64-byte identity struct) + `sysinfo`(#35, 40-byte counters) struct-write syscalls (binary structs, NOT a Linux `/proc`); `/bin/sysi` ring-3 validator `run: exit 66`, exec-smoke 8/8, sweep 7/7. The tools (cmdrs/bnrmr/mihi/iam) were blocked on these — now exist. **AGNOS-as-a-build-target effort (2026-06-06):** the kernel-interface dep **agnosys repaired → 1.4.0** (`syscall.cyr` gates uname#34/sysinfo#35/gettid/geteuid; `agnosys-core` builds `--agnos`; it's a DEP lib, not cyrius stdlib, so repaired in-repo). **`bannermanor` 1.1.0 + `commandress` 1.1.0 now build `--agnos`** (terminal tools — only Linux-isms were ioctl/getcwd/access/uname/shellout, gated inline; hostname works via uname#34, fs-detection via stat#33; pin → 6.0.56 + lib re-vendor). **`mihi`/`iam`/`chakshu` DEFERRED to 1.43.x graphics** — mihi's GPU probe pulls `ai-hwaccel → thread → atomic → Linux CLONE_VM`, and cyrius `atomic`/`thread` can't target agnos until agnos *has* threads (the future SMP arc); GPU + ioctl/terminal-size are graphics-arc concerns that belong with 1.43.x. **klug (Kernel Logs Unified Grep) bite 1 SHIPPED at 1.42.11** — `kernel/core/klug.cyr`, a 16 KB unified log ring buffer tapped by `kputc`/`kprint`/`kprintln` (all kernel output in one greppable place); `KLUG_SELFTEST` dump validator proves the boot log re-emits between markers, sweep 7/7. Kernel UNIFIES, GREP stays userland. **klug bite 2 SHIPPED at 1.42.12 → kernel logging layer COMPLETE**: leveled logging (`klog_info`/`warn`/`err`) + `klog`#36 read syscall (ring→userland, dmesg-tail, unwraps both fill cases); `/bin/klog` validator `run: exit 200`, exec-smoke 9/9, sweep 7/7, 3-lens adversarial review 0 findings (incl. a 65,679-case unwrap simulation of the boot-untested wrapped path). **`bnrmr` + `cmdrs` + `klug` BANKED onto the agnos-fs `/bin`** (2026-06-06 — the first AGNOS-tic userland tools living on agnos alongside agnsh, via `agnos/scripts/stage-tools.sh` → `install-media.sh --update-fs`; static-ELF64, runnable via the in-kernel recovery shell's `run /bin/<tool>` exec-from-disk → ride the next burn as **H5**). **`klug` 0.1.0 = the standalone `klog`#36 reader / sovereign dmesg** (its own repo; dumps the unified kernel ring, `-w`/`-e` severity lens off the `[I]`/`[W]`/`[E]` prefixes, `klug | grep …` for the rest; cut as agnos **1.42.13**). **1.42.14 hardening / audit / security sweep LANDED** (2026-06-06 — a 6-dimension multi-agent audit, 10 raw → 6 confirmed after adversarial verification: **CRITICAL ring-3 arbitrary-kernel-write via signalfd/timerfd reads FIXED** (`read(sigfd,ADDR,0)` reached a fixed-size `store32`/`store64` with `ADDR` unvalidated — helpers now require count≥4/≥8 + read#5 rejects arg3<0), MEDIUM SMAP-off-across-post-exit-work FIXED (`clac` in `kernel_resume`), LOW SFMASK widened `0x200→0x40700` (IF|TF|DF|AC); a HIGH cyrius-side `cyml.cyr` var-array stack overflow surfaced → [issue filed](https://github.com/MacCracken/cyrius), language-level fix in **cyrius 6.0.79**, then cmdrs/bnrmr re-vendor; FS/exec dimension audited clean; rebuild + sweep **7/7**). **NEXT: the ride-along burn** (validates H1–H5, incl. `run /bin/klug`). `mihi`/`iam`/`chakshu` + `kriya` (FS-layer refactor, roadmap §M11) → **1.43.x**. **agnoshi 1.4.3 (2026-06-07)** makes the 1.4.2 in-process FS verbs discoverable in `help` (the user-reported "`echo` works but is hidden" gap) and lands a first-class **`run PATH` builtin** — validated (absolute-path + `is_safe_path`) and **pre-wired** to the real exec path behind a single `RUN_EXECWAIT_READY` flag, but **kernel-gated OFF**: an adversarial-verified audit confirmed the ring-3 spawn route fatally faults (`waitpid`#4 `hlt`-spins with IF=0 → hard hang; `spawn`#3 → in-memory `elf_load` CR3-switch `memcpy` of agnsh's unmapped arena buffer → triple-fault; 4 KB-as-2 MB aliasing). **✅ 1.43.x ARC OPEN — execwait #37 LANDED (agnos 1.43.0, 2026-06-07) + run UN-GATED (agnoshi 1.4.4).** The 1.43.x arc = "graphics path / first real userland app (agnsh launches DOOM)" — non-threading: ring-3 blocking exec + a framebuffer/blit syscall + ring-3 timing + envp + `cyrius-doom --agnos`. **Bite 1 = `execwait`#37**, the ring-3 blocking-exec primitive: it exposes the proven in-kernel `elf_load_from_file` + synchronous `exec_and_wait` (the recovery shell's working `run`) as a syscall. The novelty (designed via a 3-design + 3-skeptic panel; the panel's primary "indirect stub stack-load" fix was REJECTED on re-derivation — `r10` is reused at `syscall_hw.cyr:255` to pass `user_rsp`, so clobbering it corrupts every syscall) is that it's the FIRST `exec_and_wait` from a *live ring-3 syscall frame*: (H1) it snapshots+restores the caller's resume-context globals (`exec_ctx`/`kernel_return_*`/`kpti_*`/`kernel_rsp_save`) so the caller's own `exit()` still resumes its parent, and (H2) it runs the child on a DISJOINT second SYSCALL kstack (`syscall_kstack_top2 = 0x3D0000`, baked via `exec_and_wait`'s own `syscall_init()` rebuild — no hot-path stub opcode change) so the child's syscalls can't smash the suspended caller's frame / SYSRET cells. Runs the disk load under boot CR3 (per-process CR3 doesn't map the NVMe BAR; path copied to a kernel buffer first). Validated by the `/bin/exwv` ring-3 selftest (a ring-3 program that execwaits `/bin/prog2`, exercising the full H1+H2 path) → **exec-smoke green + sweep 7/7 + check 11/11; iron rides the next burn**. agnoshi 1.4.4 then routed `process_agnos.cyr run()` onto `sys_execwait(path,len)` + flipped `RUN_EXECWAIT_READY=1` (host smoke 84/0 + 59/0; `run /bin/true` executes). **Bite 2 = FB-console ANSI/SGR color (1.43.1) + line-discipline EOF (1.43.2):** `fb_console.cyr` gained an `fb_ansi_feed` 3-state CSI machine wired into `fb_putc` — the kernel framebuffer now interprets SGR color (16 / 256-cube / 24-bit), ED/`ESC[2J` clear, and CUP cursor positioning instead of dropping the escape bytes, so the per-character **anuenue rainbow renders on iron** (`agnos` 1.43.2); `kbd_read_blocking` gained an **EOF/Ctrl-D** branch (returns 0 at line start) so stdin filters can terminate. **`anuenue` BANKED onto `/bin`** alongside bnrmr/cmdrs/klug (`stage-tools.sh`). **Iron burn `14314` (1.43.x, 2026-06-07)** ran the banked tools from `[ASSIST] >` and surfaced two userland bugs, both fixed + **QEMU-validated this cycle** (gnoboot+OVMF+KVM, HMP-sendkey driver): (1) `ls`/`ls .` → "No such file or directory" — agnsh verbs passed relative paths to the kernel, which requires absolute (no per-process CWD) → **agnoshi 1.4.5** `verb_abspath` resolves them at every path chokepoint (`ls`/`ls .`/`ls /` all list the ext2 root in QEMU); NB the real agnsh entry is `src/agnsh.cyr` (`src/main.cyr` is dead legacy). (2) `run /bin/anuenue` hangs — pure stdin filter, no pipes/EOF on agnos → **anuenue 1.1.1** adds a positional-text mode (`run /bin/anuenue AGNOS`). **`scripts/burn-prep.sh` now defaults to a BARE production kernel** (no compile-gated selftests; opt back in with `BURN_SELFTESTS=1`) — selftest code stays in-tree, just not enabled in the burn artifact now that the exec/EXT2 arc is iron-validated. **✅ envp CLOSED end-to-end (agnoshi 1.4.6, 2026-06-08).** All three layers are in: the kernel stages a real env (`HOME=/`/`PWD=/`) on the exec init stack (1.43.2), **cyrius 6.0.87** added the agnos `getenv()` branch that walks it (QEMU-validated vs the real 1.43.2 kernel — `getenv("HOME")→/`, `PWD→/`, `NOPE→null`), and **agnoshi bumped its pin 6.0.56→6.0.87 + re-vendored** so agnsh's `getenv()` IS the agnos envp-walk (`_agnos_getenv`), not the `/proc`-only stub — `history_path`/`audit_log_path` now resolve `$HOME` on agnos (`/.agnsh_history`) instead of the `/tmp` short-circuit (trailing-slash-aware concat; 3-lens adversarial review **0 findings**, host smoke 59/0, agnos `--agnos` ELF clean, ABI offset independently re-derived both sides — exact match). The **kernel deliberately holds 6.0.56** (it's the envp *producer*, not a `getenv` consumer; don't chase). Per-process env propagation (thread a caller envp through `execwait` so kybernet *seeds* rather than the kernel hard-coding a uniform default) stays a deferred follow-on — moot until a per-process env source exists. **✅ framebuffer/blit LANDED (agnos 1.43.4, 2026-06-08)** — the DOOM graphics path's core: `fbinfo`#38 (24-byte geometry query; `fb_phys` NOT exposed) + `blit`#39 (kernel-mediated ring-3→FB 32bpp copy, clipped, fail-closed on malformed-GOP `pitch<width*4`). Kernel-mediated (not FB-mmap) so raw scanout never maps into ring 3; writes `fb_phys` exactly as `fb_putc` does for stdout — **no CR3 switch**, and a 3-lens adversarial review re-derived it sound on **QEMU + iron** (archaemenid GPU BAR→PDPT[3], inherited per-process) with **0** ring-3-reachable OOB. `/bin/fbtest` exec-smoke `exit 88`, sweep 7/7; designed from a 4-source convergent audit (fbdev/simpledrm/wsfb/GOP/doomgeneric). **✅ ring-3 timing LANDED (agnos 1.43.5, 2026-06-08)** — `uptime_ms`#40 (monotonic ms since boot = `timer_ticks*10`, the frame clock) + `sleep_ms`#41 (the pacing primitive). The hard part was advancing the clock for a ring-3 loop in the no-preempt IF=0 model (user code runs IF=0 → `timer_ticks` frozen during compute; naive `hlt` = the `waitpid` hard-hang): solved via `kbd_read_blocking`'s proven recipe — `sched_active=0` (timer ISR still ticks+EOIs, `do_context_switch` early-returns) → `sti` **before** `hlt` → `cli`+restore; `SYSRET` restores user IF=0 from R11. `/bin/timetest` slept 50ms and the clock advanced exactly 50ms (exec-smoke `exit 50`, sweep 7/7, 2-lens review **0** findings — liveness-safe, no IF leak). 10ms granularity is playable for DOOM's 35Hz tics (`tick_wait` sleeps the remainder + self-corrects). ✅ 1.43.x COMPLETE — **DOOM RENDERS (agnos 1.43.6 / cyrius-doom 0.28.2, 2026-06-08):** "agnsh launches DOOM" — the first real userland *application* on AGNOS. The 584 KB ELF exec's from disk in ring 3, slurps the 4.2 MB DOOM1.WAD, parses it, and blits a 240-colour title-screen frame via `fbinfo`#38/`blit`#39. Two kernel fixes underwrote it: the PMM 2 MB pool grew **16 MB → 128 MB** (`pmm.cyr`) and `proc_create` now maps `PD[8..63]` 16–128 MB identity-supervisor so `sys_mmap`'s 2 MB `memset` can reach a high region (the WAD-alloc ring-0 #PF→#DF fix, found via `qemu -d int`). Validated by `agnos/scripts/doom-smoke.sh` (PASS); QEMU sweep 7/7; cyrius pin held 6.0.56. **KNOWN BUG (workaround-only):** the FIRST `mmap` from a freshly-exec'd ring-3 process corrupts the ring-3 return state (fault at RIP=0); cyrius-doom 0.28.2 carries a one-line warm-up-mmap workaround; agnsh unaffected. Tracked in `agnos/docs/development/planning/doom-on-agnos-render-blockers.md`. **NOT yet iron-burned on archaemenid** — the 128 MB PMM + `PD[8..63]` map meet the box's real UEFI memory map for the first time (primary risk: a reserved-region collision in 16–128 MB; PMM hardcodes `pmm_total`, reads no E820/UEFI map) → [`#tracker-1436-cycle`](iron-nuc-zen-log.md#tracker-1436-cycle). scaling + zero-copy FB-mmap deferred; userland-tool tail per [[project_agnos_userland_platform_dep_chain]] (iam/mihi land as each blocker clears). Predecessor context — **agnoshi 1.4.3 (2026-06-07)** made the 1.4.2 in-process FS verbs discoverable in `help` (the "`echo` works but is hidden" gap) and STAGED the gated `run PATH` builtin (validated + pre-wired behind `RUN_EXECWAIT_READY`, refusing cleanly while the kernel spawn route was proven to fatally fault). NB: H5's `run /bin/klug` is the **recovery shell** exec-from-disk; agnsh's `run` is the **userland** path now un-gated onto execwait. The **1.41.x shell-separation arc is iron-complete** (burn `14115`, 2026-06-06: agnsh types + echoes + dispatches on archaemenid; two non-blocking residuals → 1.42.x, of which the agnoshi `VERSION_STR` desync is fixed and backspace-on-iron awaits the recovery-shell probe). The arc (1.41.1→1.41.11) was software-complete and QEMU-validated (userland `agnsh` exec'd from disk in ring 3, in-kernel shell = recovery REPL, FAT/exFAT content-write at the syscall ABI, hardened ring-3→ring-0 ingress). **Iron burn #1 (`14111_stallout`, 2026-06-04) surfaced + fixed a ring-3-stdout regression** at **1.41.12**: a userland `write(1,…)` routed through the serial-only console device (`serial_dev_write`→`serial_print`), so the agnsh banner was invisible on the serial-less box and the boot *looked* hung at `kybernet: exec /bin/agnsh` though agnsh was alive in ring 3. Fix: `serial_dev_write`→`kprint` (serial+fb mirror). **A4 ✓** (no exec/storage/net regression — real DHCP lease). **Burn #2 (`14112`, 2026-06-04) made the banner FB-visible but agnsh `#PF`'d before reaching a prompt — NOT a keyboard issue.** QEMU `-d int` + a page-table walk traced it to a **kernel PMM bug**: `pmm_alloc`'s KASLR-seeded bottom-up first-fit scattered a 4 KB page into a random 2 MB region, so `pmm_alloc_2mb` couldn't assemble the userland code/stack/heap and the mmap heap page resolved **supervisor-not-user** (`PD[128]=0x83`); agnsh faulted on its first heap write (~50% of boots) **before ever reaching `read(fd 0)`**. Fixed `pmm_alloc` top-down (`core/pmm.cyr`) at **1.41.12**. **Typing on iron: 1.41.13's i8042-poll falsified → 1.41.14 IRQ1+`sti` → `14114` burn READ the keyboard but every line collapsed to `Command: D` (stuck shift + the `d` key) with no echo → corrected 1.41.15.** Root cause: 1.41.14's per-byte read returned after the first char, and ring 3 runs IF=0 between syscalls (`ring3.cyr:108` RFLAGS=0x002), so the SHIFT-RELEASE break landing in that inter-byte gap was dropped — `kb_shift` latched at 1, `0x20`('d')→`'D'`. **1.41.15** makes `read(0)` a continuous-IF, **line-disciplined** reader (drains the whole line under one `sti` window, terminates on Enter) with **kernel-side echo + backspace** (`kputc`) — exactly the in-kernel recovery shell that types correctly on this box; ABI O1 revised RAW→canonical-lite (`agnoshi` 1.4.1 `read_line` = one line read). IRON-VALIDATED at burn `14115` (2026-06-06) → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). **The issue-doc's two open items are now CLOSED in 1.42.x**: the deeper page-map fragility (direction B, 1.42.2 — per-process PD no longer seeds the mmap arena present-supervisor) + the rare syscall-path RBP smash (1.42.3 — user RBP preserved across the kstack switch); both QEMU-repro-clean, iron rides the next burn → [issue doc](issue/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md), [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). That forward plan is **done**: typing iron-validated (`14115`), the deeper page-map fix (1.42.2) + the syscall RBP smash (1.42.3) both landed. The **boot-pipeline de-Linux cleanup** is **✅ done (C1, 2026-06-06)**: deleted the orphaned Linux-cpio/GRUB `install.cyr` + the pre-gnoboot `test-uefi-qemu.sh` GRUB harness + purged the stale `scripts/build/` GRUB/cpio artifacts; sovereign OVMF smoke is covered by `gnoboot/tests/ovmf_smoke.sh` + `scripts/qemu-fb-smoke.sh`; paired doc fixes landed (roadmap rows, path-c steps 10/11, iso-stage4 tree, install-media.sh header) → [`roadmap.md`](roadmap.md) (backlog *C1*). · gnoboot **0.5.0**. The per-arc status is the condensed ladder below; closed-arc forensic detail lives in CHANGELOG + the iron-log, not here (this file is **NOT A LOG**).
...
>
> - **1.39.x VFS generic-write lift — CLOSED.** The full shell verb set (`cat`/`ls`/`touch`/`echo >`/`rm`/`mkdir`/`rmdir`/`mv`/`sync`) reaches FAT/exFAT including subdirectories (bites 1–9, the `vfs_*_secondary` chains consolidated behind `vfs_secondary_select`). The mount-namespace ROUTING that completes it was deferred (wrongly called "complete") and landed at **1.40.13**. Detail → CHANGELOG `[1.39.x]`.
> - **1.40.x exec-from-disk arc — iron-validated through 1.40.13** (the `14013_final*` burn, 2026-05-31, validated four cuts in one boot: exec-from-disk 1.40.9 + scheduler-reset fix 1.40.10 + boot-stack relocation 1.40.12 + mount-namespace routing 1.40.13 — `/bin/prog2`+`/bin/argv` run in ring 3, FAT verbs work while ext2 owns `/`, clean through `Activating scheduler` → DHCP lease → `Launching kybernet`). The **1.40.14 closeout** (`proc_reap` teardown/reaping) is QEMU-only; its iron re-burn folds into the 1.41.x burn. Forensic detail → CHANGELOG `[1.40.8]`–`[1.40.14]` + iron-log.
> - **1.41.x shell-separation arc — ✅ IRON-COMPLETE (burn `14115`, 2026-06-06).** The interactive shell left the kernel: PID 1 `kybernet` execs the userland `/bin/agnsh` in ring 3 from the ext2 root; the in-kernel `shell()` shrank to a recovery REPL (**1149→813 LOC**); FAT/exFAT content-write reaches the syscall ABI (`VFS_SEC_WFILE`); the ring-3→ring-0 ingress is hardened (6-dim + 3-dim adversarial audits). All QEMU-green — sweep 7/7, `fssys`/`shsys`/`agnsh-smoke` PASS, check 11/11. Locks the permanent kernel↔userland shell boundary (kernel = recovery only, `agnsh` = full interactive); mirrors the 1.37.5 font→`kashi` move. **Iron burn #1 (`14111`) → ring-3-stdout serial-only fix at 1.41.12** (`serial_dev_write`→`kprint`). **Burn #2 (`14112`) → PMM `#PF` fixed (top-down `pmm_alloc`); re-burn reached the prompt, typing dead; 1.41.13 i8042-poll falsified → 1.41.14 `read(0)` = `sti`+`sched_active=0`+IRQ1 (QEMU 7/7). Burn #3 (`14114`) → keyboard READ but every line collapsed to a stuck `Command: D` (shift-release dropped in the ring-3 IF=0 inter-byte gap) with no echo → 1.41.15 makes `read(0)` continuous-IF line-disciplined + kernel-echo (mirrors the in-kernel shell), ABI O1 RAW→canonical-lite, `agnoshi` 1.4.1. **Burn #4 (`14115`, 2026-06-06) ✅ ARC HARDWARE-CLOSE — agnsh types + echoes + dispatches correctly on archaemenid: `help`/`mode`/`version` all worked at `[ASSIST] >` past a real DHCP lease (`ip=192.168.1.163`). The 1.41.x core claim (a userland AI shell, typeable on iron) is met.** → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). Detail → CHANGELOG `[1.41.15]` + [issue doc](issue/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md). **Two non-blocking residuals → 1.42.x:** (1) stale banner `agnoshi 1.4.0` — staged-binary staleness, NOT a code bug (source is 1.4.1; `build/rootfs/bin/agnsh` was a pre-cut 1.4.0 build, refreshed 2026-06-06); design follow-up = derive `VERSION_STR` from `VERSION` so a bump can't desync. (2) backspace dead on iron — `read(0)` erase works in QEMU (`ch==8`, `0x0E`→8) but not on this box; likely the UEFI USB-legacy i8042 emulation delivers a different code for Backspace; iron-only investigation.
>
...
>
> **Open carry-forward debt:** **1.37 ext4 extent-alloc** ✓ (iron-validated) and **1.38 jbd2 journaling** ✓ (CSUM_V3 write+replay at 1.38.10; QEMU-green + Linux-e2fsck-oracle-validated; **write-side + crash-recovery iron-validated at the `13810_*` re-burn**) are DONE. **1.39.x VFS generic-write lift = verb surface (1–7) + dispatch consolidation (8) + subdir paths (9); mount-namespace ROUTING landed at 1.40.13.** **✅ MOUNT-NAMESPACE FIXED (1.40.13) + IRON-VALIDATED (`14013_final*`, 2026-05-31) — FAT shell verbs pass while ext2 owns `/`.** The shell verbs previously routed by the `ext2_active` *flag*, not a mount table — so when ext2 was mounted (always, on iron — it carries the exec arc's `/bin/prog2`) every verb took the ext2 branch and FAT/exFAT were unreachable through the shell. The 1.40.13 FAT iron burn (`14013_boot_cmd_fails`) exposed it: raw `fatfs_*` (`fatw:`) tests all PASSED, every shell-routed `vfsw:`/`vfsu:` verb FAILED (`no such directory`/`no such parent`/`no such path` — strings that ONLY come from the ext2 branch); the QEMU smoke hid it by booting with no ext2 partition. **The real VFS routing 1.39.8's `vfs_secondary_select` comment tabled as "non-goal §5" now exists** — a `{prefix → backend}` mount registry + longest-prefix resolver (`vfs.cyr`: `vfs_mount_init`/`vfs_resolve_mount`/`vfs_*_on`): ext2 at `/`, FAT at `/mnt/fat`, exFAT at `/mnt/exfat`, **all live at once**. Every shell verb (`touch`/`echo`/`rm`/`mkdir`/`rmdir`/`mv`/`cat`/`ls`) resolves abspath → owning backend → dispatch; `sh_abspath` lazy-inits CWD to `/` (killed the second half of the iron failure — bare names with no leading slash). The 8 dead `vfs_*_secondary` helpers were removed. FAT/exFAT selftests now drive `/mnt/...` paths, so they reproduce the iron config (FAT-through-shell *while ext2 owns `/`*) the bare-name version couldn't. `sweep.sh` 7/7 (FAT/exFAT read+write+subdir, ext2 regression, exec — all green). **Bonus: removes the "VFS mounts only ONE secondary FS at a time" constraint — the FAT+exFAT iron verification can collapse from two burns to one** (both backends are now reachable via their mountpoints in a single boot). The `14013_final*` re-burn confirmed it on iron (`vfsw:` FAT verbs PASS with ext2 mounted at `/`). **1.40.14 carry-forward: process teardown/reaping** (`proc_reap`/`proc_free_address_space`, QEMU-validated, **iron re-burn pending** — rides the Track-B FAT/exFAT burn) closed the per-process-memory + proc-table-slot leak that capped interactive `run` at ~3 invocations; that follow-on is now **CLOSED at 1.42.4** (`proc_reap_child` reaps the in-memory `spawn`/`elf_load` procs + full-arena munmap-on-exit; QEMU reap-stable, iron rides the next burn). Kernel-slimming: console-font→`kashi` landed at **1.37.5**; **shell→agnoshi** is **iron-complete at 1.41.15** (burn `14115`, 2026-06-06; the in-kernel shell is now a recovery REPL) and gained userland FS verbs at agnoshi 1.4.2 / agnos 1.42.6. The user-driven **FAT/exFAT iron burn** (1.34.x arc's first iron touch; brick-safe via the non-ESP guard) is now folded into the 1.39.x VFS-lift burn rubric → [`#tracker-139-cycle`](iron-nuc-zen-log.md#tracker-139-cycle) (supersedes the older [`#tracker-1341-cycle`](iron-nuc-zen-log-mvp2.md#tracker-1341-cycle) pointer).
>
...
CYML — 6.0.x LEADING EDGE (the bulk of the ecosystem, ~32 repos):
  ...
  v6.0.43:  mabda (3.0.1)
  v6.0.53:  libro (2.7.1)
  v6.0.56:  agnos (1.43.6), agnosys (1.4.0),
