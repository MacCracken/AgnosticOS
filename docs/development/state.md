...
> **Last refresh**: 2026-05-26. **agnos kernel: 1.34.2 RELEASED** · gnoboot 0.4.2. Active arc = the **1.34.x FAT-family write-completeness continuation** (FS roadmap row 21). Done + `fsck`-clean in QEMU: **1.34.0** FAT12/16/32 read+write, **1.34.1** exFAT read+write, **1.34.2** exFAT write parity (overwrite / arbitrary-truncate / PercentInUse / ENOSPC); production build **772,568 B**. Queued: **1.34.3** FAT LFN/truncate completeness → **1.34.4** cross-boundary dir runs + root-dir extension → **1.34.5** exFAT Unicode names; arc cap = ESP-write guard + user-driven FAT/exFAT iron burn. **NEXT = 1.34.3.** Plan → [`#tracker-1341-cycle`](iron-nuc-zen-log.md#tracker-1341-cycle) + agnos roadmap row 21; per-cut detail → CHANGELOG `[1.34.0]`–`[1.34.2]`.
>
