...
>
> **Last refresh**: 2026-06-09. **Current versions: agnos 1.44.0 · agnoshi 1.5.0 · kriya 1.1.2 · owl 1.3.8 · anuenue 1.1.1 · cyrius-doom 0.28.3 · gnoboot 0.5.0.**
>
> **Open cycle — 1.44.x multi-threading / preemptive scheduling.** The deep cooperative→preemptive scheduler transition ([[project_multithreading_future_arc]]). **Opening (agnos 1.44.0):** `kthread_create` (kernel-thread primitive on the shared AS, generalizing the 1.40.10 idle proc) + a `preempt_count` gate in `do_context_switch` (`preempt_disable`/`preempt_enable` — the "reentrant-or-gated" foundation so shared scratch buffers / non-reentrant FS survive preemption). `agnos/scripts/thread-smoke.sh` QEMU-green: two kthreads preemptively time-slice (counters in the tens of millions) and freeze under `preempt_disable()`; `sweep` 7/7, production byte-identical. Absorbs the 1.43.x carry-forward (per-process env · FB scaling · zero-copy FB-mmap; first-`mmap` RIP=0 already repaired at 1.43.8). Next bites: per-process-AS ring-3 preemption · FS-scratch reentrancy gating · concurrent `spawn`+`waitpid` (DOOM while the shell stays live) · SMP-AP wakeup. **Userland testing surface (done):** the **kriya/owl coreutils delegation** — agnoshi **1.5.0** dropped its in-process verbs → kriya `/bin` dispatcher (11 `/bin/<verb>` symlinks) + owl `cat`; END-TO-END QEMU-green (`agnos/scripts/agnsh-delegation-test.py`), with two consumer fixes **kriya 1.1.2** + **owl 1.3.8** (the stale-`fnptr.cyr` allocator gap from a pre-6.1.14 cyrius pin; owl also `var r = main()` → bare-call). Iron-burn pending on archaemenid → [`#tracker-144x-cycle`](iron-nuc-zen-log.md#tracker-144x-cycle).
>
