...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–25 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)**; Attempts 17–25 worked the next-bug ladder. **Attempt 25 (2026-05-15)** result: **Repair (L) confirmed** — kcp=0x19 (mem-iso final stamp at main.cyr:639), PML4 stamps all `0x07` (was `0x04` in Attempt 24), PMM stamps all `0x32/0x33` (clean), CR4 byte 2 `0x30/0x30`. Mem-iso block (3 CR3 switches + 3 SMAP brackets + kernel-PT restore) fully clean. **New bug downstream — and FIRST RACY OUTCOME of the iron-boot ladder**: kernel reaches kcp=0x19 but NOT kcp=0x13 at main.cyr:661 (post-mem-iso). Death window is main.cyr:640–660 (21 lines: cp_fb + 8 serial_print/kprint_num calls + nested if/else branch). `cp_fb(0x19)` MAGENTA paint visible *intermittently* on otherwise-identical resets ([`iron-boot-photos/attempt-25-boot-colors-racy.jpg`](iron-boot-photos/attempt-25-boot-colors-racy.jpg)); rules out deterministic build/PT errors. Working suspect set: RFLAGS.AC residue, stack misalignment from asm-heavy CR3 dance, FB MMIO TLB-flush gap on restored CR3. **Repair (M) LANDED 2026-05-15** (kernel + read-boot-log): 4-stamp port-I/O bisector at lines 640/642/649/660 with stamp values 0xE1–0xE4 (225–228 dec), plus 4 new verdict branches in read-boot-log.cyr. Pure diagnostic, +32 bytes (254,800 → **254,832 bytes**), no behavior change. read-boot-log gained ~6 KB of verdict text (now 42,288 bytes). Pre-burn ask: capture CMOS from 2-3 consecutive resets to characterize the racy outcome distribution rather than a single point. **QEMU pre-flight still permanently blocked** → iron is the only test surface. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break (multiboot2 → sovereign boot-info struct); v1.30.1 candidate carries Repair F + H + I + J + K + L (and pending M). Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
...
*Refreshed 2026-05-15 (header + iron-boot row spot-updated for **Attempt 25 result — Repair (L) confirmed, first racy outcome of the iron-boot ladder, Repair (M) drafted but unlanded**. Earlier same-day spot-updates for Attempts 21–24 + Repairs (I)/(J)/(K)/(L) summarized in the iron-boot-row narrative — full per-attempt records live in [`iron-boot-testing-log.md`](iron-boot-testing-log.md).
