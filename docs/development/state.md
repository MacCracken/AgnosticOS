...
| D | **Iron Attempt 96 — validates FIX #7+#8+#9+#10 against Attempt 95 evidence base**. Target outcome: full DHCP cycle on archaemenid (DISCOVER → OFFER → REQUEST → ACK four lines). CMOS 0x5E flips from `0x01` (background multicast leftover) to `0xFF` (broadcast OFFER) OR `0xB0` (unicast OFFER to our MAC — unambiguous FIX #7 success signal). | Iron-only. | n/a | ⏭ **DEFERRED to user direction** per [[feedback_iron_burns_block_other_work]]. Build is current: `agnos/build/agnos` at 605,064 B production. Iron-burn checklist: `cd ~/Repos/agnosticos && sudo ./scripts/install-usb.sh --update` → boot archaemenid from USB → capture boot-log photo → `sudo ./scripts/read-boot-log.sh --verbose` for CMOS 0x58-0x5F decode. |
| E | **Cycle-close sweep + Attempt 96 transcript**. CHANGELOG cycle-close summary; state/roadmap/iron-log sweep; photo catalogue. | n/a (doc-roll). | n/a | ⏳ Pending Attempt 96 outcome. |
