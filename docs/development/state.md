...
| 3a | **USB Mass Storage (BBB + SCSI)** — *next* | Yes (any USB stick / HDD) | ~600-800 | 1.31.2 cycle scope. Extends the xHCI investment. SCSI READ(10) + WRITE(10) over Bulk-Only Transport. Linux `drivers/usb/storage/usb.c` reference; structurally simpler than xHCI itself (just bulk endpoints + a command opcode set). Third iron-validatable block backend. |
...
**Iron-validation coverage at each cut**: 1.31.0 → NVMe (Crucial P3). 1.31.1 → NVMe + SATA (WD Blue SA510). 1.31.2 → NVMe + SATA + USB MS + USB optical (HP BD). 1.31.3 → same + RAM-disk + VirtIO (QEMU). 1.31.4 → same + ext2 filesystem.
...
**Validation pending:** Next iron burn (1.31.2 or later) confirms the post-RW IDENTIFY hang no longer manifests on archaemenid + WD Blue SA510. Until then, the patches are QEMU-validated only — Linux-pattern fidelity gives high confidence, but iron remains the final gate.
