...
>
> **Open carry-forward debt:** **1.37 ext4 extent-alloc** ✓ (iron-validated) and **1.38 jbd2 journaling** ✓ (CSUM_V3 write+replay at 1.38.10; QEMU-green + Linux-e2fsck-oracle-validated; **write-side + crash-recovery iron-validated at the `13810_*` re-burn**) are DONE. **1.39.x VFS generic-write lift = NEXT** — where the FAT/exFAT shell verbs (`touch`/`rm`/`mkdir`/`cat`/`ls`) + two writable FSes finally earn the abstraction. Kernel-slimming: console-font→`kashi` already landed early at **1.37.5** (retired the planned 1.40.x cycle); **shell→agnoshi** remains. Also outstanding: the user-driven **FAT/exFAT iron burn** (1.34.x arc's first iron touch; guard makes it brick-safe; [`#tracker-1341-cycle`](iron-nuc-zen-log-mvp2.md#tracker-1341-cycle)).
>
