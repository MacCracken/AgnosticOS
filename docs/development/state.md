...
That graduation event triggers the user's planned hardware migration: remove the USB NVMe dev drive, internal NVMe becomes archaemenid's AGNOS primary; the freed 2TB SATA drive consolidates with the existing 2TB in the Intel i9 ("Big boy") machine. The i9 then becomes the Intel-target iron with consolidated 4TB capacity. Captured durably in [[project_hardware_catalog]] + [[project_archaemenid_install_plan]].
