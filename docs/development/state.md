...
| 3a | **USB Mass Storage (BBB + SCSI)** — *in flight* | Yes (any USB stick / HDD) | ~600-800 | 1.31.2 cycle scope. **Phase 1 landed [Unreleased] 2026-05-20 PM** (class-interface discovery + bulk-EP enumeration + GET_MAX_LUN; ~290 LOC at `kernel/arch/x86_64/usb/msc.cyr`; QEMU-validated against `-device usb-storage,bus=xhci.0` — first-try clean). Phase 2 (bulk Normal TRBs + CBW/CSW transport) + Phase 3 (SCSI INQUIRY / TEST UNIT READY / READ CAPACITY(10)) + Phase 4 (READ(10) / WRITE(10) + `blk_register_usb_ms` registration) next. `BLK_USB_MS=4` constant reserved in `block.cyr`. |
