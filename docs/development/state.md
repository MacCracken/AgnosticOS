...
#### USB MS iron carry-forward — Phase 2.5 ✅ + Phase 2.6 ✅ landed but FALSIFIED at Attempt 85 + Phase 2.7 multi-source-converged repair landed in agnos 1.31.2 `[Unreleased]`; Attempt 86 burn pending — 1.31.2 close gated on result
...
| **2.7** | **Reset Recovery step reorder: device-side first** | ✅ Landed agnos 1.31.2 `[Unreleased]`; pending Attempt 86 | Full `msc_reset_recovery` rewrite. New order: BOT Reset → 100ms stall → CLEAR_FEATURE×2 → drain events → Stop Endpoint×2 → Reset Endpoint×2 (Halted-gated) → ring rewind → Set TR Dequeue×2. Matches convergent reference ordering. |

**Attempt 86 success rubric** in [`msc-reset-recovery-prior-art.md` § 9.4](msc-reset-recovery-prior-art.md). Per [`feedback_iron_burns_block_other_work`](../../../../.claude/projects/-home-macro-Repos-agnosticos/memory/feedback_iron_burns_block_other_work.md): § 9 IS the pre-burn audit; no further engineering proposed until Attempt 86 iron evidence lands.

**MVP gate posture**: unaffected — `msc_probe_slot` already returns 1 even on transport failure, so MVP boot-to-shell stays green regardless of Attempt 86 outcome. USB MS arc is opportunistic; closed beta depends on kybernet + agnoshi + kernel-on-iron, not on a specific block backend.

**1.31.2 close gating**: agnos VERSION stays at 1.31.2; `[Unreleased]` accumulates AHCI carry-forward + cyrius pin graduation + USB MS Phase 1-4 + Phase 2.5 + Phase 2.6 (FALSIFIED Attempt 85) + Phase 2.7. **Close decision waits on Attempt 86 iron evidence** — user calls when to burn, then decides whether to ship 1.31.2 as-is, fold the Attempt 86 result into the close, or extend with another phase.
