...
> **Last refresh**: 2026-05-15 (captures iron-boot Attempts 4–27 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)**; Attempts 17–27 worked the mem-iso next-bug ladder. **Repair (O) LANDED 2026-05-15** — structural correction after re-reading `docs/development/uefi-boot-prior-art.md`: the entire mem-iso test block at `main.cyr:383-685` (303 lines: AS1/AS2 build + CR3 dance + SMAP brackets + verify prints + all Repairs F-N bisector stamps) was deleted. Per prior-art §6: *"No loader hands off 'proper' kernel page tables — the firmware's identity map is the contract."* The mem-iso test was post-MVP verification work building AS1/AS2 PTs and switching CR3, then restoring to a kernel-built PT that doesn't reliably cover the GOP framebuffer BAR the way UEFI's identity map did. fb_phys=0 (Attempt 27's "smoking gun") was a red herring — even non-zero fb_phys would have faulted on the restored kernel PT. 9 repair letters (F-N) chased symptoms inside a test block that isn't on the boot-to-shell critical path. agnos kernel: 255,048 → **253,496 bytes** (-1,552). Attempt 28 pending USB reflash — outcome is binary: kernel either advances past kcp=0x12 (post-VFS marker) into the userland exec test, or it pegs at 0x12 (latent issue we hadn't seen). **QEMU pre-flight still permanently blocked** → iron is the only test surface. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break (multiboot2 → sovereign boot-info struct). Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
...
*Refreshed 2026-05-15 (header + iron-boot row spot-updated for **Repair (O) landing: mem-iso test block deleted from main.cyr after re-reading uefi-boot-prior-art.md confirmed it was post-MVP work breaking pre-MVP boot. Kernel: 255,048 → 253,496 bytes. Attempt 28 burn pending.**. Earlier same-day spot-updates for Attempts 21–27 + Repairs (I)/(J)/(K)/(L)/(M)/(N)/(O) summarized in the iron-boot-row narrative — full per-attempt records live in [`iron-boot-testing-log.md`](iron-boot-testing-log.md).
