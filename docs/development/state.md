...
| 4 | **RAM-disk backend** | No (RAM-only — no iron variable) | ~150 | 1.31.3 cycle scope. Pure-RAM `/dev/ram0`-equivalent over `pmm_alloc` + tag-dispatch. Useful for tests + initrd-style workflows. |
| 5 | **VirtIO-blk modern (1.x)** | QEMU only (VirtIO doesn't exist on bare metal) | ~600 delta | 1.31.3 cycle scope. Upgrade existing transitional 0.9.5 `virtio_blk.cyr` to MMIO + feature negotiation. Keeps QEMU first-class on modern machine types. |
| 6 | **ext2 read-only** | Yes (filesystem layer; consumes GPT + block) | ~1000+ | 1.31.4 cycle scope (displaced from 1.31.3). Filesystem class, not device. FAT16 read-only is the floor; ext2 buys real Linux disk semantics (inodes, indirect blocks). |

**Order through 1.31.x**: ~~GPT~~ ✅ → ~~AHCI/SATA~~ ✅ → **USB MS + Optical via USB MS** (1.31.2; AHCI carry-forward patches already landed in `[Unreleased]`) → **RAM-disk + VirtIO-blk modern** (1.31.3) → **ext2 read-only** (1.31.4).

**Iron-validation coverage at each cut**: 1.31.0 → NVMe (Crucial P3). 1.31.1 → NVMe + SATA (WD Blue SA510). 1.31.2 → NVMe + SATA + USB MS (Attempt 83 surfaced TUR-gate gap; Attempt 84 validated Phase 2.5 device-side recovery — execute-clean but stick stays wedged; Phase 2.6 controller-side commands pending) + USB optical (planned). 1.31.3 → same + RAM-disk + VirtIO (QEMU). 1.31.4 → same + ext2 filesystem.
...
#### USB MS iron carry-forward — Phase 2.5 ✅ landed + iron-validated; Phase 2.6 controller-side now justified

Third storage-class iron debut (after NVMe at Attempt 80 + SATA at Attempt 81). Phase 2.5 device-side stack (Reset Recovery + 3-retry TUR loop + INQUIRY hoist + REQUEST SENSE) shipped in `[Unreleased]` commits `90367d5` + `18bd2bd`, and validated on iron at **Attempt 84 2026-05-21** against a Silicon Motion / SMI commodity stick (`VID=0x090C PID=0x1000`, USB 2.0 / HS). All four Phase 2.5 patches fired as designed on real silicon — `msc: slot 2 Reset Recovery OK` × 3, per-step diagnostics (`data phase timeout` / `CSW transfer timeout` / `CBW transfer timeout`) replacing Attempt 83's misleading "(CSW status != 0)", INQUIRY attempted before TUR, retry loop ran to exhaustion. But the device transport stayed wedged across all three Reset Recovery cycles — each post-recovery transfer wedged at a *different* step, and a CSW tag mismatch immediately before the first recovery proved the device's bulk-IN buffer was not drained by BBB Reset.

**Phase 2.6 trigger condition met.** State.md / `usb-ms-iron-burn-audit.md` § 5 / Attempt 83 carry-forward all anticipated this: "if `xhci: transfer event timeout` repeats on a post-Reset-Recovery retry, Phase 2.6 adds the xHCI command-level half." Iron has now provided that evidence. Full prior-art walk + four-patch fix stack in [`msc-reset-recovery-prior-art.md`](msc-reset-recovery-prior-art.md).

| Item | Resolution | Mechanism |
|---|---|---|
| Bulk-IN transport wedged: Step 4 CSW receive timeout (Phase 2.5 primary) | ✅ Landed `90367d5`/`18bd2bd`, iron-validated Attempt 84 (executes cleanly; doesn't unwedge this stick) | `msc_reset_recovery` per BBB §6.7.3 + host-side ring rewind. |
| TUR retry loop (Phase 2.5) | ✅ Landed + iron-validated | 3-retry loop in `msc_probe_slot:348-371`, `msc_reset_recovery` between failed attempts. |
| INQUIRY hoist out of TUR-pass gate (Phase 2.5) | ✅ Landed + iron-validated | `msc_inquiry` runs unconditionally after Configure Endpoint per SPC-4 §6.6 (`msc.cyr:320-342`). RC10 stays inside TUR-pass branch per SBC-3 §5.10. |
| REQUEST SENSE decoder (Phase 2.5) | ✅ Landed; not exercised on Attempt 84 (transport never recovered enough for SENSE to issue) | `msc_request_sense` helper, called after all TUR retries exhausted. |
| **Controller-side EP recovery: Reset Endpoint (TRB 14) + Stop Endpoint (TRB 15) + Set TR Dequeue Pointer (TRB 16) — Phase 2.6 primary** | 🔄 Justified by Attempt 84; pending engineering | Three new TRB type constants + three helpers in `xhci_cmd.cyr`. Rewritten `msc_reset_recovery` issues controller-side commands *before* device-side control transfers + a Set TR Dequeue Pointer for each direction after host-side ring rewind. Detail: `msc-reset-recovery-prior-art.md` § 6.4. |
| Event-ring drain of stale Transfer Events (Phase 2.6 sibling) | 🔄 Justified by Attempt 84 (CSW tag mismatch traces to stale event consumption) | `xhci_drain_transfer_events(slot_id)` helper, called from `msc_reset_recovery`. `msc-reset-recovery-prior-art.md` § 5.1 / § 6.3. |
| `msc_bulk_enqueue` cycle not stored back on wraparound (latent) | 🔄 Fold into Phase 2.6 while file is open | One-line: store `cycle` back to `row + cycle_off` after toggling. `msc-reset-recovery-prior-art.md` § 5.2. |
| Link TRB cycle never updates (latent) | 🔄 Fold into Phase 2.6 while file is open | Rewrite Link TRB cycle bit in `msc_bulk_enqueue` wraparound branch. `msc-reset-recovery-prior-art.md` § 5.3. |

**Phase 2.6 audit gate** before next iron burn: `msc-reset-recovery-prior-art.md` § 6.7 checklist (TRB-type constant usage, Context State Error tolerance on Stop Endpoint, DCS bit in Set TR Dequeue parameter, controller-then-device ordering, event-ring wraparound, no iron-invisible diagnostics, fmt-clean).
