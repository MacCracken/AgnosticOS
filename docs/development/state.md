...
> **Last refresh**: 2026-05-17 — **Attempt 60 burned + FALSIFIED (HH/JJ/KK/LL stack ineffective); Attempt 61 staged with Repair (MM) — MSI-X PCI Function Mask cleared, leave Enable=1.** Attempt 60 FB: `xhci: cmd completion timeout, final_idx=1 cycle=1 events_seen=0` survives the bundled doorbell-flush + universal-readback-flush + CNR-poll + Link-TRB-cycle-fix stack. Triage decode: every readback present (no `xhci: CNR never cleared` line means KK passed; CRCR.CRR=0/IMAN=2/ERDP_lo=5316616 readback line confirms JJ writes succeeded; `cmd_submit#1 dw3=9217` confirms HH ran). The cmd-path silent-absorb arc is now narrowed to **one remaining user-validated behavioral hypothesis**: MSI-X Function Mask interfering with internal interrupter state-machine progress on AMD FCH 1022:1639 (PCI 3.0 §6.8.2 reads Function Mask as suppressing message TX only, but AMD silicon may interpret it as a stronger gate). **Repair (MM)** = `pci_enable_msix_unmasked` (new in `kernel/core/pci.cyr:191-238`) writes MSI-X control with Function Mask=0 and Enable=1; per-vector mask defaults to 1 (PCI 3.0 §6.8.2.5.3) so no spurious MSI-X messages will be transmitted. Pure-additive function + one call-site swap in `xhci_probe` (line 112 changed; FB literal updated to `xhci: MSI-X enabled (no function-mask)` for flash-proof). Build verified: `build/agnos` **368,472 B** (+528 B over Attempt 60's 367,944 B), ELF64 multiboot2 entry `0x1000a8`, new literal present, prior literal absent. **Iron status**: Phase 3 reset on port 3 still UNBLOCKED (Repair EE intact across Attempts 55-60). No VERSION bump — 1.30.6 banner retained through MM. **Attempt 61 awaits user-side USB flash + per-action burn approval (per `feedback_per_action_consent`, Attempt 60's approval does NOT cascade).** Decoupling decision: if MM falsifies, the cmd-path gate gets escalated as a controller-firmware issue or pivoted to a different bare-metal target — no `Repair (NN)` letter ladder; Phase 4/5 continues in parallel. Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) §§ Attempts 56-61. Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md) § 1.30.6 + [Unreleased]. **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
...
| 8 | **Iron-boot bring-up (NUC AMD archaemenid)** | 🔄 Phase 3 unblocked Attempt 55 (Repair EE); Phase 4 cmd-path silent-absorb open after 5 falsified arcs (FF/GG/HH/JJ/KK/LL Attempts 57-60); Attempt 61 staged with Repair (MM) — MSI-X Function Mask=0 | Closed-beta gate is boot-to-shell on iron with typeable keyboard. **Phase-3 silent-absorb arc closed 2026-05-17 Attempt 55** — root cause was `xhci_portsc_write` inner re-mask `& XHCI_PORTSC_NEUTRAL` stripping the RW1S PR bit; one-line fix in agnos@41ee6dc. 13 falsified hypotheses chasing a homegrown bug. **Phase-4 cmd-path silent-absorb open**: after 5 Linux-prior-art behavioral repairs (Attempts 56-60), Enable Slot's Command Completion Event still never posts to the event ring — `events_seen=0` survives FF (IMAN.IE=1) + GG (AMD-Vi disable) + HH (doorbell readback flush) + JJ (universal readback flush) + KK (CNR poll) + LL (Link TRB cycle init fix). **Repair (MM) staged Attempt 61 (2026-05-17)**: PCI MSI-X Function Mask cleared, Enable=1 (`pci_enable_msix_unmasked` new in `kernel/core/pci.cyr:191-238`, call-site swap in `xhci.cyr:103-115`). Per-vector mask defaults to 1 (PCI 3.0 §6.8.2.5.3) so spurious MSI-X messages remain suppressed. Hypothesis: AMD FCH 1022:1639 interprets Function Mask as a stronger gate than PCI spec implies — suppressing internal interrupter state-machine progress (DMA event posting), not just message TX. Build verified: `build/agnos` **368,472 B** (+528 B), ELF64 multiboot2, `xhci: MSI-X enabled (no function-mask)` literal present. **Decoupling decision**: if MM falsifies, no `Repair (NN)` letter ladder — cmd-path gate gets escalated as controller-firmware issue or pivoted to different bare-metal target; Phase 4/5 continues in parallel. agnos cyrius pin remains 5.11.59. **kriya 0.6.0** shipped 2026-05-17 (M5 closeout: grep + find + xargs; cyrius pin 5.11.59) — three milestones past the Attempt 54 gate version (0.3.0); gate cleared well in advance. |
