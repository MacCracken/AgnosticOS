...
>
> **Active iron work — r8169 RX of UNICAST frames on archaemenid (NUC AMD).** TX, broadcast/multicast RX, and L2 gateway reachability are all PROVEN on iron. The **unicast MAC filter is SOLVED** — 1.32.7 bite-3 read the chip's own silicon accept-counter (`rx_uc=2`), proving the physical-match filter accepts unicast and closing the entire L2 accept/filter arc. **Current blocker: RX ring/poll delivery** — `missed=158` (RxMissed FIFO-overflow) with `rx_err=0 align=0` (clean frames dropped for no free descriptor); `net_poll` pulled one frame per call and the connect loop polls once per `hlt` tick. **Fix LANDED + burn-ready (bite-4): whole-ring drain in `net_poll`** (FreeBSD `re_rxeof` / iPXE `realtek_poll`; build 624,056 B, `test.sh` 4/4 + `ext2-smoke.sh` 5/5) — awaiting iron burn; read path (`tcp_find_conn` / `net_handle_tcp` SYN+ACK) verified sound first. Queued next lever if `missed` persists: deepen the RX ring 16→64. Per-cycle trackers + per-attempt narrative live in the log: [`iron-nuc-zen-log.md#tracker-1327-cycle`](iron-nuc-zen-log.md#tracker-1327-cycle).
>
