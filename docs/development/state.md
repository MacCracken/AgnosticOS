...
> **Last refresh**: 2026-05-25. **agnos kernel 1.32.7 OPEN** · cyrius 6.0.1 · gnoboot 0.4.2.
>
> **Active iron work — r8169 RX of UNICAST frames on archaemenid (NUC AMD).** TX, broadcast/multicast RX, and L2 gateway reachability are all PROVEN on iron. The **unicast MAC filter is SOLVED** — 1.32.7 bite-3 read the chip's own silicon accept-counter (`rx_uc`>0), proving the physical-match filter accepts unicast and closing the entire L2 accept/filter arc. **Blocker: RX ring/poll DELIVERY** — clean unicast frames (`rx_err=0 align=0`) dropped for no free descriptor (`missed`). **bite-4 (whole-ring drain in `net_poll`) BURNED → FALSIFIED 2026-05-25** (`1327_TCP_FAIL_Again.jpg` + user pcap/server-log verdict: `UNICAST RX of SYN+ACK : 0` at an owned/logged endpoint, no gateway/NAT in path): `missed` went **158 → 176 (UP)** — a 64-frame drain budget can only pull the **16 descriptors that physically exist**, so a LAN-chatter burst still overflows between `hlt`-spaced polls. **bite-5 (deepen RX ring 16 → 64, RX/TX mask split) LANDED + burn-ready** (build 624,488 B, `test.sh` 4/4 + `ext2-smoke.sh` 5/5) — awaiting iron burn; 16 was an outlier vs all prior art (Linux/FreeBSD/OpenBSD 256). If `missed` still high at depth 64, the SYN+ACK drop is NOT a connect-window overflow → re-baseline to chip-side delivery or `tcp_connect` poll-cadence. Per-cycle trackers + per-attempt narrative live in the log: [`iron-nuc-zen-log.md#tracker-1327-cycle`](iron-nuc-zen-log.md#tracker-1327-cycle).
>
