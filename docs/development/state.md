...
> **Last refresh**: 2026-05-31. **agnos kernel: 1.40.9** · gnoboot 0.4.2. **The 1.40.x exec arc is QEMU-COMPLETE but the iron burn (1408) FAILED its dispositive row.** The combined VFS+exec burn went **3/4 first try** — A1 boot/mount + full ext2-write suite + jbd2-clean + A2 ENOEXEC all PASS — but **A3 (the `/bin/prog2` ring-3 run) spontaneously RESET the box** on real Zen. CMOS bisection (`CMOS[0x50]=0x20`, the pre-iretq stamp) proved the **load path fully succeeded**; the fault is the **ring-3 transition itself** on Zen (QEMU `-cpu max` passes the same binary 7/7). **1.40.9 OPEN — the silent reset is its own bug:** the IDT bare-iretq'd every vector → any transition fault triple-faulted → reset. 1.40.9 installs real #UD/#DF/#TS/#NP/#SS/#GP/#PF handlers that stamp the fault vector to `CMOS[0x54]`+magic`0x55`=0xE5 then halt (QEMU 7/7 / sweep 6/6 / check 11/11). **1.40.9 RE-BURN DONE 2026-05-31 (boot `1409_boot_lock`): THE HANDLER WORKED** — box HALTED at `exec: running /bin/prog2` (no reset); `CMOS[0x55]=0xE5` + `CMOS[0x54]=0x0E` → **vector 14 = #PF** (rubric row 159). The Zen exec fault is now a **readable page fault on the first user VA after the ring-3 `iretq`**. Static audit RULES OUT the textbook cause (intermediate-level U/S is set on every level of both the boot + per-process CR3; code/stack map `0x87` on 2 MB-aligned frames) — localizing needs **CR2 + the #PF error-code**, which the existing handler is ~3 instructions from capturing. **Next burn = a fork** (CR2-capture vs behavioral-stack — recommend CR2-capture). **Both fixes shipped 2026-05-31 (QEMU-green, folded into open 1.40.9):** (1) **CR2+errcode capture** in the #PF handler (`CMOS[0x5C-0x5F]` = CR2 bytes + #PF error code, via the proven 0x70/0x71 bank); (2) **ext2 duplicate-dirent guard** — the 1409 `e2fsck` came back **DIRTY** (dup 'prog2'/'w3a.txt' ×5, "Filesystem still has errors"); `ext2_create`/`ext2_mkdir` lacked the existence guard `ext2_link`/`ext2_rename` already have, so each boot appended a same-name dirent (the real cause of the ext2w MISMATCHes — my earlier "not corruption" read was wrong). exec-smoke 7/7 · ext2-write 19/19 · check 11/11; flash-ready `build/agnos` = 1,042,176 B. **NEXT: clean the agnos-fs (re-mkfs+re-seed or `e2fsck -fy`) → CR2 re-burn** (a corrupt dup `/bin/prog2` could make the #PF a corruption artifact, so clean first). Detail → [`iron-nuc-zen-log.md` § 1409 burn entry](iron-nuc-zen-log.md) + [`#tracker-140-cycle`](iron-nuc-zen-log.md#tracker-140-cycle). **base-maturity close now gated on the 1.40.9 re-burn** (FS-crash-safe ✓ leg done; exec-from-disk leg proven in QEMU, NOT yet on iron). The 1.39.x VFS generic-write lift is functionally COMPLETE (verb surface + subdirs; Track B was not exercised — the A3 reset ended the boot first; rides the 1.40.9 re-burn). 1.37–1.38 big-write arcs closed + iron-validated. Arc ladder since the last refresh:
