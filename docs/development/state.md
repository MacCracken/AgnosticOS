...
> **Last refresh**: 2026-06-04. **agnos kernel: 1.41.11** — the **shell-separation arc (1.41.1→1.41.11) is software-complete** and QEMU-validated: the interactive shell is now the userland `agnsh` exec'd from disk in ring 3, the in-kernel shell is a recovery REPL, FAT/exFAT content-write reaches the syscall ABI, and the ring-3→ring-0 ingress is hardened. **Iron burn PENDING** → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). · gnoboot **0.5.0**. The per-arc status is the condensed ladder below; closed-arc forensic detail lives in CHANGELOG + the iron-log, not here (this file is **NOT A LOG**).
...
CYML — 6.0.x LEADING EDGE (the bulk of the ecosystem, ~32 repos):
  ...
  v6.0.53:  libro (2.7.1)
  v6.0.56:  agnos (1.41.11), agnoshi (1.3.5), kybernet (1.3.3),
            argonaut (1.8.2)
  v6.0.61:  sigil (3.7.1)
...
| **agnos** | **1.41.11** | **6.0.56** | Kernel. The 1.41.x shell-separation arc is software-complete (full ladder in the header above); pin moved 6.0.14 → 6.0.56 at 1.41.4 (`CYRIUS_TARGET_AGNOS`). Build 1,070,720 B. Iron burn pending → [`#tracker-141x-cycle`](iron-nuc-zen-log.md#tracker-141x-cycle). |
