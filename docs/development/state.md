...
> **Last refresh**: 2026-05-14 night (captures iron-boot Attempts 4–18 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 hit closed-beta gate (CP 0x11)** ; Attempts 17–18 landed on the next bug. **Attempt 18 = ROOT CAUSE FOUND**: CR4.SMAP=0 on iron. CMOS[0x54] CR4-byte-2 dump read 0x00, proving the Path C (`#ifdef ELF64_KERNEL`) boot shim *deliberately defers* CR4 tuning to "kernel re-tunes later" — and no kernel-side code ever actually re-tunes. The legacy ELF32 shim's CPUID-gated SMEP/SMAP enable (boot_shim.cyr:70-88) only runs under `#ifndef ELF64_KERNEL`. Without CR4.SMAP=1, `stac` at main.cyr:471 is `#UD` → `#DF` → triple-fault, which is exactly the Attempts 16/17/18 stall profile (kcp pins at 0x18, reset ~1-2s later). The main.cyr comment claiming "CR4.SMAP set in the boot shim, line 52" was stale w.r.t. Path C. **Repair (F) landed 2026-05-14 night**: (1) mirror legacy CPUID-gated SMEP+SMAP enable into Path C shim post-boot_info_capture_rdi (existing CP 5 moved to *after* the CR4 block; kcp=5 now means "boot_info_capture_rdi + CR4 enable both done" — avoids CMOS collision with main.cyr:36's 0x06); (2) refresh stale main.cyr comment to reference both shim sites; (3) add second CR4 byte-2 dump at CMOS[0x55] right before stac at main.cyr:471 (post-cr3-load comparison point); (4) read-boot-log gains [0x55] readout + decode-hint expansion + refreshed kcp=4 and kcp=24 verdicts. Kernel **253,936 → 254,000 bytes** (+64). read-boot-log 31,552 → 32,376 bytes (+824). **QEMU pre-flight still permanently blocked for this kernel** → iron is the only test surface. **Next gate (Attempt 19): kcp advances past 0x18 with CMOS[0x54]+[0x55] both = 0x30 (SMEP|SMAP).** If kcp still pins at 0x18 with [0x54]=[0x55]=0x30, cp_fb-under-AS1 is the next bug. If [0x54]=0x30 but [0x55]=0x00, CR4 got cleared in the cr3_load→stac window. Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct; v1.30.1 candidate adds the Path C CR4 enable. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
