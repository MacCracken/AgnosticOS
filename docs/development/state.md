...
> **Last refresh**: 2026-05-17 — **Attempt 55 BROKE THE SILENT-ABSORB ARC. Repair (EE) — `xhci_portsc_write` was re-masking value with `XHCI_PORTSC_NEUTRAL` inside the helper, stripping PR (RW1S bit 4) on every port-reset write across Attempts 32-54 (13 falsified hypotheses, 5 days).** Caller already pre-composed `(NEUTRAL \| PR)`; helper's `& NEUTRAL` silently killed PR before `store32` hit the controller. EDK2 XhciDxe + Linux xhci-hub.c diff surfaced it — neither re-masks. **agnos@41ee6dc one-liner fix**: `store32(addr, (value & XHCI_PORTSC_NEUTRAL) | (w1c_clear & XHCI_PORTSC_W1C))` → `store32(addr, value | (w1c_clear & XHCI_PORTSC_W1C))`. Cyrius pin bumped 5.11.55 → 5.11.59 in same commit. **Iron evidence (Attempt 55)**: `CMOS[0x64]=0x04` — port 3 reset-OK, first non-zero in arc. **New gate: Phase 4 Enable Slot returns `ccode=0`** (no Command Completion Event consumed within `XHCI_CMD_TIMEOUT_SPINS`; `xhci_last_cmd_ccode` default 0 surfacing as the printed value). Per-attempt detail in [`iron-nuc-zen-log.md`](iron-nuc-zen-log.md) § Attempt 55. **Iron status**: Phase 3 (port reset) UNBLOCKED on archaemenid USB2 bank port 3 — 13-hypothesis silent-absorb arc closed as a homegrown bug, not silicon. Phase 4 (Enable Slot command-ring round-trip) is the new gate. **Phase 4/5 code surface unchanged** (1.30.5 absorbed `hid_kbd_configure` + `hid_poll` + HID-usage→PS/2-set-1 + `kb_buf` writer, kernel 350,272 B → 364,736 B; FB renders `hid: keyboard layer initialized` line cleanly per Attempt 54 baseline). CMOS `[0x86]=0x5A` / `[0x87]=0xA5` reproduced the FCH 1022:1639 extended-CMOS-offset-≥6 alias quirk from Attempt 52 — confirmed firmware-managed, not a code regression. 1.30.4 closeout (H1-H4 spec-discipline) and 1.30.5 Phase 4/5 landing remain valid. **Next enabler**: root-cause Enable Slot ccode=0 (printed value is `xhci_last_cmd_ccode` default 0 because no matching Command Completion Event consumed within `XHCI_CMD_TIMEOUT_SPINS`; classes to triage in xhci_cmd.cyr / xhci_ring.cyr / xhci.cyr — CRCR program order vs R/S, cmd-ring cycle bit on first TRB, event-ring drain interaction with PSC events firing during PR-engaged port reset). Per-repair detail in [`agnos/CHANGELOG.md`](https://github.com/MacCracken/agnos/blob/main/CHANGELOG.md). **Cyrius toolchain**: 5.11.59 active — v5.11.x cycle theme + per-patch detail in [`cyrius/CHANGELOG.md`](https://github.com/MacCracken/cyrius/blob/main/CHANGELOG.md).
