...
> **Last refresh**: 2026-05-14 night (captures iron-boot Attempts 4–16 on the **NUC AMD** primary target — Intel/Skytech queued post-AMD-proof, not concurrent. **Attempt 16 = MAJOR PROGRESS, closed-beta gate (CP 0x11) HIT**: Repair (C) sufficient. iron showed 2 new MAGENTA cells past Attempt 15's CYAN cluster — cell 0x11 MAGENTA (full 50-hlt scheduler cycle done) + cell 0x12 MAGENTA-overwrite (post VFS/initrd/memfile tests). CMOS reads kcp=0x12 but the verdict text was misleading — *visual ladder disambiguates*: CYAN at 0x12 = iter-1 idle stall, MAGENTA at 0x12 = post-VFS. Symptom class flipped: Attempts 14/15 soft-locked; **Attempt 16 resets ~1-2s after the last MAGENTA paint** — that's a triple-fault, not a hang. New fault domain: memory-isolation test (`main.cyr:393-478`) — likely the first `cr3_load(as1)` if AS1's PML4 doesn't have kernel.text mapped, or the SMAP-bracketed access if stac isn't taking effect on iron. **Repair (D) landed 2026-05-14 night**: 4 bisector CPs (0x16/0x17/0x18/0x19) instrumented across the AS-create / AS-map / first-CR3-switch / full-restore boundaries. Kernel **253,712 → 253,824 bytes** (+112). read-boot-log binary 28,080 → 29,504 bytes (+1,424) with kcp=18 verdict rewritten to reference visual-ladder disambiguation + 4 new verdicts (kcp 22-25). **QEMU pre-flight still permanently blocked for this kernel** → iron is the only test surface. **Next gate (Attempt 17): cells 0x16-0x19 painted MAGENTA + cell 0x13 MAGENTA-overwrite = memory-isolation test passed.** Path A (GRUB MB2-EFI) blocked at OVMF strict-W^X; **Path C (sovereign UEFI / gnoboot)** is MVP. agnos 1.30.0 ships kernel ABI break: entry contract switches from multiboot2 to AGNOS sovereign boot-info struct. Both `agnos/cyrius.cyml` and `agnosticos/scripts/cyrius.cyml` pin **5.11.55**.) | **Refresh cadence**: bundle with each v5.11.x patch close, full sweep when minor cuts
