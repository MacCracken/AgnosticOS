...
| Field | Value |
|-------|-------|
| Status | **✅ COMPLETE — [tentib](https://github.com/MacCracken/tentib) 1.0.0 STABLE** (2026-07-06; the one-day 0.4.1→1.0.0 arc after cyrius 6.4.6/6.4.7 answered the filed int-SIMD proposal with `iv_dp8`). All three proof-of-life checks landed: the ternary transformer **trains** (M2), the integer kernel **reproduces the model** bit-identically (M3 + the 0.4.1 SIMD kernel, 5–18× the f64-SIMD matmul, growing with layer size), and **the multiply is measurably gone AND faster** (0.5.0 benchmarks under B-series fairness; pack-once serving ~13.5k tok/s = 5.7× f64 whole-model; honest quality delta vs matched-size f64 attn11 recorded, b1.58 ≥3B parity cited). API frozen (`docs/api.md`), audited (`docs/audit/2026-07-06-audit.md`), suite 101/101. Post-1.0 = user-driven levers (GPU/mabda, hoosh/murti serving of `tx_fwd_packed`, rotation-PTQ, NEON on cyrius Phase 5). This doc is now the historical design record. |
