...
| agnosys | 1.3.2 | Kernel interface — Landlock, seccomp, syscall bindings (Cyrius, GitHub-release only) |
...
| Crate | Version | Domain |
|-------|---------|--------|
| [sandhi](https://github.com/MacCracken/sandhi) | 1.4.1 | Service-boundary layer (HTTP client+server, HTTP/2, streaming, JSON-RPC, service discovery, TLS policy). **Folded into Cyrius stdlib at v5.7.0** as `lib/sandhi.cyr` (vendored byte-identical, 376,037 B / 9,649 lines / 469 fns). Sandhi repo entered maintenance mode per [ADR 0002](https://github.com/MacCracken/sandhi/blob/main/docs/adr/0002-clean-break-fold-at-cyrius-v5-7-0.md); subsequent surface patches land via Cyrius release cycle. |
...
| [cyrius-doom](https://github.com/MacCracken/cyrius-doom) | 0.27.5 | DOOM engine in Cyrius — hardened, 5 CVEs fixed, 2.59ms/frame | standalone game / kernel demo |
...
|---------|---------|-------------|---------------|
| [attn11](https://github.com/MacCracken/attn11) | 0.5.1 | **GPT-style transformer — *trained*** — from scratch in Cyrius. Char-level: tokenizer → token+positional embeddings → pre-norm causal multi-head self-attention blocks (LayerNorm → MHSA → residual → LayerNorm → GELU MLP → residual) → weight-tied LM head → softmax cross-entropy; hand-derived backprop + **Adam**, autoregressive sampling. No BLAS / libc / autodiff — raw `f64` (IEEE-754 i64 bit-patterns via `f64_*` builtins) on flat row-major tensors, single static ELF. 0.4.0 added 4-wide SIMD matmul (~2.27× training); 0.5.0 aarch64 validation + NaN guard + atomic checkpoint. Gradients gated by finite-difference checks. The ecosystem's **ML / training reference** — proof real gradient-based learning is expressible in the "assembly-up, everything-is-i64" sovereign language. cyrius pin **6.1.6** (leading edge). | standalone binary / ML reference |
...
| **thoth** | **Sovereign agentic coding TUI** — a Claude-Code-style command-line agent (interactive REPL/TUI driver: reads a task, plans, edits files, runs tools, iterates) that can **switch the backing model** mid-session for assistance (route a turn to a different LLM / tier / provider). Backronym the user gave: **T**hinks, **H**andles, **O**rchestrates, **T**ransforms, **H**eals (code). Name = Egyptian **Thoth**, god of writing / scribes / wisdom — on-the-nose for a coding agent. The same `Thoth` already used as an `avatara` mythological ARCHETYPE (the Librarian, see `vision/maat-42.md`, `planning/hadara.md`) is **not a collision — it's the design** (user, 2026-06-08): a name being both a personality and a program is normal (cf. "Claude" the name vs. the program), and the Thoth archetype is exactly **thoth's personality overlay** — the scribe/Librarian persona helping in the coding context is a feature. So thoth **consumes `avatara`** to wear that archetype — a special on-the-nose case (name = archetype = function all aligned), not a general pattern (avatara archetypes are normally overlaid by the system, not pulled by a namesake tool). Distinct from **daimon** (the agent-orchestrator *library* + MCP tools) and **hoosh** (the LLM inference *gateway*) — thoth is the *user-facing front-end / coding-agent driver* that CONSUMES them: hoosh for model routing/switching, daimon+**bote**/**t-ron** for MCP tool execution + host registry + MCP security, avatara for personality. Name settled, no rename. Long-horizon; captured per the idea-log pattern ([[project_tools_stable_ideas]]), not scaffolded — let it ferment. | end-user dev workflow on AGNOS; consumes hoosh, daimon, bote, t-ron, avatara |
