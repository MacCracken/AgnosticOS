...
| [ark](https://github.com/MacCracken/ark) | 1.0.0 | Package manager (Cyrius). **Graduated to v1.0** — consumes **mela 1.0** (`mela_fetch_artifact` / `registry_client_new`) to fetch `.ark` packages over mela's transport (the mela v1.0 consumer gate; 263 tests). | nous, sigil, mela |
| [takumi](https://github.com/MacCracken/takumi) | 1.0.0 | Build system (Cyrius). **Rust→Cyrius port complete + graduated to v1.0** (parity reached; was 0.8.0 in-port, `rust-old/` authoritative until now). | sigil |
| [aethersafha](https://github.com/MacCracken/aethersafha) | 0.1.0 | Wayland compositor | aethersafta, mabda |
| [mela](https://github.com/MacCracken/mela) | 1.0.0 | **App & agent marketplace** — Cyrius-native (Hindi मेला, *fair/marketplace*). **Full Rust→Cyrius port complete + graduated to v1.0** (2026-06-17): all 9 Rust modules (6208 lines, `rust-old/` oracle) ported across 12 Cyrius modules, **492 parity tests** + fuzzed parsers. Both trust gates enforced end-to-end at `pipeline_install` — Ed25519 publisher signature (RFC 8032 KAT) + SHA-256 integrity, every publication appended to a hash-chained transparency log. Real I/O: HTTP/HTTPS/DNS via **sandhi**, on-disk install (zip-slip-guarded ustar), keyring disk load — no deferred seams. Ships `dist/mela.cyr` (`[lib]`); **ark consumes it (green)** for `.ark` fetch. Pin 6.2.21. Planned (not scaffolded): `mudra`/`vinimaya` for paid distribution (post-1.0). | ark (fetch), daimon (discovery); sigil / agnostik / sankoch / sandhi |
