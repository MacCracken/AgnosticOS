...
### OS & Infrastructure (28 crates)

| Crate | Version | Domain |
|-------|---------|--------|
| aegis | 1.0.1 | Security daemon — graduated from pre-1.0 in the v5.10.x window |
| agnosai | 1.1.0 | AI orchestration |
| agnostik | 1.3.1 | Shared types & domain primitives (Cyrius, GitHub-release only) — foundation for all AGNOS crates |
| agnosys | 1.4.3 | Kernel interface — Landlock, seccomp, syscall bindings (Cyrius, GitHub-release only) |
...
| daimon | 1.2.9 | Agent orchestrator (144 MCP tools, GitHub-release only) |
| hoosh | 2.4.6 | LLM gateway (15 providers) |
| ifran | 1.3.0 | LLM inference/training |
| kavach | 3.4.2 | Sandbox execution |
| libro | 2.7.4 | Cryptographic audit chain |
| mabda | 3.2.10 | GPU foundation |
...
| nein | 1.5.3 | Programmatic nftables firewall |
| patra | 1.11.4 | Structured storage & SQL — B+ tree, WAL (Cyrius-native) |
| phylax | 1.2.1 | Threat detection — YARA, entropy, magic bytes, ML |
| sakshi | 2.3.2 | Tracing, error handling, structured logging (Cyrius-native) |
| sankoch | 2.4.3 | Lossless compression — LZ4, DEFLATE, zlib, gzip |
| sigil | 3.8.0 | Trust verification & crypto — AES-NI + SHA-NI hardware accel (TLS 1.3 live-verified) |
...
| [yantra](https://github.com/MacCracken/yantra) | 1.0.0 | Sovereign UI automation — browser + mobile, as a Cyrius library (Cyrius-native). **Graduated to v1.0** (was 0.6.2 pre-1.0). `.tcyr` files include `lib/yantra.cyr` and drive Chromium / Firefox / WebKit / Android / iOS. Not a framework — `cyrius test` stays the runner. Backends: CDP, W3C WebDriver, Appium. Consumers: AGNOS E2E (owl, agnoshi, tanur when GUI lands). |
...
| [argonaut](https://github.com/MacCracken/argonaut) | 1.8.3 | Init system library (Cyrius) — depends on agnosys. v1.7.0 adds BOOT_MINIMAL agnoshi-as-no-deps-console-service (unblocks closed-beta MVP without aethersafha). |
...
| [cyim](https://github.com/MacCracken/cyim) | 1.7.3 | Sovereign modal text editor (Cyrius-native, VIM-inspired, zero attack surface, no embedded scripting). Consumes vyakarana + niyama (regex via stdlib fold v5.9.0); consumers: agnoshi, aethersafha, daimon-orchestrated agents. |
...
| [kybernet](https://github.com/MacCracken/kybernet) | 1.3.4 | PID 1 init binary (Cyrius, 140 tests) — depends on argonaut. v1.2.1 consumer pin bump for argonaut 1.7.0 BOOT_MINIMAL. |
| [mela](https://github.com/MacCracken/mela) | 1.0.0 | **App & agent marketplace** — Cyrius-native (Hindi मेला, *fair/marketplace*). **Full Rust→Cyrius port complete + graduated to v1.0** (2026-06-17, was 0.1.0 pre-1.0): all 9 Rust modules (6208 lines, `rust-old/` oracle) ported across 12 Cyrius modules, **492 parity tests** + fuzzed parsers. Both trust gates enforced end-to-end at `pipeline_install` — Ed25519 publisher signature (RFC 8032 KAT) + SHA-256 integrity, every publication appended to a hash-chained transparency log. Real I/O: HTTP/HTTPS/DNS via **sandhi**, on-disk install (zip-slip-guarded ustar), keyring disk load — no deferred seams. Ships `dist/mela.cyr` (`[lib]`); **ark consumes it (green)** for `.ark` fetch. Pin 6.2.21. Planned (not scaffolded): `mudra`/`vinimaya` for paid distribution (post-1.0). Deps: sigil / agnostik / sankoch / sandhi; consumers ark (fetch), daimon (discovery). |
...
| [takumi](https://github.com/MacCracken/takumi) | 1.0.0 | Build system (Cyrius). **Rust→Cyrius port complete + graduated to v1.0** (parity reached; was 0.8.0 in-port, `rust-old/` authoritative until now). Deps: sigil. |

### Stdlib-Folded (2 crates)

| Crate | Version | Domain |
|-------|---------|--------|
| [sandhi](https://github.com/MacCracken/sandhi) | 1.6.7 | Service-boundary layer (HTTP client+server, HTTP/2, streaming, JSON-RPC, service discovery, TLS policy). **Folded into Cyrius stdlib at v5.7.0** as `lib/sandhi.cyr` (vendored byte-identical, 376,037 B / 9,649 lines / 469 fns). Sandhi repo entered maintenance mode per [ADR 0002](https://github.com/MacCracken/sandhi/blob/main/docs/adr/0002-clean-break-fold-at-cyrius-v5-7-0.md); subsequent surface patches land via Cyrius release cycle. |
...
| [rosnet](https://github.com/MacCracken/rosnet) | 0.1.1 | Sovereign dense f64 tensor algebra (the BLAS substrate) — storage, BLAS-1, matmul + its gradient. Extraction target from attn11's ML core (with tyche); not yet folded. | attn11, future ML/agent work |
...
|--------|---------|-------------|------------|
| [shakti](https://github.com/MacCracken/shakti) | 0.7.0 | Privilege escalation (`sudo` replacement) | agnosys, sigil |
...
| [yo](https://github.com/MacCracken/yo) | 0.5.5 | `ping`-equivalent — *Yodel-Out*, Cyrius-native sovereign ICMP echo probe. English-wordplay lane. **Scaffolded** (was the network-tools "planned" family). On agnos it consumes the ring-3 `icmp_echo`#55 syscall (agnos 1.45.4). **0.5.5: folds its IPv4 codec onto [taar](https://github.com/MacCracken/taar) 0.1.0 via `[deps.taar]`** (yo+dig's byte-identical `ipv4.cyr` was the extraction trigger); host+agnos build, 365 tests green. | end-user shells; net-diagnostics |
| [dig](https://github.com/MacCracken/dig) | 0.3.3 | DNS resolver — sovereign Cyrius reimplementation of BIND `dig` (cultural double-anchor: *Domain Information Groper* + *Cyrus from The Warriors*). **Scaffolded** (was the network-tools "planned" family). On agnos it consumes the ring-3 UDP syscalls `udp_bind`/`send`/`recv`/`unbind` #51-54 (agnos 1.45.3). The second consumer whose duplication **triggered the [taar](https://github.com/MacCracken/taar) extraction (done 2026-06-15)** — **0.3.3 folds its IPv4 codec onto taar 0.1.0** via `[deps.taar]`; host+agnos build, 70 tests green. | end-user shells; whirl (hostname resolution) |
| [taar](https://github.com/MacCracken/taar) | 0.1.0 | Network-probe substrate **library** (Hindi/Sanskrit तार — *wire/string/connection*; system-lib lane). **Scaffolded 2026-06-15** at the documented extraction trigger — yo + dig shipped a byte-identical IPv4 codec. **0.1.0 ships the `ipv4` module** (`ipv4_parse` + `ipv4_format_to_buf`, packing matches agnos `net.cyr` `ip4()`); `[lib]` distlib → `dist/taar.cyr`, smoke + 17 tests green. Both consumers folded onto it (yo 0.5.5, dig 0.3.3). Per-protocol modules grow per second-consumer trigger: **planned** `socket` (UDP/ICMP shim, Linux+AGNOS backends), `icmp`, `dns`; `whirl` arrives as the third consumer adding `tcp`/`tls`/`http`. | yo, dig, whirl (all three) |
...
**✅ Scaffolded: `yo` 0.5.5 (ping) + `dig` 0.3.3 (DNS) + `taar` 0.1.0 (substrate lib)** — all three now in *Binaries & Tools (pre-1.0)* above. yo + dig consume the agnos 1.45.x ring-3 net syscalls (`yo` → `icmp_echo`#55; `dig` → UDP `udp_bind`/`send`/`recv`/`unbind` #51-54). **The `taar` extraction fired 2026-06-15**: yo + dig's byte-identical IPv4 codec lifted into `taar/src/ipv4.cyr` (taar 0.1.0); both folded onto it via `[deps.taar]` (host+agnos build, 365/70 tests green). **Still planned below: `whirl`** (the third taar consumer; HTTP/HTTPS — its TLS path is gated on the sandhi agnos socket-backend fix, `sandhi/docs/development/issues/2026-06-14-agnos-socket-backend-gap.md`).
