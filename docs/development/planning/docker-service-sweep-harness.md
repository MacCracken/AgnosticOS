...
3. **AGNOS-in-QEMU-in-Docker boot container** — ✅ **VALIDATED 2026-06-19** at [`agnosticos/docker/net-sweep/`](../../../docker/net-sweep/) (`Dockerfile` + `boot-listen.sh` + `run-sweep.sh` + README). Boots `qemu+OVMF+gnoboot+agnos` with SLIRP `hostfwd`; an external host client gets `curl http://127.0.0.1:<port>/` → `HTTP/1.0 200` `AGNOS 1.45.11 tcp_listen smoke` across the Docker boundary. The "AGNOS runs in a container" milestone is hit. (The `TCP_LISTEN_SMOKE` selftest was rewritten at agnos 1.45.11 from the 1.32.0-era 8 s raw-banner probe to a persistent HTTP server — see [`state.md`](../state.md).)
4. **Harness driver container + per-service sweep scripts** — the drivers + the 9-dimension matrix. ✅ **First cut 2026-06-19**: [`sweep.sh`](../../../docker/net-sweep/sweep.sh) runs the 4 host-reachable dimensions (D1 conn-churn/reclaim · D2 concurrency vs the 8-conn cap · D3 payload-fuzz · D4 soak) against the boot container and writes `findings.jsonl`. **It already found + drove the fix of a real bug** (the `CLOSE_WAIT`-unaccepted permanent slot-leak wedge, fixed at agnos 1.45.11). Remaining net dims (segment-fuzz / SYN-flood / netem-loss) need an in-network-namespace driver container (raw sockets don't survive Docker NAT) — deferred, logged not silently skipped. ✅ **Scheduling half also landed 2026-06-19** at [`sched-sweep/`](../../../docker/sched-sweep/) (`boot-serial.sh` + `Dockerfile` + `sched-sweep.sh` + README): boots each scheduler selftest in-container and tallies pass-rate — `ring3` 3/3 + `thread` 3/3 stable; `boot-smp4` validates the gated single-core production boot. Key finding: the selftests are **TCG-only** (KVM boot-timing races kybernet vs the selftest procs), so TCG is the default. ✅ **The concurrent-proc race-finder landed 2026-06-20**: `SCHED_STRESS_SELFTEST` (agnos `main.cyr`, gated) drives a 12-proc kernel-thread storm near the proc-table cap across 3 rounds (~720 context switches), asserting count/leak/starve/canary invariants — `sched-stress` **3/3 PASS** (scheduler robust under the storm; production byte-identical). The harness caught + drove the fix of a test bug (adjacent module-globals overlapping under the toolchain) before confirming the pass — the value of running it. Remaining: a cap-invariant over-spawn probe (deferred — needs distinct throwaway stacks).
