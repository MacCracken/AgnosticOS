...
| [mirshi.md](mirshi.md) | Phase 20 / swallow | AGNOS↔Linux syscall-ABI mirror-shim (scaffolded 0.1.0 2026-06-29). v1 = agnos userland in a plain Docker container, no QEMU (direction 1); userland pico-process supervisor (ptrace→seccomp-notify), NOT kernel-emu/VM. 3 weights: test-fan-out / Linux-swallow / cloud-bridge. Complements QEMU+iron. |
| [kavach-agnos-compat.md](kavach-agnos-compat.md) | Phase 20 / swallow | kavach↔agnos-kernel compat audit (2026-06-29): scanner+audit+policy works today, confinement is capability-layer (not in kernel); fixed the Linux-syscall-aliasing defect (kavach 3.5.3); sandbox primitives are swallow-stage + capability-first; two near-term kernel items queued 1.50.x |
| [ark-v2-sovereignty-path.md](ark-v2-sovereignty-path.md) | base→server / pkg-mgmt | ark v2 sovereignty roadmap (2026-06-29): de-apt the install path so `agnova install` works on a no-apt box (the agnova prerequisite). Keep apt behind a `system_backend` seam (ADR 0002 default-flip, not deletion); M0–M2 host de-apt → M3 on-agnos install of a prebuilt `.ark` (base) → M5/M6 takumi self-host (server flagship). Surfaces real agnos gaps: no symlink syscall, execwait#37 refuses nested-exec, tiny argv/env caps, no boot-slot primitive. cyrius server-socket peer is RESOLVED. |
| [audio-arc-152x.md](audio-arc-152x.md) | agnos 1.52.x / audio | Audio-output arc scope (2026-06-29): HDA/Azalia driver (`kernel/core/hda.cyr`, 6 bites, polled, reuses pci/r8169/nvme pattern) + ring-3 audio syscall band `#63-68` (mirrors net band) + the two-sided cyrius `vani` agnos backend + a `vanitone` proof-app (NOT a naad/dhvani tracker — naad is still Rust). archaemenid: generic HDA `04:00.6` + Realtek ALC897 + front jack. QEMU `intel-hda`+wav self-checks → iron front-jack. Filed agnos 1.52.x. |
