...
# Iron Boot Test Log — MVP Era (Attempts 1 – 68)

Append-only running log of AGNOS **boot-to-shell-on-iron** test
attempts from project start through the closed-beta MVP gate. Primary
target was the **NUC AMD** (x86_64, archaemenid Beelink SER) — where the
bulk of kernel engineering work was validated — with the Pi 4 (aarch64)
as the secondary axis. Intel hosts were queued after AMD proved out.

This log captures each attempt's symptom, root cause, repair, and
verification state across the boot-pipeline arcs: gnoboot (Path A
multiboot2 → Path C sovereign UEFI), kernel arch bring-up, scheduler /
userland / kybernet / agnoshi, and the xhci HID keyboard arc
(Phases 1 → 5) that closed the MVP functional half.
...
**MVP gate hit**: Attempt 68, 2026-05-18 ~21:30 PDT — `agnos> echo "Assembly Up!"` echoed back on the iron Logitech (VID=1452 PID=591). Closed-beta MVP definition (kernel + kybernet + agnoshi typeable on iron archaemenid) complete.
