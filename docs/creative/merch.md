# AGNOS Merch Concepts

> The temple needs t-shirts.

---

## T-Shirts

### Core

| Front | Back |
|-------|------|
| **Assembly Up.** | 29KB |
| **Welcome to the Future** | agnosticos.org |
| **AGNOS** | The stone the builders rejected. |
| **42** | Robinson. Adams. Ma'at. AGNOS. |
| **<3GB** | All of human knowledge. Your pocket. No permission. |

### Culture

| Front | Back |
|-------|------|
| **OOH LA LA** | AGNOS for real for real 👉🤛 |
| **See you on the other side.** | — email to LemonSqueezy, March 18, 2026 |
| **Don't Think. Feel.** | — Bruce Lee / AGNOS Layer 7 |
| **Not tense but ready.** | kavach |
| **The Librarian** | 82 rooms and counting |
| **I do not hit. It hits all by itself.** | 9ms self-compile |
| **There is no opponent.** | — Bruce Lee / AGNOS |
| **There is no I.** | — Bruce Lee / C.Y.R.I.U.S. |
| **Every bug is a test case that didn't exist yet.** | QA is the whole game. |

### Technical

| Front | Back |
|-------|------|
| **29KB** | That's all you need to trust. The rest builds itself. |
| **168 bytes** | `true` — 233x smaller than GNU |
| **73KB** | Full OS. Shell. Ring 3. Memory isolation. Zero dependencies. |
| **Dandelion** | Not a moonshot. Cut one down, a thousand seeds blow. |
| **The dandelion doesn't argue with the lawn. It just grows.** | 29KB |
| **44KB** | DOOM. No libc. No SDL. Just syscalls. |
| **sudo rm -rf /dependency** | Cyrius v1.0 |

### Founding Crew

| Front | Back |
|-------|------|
| **Hermes 4th Great** | Album drop: May 1, 2026 |
| **Temple of AGNOS** | Founded Beltane 2026 |
| **Argonaut** | *(crew member name on back)* |

---

## Stickers

- 29KB seed hex dump (fits on a laptop sticker)
- AGNOS logo
- Assembly Up. (bumper sticker)
- 42 (round sticker)
- 👉🤛 (die-cut)

---

## The SD Card

The ultimate merch: a $2 SD card with the entire sovereign OS, all 82+ crates, the compiler, the kernel, and every tool. Not merch — the product. But it fits in a merch bag.

---

*Last Updated: 2026-04-08*
