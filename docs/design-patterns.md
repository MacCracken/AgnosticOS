...
### Sovereign substitution at the protocol layer — Wayland refused (added 2026-07-06)

Pattern instance of **§9 (Reference, Don't Mimic)** + **§0 (Refusal as Architecture)** at the desktop / display layer — the same move that produced seed → Cyrius, GRUB → gnoboot, and Linux → native kernel (and that the still-pending ISO/xorriso → sovereign image builder will extend), now applied to the display protocol. **AGNOS will not port Wayland.** The sovereign widget toolkit (`dhancha`, complete on the client side at 0.5.0 — widget tree, flexbox layout + intrinsic measure, sadish/rekha draw, a full event model) and the compositor (`aethersafha`) will speak a **native, first-principles protocol** — "Wayland-esque" only in the loosest shape (clients submit surface buffers; a compositor composites and routes input), sovereign in wire, lifecycle, and paradigm. aethersafha's ported-from-Rust Wayland-server work (its ~3360-line "Bite F") is **retired, redefined native**; it already composites through its own window model on bhumi with zero Wayland, so the sovereign path is the de-facto reality, not a leap. (puka's `wl_*` client speaks to *host* Hyprland — a Linux-desktop dev crutch, not the AGNOS path, and not a template.)

**Why porting is refused (the §0 interrogation).** Wayland is a decade of accreted compromises for the Linux desktop's constraints: the client-allocates-opaque-buffers model, the `wl_shm`/`xdg_shell`/registry handshake, the extension sprawl. Each encodes a design decision AGNOS never made; adopt the protocol and you silently adopt them all — *accepting design decisions we never made.* "For compatibility with Wayland" fails the living-reason test: the sovereign desktop was never meant to run foreign Wayland clients (that is the separate, firewalled compat lane — the empire-defense layers — not the native protocol's job), so the compat defense points at something AGNOS isn't shipping into.

**"Follow what works, leave what doesn't" (the §9 discipline).** Wayland is *prior art*, not a template: the surface/role separation, client-submits-buffer, compositor-owns-composition-and-input, damage tracking, and event-ordering discipline are load-bearing concepts to study and keep as design inputs — then redesign clean into Cyrius, dropping the wire, the lifecycle protocol, and the extension mess. Sovereign ≠ ignorant of precedent; sovereign = deciding for yourself which parts earned their place. (The multi-source-prior-art audit discipline, applied to a *protocol* instead of a driver.)

**The positive freedom — "what AGNOS requires."** The sharper argument isn't cost-avoidance; it's capability. Wayland's client-opaque-buffer model *structurally forbids* what an AI-native OS wants: the compositor cannot see inside a client's surface, so it cannot introspect a widget tree, cannot let an agent drive or reason about another surface, cannot do content-aware OS-level composition. aethersafha already carries an "agent window" concept and mehman foreign guests; a native protocol can make **agent-owned, introspectable surfaces first-class**. That capability is *forfeited* by porting Wayland, not gained by it.

**Timing is why this is the crux.** There is no protocol to be locked into yet — the seam between every app and the compositor doesn't exist. The cheapest possible moment to make the sovereign choice is *before* that ABI is minted; port Wayland first and reclaiming sovereignty later becomes a migration across every app's surface contract. The client side is done and paradigm-neutral; the compositor protocol + the AGNOS desktop paradigm (the AI-native window/interaction model — the part that most emphatically must not be a port of GNOME/KDE) are the remaining, founder-led design.

**See also.** dhancha plan doc `dhancha/docs/development/sovereign-desktop.md` (the cross-repo seam, bites, and open decisions); memory `project_dhancha_arc.md` (the arc + the WAYLAND-DEAD directive). **Promotion path:** when the native protocol + desktop paradigm are designed and the compositor ships client support — or a second protocol-layer refusal lands — hoist into a full §12 on sovereign protocol/format substitution (anchored on the shipped seed/Cyrius + GRUB/gnoboot cases, with the pending image-builder and native-display instances as the protocol/format extensions, and the "prior-art-not-blank-slate" + "porting-forfeits-a-capability" rules named).
