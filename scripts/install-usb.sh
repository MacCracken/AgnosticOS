...
if [[ "$MODE" == "update" ]]; then
    ...
    echo ""
    echo "What you'll see on the screen (Attempt 15+ diagnostic ladder):"
    echo "  - WHITE stripe at top  = boot_shim entry canary (256 px @ y=0)"
    echo "  - YELLOW 4×4 cells     = early arch reached (GDT/TSS/IDT)"
    echo "  - GREEN  4×4 cells     = subsystem init (APIC, PMM, heap, ACPI, VFS)"
    echo "  - CYAN   4×4 cells     = scheduler arming + first hlt loop (CP 0x10 = closed-beta gate)"
    echo "  - MAGENTA 4×4 cells    = post-scheduler / userland reached (CP 0x11 = full 50-hlt cycle done)"
    echo "  Cells form a 16-wide grid starting at y=8. Stop-point cell tells you the last CP that fired."
    echo ""
    echo "If boot stalls (no shell, screen frozen at some cell):"
