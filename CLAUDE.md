...
| **aethersafha** | Wayland compositor | **Porting** (0.5.0 — built-in-apps framework; backends bhumi 1.0.0 + mehman 1.0.0) |
...
| **tentib** | Integer-native / ternary (1.58-bit) ML reference in Cyrius (`bitnet` reversed) — attn11's everything-is-i64 thesis applied to the *weights*: BitNet b1.58 ternary {−1,0,+1} weights → a matmul-free (add/sub/skip) weight·activation path + int8 activations + the straight-through estimator (the canonical finite-difference-gate case study). Sibling reference to attn11 (Transformer) + tarka (RL/reasoning) on the same f64 substrate (rosnet/tyche/akshara). **Stable at v1.0** (2026-07-06, the one-day 0.4.1→1.0.0 arc): ternary transformer trains via STE; whole-model matmul-free integer inference **bit-identical** to the f64-latent reference; the cyrius-6.4.x `iv_dp8` integer-SIMD kernel (faster than the f64-SIMD matmul, advantage growing with layer size); pack-once deployment serving (`tx_fwd_packed` — the hoosh/murti seam); benchmarks + frozen API + security audit. Binary. | **Native** |
...
| **tula** | तुला (*balance/scale*) — sovereign **ML weight-file format** lib (safetensors/GGUF analog with a sigil-signed header they lack): typed manifest (f64/int8/ternary/nf4) + aligned payload + Ed25519 sign/verify + heap/mmap read. **M0 of the Type-3 weight-import chain** (`docs/development/planning/type3-weight-import.md`). Format v1 frozen at v1.0.0. Library. | **Native** |
| **rupantara** | रूपान्तर (*transformation*) — sovereign **transformer-FORWARD** lib extracted from attn11 (attn11→libs #4) so the Type-3 importer has a consumable forward without duplicating attn11. `ru_*` leaf ops + composition + KV-cache decode; whole-forward parity-proven bit-identical vs attn11 (which re-folds onto it, 1049 grad-checks green in one binary). Library. | **Native** |
| **anukūlana** | अनुकूलन (*adaptation*) — sovereign **Type-3 "Pre-Trained" reference**: imports a **real foreign checkpoint** (GPT-2-small safetensors, own parser) and runs its forward on rupantara/rosnet, then adapts it (LoRA → QLoRA/NF4). The "sovereign AND interoperable with the world's weights" proof — NOT a from-scratch trainer (that's attn11/tarka/tentib/prajna). NF4 import-quant lives here, not tentib. Binary. | **Native** |
